...
### 2026-09-11: authorization and audit for the complete solver

The removal checkpoint was published as
75da68b446a1197b0eae475f0f17b715efaeae9b, parent
5b900ac1ae36aaea6e47e0c17a71d169b78d4274, tree
87b6252486fe9833980d403ef7684dccd7869472.  Its non-forced push,
fetch, and head/parent/tree/worktree comparisons passed.

The user reviewed and authorized the complete development: one Lean
program compiled by LeanExe, one local WASM process and thread, runtime
grid size, and proofs for both source and exact generated bytes.  After
those proofs pass, run 192, plot 192, run 800, and plot 800.  Both results
are additional datasets.  The user accepted local execution after the
discussion of single-core execution and memory requirements.

Read README, AGENTS, DEVELOPING, the operating instructions, the installed
leanrunner skill, the source language documentation, the three Euler
source kernels, the mathematical cell/sweep/run specifications, the host
initialization and timestep code, the array mapper and allocator emitter,
and the pinned Lean and Talos arithmetic definitions.  The first broad
reads were truncated.  Narrower reads recovered the relevant definitions.
Several guessed documentation/source paths were absent.  File listings
identified the installed paths before further inspection.

The current cell theorem quantifies over raw inputs and proves exact
terminating cell execution.  RunnerSpecFor is conditional on the pure
finite-ratio recurrence succeeding.  It does not prove the native grid or
time loop.  Array.set copies whole arrays, while Array.map allocates one
output and writes each item.  The existing general proof support includes
copy, fold, and a scalar map-add example.  The complete solver needs
grid traversal and storage reuse proofs.

The source calls LeanExe.Float64 operations implemented with Lean Float.
Lean 4.34.0-rc2 defines the five required operations and bit conversions
through Float.Model.  Talos defines separate pure integer-based IEEE64
operations.  Searches found no existing equivalence bridge in the pinned
proof dependencies or project.  This is an additional proof obligation,
not permission to assume equivalence.  The host's bounded step/retry
loops likewise do not prove successful completion at time 0.8.

The first bounded edit adds plans/euler-riemann-complete.md and records
the current authorization in plan.md, plans/talosfp-euler-operations.md,
devnotes.md, and this journal.  Status was inspected before editing.
The unrelated report intermediates remain untouched.  No new solver run
or proof success is claimed.

Added LeanExe/Examples/EulerRiemann/Grid.lean, source proofs in
Project/EulerRiemann/Geometry.lean and Indices.lean, and
test/euler_riemann_grid.js.  The source accepts sizes 2 through 800.
The geometry theorems prove the interface-fraction numerator bound and
clamped-neighbor bounds.  The index theorem proves the exact ordered
sequence from zero through n*n-1 and its length.  Public axiom reports
contain only propext, Classical.choice, and Quot.sound.

The first geometry build failed on the absent Mathlib.Tactic.Omega import.
Replaced it with the installed Init.Omega.  The next build exposed an
unreduced Boolean branch, which was reduced before the arithmetic tactic.
The index proof required separate array-to-list rewrites before reasoning
about the prefix.  The corrected source proofs passed in under a second.

The first index implementation recursed before constructing its returned
array.  The compiler report rejected that non-tail recursion.  Changed
the source to double an accumulated index array in tail position and
proved the resulting sequence.  The ownership report accepts all four
functions and reports no emitted releases or fresh-result summary for
the recursive helper.  Its bounded initialization allocations still need
an exact-WASM memory argument.

The grid test's first invocation failed before child launch with
spawnSync tools/leanrun EPERM.  Retried through the required escalation
with the reusable node test/euler_riemann_grid.js prefix.  The test passed
48 cases, including every returned index at n=192 and n=800, all four
neighbors of every cell in a three-by-three grid, rejected size bounds,
and the 192/800 interface fractions.  It retained the three generated
WASM modules in tmp/euler-riemann-grid-19kVAU.  These are grid-helper tests.
No Riemann evolution was run.  Compiler and runtime child commands use
tools/leanrun with bounded timeouts and standard local limits.

Added the numerical initial-state definitions and began their proof in
Project/EulerRiemann/Initial.lean.  The monolithic Fin6-by-Fin6 guard proof
reached the three-minute timeout with final status 124 and no theorem
diagnostic.  Divided the definitions into InitialModel.lean and introduced
InitialStates.lean to prove the four conservative-state raw-word results
once.  That target passed in 2.9 seconds.  The next initial-state proof
uses those lemmas before checking the weighted states.  A separate
Neighbors.lean proves correspondence to clamped coordinate neighbors.
Its first check left an addition-reordering goal, now reduced to linear
arithmetic.  The revised initial-state and neighbor proofs remain pending.

The compiler currently recognizes five LeanExe.Float64 names.  Asked
the user whether to add mappings for the existing Wasm.IEEE64 definitions
so source and artifact proofs can share that arithmetic model, or retain
the native Float source and prove the two models equivalent.  This is a
compiler design choice.  Its answer remains pending, and no compiler or
arithmetic source change has been made.  Independent grid and initial-state
proof work continues.  The 105-file documentation check and whitespace
check passed before the latest proof-only edits.

The revised Neighbors target passed in 668 milliseconds, with only
propext and Quot.sound in both audits.  InitialStates passed after the
explicit exponentiation threshold was added, with only propext and
Quot.sound in its four audits.  The combined weighted-state target again
reached the three-minute limit with status 124 and no diagnostic.

Read Lean/Elab/Tactic/Decide.lean in the pinned toolchain.  Its +kernel
option constructs and checks a proof through the kernel and caches the
auxiliary lemma.  The default first reduces the decision instance in the
elaborator.  Added the smaller InitialRows.weighted_guard_0 theorem using
the checked state lemmas and decide +kernel.  It passed in 9.1 seconds,
with only propext and Quot.sound.  The complete weighted_guard now splits
the first Fin6 argument, reuses that row, and checks the remaining five
rows with the same kernel method.  Initial passed in 16 seconds, and
weighted_guard and initial_safe both audit to propext, Classical.choice,
and Quot.sound.  Every revised target changed its proof boundary after
the preceding timeout.  No native_decide or additional axiom was used.

The completed checkpoint consists of the grid source, seven source/model
proof modules, the grid test, and the six plan/note files.  All new proof
modules have passed their focused builds.  The grid test retained its
48 passing cases and artifacts.  The complete solver, allocator proof,
time-control proof, exact-artifact proof, and both requested production
...
The reviewed 17-path checkpoint passed whitespace and 105-file
documentation checks and was published as
093f246df7a2b0211553a307e974ce9c7d4e91c2.  The exact commit identity is
verified in the publication command output.  Added ExecutionScalar to
reuse the checked scalar flux and conservative-update regions, then prove
the complete module's two forwarding functions by call composition.
Its focused check is pending.

Correction: the preceding entry mistyped the commit identity.  The
verified commit is 093f246ec9e6a3d16a3b362018773ebb531b71b0, parent
404521c56ccb0529b94416eb09aafec561da65b9, tree
be8a70efdf874831aae47a7d3d9a530c0b932fde.  Non-forced SSH push,
fetch, and exact commit/tree/index/tracked-worktree comparisons passed.

ExecutionScalar checked both renamed scalar regions on its first attempt.
The two forwarding proofs needed explicit constant-local simplification,
followed by the final empty-list equality.  Two failed elaborations retain
their diagnostics.  The corrected module passed all 3,368 jobs in its
focused build, with module time 3.8 seconds and standard-only axiom audits
for all six declarations.

ExecutionFlux composes the proved side and scalar calls against function
47 of the complete generated module.  Its proof shares the continuation
across both speed-selection branches.  The 3,408-job focused build passed,
including a rebuilt existing flux proof in 29 seconds and the new module
in 31 seconds.  Rejection and all-input flux execution use only standard
axioms.  The final simplifier reports one unused argument.  Added the
cell-update composition in ExecutionCell against function 58.  That
module's focused check is next.  Read-only storage inspection now reports
59 GiB available on the home filesystem.  No saved files were removed.

ExecutionCell passed the 3,413-job focused build.  The existing cell
execution module rebuilt in 58 seconds and the complete-module proof
checked in 61 seconds.  Positive-word, rejection, and all-input cell
execution audits contain only the accepted standard axioms.  Added
ExecutionScanCell for the source's two-direction speed-fold callback.
Its first check completed the WASM instruction paths but left four
maximum-value equalities.  Generic max simplification selected lattice
notation before the intended word comparison.  The revision first
unfolds the exact UInt64 max comparison and then uses the branch facts.

The revised scan callback proof resolved the first two comparison cases.
The remaining two needed the existing flux proof's exclusion of
UInt64.not_le from simplification, preserving the negative branch fact.
ExecutionScanCell then passed all 3,423 jobs, with module time 4.3 seconds
and only standard axioms.  Reviewed the four new execution modules and
their proof outputs.  They change no compiler source, numerical source,
...
Published and fetched 809f5ecd870f5ecbbb985d84d51733749e9e6361,
"Prove computed Riemann wave-speed adequacy", parent
6bafdb4b0f01a5e8215e5ebe2b8c76ec0a9c7348, tree
faa8622ebec210f0e910b013624eae7d31d95be7.  The fetched commit,
parent, title, tree, index, and all fourteen checkpoint worktree files
match.  Documentation and whitespace checks passed.

Added F64ProductBound to derive an exact product at most 51/100
from a positive rounded product at most 1/2, retaining the multiplication
underflow term.  Added RealCflStep with an explicit positive center
weight.  This permits the real positivity argument to accommodate the
rounded CFL comparison.  Both new proofs await their focused checks.

F64ProductBound passed in 1.5 seconds with a standard-only audit.
Added NumericsCfl to discharge its arithmetic range premise from
the current ratio and interface bounds and transfer the selected
maximum to the sum of the two exact interface Courant numbers.
This source-independent arithmetic adapter awaits its focused check.

RealCflStep passed in 1.3 seconds with a standard-only audit.
Added NumericsRealStep to combine source interface alpha bounds,
the accepted rounded CFL test, and exact split-state positivity.
Its reference update uses decoded input states, exact physical
fluxes, and the source's computed alpha values.  The target retains
49/100 of center density and internal energy.  Flux and conservative
update rounding still require the subsequent perturbation bound.

NumericsCfl's first check accepted the individual product theorem.
The selected-product proof introduced the conclusion's let-bound
alpha instead of its first hypothesis.  Added explicit let reduction
before introducing the two CFL hypotheses.  The rejected draft is
not accepted proof evidence.

NumericsCfl and NumericsRealStep passed in 1.3 seconds each, with
four standard-only audits.  Reviewed the accepted statements and
the earlier let-introduction failure.  The explicit center-weight
theorem and shared rounded-product bound close the CFL-to-reference
positivity composition under the stated cell and ratio bounds.
The result concerns exact physical fluxes with computed alpha values.
Physical-flux and update rounding, quantitative guard acceptance,
global preservation, final-time success, and complete artifact
execution remain open.

Prepared the four-module CFL checkpoint with devnotes.md, plan.md,
plans/euler-riemann-complete.md, and this journal.  All checks ran
locally through tools/leanrun with six-minute timeouts and standard
limits.  There were no compiler, numerical-source, generated-WASM,
or LTG catalog changes.  All existing unrelated state is preserved.

Published and fetched 5c8c9defedecf177907b1b84eefc54caa1b6afdb,
"Prove rounded CFL reference-step positivity", parent
809f5ecd870f5ecbbb985d84d51733749e9e6361, tree
12e289032a0b0c52ada3aed81f002e90d6e3543c.  The fetched commit,
parent, title, tree, index, and all eight checkpoint worktree files
match.  Documentation and whitespace checks passed.

Added RealProductError.product_error with separate factor bounds and
error budgets.  Energy-flux composition needs this distinction between
rounded velocity and exact enthalpy.  The existing symmetric
perturbation theorem remains unchanged.  The new theorem awaits its
focused check before the source flux composition.

RealProductError passed in 647 milliseconds with two standard-only
audits.  Added NumericsFluxReference to compare all four returned
side fluxes with the exact Euler formulas on decoded conserved inputs.
Its energy-flux proof propagates velocity, pressure, enthalpy, and
final multiplication error through the shared product lemma.  The
new source composition awaits its focused check.

NumericsFluxReference passed in 1.9 seconds with a standard-only
audit.  The mass flux is exact.  Momentum, transverse-momentum,
and energy-flux errors are at most 20 epsilon M cubed, 3 epsilon
M cubed, and 48 epsilon M to the fifth power, respectively.
Added a shared reference-flux error composition and its Rusanov
component consumer.  The latter targets 304 epsilon M to the fifth
power against exact physical input fluxes with the computed alpha.

Added NumericsVectors to collect finite words, magnitude bounds,
and physical-flux errors by component index.  This permits one
Rusanov component theorem to serve all four interface fields.
The source recurrence and generated module remain unchanged.

The shared RealRusanovError extension passed in 1.4 seconds with
two standard-only audits.  NumericsComponentReference referred to the
two-dimensional alias without importing its module.  The ensuing
type-recovery attempt exhausted the default heartbeat limit.  Changed
both references to the original EulerDynamicFlux declaration already
imported by NumericsComponent.  No proof or resource limit changed.

NumericsComponentReference passed in 1.1 seconds with a standard-only
audit.  Added NumericsInterfaceReference to identify each returned
component from source acceptance and instantiate the common error
theorem with the checked vector bounds.  This proof targets the
complete returned interface vector against the exact Rusanov formula
using the returned alpha.  Its check is pending.

NumericsVectors and NumericsInterfaceReference passed in 1.7 and
1.4 seconds, with five standard-only audits.  Added
NumericsUpdateReference to retain the timestep factor when both
interface errors enter the conservative update.  It reuses the
checked affine-error lemma and targets epsilon M plus 1004 epsilon
times the ratio times M to the fifth power, plus twice the
half-subnormal unit.  Its focused check is pending.

NumericsUpdateReference passed in 1.1 seconds with a standard-only
audit.  Added vector bounds for the returned interface words and
NumericsCellReference to prove status-zero finite component updates
and their uniform error against the exact reference cell update.
The final thermodynamic check on that candidate cell remains a
separate obligation.  The new composition awaits its focused check.

NumericsCellReference's first check rejected the continuation layout
of a let-binding inside the component quantifier.  Moved the binding
to its own line so its remaining arguments belong to the update call.
The vector-bound extension and its interface consumer passed during
that check.  The cell composition awaits checking again.

NumericsCellReference passed in 1.1 seconds with a standard-only
audit.  Reviewed the accepted proof and the alias/import and layout
failures.  The shared real product, reference-flux, and affine-error
lemmas support one component-indexed derivation for all four returned
interface fields and candidate updates.  Every candidate scalar
update succeeds and stays finite under the input StateBounds and
positive ratio-at-most-one premises.  Final candidate-cell guard
acceptance and the full-run invariant remain open.

Prepared the eight-module physical-flux and update checkpoint with
devnotes.md, plan.md, plans/euler-riemann-complete.md, and this journal.
Every changed proof has a passing focused check and standard-only
public audits.  All checks used the local standard tools/leanrun
limits and six-minute timeouts.  No numerical source, compiler,
generated bytes, or LTG catalog changed.  No production run started.

Published and fetched 46a988d90bbe21fd83c278f401f145727fb73a68,
"Prove full Riemann flux and update error bounds", parent
5c8c9defedecf177907b1b84eefc54caa1b6afdb, tree
f8adc9c21a4343b7d580c0e01de2a34a802bd702.  The fetched commit,
parent, title, tree, index, and all twelve checkpoint worktree files
match.  Documentation and whitespace checks passed.

Strengthened the reference-step proof to retain its exact center
weight, 1 minus half the sum of the two exact Courant numbers.
The existing 49/100 theorem now follows as a corollary with the
same statement.  Retry analysis needs this timestep-dependent lower
bound as well as the timestep-dependent rounding bound.  The revised
module awaits its focused check.

Added NumericsReferenceBounds to bound each exact interface component
and the reference update while retaining the timestep factor.  Added
RealMarginBounds to convert the weighted density and internal-energy
bounds into a squared-weight energy-margin bound.  Both modules await
their focused checks before candidate perturbation composition.

NumericsRealStep passed in 1.3 seconds with three standard-only
audits, including its original public theorem.  NumericsReferenceBounds
needed the absolute-difference orientation fixed before inferring the
shared magnitude theorem's arguments.  Rewrote the supplied error
inequality explicitly.  Its reference-update derivation otherwise
passed in that failed draft.

Added NumericsCandidateBounds to combine the component errors,
reference magnitude, exact center weight, and energy-margin
perturbation.  It derives lower bounds for candidate density and
energy margin without assuming candidate guard acceptance.  The
word-vector helper expands the existing source flux and update
definitions.  The new composition awaits its focused check.

RealMarginBounds and NumericsReferenceBounds passed in 1.1 and
1.2 seconds with four standard-only audits.  NumericsCandidateBounds
needed its supplied weight inequality expressed using the local
weight abbreviation before the arithmetic tactic.  Added that
definitional conversion.  The remaining obligations passed in the
failed draft, and the resource limits remain unchanged.

NumericsCandidateBounds passed in 1.2 seconds with a standard-only
audit.  Extracted stateGuard_of_energyMargin from the existing
perturbation theorem and added NumericsCandidateGuard.  Its acceptance
condition combines the derived density and margin bounds with the
guard's explicit normalization conditions.  Discharging those
conditions throughout the reachable run remains open.  Both revised
guard-proof modules await their focused check.

NumericsMargin and NumericsCandidateGuard passed in 1.3 and
1.2 seconds with four standard-only public audits.  The former retains
its existing exponentiation-threshold warning at the normalized-margin
arithmetic boundary.  The warning did not prevent checking.  All six
candidate-margin modules now have passing focused checks under the
local standard runner limits and six-minute timeouts.

The candidate theorem derives finite words and lower bounds for
density and energy margin from accepted rounded CFL and three input
StateBounds predicates.  Guard acceptance still requires the stated
normalization and quantitative inequalities.  Those hypotheses have
not been established over the reachable simulation.  The checkpoint
does not establish full-cell acceptance, successful retry, or final-time
completion.  No numerical source, generated artifact, compiler, or LTG
catalog changed, and no production calculation started.

Prepared the six proof modules with devnotes.md, plan.md,
plans/euler-riemann-complete.md, and this journal for an explicit-path
checkpoint.  The shared reference-weight and real-margin results
...
Published and fetched ce0c082a45ad6aa31643f4572166fb8653e0a0df,
"Check failure of fixed Riemann state bounds", parent
abbbee534e675ae2279fc7ab3097f785bbfb8f59, tree
e6db3926e7216e3a136d890210912cf6869c339a.  The fetched commit,
parent, title, tree, index, and all five checkpoint worktree files
match.  Documentation and whitespace checks passed.

The user approved the proposed proof revision.  Updated plan.md,
plans/euler-riemann-complete.md, plans/talosfp-euler-operations.md,
and devnotes.md to require complete terminating source and exact-byte
behavior with the full memory bound, including explicit failure
returns.  Status zero must imply the specified numerical trace through
time 0.8.  Both production results must report that status and time.
The run and plot order remains unchanged.  The numerical program,
generated bytes, and permitted axioms remain unchanged.

Read the current annotation documentation and LTG package instructions.
Searched the compiler-motif and proof-construction indexes for guard,
floating-point, shift, and portable execution support.  The matching
fuel-guard and array entries support later loop composition.  No
function-renaming entry appeared in that query.  The existing checked
FunctionRegion library provides the scalar-body transfer.  Current
compiler annotations identify the revised component and update call
indices.  Updating ExecutionScalar against those function identities
and checking the exact decoded instruction matches is the first
execution target under the revised requirement.

ExecutionScalar passed in 7.1 seconds with six standard-only audits.
Both region shifts match the current decoded bodies and call targets.
The helper and wrapper execution theorems retain their exact result
words and complete store preservation.  Added ExecutionGuardBase to
transfer the unchanged classification, narrow guard, normalization,
residual, and rejected-side functions from their checked module.
Its two closed call-domain matches and execution adapters await a
focused check.  No generated program was edited.

ExecutionGuardBase passed in 3.4 seconds with fourteen standard-only
audits.  Its two closed call-domain matches reuse the existing
FunctionRegion theorem, and twelve execution adapters preserve exact
words and the store.  Added ExecutionGuardMomentum for the four new
tiny-momentum functions at compiler-described indices 13, 14, 17,
and 18.  Their proofs use the shared call and branch tactics and the
checked normalization helpers.  The focused check is pending.

ExecutionGuardMomentum's first check accepted the tiny predicate but
stopped at the two Boolean-call continuations.  The generic guard_call
tactic advances before the returned Boolean is split, whereas the
checked energy-guard proof splits that Boolean first.  Changed these
continuations to unpack the call result and split the Boolean before
advancing the instruction proof.  No theorem statement or resource
limit changed.  The remaining residual composition follows the same
checked helper call sequence.

The second momentum check passed the previously blocked false
normalization branch.  Destructuring store equality with rfl had
removed the named initial store needed by subsequent calls.  Restored
the established explicit store substitution and removed the reported
unused simplifier arguments.  The third focused check is pending.

ExecutionGuardMomentum passed in 5.3 seconds with four standard-only
audits.  Removed its two remaining unused simplifier arguments, which
the next dependent build will check.  Added ExecutionGuard to compose
all classification, normalization, residual, threshold, and narrow-
guard branches at current functions 19 and 20.  The proof uses the
existing guarded-call structure, with rejection simplification tied
to the revised source definition.  Its focused check is pending.

ExecutionGuard's first check rejected the macro binder named condition,
which the tactic syntax had already reserved.  Renamed the binder to
test.  The second check passed in 8.6 seconds with two standard-only
audits.  Removed the reported unused simplifier arguments and a
redundant final guard_peel.  The dependent build checked the cleaned
module in 8.4 seconds without its own warnings.  The momentum-helper
cleanup also passed that dependency check.

Replaced ExecutionSide's old whole-side shift with the established
instruction proof composed through the new state_guard_exact and the
checked scalar helpers.  Its result now names Numerics.sideCheckedBits
at current function 22.  Every rejection branch and floating-point
operation remains in the execution proof.  The focused standard local
runner check passed in 33 seconds with a standard-only audit.  No
compiler output or numerical definition changed.  The region library
supplied the unchanged-helper transfers, and the existing guarded-call
proof supplied the changed consumer's branch structure.

Prepared ExecutionFlux for current function 54 and the revised side
model.  Its focused six-minute runner check is in progress.  Keeping
that draft outside the present checkpoint.  The checkpoint contains
the approved gate revision, the scalar and side execution updates,
ExecutionGuardBase, ExecutionGuardMomentum, ExecutionGuard, plan.md,
plans/euler-riemann-complete.md, plans/talosfp-euler-operations.md,
