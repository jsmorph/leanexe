...
The group64 implementation has complete execution and exact-binary proofs, retained runtime and memory measurements, generated texts, and conditional numerical bounds.  The `iogpt` merge passes the aggregate source and artifact checks in the existing checkout on 2026-09-25, along with the complete execution, byte-I/O, running-sum, WAT/binary comparison, and conformance checks.  Quantized inference remains opt-in.

The I/O merge's [refreshed prefix test](iogpt-cached-test.json) reproduces all 6,432,896 reference logits and every attention-cache byte across 128 prefixes.  Greedy choices agree with FP32 on the same 120 prefixes as the retained grouped experiment.  The controlled timing and completion records below describe the earlier compiler output.
...
The [current model record](model.json) pins the [28,017-byte binary](../../proofs/artifacts/gpt2_quantized_cached/87026cf4bd73cc7ae793195ccc1a6e0469d2eb3d007d1e01cad676e551176aa8/program.wasm) from the I/O merge, SHA-256 `87026cf4bd73cc7ae793195ccc1a6e0469d2eb3d007d1e01cad676e551176aa8`.  Its [prefix test](iogpt-cached-test.json) reproduces every retained grouped logit and cache value.  The [scheme-2 checkpoint manifest](group64-manifest.json) records the same 127,695,972-byte file size and tensor payload as scheme 1.  The changed header distinguishes the grouped arithmetic.  Its weight SHA-256 is `9d60657659e502b8dae9f11c2e73583643962b42c662cb9b51aa53e8730ec314`.
