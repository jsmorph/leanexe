# Checked Sod shock tube — Wasmtime revision
...
The persistent C host uses the pinned Wasmtime44 C API with contraction and
fast-math disabled. Node prepares files and scientific diagnostics; it never
executes WASM. The runtime loads the exact registered grid-scan and grid-step bytes named in
...
outside the formal theorem. The C/JS runtime assertions check the host obligations; this does not
constitute a formal proof of either host driver.
...
This revision corrects the runtime policy violation in [v1](../euler-sod-v1/README.md).
Every retained raw output, time-step history, frame and diagnostic agrees
exactly with v1 at all four resolutions. Its data files remain preserved;
v2 is the maintained Wasmtime reproduction target.
