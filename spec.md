...
`LeanExe.Examples.JsonCollatzLength.transform : ByteArray -> ByteArray` accepts `{ "collatzLengthFor" : digits }` with the same ASCII whitespace rules and returns `{"length":<number>}` through `Ascii.Json.object1UInt64`.  The length counts sequence terms, so input `41` returns `110`.  The program rejects zero, decimal parse overflow, `3n+1` overflow during sequence evaluation, fuel exhaustion before reaching `1`, malformed input, non-ASCII input, and trailing input.

`LeanExe.Examples.JsonTools.transform : ByteArray -> ByteArray` demonstrates generated JSON output through `object1UInt64`.  `LeanExe.Examples.JsonTools.lookup : ByteArray -> UInt64` demonstrates generic object-field lookup, including skipping an unknown nested object before the requested field.  The split reflects a current compiler limitation: demanding an `Option UInt64` payload found after a skipped field inside a `ByteArray`-returning match can produce a WASM trap that Lean execution does not produce, so byte-output programs should stage that pattern through a scalar entry or use exact schema parsers until the structured-value lowering is fixed.
