...
Nonrecursive project-local helper calls are inlined into the caller with lazy argument thunks.  Recursive helper calls that remain as Wasm calls still use Wasm's strict argument evaluation, so the extractor rejects those calls when an argument expression contains a direct partial array operation such as `a[i]!` or `a.set! i v`.  The guard is syntactic and does not expand called helpers inside the argument.  Demand analysis or call-site loop inlining remains planned so the compiler can accept more safe recursive-helper calls and reject indirect trapping arguments for the same reason.
