...
| `Option` | Planned | Implemented internally for first-fragment payload values, including construction, matching, `getD`, `orElse`, `elim`, `map`, `bind`, `isSome`, and `isNone`; `Option` entry parameters and `Option` entry results are rejected |
