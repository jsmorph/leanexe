...
| `UInt8` | Implemented | Implemented for the demo validator and as an internal generic scalar for `ByteArray.get!`, `UInt8` literals, `UInt8.ofNat`, `UInt8.toNat`, local lets, project-local helper parameters and results, comparisons, wrapping addition, subtraction, multiplication, `&&&`, `|||`, `^^^`, and bitwise complement; `UInt8` entry parameters and results are rejected |
