...
| `PSum α β` | No | No | Yes | Internal sum values are accepted for Lean's generated two-branch mutual-recursion helpers. |
| Structure | Yes | Yes | Yes | Monomorphic, nonrecursive structures with supported runtime fields.  Internal-only structures may contain recursive-inductive pointer fields. |
| User inductive | Yes | Yes | Yes | Monomorphic, nonrecursive inductives with supported runtime fields.  Internal-only tagged values may contain recursive-inductive pointer payloads. |
