...
Pure `Id.run` `for` loops compile when Lean elaborates them to `ForIn.forIn` over `ByteArray` or a fixed-width `Array`, and every loop step yields the next accumulator through `ForInStep.yield`.  The accumulator may be a scalar, an `Array` pointer value with supported fixed-width elements, a product, a structure, a nonrecursive tagged value, or a recursive-inductive pointer value, provided its flattened representation contains no `ByteArray` field.  The accepted loop body may use `let mut` assignments that elaborate to local lets.  `ByteArray` accumulator values, `ForInStep.done`, `break`, range loops, maps, polymorphic iterators, and monadic effects are unsupported.
