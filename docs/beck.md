...
Source proofs establish that the executable constructs a nonzero direction preserving every protected category, gives frozen jobs zero direction, and bounds every coefficient's magnitude by 120.  The boundary scan chooses the first minimum boundary ratio using exact word-sized products.  A round preserves coordinate bounds and protected category sums, keeps frozen jobs fixed, and freezes an additional job.  Starting from zero, the full rounding loop finishes with every job frozen, a nonzero denominator, and denominator at most `120^6`.  These theorems assume at most six jobs, binary incidence entries, and the overlap bound.  They use counting, determinant bounds, Cramer's identity, a bordered-determinant identity, and interval inequalities.  A general release lemma gives final discrepancy at most `2t-1` after a category has at most `t` undecided jobs.  Its connection through the full loop, input validation, allocation bounds, and exact-WASM execution remains work in progress.
