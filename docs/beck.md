...
Checked general lemmas establish that the protected category count is smaller than the undecided job count, moving to the first boundary freezes a coordinate, and releasing a category with at most `t` undecided jobs gives a final discrepancy at most `2t-1`.  Source proofs establish that the executable constructs a nonzero direction preserving every protected category, gives frozen jobs zero direction, and bounds every coefficient's magnitude by 120.  The proofs cover the live counter, flattened matrix, determinant recursion, maximal basis search, free-column search, and direction-array writes.  They use counting, determinant bounds, Cramer's identity, and a bordered-determinant identity.  Signed-word arithmetic lemmas establish denominator growth and update bounds under the rounding invariants.  Input validation, boundary selection, rounding invariants, and exact-WASM execution remain work in progress.
