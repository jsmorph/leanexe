...
The current `leanexegen` proof task cannot use most of this material.  Its import validator permits generated modules, `CodeLib`, `Interpreter`, `Mathlib`, `Std`, and `Init`, while rejecting `Project.Common`, `Project.WpScaffold`, and the runtime modules.  The proof prompt and isolated task context also omit a catalog, so an agent has neither permission nor a direct discovery path for repository-owned proof support.
...
The orchestrator should classify the generated module before starting Codex.  The initial feature set should distinguish scalar arithmetic, branches, loops, calls, memory reads and writes, allocation, retain and release, recursive ownership, and function-index transport.  The task context can then identify relevant catalog sections and a small number of completed examples while leaving the complete library available through ordinary Lean imports.
...
A from-scratch `leanexegen` run then generated the same WASM artifact as the retained baseline and used both cataloged tactics.  The behavior proof fell from 329 to 321 lines, and the independent verifier accepted the packaged artifact theorem.  Stage five took 253.925 seconds, compared with 238.557 seconds in the baseline, so the run demonstrates discovery and proof reduction but supplies no evidence of faster generation; the [retained process streams](../demo/README.md#live-proof-kit-generation) record the measurement.
...
| 7. Distillation cycle | Review each later proof for repeated semantics, mechanical rewrite sequences, expensive boundaries, and missing catalog entries. | A promoted declaration satisfies the two-consumer rule, has a focused test, updates the catalog, and does not regress measured consumers beyond an explained bound. |
...
Performance evidence should measure elaboration rather than source line count.  Each promoted tactic or scaffold needs before-and-after times for its first two consumers, along with the target and toolchain identity.  A timeout without a diagnostic requires a smaller module or theorem boundary before another unchanged run, following the repository proof policy.
