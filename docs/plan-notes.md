...
Parameterize shared theorems, frames, and tactics over generated local indices, counts, strides, and capacities when two artifacts repeat an instruction pattern.  The earlier rule that kept all local-index manipulation in artifact modules is withdrawn: the completed depth work showed that abstract-frame theorems with index hypotheses (`missingSearchProg_empty`, `missingBumpProg_spec`) compose as cleanly as concrete adapters and remove the clone modules.  An artifact module should shrink to instantiations of shared parameterized theorems plus the facts unique to its semantics.
...
| Depth empty-search loop | [`missingSearchProg_empty`](../proofs/talos/lean/Project/ClobDepth/MissingSearch.lean) | Direct for depth branches | Preserve any 26-local frame whose free-head local 26 is zero.  First consumers: the missing branch composition and the found allocation chain. |
...
Parameterized instruction theorems are now in scope.  A shared theorem may take generated local indices, instruction slices, strides, and capacities as arguments when the same pattern appears in more than one artifact, and the overhaul in progress replaces the per-artifact adapter clones with such theorems.  Measure each candidate by the elaboration cost of its abstract statement against the concrete clones it removes, and record the comparison here.
