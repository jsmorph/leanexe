...
2. The interpreter package is third-party AGPL code in the Talos repository, pinned by commit.  The decision: fork it, instrument the model in the fork, pin the proof workspace to the fork, and offer the instrumentation upstream as pull requests, moving the pin back to upstream as they merge.  Instrumentation should be additive — new definitions and theorems beside `run`, with `run` itself unchanged where possible — to keep the pull requests small and mergeable.  The fork stays private and internal, which keeps AGPL obligations dormant.
...
4. Instrument the forked model once with a cost semantics: step count, frame depth, and a heap watermark serving the time, stack, and heap rows together, offered upstream as one reviewed change.
