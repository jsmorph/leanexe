...
The production annotation command rebuilt the exact Demo 1 artifact digest, generated the transition equations, and completed independent package verification.  The proof recipe names the lifted condition and body equations and instructs the proving agent to rewrite with them before reducing either descriptor evaluator.  A matched reproof will determine their effect on Stage 5 time and accepted proof structure.
