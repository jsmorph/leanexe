...
Every validated region contributes task features and candidate annotation kinds to structured LTG retrieval.  The task bundle includes only entries that pass category, feature, annotation, maturity, consumer, and artifact-exclusion filters.  Proof recipes name the resulting declarations and tactics so the agent can retrieve detailed guidance after inspecting its residual goal.
