...
The recipe registry succeeds only if attempted proof assets reduce total proof-generation time across the corpus.  Accepted proof structure remains secondary evidence: fewer proof steps, less local scaffolding, or greater shared theorem use can reveal a useful improvement and justify refinement, but raw identifier length has no negative weight and structural improvement cannot promote a slower configuration by itself.  The retained attempt records should identify which direct, indirect, and guidance methods earned their place in the active registry.
