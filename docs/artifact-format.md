...
Artifact verification starts with the exact bytes of one WebAssembly binary.  A Lean `ByteArray` contains those bytes, and a checked file comparison connects that value to the distributed `program.wasm`.  The proof checker decodes that value without invoking LeanExe.  A source-agreement theorem imports the Lean definitions that specify its behavior.
...
The type section accepts function types whose parameters and results contain `i32`, `i64`, and `f32`.  The function section retains every type index, while the code section retains every local declaration group and structured instruction body.  Validation later checks all type indices, the agreement between function and code counts, the accepted function-result arity, and the expanded local count.
...
The code profile consists of `unreachable`, `drop`, structured `block`, `loop`, and `if`, direct branches, `return`, direct calls, local and global access, integer scalar constants, the integer operations emitted by LeanExe, `f32.add`, `f32.sub`, `f32.mul`, `f32.div`, `f32.sqrt`, `i32.reinterpret_f32`, `f32.reinterpret_i32`, their binary64 counterparts, six memory operations, and memory size or growth.  Quantized inference adds `f32.nearest`, `i32.trunc_sat_f32_s`, `f32.convert_i32_s`, and `i32.extend8_s`.  The saturating conversion uses prefix `0xfc` followed by unsigned-LEB subopcode zero.  The decoder accepts legal nonminimal encodings of that subopcode and rejects other subopcodes.  Structured control accepts empty, `i32`, `i64`, or `f32` block results.  Floating-point constants enter through integer reinterpretation.  The decoder rejects `f32.const`, `f64.const`, and explicit binary64 parameter, result, local, and block types.  Binary64 values can occur on the operand stack.  Memory instructions retain alignment and offset immediates, and memory size or growth retains the memory index that validation restricts to zero.
...
When current compiler output changes, a historical binary can retain its execution cache in `FrozenProgram` and its specification in `FrozenSpec`.  Their declarations use the case's `Frozen` namespace.  Manifest validation requires that complete naming convention and maps every registered behavioral theorem into that namespace.  The artifact theorem still proves equality with the decoded frozen bytes.  The current `Program` and source specification can then change without altering the historical binary's proof subject.
...
historical inputs.  Both current warm gates must produce matching receipts,
after which an immutable source revision and the deferred cold-checkout gate
remain before the new record can become ready.
