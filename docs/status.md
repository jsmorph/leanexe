...
| Byte I/O | `ByteIO.read` and `ByteIO.write` run through the nonblocking WASI host. The resumed native host, I/O, ownership, and non-release execution checks pass. The separate [byte-I/O gate](../proofs/byte-io/README.md) checks host-model contracts, byte-transfer protocol laws, and six exact-binary execution cases. C host, Wasmtime, and OS behavior remain external assumptions checked by execution tests. |
