...
ProofKit now exposes `EncodedIndexDecoder.resultFrame_get_of_ne`, `EncodedIndexDecoder.resultFrame_validIndex`, `FixedArrayAllocatorWindow.allocFrame_shape`, and `FixedArrayAllocatorWindow.allocFrame_validIndex`.  These declarations address repeated decoder and allocator frame reconstruction found in Demo 12, ClobDepth, and prior allocation-heavy demos.  Focused Lean checks accept both modules, while a new proof-generation measurement remains pending.
