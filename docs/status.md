...
Demo 12 supplies the accepted structurally different artifact: its five loops implement early-exit search, allocation, prefix copy, and shifted-suffix copy.  ProofKit now contains checked theorems for the compiler's one-word literal-key search, public erase-result reconstruction, and the exact raw-cell prefix and shifted-suffix loop pair under symmetric source-target nonoverlap.  The compiler emits `leanexe.array.find-idx-eq.v1` and `leanexe.array.erase-copy.v1`; a production annotation pass preserved Demo 12's digest, checked the copy region against `FixedArrayCopy.program 1 8 14 11 12 15`, and passed independent package verification.  The copy shape occurs in Demo 12 at width one and in the CLOB `matchFuel` and `limit` functions at width five, although the current CLOB proofs require stronger continuations and do not use the copy theorem.  An exploratory Demo 12 reproof completed the theorem in a focused check using the search and reconstruction support, but concurrent ProofKit editing prevented the ordinary package gate and leaves no accepted timing result.  The next clean fixed-artifact reproof will evaluate the settled search, copy, reconstruction, annotation, and LTG support before the compiler-theorem-directed experiment continues.
