...
ProofKit now exposes `EncodedIndexDecoder.resultFrame_get_ne`, `EncodedIndexDecoder.resultFrame_validIndex`, `FixedArrayAllocatorWindow.allocFrame_shape`, and `FixedArrayAllocatorWindow.allocFrame_validIndex`.  The decoder getter preserves parameter, valid internal-local, and out-of-range reads outside the scratch pair and destination.  These declarations address repeated decoder and allocator frame reconstruction found in Demo 12, ClobDepth, and prior allocation-heavy demos, while a new proof-generation measurement remains pending.
