...
| Compiler theorems | The [general source-to-exact-WASM theorem](arithmetic-correctness.md) covers the admitted UInt64 grammar: arithmetic, lets, conditionals with propositional and Boolean negation, pure Id operations, local functions, and one bounded strided range loop with continue and break. It proves exact bytes, full module validation and terminating exported invocation. Loop steps include monadic/result bindings and scalar or step-result functions of two UInt64 arguments. The full Boolean-negation checkpoint passed all nine compiler audits and 3,298 native Lean/V8 comparisons across 165 declarations. Local helpers defined before loops then passed all nine audits and 2,737 focused comparisons across 115 range declarations, preserving all 105 preceding range modules byte for byte. Ordinary lets of range results then passed all nine audits and 2,977 focused comparisons across 125 range declarations, preserving all 115 preceding range modules. Earlier compiler-side scalar-certificate theorems continue to cover selected IR emitters used by annotation checks. Broader dialect coverage is being added incrementally. |
