...
| Compiler theorems | Compiler-side scalar-certificate theorems prove agreement between selected IR emitters and the structured WASM instruction sequences used by annotation checks.  The current Phase 3 increment applies this path to the recurring zero-or-index-plus-one decoder.  A [general source-to-exact-WASM correctness theorem](arithmetic-correctness.md) now covers the admitted UInt64 scalar grammar: arithmetic, lets, conditionals, pure Id operations, local functions and one bounded range loop with continue and break, including full module validation and terminating exported invocation. The range checkpoint passed 582 native Lean/V8 comparisons across forty-one declarations; subsequent loop-step increments culminate in 1,129 focused comparisons across forty-eight range declarations and all nine general compiler audits, covering local functions, monadic bindings, branch continuations, continue, early exits, direct step constructors, standard Nat literal bounds and nested Id/result bindings and functions taking complete step results. Inequality and repeated propositional negation then passed all nine audits and the full 1,764-result/94-declaration engine group; all eighty-two earlier modules retained identical bytes. Nonzero literal starts then passed all nine audits and 1,465 focused comparisons across sixty-two range declarations, preserving all fifty-one preceding range modules byte for byte. Dynamic starts then passed all nine audits and 1,729 focused comparisons across seventy-three range declarations, preserving all sixty-two preceding range modules byte for byte. Positive literal strides then passed all nine audits and 2,017 focused comparisons across eighty-five range declarations, preserving all seventy-three preceding range modules byte for byte. Two-argument local scalar functions then passed all nine audits and the full 2,850-result/143-declaration engine group, preserving all 128 preceding modules byte for byte. Two-argument step-result functions then passed all nine audits and 2,401 focused results across 101 range declarations, preserving all 91 preceding range modules byte for byte. Broader dialect coverage is being added incrementally. |
