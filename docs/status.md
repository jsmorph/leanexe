...
| Compiler theorems | Compiler-side scalar-certificate theorems prove agreement between selected IR emitters and the structured WASM instruction sequences used by annotation checks.  The current Phase 3 increment applies this path to the recurring zero-or-index-plus-one decoder.  A [general source-to-exact-WASM correctness theorem](arithmetic-correctness.md) now covers successfully admitted UInt64 arithmetic declarations, including full module validation and terminating exported invocation. Broader dialect coverage is being added incrementally. |
