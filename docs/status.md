...
This report describes the repository state on 2026-09-24.  The source-driven registry contains seventy-two Talos cases, of which seventy-one are complete.  The separate exact-artifact registry contains forty-six frozen packages, and the source-driven proof tree tracks one untrusted `Program.lean` execution cache for each of its seventy-two cases.  The demonstration index contains eleven current array-interface programs and the original scalar example.  The root [Development Plan](../plan.md) owns remaining work, while repository tools and registries own changing counts and release identities.
...
| Quantized GPT-2 | The [scalar projection](../proofs/talos/lean/Project/Gpt2QuantizedLinearRows/README.md) has checked accumulation, execution, ownership, release, and exact-binary proofs.  Its independent package check passes for the 4,757-byte binary.  The [grouped projection](../proofs/talos/lean/Project/Gpt2QuantizedGroupedRows/README.md) now has a complete exact-binary theorem and a passing independent package check for its 5,441-byte module.  All 38 focused quantization cases pass.  The grouped cached candidate matches 6,432,896 independent-reference logits and every cache through 128 prefixes, passes rejection and cleanup tests, and reproduces all nine grouped completion streams.  Greedy choices agree with FP32 on 120/128 retained prefixes and 85/101 completion-prompt prefixes.  Three measured warm traces give a 3.59× median speedup and 34.7% lower WASM linear memory.  The 127,695,972-byte model reduces weight storage by 74.3%.  The [evaluation record](../data/gpt2-quantized-v1/README.md) retains identities, failures, comparisons, and measurements.  The complete cached session has checked execution, termination, allocation, and buffer-release proofs.  The exact 28,315-byte package passes independent verification and the declaration audit with standard logical axioms.  Conditional numerical propagation and its outward-rounded session evaluator pass.  Forward bounds certify no token choices across 302 evaluated prefixes.  Separate certificates from measured logits establish 232 individual greedy choices.  The complete execution, WAT/binary comparison, and conformance suites pass.  The final aggregate source and clean-checkout artifact checks remain active. |
| Source-driven proofs | The registry contains seventy-two source entries and seventy-one completed specifications, including the FP32 and quantized GPT-2 sessions, all eight CLOB exports, and the complete Riemann solvers.  Current-source proof repairs pass their focused checks.  The aggregate source check is running.  The incomplete tiny GPT-2/128 sequence case remains paused. |
| Exact-artifact proofs | The 2026-09-24 aggregate passed all forty-six frozen packages, their behavioral specifications, and registered declaration audits.  Each package embeds exact bytes, decodes and validates them, proves translation equality with its Talos execution module, and connects that module to its behavioral theorems.  A clean-checkout repeat remains pending after the final shared proof changes. |
...
The 2026-09-19 aggregate artifact receipt covers the 43 packages registered then.
