...
The current demonstrations show useful transfer for fixed-array wrappers, searches, maps, filters, scalar post-test loops, array folds, frame accessors, allocator composition, and singleton results.  Closely related addition, multiplication, and XOR folds do not establish transfer to a different accumulator or control-flow shape.  The root [Development Plan](../plan.md) therefore requires a structurally different fold before broader promotion of the current completion adapters.
