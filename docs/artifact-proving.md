...
The artifact package checks the descriptor again against the exact decoded region.  Its JavaScript matcher checks every decoder opcode, branch body, constant, and local role, and generated Lean proves equality between the selected interval and `EncodedIndexDecoder.program`.  Recipe version two records the corresponding resolved-tail theorem in `direct.tailEquality`; the same field now covers every direct semantic recipe with a checked program boundary.  The generator remains outside the trust boundary because Lean accepts each declaration only when reduction over the exact decoded program proves it.
