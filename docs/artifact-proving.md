...
The artifact package checks the descriptor again against the exact decoded region.  Its JavaScript matcher checks every decoder opcode, branch body, constant, and local role, and generated Lean proves equality between the selected interval and `EncodedIndexDecoder.program`.  The consumer also generates `<region>_tail_eq`, which decomposes the resolved artifact tail at the region start into `EncodedIndexDecoder.program` followed by the resolved tail at the region end.  The tail generator remains outside the trust boundary because Lean accepts the declaration only when reduction over the exact decoded program proves it.
