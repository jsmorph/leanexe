...
`record` turns an accepted proof package into an experimental knowledge package containing the accepted proof, frozen Talos program, generated annotation theorems, journal, annotations, recipes, task features, telemetry, and prior knowledge identity.  Each invocation receives a generated attempt identity, so repeated records over one accepted proof remain separate packages.  Its catalog entry excludes the motivating artifact, and the package remains outside every selected forest until promotion.

`propose` gives the same run evidence and archived knowledge snapshot to a separate headless Codex task.  The task can inspect the exact generated artifact adapters, search package indexes, inspect checked ProofKit source, and import package-local modules selected for the accepted proof.  A checked candidate records the packages that own its direct imported modules, while guidance and worked examples record no Lean dependencies.  A candidate becomes an experimental package, while a declined proposal records its generated proposal identity in `learning-report.json` beside `proposal.json` and `learning-journal.md`.
...
The artifact-proof workspace includes `PROOF_IMPORT_CHECK.js`, generated from the same proof-kit and generated-module allowlist used by outer acceptance.  Codex runs this check before every prescribed Lean build, so an unsupported direct import becomes an in-session diagnostic rather than a publication-time rejection.  The live knowledge view contains the selected catalogs and checked Lean sources but omits package evidence, including prior proof sources and journals; the published package archives the complete filtered knowledge task.  `KNOWLEDGE_USE.json` records one used or rejected result and reason for each knowledge entry the agent inspected.  Outer acceptance repeats the import audit independently and publishes the knowledge-use record only after accepting the proof.
...
The [knowledge forest](ltg.md) contains separately versioned packages of checked proof assets, compiler-motif support, guidance, proof-generation methods, and worked examples.  Each package retains its own category JSONL indexes and canonical entries, while the forest selects the packages available to one run.  Every proof task receives an artifact-filtered forest view and the selected strategy guide.  Schema-nine packages preserve the exact forest, package versions, package-local Lean sources, exclusions, content digests, and accepted knowledge evaluation used during proof generation.
...
The [artifact-proof strategy notes](proof-strategies.md) describe proof structures distilled from accepted Talos proofs.  Extractor version ten computes generic control and resource features from the frozen `Program`, then derives equality-node, search-key, length-dispatch, literal-key first-match, erase-copy, and encoded-index-decoder features from annotation matches against the decoded instructions.  The erase-copy feature records its source width and five local roles, while each decoder records its encoded source, scratch start, destination, and encoding.  Leanexegen supplies only the matching marked sections, and the generated `Behavior` module must contain every checked proof term used by the artifact theorem.
...
Current generation and current-interface reproof publish schema nine, which archives the artifact-filtered knowledge forest and accepted knowledge evaluation.  Schema eight archived the forest without an evaluation, schema seven archived one structured LTG catalog, and schema six introduced the `heapReserveBytes` formal interface without a catalog archive.  The verifier accepts schemas three through nine, preserving the recorded boundary of each historical package.  New knowledge-task records use schema two and exact artifact digests, while validation retains schema-one records that contain derivative-group fields.
...
| `knowledge-task.json` | Selected package versions, dependencies, exact-artifact filter, entry exclusions, package-local Lean modules, and forest and package digests. |
| `knowledge-evaluation.json` | Inspected entries used or rejected with reasons, accepted proof identity and size, and Stage 5 time. |
...
The final theorem states the fixed formal property of the validated Talos translation of one embedded WASM byte sequence.  A newly generated sidecar binds that sequence to SHA-256, byte length, theorem names, task-source hashes, task reports, exact Lean 4.34.0-rc2, Talos revision `87e3aa5e8f6e6f3b3eb5e7e4c5aba43071002d47`, the proof-workspace manifest, and the verifier-source digest; historical sidecars retain their recorded tool pins.  The declaration audit rejects `sorryAx` and axioms outside the artifact gate's configured allowance.
