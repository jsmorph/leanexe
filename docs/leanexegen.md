...
The [artifact-proof strategy notes](proof-strategies.md) describe proof structures distilled from accepted Talos proofs.  Leanexegen computes generic control and resource features from the frozen `Program`, then derives annotated equality-node, search-key, and length-dispatch features from the annotation matcher that checks those regions against the decoded instructions.  It supplies only the matching marked sections, and the generated `Behavior` module must contain every checked proof term used by the artifact theorem.
