...
`propose` gives the same run evidence and archived knowledge snapshot to a separate headless Codex task.  The task searches the active package indexes, inspects relevant entry bodies, and examines compiler recipes and checked ProofKit source before selecting a missing proof boundary.  It may propose one guidance entry, worked example, checked lemma, or checked tactic, or it may conclude that the run supplies no useful entry.  A candidate becomes an experimental package, while a declined proposal becomes a small assessment directory containing `learning-report.json`, `proposal.json`, and `learning-journal.md`.
