...
The invalid-module stage extracts each configured official module and requires the exact decoder or validator error constructor recorded in the configuration.  `wasm-tools` adds custom name sections when encoding text-origin `assert_invalid` modules, so the command strips custom sections from those cases before classification.  It preserves raw `assert_malformed` binary modules byte-for-byte, and the 2026-08-11 run matched all fifteen classifications, including truncation, version, section, integer-width, alignment, stack, and memory-limit failures.

Talos executes supported assertions and reports unsupported command kinds as skips, so the invalid-module stage supplies separate evidence for the artifact decoder and validator.  The 2026-08-11 execution run produced 3,853 passes, six known assertion failures, and 627 skips in Talos, while Wasmtime passed all twenty-five files with `function-references=y`.  The command warns only when the failures exactly match the six imported-memory rows recorded for `memory_grow.wast`; an upstream repair removes the warning, while any changed or additional failure stops the gate.
