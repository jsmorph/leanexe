...
The definitions below combine independently stated runtime typing rules with an implementation-indexed acceptance relation.  Central acceptance premises, including recursor recognition and expression extraction, refer to implementation function graphs.  The complete acceptance discipline has no mechanized soundness theorem.  The [independent core type-safety development](type-safety.md) proves progress and preservation for a smaller, separately specified first-order language; correspondence with this acceptance discipline remains open.  A rule marked “implemented” names the executable predicate that fixes its premises.  A proposed preservation statement for extraction appears only under proof obligations.
