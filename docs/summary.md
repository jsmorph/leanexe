...
Talos verification lives in `proofs/talos`, described in the workspace [README](../proofs/talos/README.md).  The source-driven path emits WASM, renders WAT, asks Talos to generate a module value, and checks a handwritten specification; all twenty registered cases have completed specifications.  The exact-artifact path starts from twenty frozen binaries and proves byte identity, sound decoding, profile validity, equality with each specification's execution module, and the registered behavior without reading source or invoking the compiler.

The workspace organizes the artifacts around a shared proof library.  The four runtime functions every module ships are pinned to shared definitions by `rfl` checks, and their behavior is proved generically over the module and function index.  A generic teardown theorem, `release_frees_tree`, covers recursive release of slots-kind ownership trees, while shared tactics handle read-over-write and address-normalization goals.

Five repository tools implement model generation, source-driven proof, exact-artifact proof, semantic conformance, and release evidence.  The source preparation tool refreshes ignored WASM and WAT and tracked `Program.lean` proof caches, while the source proof tool compares generated candidates without changing tracked caches and the artifact tool consumes only registered frozen packages and the proof workspace.  [Verifying a Program](verifying.md) documents every input, output, human decision, and final-gate failure.

## Documentation

The [Language Specification](spec.md) defines the accepted subset, ABI, memory management, numeric semantics, and unsupported behavior.  The [LeanExe User Manual](manual.md) teaches source patterns and diagnostics, while [Developing LeanExe](../DEVELOPING.md) defines setup, local gates, generated files, and troubleshooting.  [Talos Proofs](../proofs/talos/README.md) owns the current theorem inventory, [Verifying a Program](verifying.md) owns the proof procedure, the [Development Plan](../plan.md) owns future work, and the [Development Journal](../devnotes.md) owns history and test records.  The old [Development Agenda](history/agenda.md) is archived, and the two `leanexe-talos` documents are marked as historical experiments.

## Current State and Observations

Several structural facts govern further work.  Extraction remains concentrated in the large mutual block in `LeanExe/Extract/Core.lean`, despite the surrounding module splits.  A semantically neutral code-generation change can move every generated model and require proof repair; the runtime lemma library reduces that work, but entry proofs still track the current instruction stream.  The gap between the Talos WASM model and Wasmtime execution remains a trusted-base assumption.  The compiler releases constructor fields, fold sources and accumulators, and owned temporaries in supported positions, but it does not check the source-level ownership precondition for `LeanExe.Runtime.release`.  The formal stages between differential testing and artifact proofs, including `extract_correct`, verified IR passes, and a general `wasm_lower_correct`, have no proof development in this repository.
