...
Artifact verification lives in the Talos proof workspace at `proofs/talos`, described in the workspace [README](../proofs/talos/README.md).  LeanExe emits WASM, `wasm-tools print` renders WAT, Talos decodes the WAT into a Lean module value in a generated `Program.lean`, and a handwritten `Spec.lean` proves a theorem about the decoded module.  Nineteen completed proofs cover scalar algorithms, recursive data, byte processing and allocation, exact runtime accounting, the self-compiled unsigned LEB128 encoder, and the completed CLOB exports; the proof README gives each theorem's exact scope.
...
Two tools implement the artifact and proof phases.  They regenerate ignored WASM, WAT, and `Program.lean` outputs from the current source and compiler, while creating Talos's Cargo-shaped input in a disposable temporary directory.  [Verifying a Program](verifying.md) documents every input, output, human decision, and final-gate failure.
