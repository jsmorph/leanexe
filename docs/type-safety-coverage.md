...
| Structural equality | Numeric/Boolean source equality; exact raw value/list comparison; independent `EqTy` domain with a proved exact terminating checker. | No compound source equality primitive yet. Integrate strict evaluation and the checked admission domain with all metatheory. Bytes remain absent. Arbitrary selected `BEq` functions are separate. |
