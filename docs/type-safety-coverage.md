...
| Structural equality | Numeric/Boolean source equality; total raw value/list comparison with exact iff-equality proofs, including compound and finite recursive values. | No compound source equality primitive yet. Prove the documented acyclic `EqTy` admission domain and integrate strict evaluation with all metatheory. Bytes remain absent. Arbitrary selected `BEq` functions are separate. |
...
| Option/Except APIs | Sums and nominal patterns supply representation and elimination ingredients. | Choose representations and prove constructors, map/bind/filter/default/tests/fallback/map-error/conversion APIs. Payload-discarding observers need an explicit policy consistent with relevance; dummy uses are not a solution. Captured callback expansions may need proved renaming. |
