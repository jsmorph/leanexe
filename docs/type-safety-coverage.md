...
| Boolean operations | Bool literals and elimination; conditional expressions can express Boolean functions. | Add and prove named derived NOT/AND/OR/XOR/equality forms, specifying strict versus conditional operand evaluation. |
| Structural equality | Numeric equality comparisons only. | Specify the equality domain and computation for products, sums, arrays, bytes, and data. Do not confuse structural equality with arbitrary user-selected `BEq` functions. |
...
| Option/Except APIs | Sums and nominal patterns supply representation and elimination ingredients. | Choose representations and prove constructors, map/bind/filter/default/tests/fallback/map-error/conversion APIs. A representable type is not already a verified combinator library. |
