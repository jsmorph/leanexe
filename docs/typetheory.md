...
All source and proof claims inherit the trust properties of their pinned Lean kernel.  The compiler root now pins exact Lean 4.34.0-rc2.  During the staged `talosfp` migration, the existing artifact proof workspace still pins Lean 4.31.0 and retains the known kernel-unsoundness record described in the [artifact verification format](artifact-format.md).  No mixed-toolchain state is accepted as new proof evidence; aggregate proof gates resume after the proof workspace reaches the same 4.34.0-rc2 pin.
