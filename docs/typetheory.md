...
All source and proof claims inherit the trust properties of the pinned Lean kernel.  This repository pins Lean 4.31.0 and records a known kernel-unsoundness reproduction accepted by that version.  The project owner accepted that defect for the artifact work after the narrow lexical audit recorded in the [artifact verification format](artifact-format.md); the audit does not repair the kernel, examine every dependency, or establish soundness for the Lean source theory.
