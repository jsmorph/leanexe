...
`LeanExe.Float64.addBits` and `mulBits` form a deliberately restricted numeric bridge: source-visible arguments and results remain `UInt64` bit patterns, while lowering reinterprets them as binary64 for one WebAssembly addition or multiplication and reinterprets the result back.  This bridge does not admit general Lean `Float`, `f32`, division, square root, classification, or floating-point comparisons.
