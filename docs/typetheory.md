...
All source and proof claims inherit the trust properties of the pinned Lean kernel.  This repository pins Lean 4.31.0 and records a known kernel-unsoundness reproduction accepted by that version.  The project owner accepted that defect for the current artifact release after a narrow lexical audit for the reproduction's environment-mutation identifiers, as recorded in the [verification review](reviews.md).  That audit does not repair the kernel, examine every dependency, or establish soundness for the Lean source theory.
