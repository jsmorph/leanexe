...
`LeanExe.Float64.addBits`, `subBits`, `mulBits`, `divBits`, and `sqrtBits` keep source-visible arguments and results as `UInt64` bit patterns.  Lowering reinterprets operands as binary64, applies the corresponding WebAssembly arithmetic instruction, and reinterprets the result back.  Square root has a unary IR expression; the other four operations are binary.  This interface does not admit general Lean `Float`, `f32`, classification intrinsics, or floating-point comparison instructions.
...
| Unsupported data and arithmetic | Runtime `String`, runtime `Char`, signed integers, general floating point beyond the two raw-bit binary64 intrinsics, arbitrary-precision runtime `Nat`, recursive structures, indexed inductives, and public recursive values |
...
All source and proof claims inherit the trust properties of their pinned Lean kernel.  The compiler root and artifact proof workspace now pin exact Lean 4.34.0-rc2, and the proof workspace pins Talos revision `87e3aa5e8f6e6f3b3eb5e7e4c5aba43071002d47`.  The known Lean 4.31.0 kernel-unsoundness record described in the [artifact verification format](artifact-format.md) remains historical evidence for the earlier release inputs, not evidence for this migration.  The prior aggregate artifact result is likewise historical for its exact input; matching aggregate proof, semantic conformance, immutable source identity, and cold-checkout evidence remain required before a current release claim.
