...
The first specialization slice is implemented.  LeanExe recognizes class evidence through Lean's imported class-extension entries without enabling imported initializer execution, treats that evidence as static in inline-specialized helpers, and runs a bounded normalizer over specialized bodies so method projections reduce to ordinary first-order expressions.  The comparison suite covers built-in `BEq`, a custom `BEq` instance whose method disagrees with structural equality, built-in `Inhabited`, a source-defined `TypeclassScore` class over scalars and structures, a dependent `Option` instance, class methods inside `Array.foldl`, `Array.any`, and `Array.find?` direct-lambda callbacks, and generic helpers whose specialized bodies call `List.foldl` or `List.find?`.

Direct `List.any` remains supported in its accepted closed-predicate shape, but generic class-constrained helpers that call `List.any` still need a separate predicate-extraction improvement.  The current implemented path follows class evidence through inline specialization and discovers the structural recursion required by the specialized list helper body.  The follow-up belongs in the generic closed-predicate lowering path, because direct class evidence already normalizes before extraction.

Runtime dictionaries remain unsupported.  The compiler now rejects public entries that expose class evidence with a specific `runtime class evidence is not supported` diagnostic, while the report command describes internal evidence-bearing declarations as static-specialization requirements.  The generated WASM for this slice should contain ordinary specialized code, and no exported or internal function should receive class evidence as a runtime argument.  If evidence does not normalize to accepted first-order code, the program must reject rather than compile a partial dictionary representation.
