...
An array-map instruction carries per-element local bindings when its result spans several slots.  Extraction materializes that result once and records its field slots and ownership information.  The backend executes the bindings inside the traversal loop before storing the fields.  Call-index shifting, scratch bounds, liveness, and release analysis include those bindings.
