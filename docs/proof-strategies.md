...
Apply `Project.ProofKit.FixedArraySearchTree.Tree.program_spec` when the checked equality, less-than, and pair-result regions form a complete fixed binary search tree.  A `Tree.branch` or `Tree.leaf` descriptor records every array index and result destination, while `Tree.program` supplies the exact artifact fragment and `Tree.result` supplies its semantic lookup function.  Prove the descriptor's `Tree.Valid` predicate from the fixed input size, rewrite the decoded region with its generated `AnnotationMatches` equality, and leave one equation between the formal expected result and `Tree.result`.
