...
Selection distinguishes generation provenance from theorem dependencies.  Current package schema 7 archives the extracted notes, feature manifest, source-document digest, extractor version, annotations, proof recipes, LTG task bundle, and their identities, allowing a later audit to reconstruct the proof agent's context.  Independent Lean verification does not import prose files or require the checkout's guidance to match, because the accepted `Behavior.lean` contains every proof term that reaches the theorem.

The structured `LTG/` tree has a different role.  Its category indexes and canonical entries describe checked declarations, compiler motifs, proof methods, and worked examples that Codex may inspect, while each imported proof-kit declaration contributes to the checked proof and verifier-source identity.  Leanexegen presents the task-filtered LTG tree and selected strategy notes separately, and the retained `Behavior.lean` records which allowed modules the final proof imported.
