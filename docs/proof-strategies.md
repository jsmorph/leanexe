...
A dynamic `Array.findIdx?` scan needs an invariant that relates the current index to `Array.findIdx?.loop`, the encoded option result, the represented input, and the generated local frame.  Rewrite one step with `Array.findIdx?.loop.eq_def`: the exhausted case establishes `none`, a matching element establishes `some index`, and a nonmatching element advances the invariant and decreases `input.size - index`.  Convert between `input[index]` and `input[index]!` with `getElem!_pos` only at the machine-load boundary so the semantic invariant retains the standard array notation.

An in-bounds `Array.eraseIdx!` result divides into allocation, a prefix copy over `[0, index)`, a shifted-suffix copy from `[index + 1, input.size)`, and representation reconstruction.  Carry the represented source array through both loops, state each invariant by its completed destination interval, and use `Array.getElem_eraseIdx_of_lt` and `Array.getElem_eraseIdx_of_ge` to identify the two output ranges at completion.  Demo 12 proves this composition locally; a checked continuation-generic interface for the encoded search and copy-shift regions remains open proof-library work.
