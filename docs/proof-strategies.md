...
Selection distinguishes generation provenance from theorem dependencies.  Current package schema 8 archives the extracted notes, feature manifest, source-document digest, extractor version, annotations, proof recipes, knowledge-forest task snapshot, and their identities, allowing a later audit to reconstruct the proof agent's context.  Independent Lean verification does not import prose files or require the checkout's guidance to match, because the accepted `Behavior.lean` contains every proof term that reaches the theorem.
