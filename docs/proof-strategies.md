...
State checked projection lemmas immediately after defining a proof-local fold frame.  Record its parameters, internal-local length, value stack, and every getter used by setup, body, result, or measure theorems, even when each proof is `rfl`.  Pass these facts as named arguments at composition boundaries so elaboration does not unfold the frame into a long update sequence before a supplied theorem can match it.
