...
An in-bounds `Array.eraseIdx!` result divides into allocation, a prefix copy over `[0, index)`, a shifted-suffix copy from `[index + 1, input.size)`, and representation reconstruction.  Carry the source reads through both loops and state each invariant by its completed destination interval, then apply `Project.ProofKit.UInt64Array.At.eraseIdx!_of_reads` to the target length, unchanged-prefix reads, and one-position-shifted suffix reads.  The theorem reconstructs the public target representation with `Array.getElem_eraseIdx_of_lt` and `Array.getElem_eraseIdx_of_ge`; allocation and copy-instruction execution remain separate artifact obligations.
