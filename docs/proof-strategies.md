...
These notes describe proof strategies that recur in checked Talos artifact proofs.  They concern the `Program` produced from exact WASM bytes and the `Wasm.TerminatesWith` or weakest-precondition theorem proved about that program.  [Artifact Proving](artifact-proving.md), structured [LTG entries](../ltg/README.md), retained proof journals, and the checked modules cited below supply the evidence for each recommendation.
...
Twelve frozen `leanexegen` demonstrations add acceptance evidence for the generated array ABI and structured proof support.  Demo 1 combines a scalar computation with a singleton wrapper, Demo 2 proves an unrolled association-list search and two-word result, and Demos 4–12 exercise maps, filters, scalar loops, counter transfer, folds, and dynamic first-match removal.  The [demonstration index](../demos/README.md) defines each program and links to its representative proof and retained experiments.
...
The current `leanexegen` allowlist admits the ProofKit modules enumerated in `tools/leanexegen-lib.js`, generated package modules, Talos and Mathlib dependencies, and the exact-artifact verifier modules.  It excludes case-local `Project` modules such as CLOB proof files even when this guide cites them as evidence.  Codex must prove every remaining case-local fact from generated modules or an explicitly permitted checked dependency, and the independent import audit enforces that boundary.
...
Use `Project.ProofKit.Frame.internal_getElem?_of_get` or `internal_getElem_of_get` when a loop invariant states a combined `Locals.get` fact and symbolic execution requests an internal-list getter.  The declarations take the parameter count, internal index bound, and invariant fact, then produce the optional or indexed getter directly.  This avoids repeating `Wasm.Locals.get` unfolding, `List.getElem?_eq_getElem`, and `Option.some.inj` for every live loop local.

Prefer a semantic region's frame projections when the current frame already uses a ProofKit constructor.  `FixedArrayCapacity.capacityFrame_get_capacity` and `capacityFrame_internal_get_capacity` expose the normalized capacity in both coordinate systems, while `FixedArrayAllocatorWindow.allocFrame_get_root` exposes the returned array root.  Their frame-shape companions preserve parameter lists, internal-local lengths, and operand stacks across the named boundary.
...
Use `Project.ProofKit.FixedArrayLengthDispatch` when the wrapper's entry region stores the input pointer, checks the represented length against a fixed size, and selects valid or invalid code.  Run `wp_fixed_array_length_dispatch inputLocal, expectedSize` for normalized inequality, `wp_fixed_array_length_eq_dispatch inputLocal, expectedSize` for normalized equality, or `wp_fixed_array_length_le_dispatch inputLocal, maximumSize` for a direct unsigned upper bound; a checked `PROOF_RECIPES.json` entry selects the exact variant.  Use the corresponding `_from hInput at inputLocal, expectedSize` form when Lean cannot infer the represented pointer and array before it generates frame premises, then apply the boundary before processing query initialization or result allocation.

A `leanexe.array.fold.v1` recipe identifies a fixed-array traversal nested inside allocation or branch code.  Read its accumulator, element, index, effective-stop, done, staging, and result locals before defining the invariant, and use `ArrayFold.foldPrefix` as the semantic accumulator coordinate when the specification uses `Array.foldl`.  Apply `foldPrefix_succ` only at the continuing back edge and `foldPrefix_size` only after the checked guard establishes completion, while keeping allocator and input-preservation facts outside the prefix algebra.

For a forward, one-word, one-accumulator fold over the complete input array, use the generated `<region>_setup_eq` before reducing setup instructions.  `FixedArrayFold.forwardSetupProgram_spec` performs both checked length loads, initializes the index and accumulator, selects the effective stop, and passes the exact `forwardSetupFrame` to the loop continuation.  Use `FixedArrayTraversalInput.continuingProgram_exit_spec` when the index equals the effective stop, then rewrite with `<region>_result_eq` and apply `resultProgram_spec` without reducing the surrounding loop body or result suffix.

The generated `<region>_setup_frame_eq` normalizes `forwardSetupFrame` when its input uses the generated continuing-frame representation.  Its right side records the compiler-selected array, length, index, stop, accumulator, release, and effective-stop values while preserving every other generated local.  Apply this equality after rewriting the pre-setup frame into that representation so Lean does not reduce the complete local-update chain inside the loop theorem.

Use `FixedArrayFoldBody.continuingGuardedProgram_spec` on the strict-index branch when the recipe supplies the continuing loader, guarded step, loaded-frame equality, and scalar evaluator theorems.  Supply every structural and dependent premise by name, and fix `module_`, `env`, `st`, and `frame` when callback inference would generalize the weakest-precondition context.  Convert generated compact states with `.toState`, unfold only the proof-local frame aliases needed by the loaded-frame equality, and rewrite the generated continuing evaluator directly.

Use the generated continuing-frame accessors before writing proof-local projection lemmas.  Apply `<region>_continuing_frame_params`, `<region>_continuing_frame_locals_length`, `<region>_continuing_frame_values`, or `<region>_continuing_frame_get_<index>` through `simp only [<semantic-frame>, <generated-accessor>]`; the restricted simplifier unfolds the semantic abbreviation without reducing `Wasm.Locals.get` before the exact theorem can match.  After result placement, use `FixedArrayFold.resultFrame_get_result` for the written value and `resultFrame_get_of_ne` for a preserved distinct local, discharging their frame premises from the generated shape accessors.
...
A dynamic `Array.findIdx?` equality scan should first be compared with a generated `leanexe.array.find-idx-eq.v1` program equality and `Project.ProofKit.FixedArrayFindIdxEq.program_spec`.  The theorem covers the compiler's exact one-word forward loop for a literal `UInt64` key, exits on the first match, and emits zero for `none` or `encodedIndex index` for `some index`; scratch offsets, trailing locals, and continuations remain parameters.  The none and some callbacks receive the corresponding `Array.findIdx?` equation and named final frame, while the successful callback also receives `index < input.size`.

Keep `someFrame` folded when proving the successful callback.  Normalize the producer frame with `someFrame_params`, `someFrame_locals_length`, and `someFrame_values`, and use `encodedIndex_eq_ofNat_succ`, `encodedIndex_ne_zero`, `encodedIndex_not_lt_one`, and `encodedIndex_sub_one` instead of expanding the modular expression.  Apply the atomic scalar rules only until the following `iff`, block, or loop becomes explicit, then use the corresponding structured-control theorem.  Do not apply broad `wp_simp` to a term containing the nested search frames and a large continuation.

A version-two `leanexe.option.encoded-index.v1` recipe records `<region>_tail_eq` in `direct.tailEquality`, which rewrites the resolved artifact tail as the checked decoder followed by its exact suffix.  Apply that equation before `Project.ProofKit.EncodedIndexDecoder.program_spec`; use `<region>_eq` when the goal refers to the extracted region instead of its tail.  Supply the encoded source getter, internal-local bounds for the two consecutive scratch slots and destination, and the continuation from `resultFrame`.  Use `resultFrame_params`, `resultFrame_locals_length`, `resultFrame_values`, `resultFrame_validIndex`, `resultFrame_get_ne`, and `resultFrame_decoded` to enter the next proof boundary without selecting either nested decoder branch by hand.  `resultFrame_get_ne` preserves parameter, valid internal-local, and out-of-range getters without separate read-bound premises.

The decoder theorem handles only the emitted representation conversion from zero or index plus one to zero or the predecessor.  Retain the producer's equation and index bound across the decoder, because the destination value alone cannot distinguish no result from a successful result at index zero.  Search correctness, index validity, and the application branch therefore remain premises of the surrounding proof.

For a scan outside that exact shape, use the invariant `Array.findIdx?.loop predicate input index = input.findIdx? predicate` and the measure `input.size - index`.  Rewrite one step with `Array.findIdx?.loop.eq_def`, splitting the exhausted, matching, and continuing cases before executing the corresponding WASM branch.  This form covers dynamic keys, other predicates, wider elements, and different generated local layouts without treating the narrow annotation theorem as a match.

A `leanexe.array.erase-copy.v1` recipe identifies the complete block-wrapped prefix and shifted-suffix copy loops after allocation and the target-length store.  Rewrite with its generated region equality and apply `Project.ProofKit.FixedArrayCopy.program_spec` before defining proof-local loop invariants.  The theorem accepts raw cells at any recorded source width, either ordering of nonoverlapping source and target regions, complete 32-bit and memory bounds, and exact local getters.  Its continuation receives page, target-header, source-read, copied-prefix, shifted-suffix, and final-counter facts.

Use `prefixProgram_spec` or `suffixProgram_spec` when the proof needs a boundary between the loops.  Their postconditions retain the corresponding source and target interval facts, while `counterFrame_params_length`, `counterFrame_locals_length`, `counterFrame_values`, `counterFrame_get_counter`, and `counterFrame_get_ne` expose the final frame without unfolding its update.  Allocation, the target header store, result transfer, globals, and full outside-region or ownership framing remain separate proof boundaries.

For a one-word in-bounds `Array.eraseIdx!` result allocated after its source, apply `FixedArrayCopy.eraseIdxProgram_spec`.  This adapter accepts the represented source, erase-index bound, target bounds, and already-written target length, then applies `UInt64Array.At.eraseIdx!_of_reads` to establish the public target representation.  When the exact loop shape does not match or the continuation requires a stronger memory relation, carry the source reads through both loops and use `eraseIdx!_of_reads` directly after proving the prefix and shifted-suffix equations.
...
Use `Project.ProofKit.FixedArrayResult` when application-specific instructions separate allocation from result construction or produce values between payload stores.  Its continuation-generic `lengthStore_spec`, `lengthStoreLocal_spec`, and `payloadStore_spec` theorems cover the standard emitted address sequence at arbitrary combined-local indices, while `finishProgram_spec` covers the final root-to-destination-to-return transfer.  `lengthStoreLocal_spec` reads the dynamic length from a local and preserves the existing operand stack.  The `singletonStore_at` and `pairStore_at` theorems reconstruct the public representation from the named store transformers after the artifact proof has supplied the result values.
...
Check for a constant result-capacity recipe before proving the allocator premise.  `Project.ProofKit.FixedArrayCapacity.constantProgram_spec` executes the compiler's header-plus-payload calculation, eight-byte alignment, minimum-capacity branch, and capacity-local assignment for arbitrary constant result length and stride.  The length-dispatch consumer supplies separate exact equalities for matching valid and invalid branch prefixes, so a proof can continue from `normalizedCapacity` and `capacityFrame` without declaring branch-specific capacity theorems.

Apply `Project.ProofKit.FixedArrayAllocatorWindow.constantCapacityRegion_spec_withTail` when the constant prefix is immediately followed by the standard shifted allocator and writes combined local `offset + 9`.  The theorem derives the capacity-frame shape, internal capacity getter, and minimum-capacity fact before invoking the allocator theorem.  This boundary leaves the memory fit, allocator globals, and post-allocation continuation visible while avoiding a large intermediate weakest-precondition goal.

Use `capacityFrame_internal_get_capacity` for the shifted allocator's internal-local capacity premise.  After `region_spec_withTail`, use `allocFrame_get_root` for the returned-pointer premise of a length store or final root transfer.  These getters retain the semantic frame boundary and require only the frame dimensions already supplied to the capacity and allocator theorems.

Account for a uniform local-operand shift and unused trailing locals during template matching.  Demo 2's allocator has the same instructions as the checked fourteen-local allocator, but ten application locals shift every allocator operand and its root destination.  `Project.ProofKit.FixedArrayAllocatorWindow.region_spec_withTail` accepts both the offset and a trailing-local count, while `region_spec` retains the exact-frame specialization.  Supply the shifted capacity-local fact and the exact instruction decomposition before applying either theorem.
...
Selection distinguishes generation provenance from theorem dependencies.  Current package schema nine archives the extracted notes, feature manifest, source-document digest, extractor version, annotations, proof recipes, knowledge-forest task snapshot, knowledge evaluation, and their identities, allowing a later audit to reconstruct the proof agent's context.  Independent Lean verification does not import prose files or require the checkout's guidance to match, because the accepted `Behavior.lean` contains every proof term that reaches the theorem.

The structured catalogs under `KNOWLEDGE/packages/` have a different role.  Their category indexes and canonical entries describe checked declarations, compiler motifs, proof methods, and worked examples that Codex may inspect, while each imported proof module contributes to the checked artifact theorem.  Leanexegen presents the task-filtered forest and selected strategy notes separately, and the retained `Behavior.lean` records which allowed modules the final proof imported.
