...
Apply `Project.ProofKit.FixedArrayEqNode.program_spec` to a generated unrolled-search node that loads one indexed word, compares it with a saved key local, normalizes the equality Boolean, and selects two branch programs.  `keyFirstProgram_spec` covers the equivalent instruction order in which the artifact pushes the saved key before the checked load; it uses the stack-preserving `FixedArrayTraversalInput.program_stacked_spec` theorem.  Use `wp_fixed_array_search_key 10, 0, keyLocal` for the query initialization, followed by `wp_fixed_array_eq_node` for loaded-first nodes or `wp_fixed_array_key_eq_node` for key-first nodes, so Lean infers each following program and remainder.

When checked annotations enumerate a search-key region and several equality-node regions, follow the recipe order rather than scanning the complete program again.  State one `Project.ProofKit.FixedArrayEqNode.SearchFrame` after the key load, use `SearchFrame.afterLoad` for each saved-key premise, and use `SearchFrame.branch` before entering either child.  Represent the remaining nested continuations with `FixedArrayEqNode.branchN`; for a standard pair result, `Project.ProofKit.FixedArraySearch.pairPost_branchN_conseq` composes the result through that branch count.

Apply `Project.ProofKit.FixedArrayLtNode.program_spec` when a tree node pushes the saved key, performs the checked indexed load, compares the key with the element using unsigned less-than, and selects the next subtree.  The `wp_fixed_array_lt_node offset, index, keyLocal` tactic consumes the same `SearchFrame` as an equality node and returns semantic obligations for `key < input[index]` and its negation.  Follow the interleaved equality and less-than recipes in structured instruction order, since that order describes the generated tree traversal and preserves the comparison facts needed by each child.
