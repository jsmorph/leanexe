...
This result rejects the complete current annotation package as a proof-time improvement and leaves the scalar boundary unresolved.  The next controlled package will retain only the scalar-loop annotation and remove the wrapper and call recipes before proof planning, providing a causal comparison with the unannotated baseline.  The scalar journal still identifies descriptor-evaluator and scratch-state work, so checked branch-transition equations remain the next theorem increment if the isolated trial is neutral or slow.
