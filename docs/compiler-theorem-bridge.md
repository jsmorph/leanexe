...
The production Demo 1 package generated five semantic body branches and built both transition equations under Lean 4.31.0.  An independent `leanexegen verify` run accepted the package with the new proof-kit module included in its checked identity.  A matched reproof still determines whether the smaller application-level elaboration boundary reduces Stage 5 time.
...
The matched journals identify two next boundaries.  Checked one-step equations should expose the complete fixed-frame result for each loop branch without repeated descriptor-evaluator simplification, and a shared singleton-wrapper theorem should remove entry and allocator alignment from both comparison configurations.  The next fixed-artifact screen will test those changes before freezing or exposing the held-out scalar demo.
...
This theorem removes the wrapper instructions from the proving agent's semantic search, but it has no timing result yet.  The deterministic starter reaches the checked wrapper boundary and leaves the scalar callee theorem plus the two formal-result equations.  The checked fixed-frame transition equations now provide the second boundary, so the next matched Demo 1 screen can measure both additions together.
