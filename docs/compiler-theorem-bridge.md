...
The first matched Codex 0.147.0 experiment compared an unannotated baseline with the checked scalar descriptor while preserving the formal specification, Lean program, WASM digest, proof kit, task limits, and semantic tool pins.  The baseline completed Stage 5 in 2,017.931 seconds, while the descriptor trial took 3,692.913 seconds, an increase of 1,674.982 seconds or 83.0 percent.  Independent verification accepted both packages, and the descriptor proof used the exact decoded-region equality and `ScalarTransition.whileProgram_spec`.  The descriptor reduced the accepted source from 734 to 704 lines, but its journal records extra iterations proving descriptor-evaluator equalities, exact scratch-local states, and conversions between the descriptor state and WASM locals.
