...
The proof of concept succeeds if the descriptor remains generic, the exact artifact rechecks it, and the scorecard shows useful effects on proof construction.  The scorecard includes retrieval, agent revisions, accepted proof structure, shared theorem use, compiler-derived evidence use, applicability, and Stage 5 time.  Raw source bytes, word length, and identifier length have no negative weight because descriptive names often identify reused results.  Demo 6 should confirm that an improvement survives a different loop invariant, branch shape, and arithmetic operation before the method expands to array-loop bodies.
...
The annotation consumer now evaluates each scalar descriptor over a compact `UInt64` state and generates named equations for the condition and complete body transition.  Generic correspondence theorems lift those equations to `ScalarTransition.State`, preserving the exact descriptor program while summarizing intermediate scratch-local updates in one fixed-frame result.  Constant propagation removes compiler-generated Boolean normalization branches before Lean checks the generated equation, and two shared zero-or-one comparison lemmas handle the remaining Boolean word facts.

The production Demo 1 package generated five semantic body branches and built both transition equations under Lean 4.31.0.  A matched control retained every candidate annotation except the scalar-loop region, and both packages used the same complete singleton-wrapper composition.  Independent `leanexegen verify` runs accepted both packages with the new proof-kit module included in their checked identity.

The first Codex 0.147.0 experiment compared an unannotated baseline with the complete current annotation package while preserving the formal specification, Lean program, WASM digest, proof kit, task limits, and semantic tool pins.  The baseline completed Stage 5 in 2,017.931 seconds, while the annotated trial took 3,692.913 seconds, an increase of 1,674.982 seconds or 83.0 percent.  Independent verification accepted both packages, and the annotated proof used the scalar equality and `ScalarTransition.whileProgram_spec`, but the package also supplied length-dispatch and direct-call recipes.  Its journal records many iterations forcing the wrapper dispatch theorem and normalizing its branch frame, so the result does not isolate the scalar theorem's effect.

This result rejected the complete annotation package as a proof-time improvement and left the scalar boundary unresolved.  The following controlled package retained only the scalar-loop annotation and mandatory call recipes, removing the length-dispatch recipe before proof planning.  That isolated experiment established the descriptor's independent cost and identified descriptor-evaluator and scratch-state work as the next theorem boundary.
...
The matched journals identified two boundaries.  Checked one-step equations needed to expose the complete fixed-frame result for each loop branch without repeated descriptor-evaluator simplification, while a shared singleton-wrapper theorem needed to remove entry and allocator alignment from both comparison configurations.  The later fixed-artifact screen tested both changes before exposing a held-out scalar demo.
...
The matched control completed Stage 5 in 2,262.084 seconds, while adding the checked scalar-loop region reduced the candidate to 1,965.454 seconds.  The 296.630-second reduction, or 13.113 percent, is the first matched proof-time evidence that the compiler-derived scalar descriptor helps a WAT proof after artifact-side rechecking.  The final proof imports no compiler declaration: it uses exact decoded-region equality, neutral scalar semantics, and the generated fixed-frame equations.

The candidate used three condition-equation rewrites and five body-equation rewrites, reducing `wp_run` applications from 36 to 12 and proof lines from 643 to 635.  The control journal records repeated reconstruction of division guards, Boolean normalization, and branch sequencing that the candidate equations summarize.  Both journals identify entry argument order, fixed-store quantification, and loop-head frame conversion as the next general compiler-directed annotation boundary.
...
The isolated pilot rejects the raw scalar descriptor as a proof-time improvement.  Its journal and accepted source locate the remaining work in application invariants, descriptor evaluator cases, fixed-width representation, and wrapper alignment rather than WAT branch discovery.  The implemented fixed-frame transition equations address the evaluator boundary, and the next Demo 1 screen will measure them with the shared wrapper composition before any extension to array-loop bodies, calls, or allocation cut points.
