...
The proof of concept succeeds only if the descriptor remains generic and lowers median Stage 5 time.  Accepted proof structure remains secondary evidence: the current candidate's reduction from 722 to 674 lines and its increased shared theorem use justify refining the boundary, but they do not promote a configuration whose proving time regressed.  Raw source bytes, word length, and identifier length have no negative weight because descriptive names often identify reused results.  Demo 6 should confirm that a later improvement survives a different loop invariant, branch shape, and arithmetic operation before the method expands to array-loop bodies.
