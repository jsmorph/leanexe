...
| `OPA-011` | Exact generated accessors for a compact frame | Continuing-frame and result-frame projections used by traversal and singleton-suffix theorems | Generated accessors checked on Demos 9 and 10.  Demo 9 supplied complete fresh retrieval; Demo 10 used two generated accessors and both shared result getters but missed the LTG entry and discharged eleven frame facts by reduction.  Both reproofs were slower than their preceding fold-body screens. |
...
The task already contained the entry, selected frame strategy, and complete generated recipe, so the missing use reflects residual-goal retrieval rather than knowledge availability.  The proof prompt now requires another LTG and recipe query when a new residual goal class appears, with frame projections named as the first checked case.  A later fixed-artifact screen will test that instruction on a fold artifact that was not used to formulate it.
