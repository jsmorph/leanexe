...
| `OPA-009` | Separate compact suffix theorem | Completed fold frame before result placement and the public continuation | Accepted and independently verified on Demo 10.  An inline compact consequence retained the six-gibibyte elaboration failure; a preceding theorem declaration restored diagnostics, and a later generic ProofKit theorem captured the same emitted suffix for a fresh-use screen. |
...
The sixth experiment will give the complete package and LTG guidance to a fresh fixed-artifact proving agent.  The journal will record whether it selects the function equality, keeps branch programs folded until their existing semantic boundaries, and avoids rebuilding the proof around concrete WAT.  Proof-generation time, proof structure, generated-module size, and independent package verification will determine whether the package remains automatic support or a research example.
