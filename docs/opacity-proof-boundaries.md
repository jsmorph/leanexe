...
| `OPA-010` | Exact frame equality followed by a continuation-generic helper | Generated traversal and guarded back edge under the complete loop assertion | Accepted and independently verified on Demo 10.  The frame equality removed two heartbeat failures, while the local loop-body helper exposed compact completion and repeat callbacks; a shared parameterized theorem remains unimplemented. |
...
`OPA-010` records the new unresolved boundary.  The traversal-exit theorem, generated continuing theorem, scalar evaluator equations, and guarded-back-edge theorem already exist, but the agent had to compose them in a 155-line local helper before the enclosing loop rule became tractable.  A shared theorem should parameterize the semantic invariant, measure, completion callback, repeat callback, scalar descriptors, and exact frame view, leaving multiplication and `ArrayFold.foldPrefix` outside the compiler-motif layer.
...
The seventh experiment will extract the recurring traversal-plus-guarded-back-edge composition from `productLoopBody_spec`.  Its shared interface must accept arbitrary application invariants and measures while consuming exact compiler-generated traversal and scalar-transition evidence.  Demo 9 provides the out-of-sample sum fold after the theorem and recipe pass Demo 10's fixed-artifact screen.
