...
The exercise reported three representation gaps, one of which belonged to its archived input.  The current capacity and allocation entries already expose `FixedArrayAllocatorWindow.constantCapacityRegion_spec_withTail`, and a catalog test now protects both index records.  The annotation generator now emits the fold setup-frame equality, while extractor version six reports the checked length dispatch and its encoding.  The [retained exercise](../benchmarks/leanexegen/demo9-fold-sum/stateful-knowledge-1/) preserves the older proof package, proposal evidence, journals, evaluation, and follow-up candidate that produced these observations.
