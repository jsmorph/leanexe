...
The journals led to decoder preserved-local and valid-index theorems, together with allocator shape and valid-index theorems.  Recipe version two now records a checked tail equality for every direct semantic recipe with an exact program boundary, replacing the earlier decoder-specific naming convention.  The current evidence does not justify another application tactic: the remaining continuation failures differ in theorem argument order and residual-program shape, so the next proof run will record them before adding another proof-search choice.  Bound-eight arithmetic cases and the complete find-zero-and-erase composition remain worked examples pending evidence from another artifact.
