...
The provisional `encoded-index-decoder` entry isolates the compiler's recurring conversion from zero or index plus one to zero or the predecessor.  Its recipe combines a decoded-artifact region equality with `EncodedIndexDecoder.program_spec`, which records the exact scratch and destination-local frame.  Search semantics and index bounds remain with the producer theorem rather than entering the decoder entry.

The Demo 12 annotation pass preserved its exact artifact, generated and checked the decoder equality, and passed separate package verification against the existing behavior proof.  ClobDepth supplies cross-program semantic evidence: its checked source proof now applies `EncodedIndexDecoder.program_spec` twice and removes four decoder-specific branch selections and four associated scalar-reduction calls.  That refactor grows by 21 lines because it states exact region decompositions and theorem premises without a generated adapter, and neither case measures fresh proof generation or LTG retrieval.  The entry therefore remains provisional pending an end-to-end proof-generation use or a genuine held-out artifact.
