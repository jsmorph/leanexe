...
The journals identify the next general candidates more strongly than another complete erase-specific theorem.  Decoder and allocator frames need preserved-local and valid-index accessors, while continuation-heavy theorems need application forms that infer the frame and residual program before elaborating dependent premises.  A bounded prefix-to-tail tactic could combine atomic prefix steps, the generated decomposition, and a direct region theorem.  Bound-eight arithmetic cases and the complete find-zero-and-erase composition remain worked examples pending evidence from another artifact.
