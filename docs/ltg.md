...
The exercise also found three representation gaps.  Current indexes omit the existing constant-capacity-plus-allocation composition theorem, the annotation generator omits the initial setup-frame equality needed by the loop invariant, and the task-feature extractor omits a length dispatch present in the checked annotations.  The [retained exercise](../benchmarks/leanexegen/demo9-fold-sum/stateful-knowledge-1/) preserves the proof package, proposal evidence, journals, evaluation, and follow-up candidate.
