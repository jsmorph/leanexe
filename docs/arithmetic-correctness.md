...
with one UInt64 accumulator. The start is a standard Nat literal smaller than
2^64; omitting it means zero. The stop is a supported UInt64 expression, and
iteration has unit step. Steps may yield or finish early with `break`.
...
a separate capability. Additional accumulators, multiple/nested loops, dynamic range starts and other steps remain
...
Literal starts may be nonzero, including `[3:8]` and `[1:count.toNat]`. An empty
interval preserves the initial accumulator when the start equals or exceeds the
stop. Internally the loop counts from zero to the natural truncated distance;
each source index is the start plus that counter. Checked distance and index
lemmas connect this representation to native interval iteration. Zero-start
ranges keep their preceding output bytes.
...
and all reserved names, compiles 105 fresh declarations with that command,
...
It is configured to compare 2,028 results against native Lean evaluation, including overflow, zero
...
checks the fixed range fixture group: 1,465 results across sixty-two declarations,
