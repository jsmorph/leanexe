...
the start means zero. Iteration has unit step. Steps may yield or finish early with `break`.
...
a separate capability. Additional accumulators, multiple/nested loops and other steps remain
...
ranges keep their preceding output bytes. Computed starts retain their original
...
unsupported helper calls remain excluded.
...
and all reserved names, compiles 116 fresh declarations with that command,
...
It is configured to compare 2,292 results against native Lean evaluation, including overflow, zero
...
checks the fixed range fixture group: 1,729 results across seventy-three declarations,
