...
This snapshot uses LTG and proof-kit content at repository revision `5edf33ad1abef9dfaa95dd6d289475358e60bb25`.  Its complete task-bundle digest is `fb5e1489d8299ad146d3b8b23d535bc00e92cf3ed9a356718d9da43ec3c16bcd`, which identifies the exact catalog files presented to an unfiltered proof task.  Later reports can compare the dated revision, bundle digest, and schema version before comparing counts.
...
The catalog contains 53 unique feature strings across 113 assignments and 12 unique annotation kinds across 29 assignments.  Every entry names at least one annotation kind, while the scalar post-test annotation has the broadest binding at 6 entries.  The catalog records 48 consumer assignments across Demos 1 through 9, and every entry has at least one recorded consumer.

| Lean-support measure | Indexed by entries | Complete supplied proof kit |
|----------------------|-------------------:|----------------------------:|
| Module references | 27 assignments, 16 unique | 26 modules |
| Declaration references | 61 assignments, 51 unique | 365 source headers |
| Public named local declarations | 45 indexed | 316 available |
| Theorem headers | 35 indexed | 175 total |
| Lemma headers | 0 indexed | 0 total |
| Definition headers | 10 indexed | 148 total |
...
Entries mention 16 of the 26 supplied proof-kit modules, a module coverage ratio of 0.615.  Their declaration arrays expose 45 of 316 public named local source declarations, a lexical discoverability ratio of 0.142, and reference 6 additional imported declarations whose kinds this command does not infer.  The remaining proof-kit source is usable through imports but absent from structured declaration retrieval, which identifies a substantial indexing backlog rather than missing Lean implementation.

The complete proof kit has 365 recognized source headers: 175 `theorem`, 148 `def`, 29 `macro`, 7 `inductive`, 5 `structure`, and 1 `abbrev`.  Of these headers, 345 are public and 20 are private.  The source contains no recognized `lemma`, `class`, `instance`, `opaque`, or `elab` header at this revision.
...
|-----------------|------:|------:|
| Canonical entry content | 40 | 55,951 |
| Canonical catalog | 42 | 58,757 |
| Catalog Markdown | 21 | 29,555 |
| Metadata JSON | 21 | 29,202 |
| Generated category indexes | 7 | 89,685 |
| Physical catalog | 49 | 148,442 |
| Complete unfiltered task bundle | 49 | 148,442 |
| Proof-kit Lean sources | 26 | 319,030 |
| Proof-kit README | 1 | 41,806 |
| Generated declaration check | 1 | 3,562 |
| Combined physical knowledge | 76 | 509,278 |

Generated indexes account for 89,685 of the physical catalog's 148,442 logical bytes.  This duplication buys search locality and does not increase the 55,951-byte canonical entry inventory.  The combined total counts each physical task-catalog and proof-kit file once and excludes the generated declaration check, whose size appears on its own row.

The relation graph contains 80 directed related-entry links, including 29 reciprocal pairs and 22 asymmetric links, with no entry isolated from the graph.  Four worked examples carry exclusions covering four exact artifact digests and three derivative groups.  Catalog validation found no dangling category, allowed-module, or related-entry reference, and the local source inventory found no missing `Project.ProofKit` declaration name.
