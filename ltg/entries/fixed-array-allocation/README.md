...
Use `wp_alloc_to_store` when broad reduction crosses the next memory operation or structured control boundary.  It reduces the emitted local and global state prefix while stopping before loads, stores, comparisons, and branches, and its default rules normalize concrete list lengths and head or successor lookups.  Name the post-allocation store and frame before repeated use so each call reduces a bounded expression rather than the allocator's nested updates.
