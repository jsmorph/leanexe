...
The [general-tail and frame reproof](experiments/general-tail-frame-reproof.proof/) evaluates the subsequent recipe and ProofKit changes against the same artifact.  Its accepted proof uses generated tail equalities for the first-match search, encoded-index decoder, and erase-copy region, together with `EncodedIndexDecoder.resultFrame_get_ne` and `Frame.withValues_get`.  Independent verification rebuilt the embedded bytes, decoded program, behavior theorem, and final exact-artifact theorem.

The baseline [compiler annotations](program.proof/program.annotations.json) contain one `leanexe.array.length-dispatch.v1` region.  The [generated annotation equalities](annotation-matches.lean) and [proof recipe](program.proof/proof-recipes.json) establish the exact dispatch match and expose the checked invocation `wp_fixed_array_length_le_dispatch_from hArray at 8, 8`.  That baseline package has no generated annotation for the dynamic first-match search, prefix copy, shifted suffix, or complete erase path.
...
The general-tail and frame reproof took 5,903.365887 seconds and produced a 1,059-line, 50,046-byte proof with 55 journal paragraphs that mention an import check.  This run is 75.087 percent slower and 324 lines larger than the guided run, while it is 5.328 percent slower and has 19 fewer import-check paragraphs than the unguided encoded-index baseline.  Its main theorem spans 599 lines, compared with 700 in the guided proof, but a 460-line pre-theorem block contains 45 private definitions, abbreviations, and theorems for named continuations and nested frame adapters.  The run confirms retrieval and use of the generalized boundaries and decoder getter, but it provides no overall proof-time or proof-size improvement.

The baseline [stage reports](program.proof/stage-reports.json) record one accepted attempt for the formal specification, Lean program, and artifact proof.  Every retained reproof package covers the unchanged SHA-256 digest `7cdd8adba75d4f076d0a142f824a19a0d34d6a5cedd1a810a417a7fc5789f7b6`.  Separate `tools/leanexegen verify -s` invocations accepted the full-support, guided encoded-index, and general-tail packages under their frozen proof contexts.
...
| [General-tail and frame reproof](experiments/general-tail-frame-reproof.proof/) | The independently verified reproof that used all three version-two semantic tails and the broader decoder frame getter. |
