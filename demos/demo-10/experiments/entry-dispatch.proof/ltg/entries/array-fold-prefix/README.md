...
A `leanexe.array.fold.v1` annotation identifies the exact decoded fold interval and records the source width, direction, stop expression, accumulator and element locals, staged-value locals, done local, and result placement.  Use those local roles to state the invariant before opening the block, then apply the generated array-fold recipe at the annotated structured path.  The annotation checker has already matched the guard, staged accumulator copies, done test, index update, back edge, and selected result slot against the frozen artifact.
