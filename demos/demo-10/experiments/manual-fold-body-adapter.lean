...
theorem productLoopBody_spec
  ...
  · have hIndexLt : index < input.size := by omega
    ...
    apply FixedArrayFoldBody.continuingGuardedProgram_spec
      ...
      (stopValue := UInt64.ofNat input.size) (input := input) (index := index)
      ...
      (result := false) (Q := Q)
      (hValues := by rfl) (hArrayLocal := by rfl)
      (hIndexLocal := by rfl) (hStopLocal := by rfl)
      (hIndexValue := by rfl) (hContinueGuard := hContinue)
      (hItem :=
