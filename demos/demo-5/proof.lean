...
open Wasm Project.ProofKit Project.ProofKit.FixedArrayAllocatorWindow

private def keptPrefix (input : Array UInt64) (index : Nat) : Array UInt64 :=
  (input.extract 0 index).filter fun element => element < (100 : UInt64)

private theorem keptPrefix_succ_pos (input : Array UInt64) (index : Nat)
    (hIndex : index < input.size) (hKeep : input[index]! < (100 : UInt64)) :
    keptPrefix input (index + 1) =
      (keptPrefix input index).push input[index]! := by
  unfold keptPrefix
...
private theorem keptPrefix_succ_neg (input : Array UInt64) (index : Nat)
    (hIndex : index < input.size) (hKeep : ¬input[index]! < (100 : UInt64)) :
    keptPrefix input (index + 1) = keptPrefix input index := by
  unfold keptPrefix
  rw [Array.extract_succ_right (by omega) hIndex]
...
private theorem keptPrefix_size_le (input : Array UInt64) (index : Nat)
    (hIndex : index ≤ input.size) :
    (keptPrefix input index).size ≤ index := by
  unfold keptPrefix
  calc
    ((input.extract 0 index).filter fun element =>
      element < (100 : UInt64)).size ≤ (input.extract 0 index).size :=
        Array.size_filter_le
    _ = index := by simp [hIndex]

private theorem keptPrefix_all (input : Array UInt64) :
    keptPrefix input input.size =
      input.filter fun element => element < (100 : UInt64) := by
  simp [keptPrefix, Array.extract_eq_self_of_le]

private def entryFrame (inputPtr : UInt64) : Locals :=
  { params := [.i64 inputPtr]
...
private def filterSuffix : Wasm.Program :=
  [
  .constI64 0,
  .localSet 11,
  .localGet 8,
...
private def validProgram : Wasm.Program :=
  validPrefix ++ validAllocatorRegion ++ filterSuffix
...
private def wrapperProgram : Wasm.Program :=
  FixedArrayLengthDispatch.leProgram 5 8 validProgram emptyProgram ++
    [.localGet 4]
...
private def validFrame (inputPtr heapTop capacity : UInt64)
  ...
  { params := [.i64 inputPtr]
    locals := [.i64 last, .i64 0, .i64 0, .i64 0,
      ...
      .i64 (UInt64.ofNat (keptPrefix input index).size),
...
private def validInv (initial : Store Unit)
  ...
  fun st frame =>
    ∃ index last, index ≤ input.size ∧
      frame = validFrame inputPtr heapTop capacity input last index ∧
      st.mem.pages = initial.mem.pages ∧
      UInt64Array.At st inputPtr input ∧
      ∀ (i : Nat) (hi : i < (keptPrefix input index).size),
        st.mem.read64
          (heapTop + 48 + UInt64.ofNat (8 * (i + 1))).toUInt32 =
          (keptPrefix input index)[i]
...
theorem artifact_behavior :
    LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.ArtifactSpec
      LeanExeGen.GeneratedRc28a1499719cbaa0.«module» := by
  refine ⟨0, rfl, ?_⟩
  ...
  have hAtIff (st : Store Unit) (ptr : UInt64) (values : Array UInt64) :
      LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.UInt64ArrayAt
          st ptr values ↔ UInt64Array.At st ptr values := by
    rfl
  ...
  apply Wasm.wp.conseq (Q :=
    FixedArrayPairResult.publicPost
      (LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.expected input))
  · intro continuation hPost
    cases continuation <;> simp_all [FixedArrayPairResult.publicPost]
  change wp LeanExeGen.GeneratedRc28a1499719cbaa0.«module»
    LeanExeGen.GeneratedRc28a1499719cbaa0.func0
    (FixedArrayPairResult.publicPost
      (LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.expected input))
    initial (entryFrame inputPtr) env
  have hProgram : LeanExeGen.GeneratedRc28a1499719cbaa0.func0 =
      wrapperProgram := by
    rfl
  rw [hProgram]
  unfold wrapperProgram
  apply FixedArrayLengthDispatch.leProgram_spec 5 8 _ _ _
    LeanExeGen.GeneratedRc28a1499719cbaa0.«module» env initial
  ...
  · intro hSize
    have hExpected :
        LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.expected input =
          input.filter fun element => element < (100 : UInt64) := by
      simp [LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.expected, hSize]
    let capacity : UInt64 :=
    ...
    have hCapacityNat : capacity.toNat = 8 * (input.size + 1) := by
      ...
      simp only [capacity, UInt64.toNat_mul, UInt64.toNat_div,
        UInt64.toNat_add, UInt64.toNat_ofNat_of_lt' hArray.size_lt,
        hEight, hSeven]
      norm_num
      omega
    ...
    have hFitCapacity :
      ...
      rw [hCapacityNat]
      simpa [LeanExeGen.GeneratedRc28a1499719cbaa0.FormalSpec.heapReserveBytes,
        hSize, Nat.add_assoc] using hHeapFitMemory
    change wp LeanExeGen.GeneratedRc28a1499719cbaa0.«module» validProgram _
      initial
      { params := [.i64 inputPtr]
    ...
    wp_run
    simp [hArray.pointerAddress_eq, hArray.lengthRead]
    ...
    simp only [List.take_zero, List.drop_zero, List.nil_append]
    change wp LeanExeGen.GeneratedRc28a1499719cbaa0.«module»
      (validAllocatorRegion ++ filterSuffix) _ initial
      { params := [.i64 inputPtr]
        locals := [.i64 0, .i64 0, .i64 0, .i64 0, .i64 inputPtr,
    ...
    apply validAllocatorRegion_spec
      LeanExeGen.GeneratedRc28a1499719cbaa0.«module» env initial _
    ...
    · have hBump := Allocation.bumpFacts heapTop capacity initial.mem.pages
      ...
      · apply wp_block_cons
        apply wp_loop_cons
          (Inv := validInv initial inputPtr heapTop capacity input)
        ...
        · rintro st frame
          ...
          · subst index
            have hGuard :
              ...
              simp
            rw [hGuard]
            let hOutput := keptPrefix input input.size
            have hOutputEq : hOutput =
                input.filter fun element => element < (100 : UInt64) := by
              simpa [hOutput] using keptPrefix_all input
          ...
          · have hIndexLt : index < input.size := by omega
            ...
            rw [hLoadRead']
            by_cases hKeep : input[index]! < (100 : UInt64)
            · have hPrefixSucc :=
                keptPrefix_succ_pos input index hIndexLt hKeep
              have hPrefixSize := keptPrefix_size_le input index (by omega)
              have hCountSucc :
                  UInt64.ofNat (keptPrefix input index).size + 1 =
                    UInt64.ofNat
                      ((keptPrefix input index).push input[index]!).size := by
                rw [Array.size_push]
                apply UInt64.toNat.inj
                rw [UInt64.toNat_add,
                  UInt64.toNat_ofNat_of_lt' (by
                ...
                omega
              have hWordFit :
                  8 * ((keptPrefix input index).size + 1 + 1) ≤
                    capacity.toNat := by
                rw [hCapacityNat]
                omega
              have hOutputAddress :
                  UInt32.ofNat
                      ((heapTop.toNat + 48 +
                        ((keptPrefix input index).size + 1) * 8) %
                          4294967296) =
                    (heapTop + 48 + UInt64.ofNat
                      (8 * ((keptPrefix input index).size + 1))).toUInt32 := by
                ...
                simpa [Nat.mod_eq_of_lt hRoot64, Nat.mul_comm, Nat.add_assoc]
                  using hBump.wordAddress
                    ((keptPrefix input index).size + 1) hWordFit
              have hStoreBound :
                  (heapTop + 48 + UInt64.ofNat
                    (8 * ((keptPrefix input index).size + 1))).toUInt32.toNat + 8 ≤
                      st.mem.pages * 65536 := by
              ...
              · unfold validInv
                ...
                · intro i hiNext
                  ...
                  have hRead :
                      (st.mem.write64
                          (heapTop + 48 + UInt64.ofNat
                            (8 * ((keptPrefix input index).size + 1))).toUInt32
                          input[index]!).read64
                          (heapTop + 48 + UInt64.ofNat (8 * (i + 1))).toUInt32 =
                        ((keptPrefix input index).push input[index]!)[i] := by
                    by_cases hEq : i = (keptPrefix input index).size
                    · subst i
                      simpa [hIndexLt] using
                        (Mem.read64_write64_same st.mem
                          (heapTop + 48 + UInt64.ofNat
                            (8 * ((keptPrefix input index).size + 1))).toUInt32
                          input[index]!)
                    · have hiOld : i < (keptPrefix input index).size := by
                            ...
                            st.mem.read64
                              ...
                              rw [hBump.wordAddress_toNat (i + 1) hWordFitI,
                                hBump.wordAddress_toNat
                                  ((keptPrefix input index).size + 1) hWordFit]
                              omega
                        _ = (keptPrefix input index)[i] := hPayload i hiOld
                        _ = ((keptPrefix input index).push input[index]!)[i] := by
                          exact (Array.getElem_push_lt hiOld).symm
                  simpa only [hPrefixSucc] using hRead
              · simp [validMeasure, hIndexNat]
                omega
            · have hPrefixSucc :=
                keptPrefix_succ_neg input index hIndexLt hKeep
              simp [hKeep]
              refine wp_iff_cons rfl ?_
              rw [if_neg (by simp)]
              wp_run
  ...
  · intro hSize
    ...
    change wp LeanExeGen.GeneratedRc28a1499719cbaa0.«module» emptyProgram _
      initial
      { params := [.i64 inputPtr]
        locals := [.i64 0, .i64 0, .i64 0, .i64 0, .i64 inputPtr,
          .i64 0, .i64 0, .i64 0, .i64 0, .i64 0, .i64 0,
