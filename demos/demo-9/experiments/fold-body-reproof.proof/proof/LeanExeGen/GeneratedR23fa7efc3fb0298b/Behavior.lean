...
@[simp] theorem foldFrame_params (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).params = [.i64 inputPtr] := by
  rfl

@[simp] theorem foldFrame_locals_length (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).locals.length = 20 := by
  rfl

@[simp] theorem foldFrame_values (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
...
@[simp] theorem foldFrame_get_accumulator (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).get 1 = some (.i64 (sumPrefix input index)) := by
  rfl

@[simp] theorem foldFrame_get_root (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).get 7 = some (.i64 root) := by
  rfl

@[simp] theorem foldFrame_get_input (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).get 11 = some (.i64 inputPtr) := by
  rfl

@[simp] theorem foldFrame_get_index (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
...
@[simp] theorem foldFrame_get_stop (inputPtr root : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (foldFrame inputPtr root input index item temporary scratch staged
      releaseReady).get 15 = some (.i64 (UInt64.ofNat input.size)) := by
  rfl

@[simp] theorem foldResultFrame_get_root (inputPtr root value : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (FixedArrayFold.resultFrame
      (foldFrame inputPtr root input index item temporary scratch staged
        releaseReady) 10 value).get 7 = some (.i64 root) := by
  rfl

@[simp] theorem foldResultFrame_get_result (inputPtr root value : UInt64)
    (input : Array UInt64) (index : Nat)
    (item temporary scratch staged releaseReady : UInt64) :
    (FixedArrayFold.resultFrame
      (foldFrame inputPtr root input index item temporary scratch staged
        releaseReady) 10 value).get 10 = some (.i64 value) := by
  rfl
            ...
            · rintro st frame
              ...
              · subst index
                ...
                · simp only [List.take_zero, List.drop_zero, List.nil_append]
                  ...
                  · apply FixedArrayFold.singletonResultProgram_spec
                      ...
                      (hValues := foldFrame_values inputPtr (heapTop + 48)
                        input input.size item temporary scratch staged
                        releaseReady)
                      (hAccumulator := foldFrame_get_accumulator inputPtr
                        (heapTop + 48) input input.size item temporary scratch
                        staged releaseReady)
                      (hRoot := foldResultFrame_get_root inputPtr
                        (heapTop + 48) (sumPrefix input input.size) input
                        input.size item temporary scratch staged releaseReady)
                      (hResult := foldResultFrame_get_result inputPtr
                        (heapTop + 48) (sumPrefix input input.size) input
                        input.size item temporary scratch staged releaseReady)
                    · norm_num
                    · norm_num [Wasm.Locals.validIndex]
                    · have hPayloadNat :
                      ...
                      omega
                    · norm_num [FixedArrayFold.resultFrame]
                    · norm_num [Wasm.Locals.validIndex,
                        FixedArrayFold.resultFrame]
                    · norm_num [FixedArrayFold.resultFrame]
                    · norm_num [Wasm.Locals.validIndex,
              ...
              · have hContinue : index < input.size := by omega
                ...
                refine FixedArrayFoldBody.continuingGuardedProgram_spec
                  ...
                  (hValues := foldFrame_values inputPtr (heapTop + 48) input
                    index item temporary scratch staged releaseReady)
                  (hArrayLocal := foldFrame_get_input inputPtr (heapTop + 48)
                    input index item temporary scratch staged releaseReady)
                  (hIndexLocal := foldFrame_get_index inputPtr (heapTop + 48)
                    input index item temporary scratch staged releaseReady)
                  (hStopLocal := foldFrame_get_stop inputPtr (heapTop + 48)
                    input index item temporary scratch staged releaseReady)
