...
Keep this result sequence outside the enclosing loop theorem when its complete postcondition makes `Wasm.wp_loop_cons` exceed the elaboration resource limit.  The `compact-loop-suffix-boundary` entry records how to state the completed fold frame and output assertion as a separate theorem declaration.  Applying that theorem after loop completion lets Lean elaborate the suffix once without carrying the public continuation through the loop rule.
