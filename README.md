...
Complete cached-model and exact-binary proofs remain open.
...
The 2026-09-19 aggregate artifact check passed all forty-three frozen packages, their behavioral specifications, and registered declaration audits.  A refreshed release draft records the current package membership.  Its receipts must match the current release inputs.  Semantic conformance, an immutable source revision, and cold-checkout evidence remain separate release work.  [Development Plan](plan.md) is the work queue, and `devnotes.md` records decisions and test evidence.
