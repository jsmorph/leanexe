...
`Option` and `Except` do-notation is accepted when Lean elaborates it to first-order `Pure.pure` and `Bind.bind` applications with direct lambdas.  `Except` bind chains may call helpers that use accepted pure `Id.run` loops and may return structured, tagged, array, or byte-array payloads.  Error results short-circuit later binds, matching standard Lean behavior.
