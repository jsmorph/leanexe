...
The hidden-state body and single-logit function have proved execution and
store-preservation theorems in the full inference module.
The vocabulary-output loop proves termination, all 256 raw-bit logits,
checkpoint preservation, and a fixed page count under its memory reservation.
Inference entry and exit, remaining checkpoint range certificates, and the
