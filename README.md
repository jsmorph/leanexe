...
The release record identifies the source revision and successful warm gates, while its cold-checkout receipt remains deferred.  `tools/artifact-release.js check-ready` will continue to reject that draft record until the deferred receipt exists.  [Development Plan](plan.md) is the sole work queue, and `devnotes.md` records decisions and test evidence.
