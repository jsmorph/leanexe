...
The compiler emits `release` for local heap temporaries when the temporary comes from a visible fresh allocation and the function returns only scalar or fixed non-heap data.  It also computes helper-result ownership summaries from the extracted IR, then releases array and byte-array owner slots in scalar-result callers when the callee result is proven fresh, including helpers that receive heap-bearing parameters.  It keeps heap-pointer results, recursive-inductive call results, and helper results that may borrow from heap arguments conservative: hosts remain responsible for releasing returned arrays and byte arrays after reading them.
