...
The compiler emits `release` for a conservative class of local heap temporaries: the temporary must come from a visible fresh allocation in a local expression or local binding, and the function result type must contain no heap pointer.  This lets scalar-result helpers reclaim internal arrays, byte arrays, and recursive values before returning.  It also releases owner slots in helper-call results when the callee has no heap-bearing parameters, because such a result cannot be borrowed from a caller-owned heap argument.  Calls to helpers that receive arrays, byte arrays, recursive values, or structures containing heap pointers remain conservative until the compiler has a real ownership summary for helper results.  The compiler does not yet perform full ownership analysis for heap-pointer results, loop-carried values, or temporaries whose lifetime ends before function exit.
