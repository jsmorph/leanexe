...
theorem output_prepare_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hStart : params.length + saved.length = 61)
    ...
    (hLength : (FixedArraySearch.frame params saved tail (UInt64.ofNat (capacity (count + 1)))
      previous current capacity' next result).get 55 = some (.i64 (UInt64.ofNat (count + 1))))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous' : UInt64, wp module rest Q
      (OutputMemory.prepare initial start (count + 1) allocations)
      (resultFrame (TinyGpt2Infer.Spec.outputAllocationFrame params saved tail start count previous') 56
  ...
  have hRoot : allocatedFrame.get 66 = some (.i64 (node start (count + 1)).root) := by
  ...
  have hLength' : allocatedFrame.get 55 = some (.i64 (UInt64.ofNat (count + 1))) := by
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 55 (by omega)] at hLength
    change (FixedArraySearch.frame _ _ _ _ _ _ _ _ _).get 55 = _
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 55 (by omega)]
    exact hLength
  have hLower : allocatedFrame.params.length ≤ 56 := by simpa only [allocatedFrame, TinyGpt2Infer.Spec.outputAllocationFrame,
    FixedArraySearch.frame, hParams] using (show 6 ≤ 56 by decide)
  have hValid : allocatedFrame.validIndex 56 := by
    simp only [allocatedFrame, TinyGpt2Infer.Spec.outputAllocationFrame, FixedArraySearch.frame, Locals.validIndex,
      List.length_append, List.length_cons, List.length_nil]
    omega
  change wp module ((ScalarTransition.Stmt.assign 56 (.get 66)).program 71 ++
    FixedArrayResult.lengthStoreLocalProgram 56 55 ++ rest) Q _ allocatedFrame env
  apply ScalarTransition.Expr.assign_frame_spec (.get 66) 71 56 allocatedFrame
  ...
  apply FixedArrayResult.lengthStoreLocal_spec module env _ _ (node start (count + 1)).root
    (UInt64.ofNat (count + 1)) 56 55
    (resultFrame_get_result allocatedFrame 56 _ hLower hValid)
    ((resultFrame_get_ne allocatedFrame 56 55 _ hLower (by decide)).trans hLength')
