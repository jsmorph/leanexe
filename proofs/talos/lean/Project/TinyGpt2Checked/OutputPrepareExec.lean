...
theorem output_prepare_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hLength : (FixedArraySearch.frame params saved tail (UInt64.ofNat (capacity (count + 1)))
      previous current capacity' next result).get 57 = some (.i64 (UInt64.ofNat (count + 1))))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous' : UInt64, wp module rest Q
      (OutputMemory.prepare initial start (count + 1) allocations)
      (resultFrame (TinyGpt2Infer.Spec.outputAllocationFrame params saved tail start count previous') 58
        (node start (count + 1)).root) env) :
    wp module ((outputBody.drop 69).take 21 ++ rest) Q initial
  ...
  have hRoot : allocatedFrame.get 68 = some (.i64 (node start (count + 1)).root) := by
  ...
  have hLength' : allocatedFrame.get 57 = some (.i64 (UInt64.ofNat (count + 1))) := by
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 57 (by omega)] at hLength
    change (FixedArraySearch.frame _ _ _ _ _ _ _ _ _).get 57 = _
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 57 (by omega)]
    exact hLength
  have hLower : allocatedFrame.params.length ≤ 58 := by simpa only [allocatedFrame, TinyGpt2Infer.Spec.outputAllocationFrame,
    FixedArraySearch.frame, hParams] using (show 6 ≤ 58 by decide)
  have hValid : allocatedFrame.validIndex 58 := by
    simp only [allocatedFrame, TinyGpt2Infer.Spec.outputAllocationFrame, FixedArraySearch.frame, Locals.validIndex,
      List.length_append, List.length_cons, List.length_nil]
    omega
  change wp module ((ScalarTransition.Stmt.assign 58 (.get 68)).program 73 ++
    FixedArrayResult.lengthStoreLocalProgram 58 57 ++ rest) Q _ allocatedFrame env
  apply ScalarTransition.Expr.assign_frame_spec (.get 68) 73 58 allocatedFrame
  ...
  apply FixedArrayResult.lengthStoreLocal_spec module env _ _ (node start (count + 1)).root
    (UInt64.ofNat (count + 1)) 58 57
    (resultFrame_get_result allocatedFrame 58 _ hLower hValid)
    ((resultFrame_get_ne allocatedFrame 58 57 _ hLower (by decide)).trans hLength')
