...
structure OutputSaved (owner pointer empty : UInt64) (x : Row) (frame : Locals) : Prop where
  ...
  limit : frame.get 50 = some (.i64 256)
  step : frame.get 51 = some (.i64 1)
  scratch : I64LocalRange frame 61 67

theorem OutputSaved.valid {owner pointer empty : UInt64} {x : Row} {frame : Locals}
    (h : OutputSaved owner pointer empty x frame) (index : Nat) (hIndex : index < 71) :
...
theorem OutputSaved.transport {owner pointer empty : UInt64} {x : Row} {frame next : Locals}
    ...
    (hScratch : I64LocalRange next 61 67) : OutputSaved owner pointer empty x next := by
  exact ⟨hParams.trans h.params, hLocals.trans h.locals, hValues,
    ...
    (hGets 22 (by decide)).trans h.empty, (hGets 50 (by decide)).trans h.limit,
    (hGets 51 (by decide)).trans h.step, hScratch⟩
...
theorem OutputSaved.result {owner pointer empty : UInt64} {x : Row} {frame : Locals}
    ...
    (hLower : 6 ≤ index) (hUpper : index < 71) (hPreserved : index ∉ savedOutputLocals) :
...
theorem OutputSaved.counter {owner pointer empty : UInt64} {x : Row} {frame : Locals}
    (h : OutputSaved owner pointer empty x frame) (count : Nat) (hValid : frame.validIndex 57) :
    OutputSaved owner pointer empty x (FixedArrayCopy.counterFrame frame 57 count hValid) := by
  apply h.transport (FixedArrayCopy.counterFrame_params_length ..)
    (FixedArrayCopy.counterFrame_locals_length ..) rfl
  · intro read hRead
    exact FixedArrayCopy.counterFrame_get_ne frame 57 count read hValid (by
      simp only [savedOutputLocals, List.mem_cons, List.not_mem_nil, or_false] at hRead
      omega)
  · exact h.scratch.counter 57 count hValid
