import Project.TinyGpt2Checked.OutputLogitExec
import Project.TinyGpt2Checked.OutputCapacityExec
import Project.TinyGpt2Checked.OutputAppendExec
import Project.TinyGpt2Checked.OutputAdvanceExec
...
structure OutputLoopLocals (owner pointer empty : UInt64) (x : Row) (root : UInt64)
  ...
  counter : frame.get 49 = some (.i64 (UInt64.ofNat count))
  owned : frame.get 70 = some (.i64 (if count = 0 then 0 else 1))
...
theorem output_iteration_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hNext : ∀ (final : Store Unit) (finalFrame : Locals),
      ...
      OutputLoopLocals owner pointer empty x (node start (count + 1)).root (count + 1) finalFrame →
  ...
  apply output_logit_spec env initial frame owner pointer empty (node start count).root
  ...
  let logged := outputLogitFrame frame owner pointer (node start count).root (UInt64.ofNat count) value x
  have hLogged : OutputSaved owner pointer empty x logged := outputLogitFrame_saved hLocals.toOutputSaved _ _ _
  obtain ⟨hValue, hOutput, hCurrent, _, hCounter, hOwned⟩ :=
    outputLogitFrame_get frame owner pointer (node start count).root (UInt64.ofNat count) value x
      hLocals.params hLocals.locals
  apply output_capacity_spec env initial logged owner pointer empty (node start count).root value
  ...
  have hCapacity : OutputSaved owner pointer empty x capacityFrame := outputCapacityFrame_saved hLogged _ _ _
  ...
  apply output_append_spec env initial capacityFrame owner pointer empty value x start count
  ...
  have hCounter' : released.get 49 = some (.i64 (UInt64.ofNat count)) :=
