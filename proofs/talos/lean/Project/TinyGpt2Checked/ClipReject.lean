...
theorem reject_program_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    wp Project.TinyGpt2Checked.module (rejectProgram ++ rest) Q initial (prepareFrame count bound owner ptr 0) env := by
  ...
  apply FixedArrayCapacity.constantProgram_spec 0 1 21 Project.TinyGpt2Checked.module env initial _
    rfl (by change 4 ≤ 21; decide) (by change 21 < 29; decide)
  change wp Project.TinyGpt2Checked.module _ Q initial
    (rejectAllocationFrame count bound owner ptr 8 0 0 0 0 0) env
  apply FixedArrayAllocateNone.program_spec Project.TinyGpt2Checked.module env initial
    [.i64 count, .i64 bound, .i64 owner, .i64 ptr]
    (rejectSaved count bound owner ptr)
    [.i64 0, .i64 0] 21 rfl (FixedArrayReuse.program 21 1) base 8 1 0 0 0 0 0 allocations []
  · simp [hGlobals]
  ...
  · intro previous
    ...
    apply FixedArrayResult.lengthStore_spec Project.TinyGpt2Checked.module env
      (clipAllocate initial base 0 allocations) _ (base+48) 0 17 rfl rfl
    · rw [Memory.toUInt32_toNat, hWords.2, Nat.mod_eq_of_lt (by omega), hAllocatedPages]
      omega
    · wp_fixed_frame [finishOwnedProgram, List.cons_append, List.nil_append]
      apply hNext
      exact ⟨rfl, rfl, rfl, hOutput, hInitializedInput, hInitializedPages, WritesRange.refl ..⟩
