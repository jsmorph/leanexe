...
open Wasm Project.ProofKit UInt64Array Memory Project.F64Clip

def mapSuffix : Wasm.Program :=
  [.localGet 19, .localGet 20, .constI64 1, .mulI64, .constI64 1, .addI64,
   .constI64 8, .mulI64, .addI64, .wrapI64,
   .localGet 1, .localSet 10, .localGet 9, .localSet 11, .localGet 10, .localGet 11,
   .call 5, .localSet 12, .localGet 12, .store64 0,
   .localGet 20, .constI64 1, .addI64, .localSet 20, .br 0]

def mapBody : Wasm.Program :=
  FixedArrayTraversalInput.continuingProgram 17 20 18 9 ++ mapSuffix

def mapFrame (count bound owner ptr root : UInt64) (size index : Nat)
    (last argument result : UInt64) (tail : List Value) : Locals :=
  { params := [.i64 count, .i64 bound, .i64 owner, .i64 ptr]
    locals := [.i64 count, .i64 bound, .i64 owner, .i64 ptr, .i64 1,
...
macro "wp_checked_clip_map" : tactic => `(tactic|
...
def mapInv (initial : Store Unit) (count bound owner ptr root : UInt64) (w : Array UInt64)
    (tail : List Value) : AssertionF Unit := fun st frame =>
  ∃ index last argument result, index ≤ w.size ∧
    frame = mapFrame count bound owner ptr root w.size index last argument result tail ∧
    PrefixAt st root (w.map (clip bound)) index ∧
    WritesRange initial st root.toNat (root.toNat+8*(w.size+1))

def mapDone (initial : Store Unit) (count bound owner ptr root : UInt64) (w : Array UInt64)
    (tail : List Value) : AssertionF Unit := fun st frame =>
  ∃ last argument result,
    frame = mapFrame count bound owner ptr root w.size w.size last argument result tail ∧
    UInt64Array.At st root (w.map (clip bound)) ∧
    WritesRange initial st root.toNat (root.toNat+8*(w.size+1))

def mapMeasure (w : Array UInt64) (_ : Store Unit) (frame : Locals) : Nat :=
  match frame.get 20 with
  | some (.i64 index) => w.size-index.toNat
  | _ => 0

theorem map_step (env : HostEnv Unit) (initial : Store Unit) (count bound owner ptr root : UInt64)
    (w : Array UInt64) (tail : List Value) (hInput : UInt64Array.At initial ptr w)
    (hBefore : ptr.toNat+8*(w.size+1) ≤ root.toNat)
    (st : Store Unit) (frame : Locals) (hInv : mapInv initial count bound owner ptr root w tail st frame) :
    wp Project.TinyGpt2Checked.module mapBody
      (BlockLoop.stepPost (mapInv initial count bound owner ptr root w tail)
        (mapDone initial count bound owner ptr root w tail) (mapMeasure w) (mapMeasure w st frame)) st frame env := by
  ...
  · subst index
    unfold mapBody
    refine FixedArrayTraversalInput.continuingProgram_exit_spec 17 20 18 9
      Project.TinyGpt2Checked.module env st _ (UInt64.ofNat w.size) rfl
  ...
  · have hi : index < w.size := by omega
    have hlt : UInt64.ofNat index < UInt64.ofNat w.size := by
      ...
      exact hi
    unfold mapBody
    refine FixedArrayTraversalInput.continuingProgram_spec 17 20 18 9 Project.TinyGpt2Checked.module env
    ...
    unfold mapSuffix
    wp_checked_clip_map
    have hAddress := Project.F64Clip.Spec.map_address root index
...
theorem map_program_spec (env : HostEnv Unit) (initial : Store Unit)
    (count bound owner ptr root : UInt64) (w : Array UInt64) (tail : List Value)
    ...
    (hNext : ∀ st last argument result,
      UInt64Array.At st root (w.map (clip bound)) →
      WritesRange initial st root.toNat (root.toNat+8*(w.size+1)) →
      wp Project.TinyGpt2Checked.module rest Q st
        (mapFrame count bound owner ptr root w.size w.size last argument result tail) env) :
    wp Project.TinyGpt2Checked.module ([.block 0 0 [.loop 0 0 mapBody]] ++ rest) Q initial
      (mapFrame count bound owner ptr root w.size 0 0 0 0 tail) env := by
  refine BlockLoop.program_spec Project.TinyGpt2Checked.module env initial _ mapBody
    (mapInv initial count bound owner ptr root w tail) (mapDone initial count bound owner ptr root w tail)
    (mapMeasure w) ?_ ?_ ?_ (map_step env initial count bound owner ptr root w tail hInput hBefore) _ _ ?_
