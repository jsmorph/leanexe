...
theorem output_prepare_frame_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hLength : frame.get 53 = some (.i64 (UInt64.ofNat (count + 1))))
    ...
    (hNext : ∀ finalFrame : Locals, OutputSaved owner pointer empty x finalFrame →
      finalFrame.get 54 = some (.i64 (node start (count + 1)).root) →
      (∀ index : Nat, index ≠ 54 → (index < 59 ∨ 65 ≤ index) →
  ...
  let saved := frame.locals.take 53
  let tail := frame.locals.drop 59
  have hStart : params.length + saved.length = 59 := by
    simp [params, saved, hSaved.params, hSaved.locals]
  obtain ⟨need, previous, current, capacity', next, result, hFrame⟩ :=
    hSaved.scratch.window 53 (by rw [hSaved.params])
  ...
  apply hNext (resultFrame allocated 54 (node start (count + 1)).root)
    (hAllocated.result 54 _ (by decide) (by decide) (by decide))
    (resultFrame_get_result allocated 54 _ (by rw [hAllocated.params]; decide)
      (hAllocated.valid 54 (by decide)))
  intro index hIndex hOutside
  rw [resultFrame_get_ne allocated 54 index _ (by rw [hAllocated.params]; decide) hIndex]
