...
theorem output_prepare_frame_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hLength : frame.get 57 = some (.i64 (UInt64.ofNat (count + 1))))
    ...
    (hNext : ∀ finalFrame : Locals, OutputSaved owner pointer empty x finalFrame →
      finalFrame.get 58 = some (.i64 (node start (count + 1)).root) →
      (∀ index : Nat, index ≠ 58 → (index < 63 ∨ 69 ≤ index) →
  ...
  have hStart : params.length + saved.length = 63 := by
  ...
  apply hNext (resultFrame allocated 58 (node start (count + 1)).root)
    (hAllocated.result 58 _ (by decide) (by decide) (by decide))
    (resultFrame_get_result allocated 58 _ (by rw [hAllocated.params]; decide)
      (hAllocated.valid 58 (by decide)))
  intro index hIndex hOutside
  rw [resultFrame_get_ne allocated 58 index _ (by rw [hAllocated.params]; decide) hIndex]
