import Project.ClobDepth.FrozenEntry
import Project.ClobDepth.FrozenProperties
import Project.ClobDepth.FrozenRepresentation
...
namespace Project.ClobDepth.Frozen.Scan

open Wasm Project.Common Project.ClobDepth Project.ClobDepth.Frozen.Model
  Project.ClobDepth.Frozen.Properties Project.ClobDepth.Frozen.Representation
...
theorem scanLoop_spec {env : HostEnv Unit} {st : Store Unit}
  ...
  unfold Entry.scanLoopProg Entry.scanProg Project.ClobDepth.Frozen.func3
  apply wp_block_cons
  apply wp_loop_cons
    (Inv := fun st' s =>
      st' = st ∧
...
theorem scanProg_spec {env : HostEnv Unit} {st : Store Unit}
  ...
  unfold Entry.scanPrepareProg Entry.scanProg Project.ClobDepth.Frozen.func3
  simp [entryFrame]
  rw [if_neg (Nat.not_lt.mpr hLevels.1.2), hLevels.1.1]
  apply scanLoop_spec levels hLength hLevels
  · intro hIndex f4 f5
    unfold Entry.scanFinishProg Entry.scanProg Project.ClobDepth.Frozen.func3
    wp_run
    simp
    refine wp_iff_cons rfl ?_
    rw [if_neg (by simp)]
...
end Project.ClobDepth.Frozen.Scan
