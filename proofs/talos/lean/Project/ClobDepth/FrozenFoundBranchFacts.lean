import Project.ClobDepth.FrozenFoundStore

/-!
# Found-price branch facts

These facts connect bump allocation and length initialization to the
same-length copy invariant.  They also transport the completed replacement
state back to the input allocator state and owned source array.
-/

namespace Project.ClobDepth.Frozen.FoundBranchFacts
...
open Wasm Project.Common Project.Clob Project.ClobDepth
  ...
  Project.ClobDepth.Frozen.FoundCopyInvariant
...
def allocatedStore (st : Store Unit) (g0 g2 : UInt64)
  ...
  MissingFinish.finishStore
    ...
    (target g0) (UInt64.ofNat levels.length) g2
...
theorem allocatedStore_fresh
  ...
  have hWritten := FreshFixedArrayAt.write64_data
    (value := UInt64.ofNat levels.length) hFresh
...
theorem copyInvariant_initial
  ...
  apply FoundCopyInvariant.initial _ _ _ _ _ _ hLocals hValues hCounter
...
structure ResultState (st0 st : Store Unit) (g0 g2 source sourceCapacity :
    UInt64) (levels : List LevelL) (i : Nat) (level : LevelL) : Prop where
...
theorem ResultState.of_finish
    ...
    {levels : List LevelL} {i : Nat} {level : LevelL}
    (hFinish : FoundStoreFacts.ReplaceState
      (allocatedStore st0 g0 g2 levels) st (target g0) source
      (capacity levels) levels i level)
    (hSource32 :
      source.toNat + fixedArrayBytes levels.length 2 < 4294967296)
    (hSource48 : 48 ≤ source.toNat)
    (hSourceCapacity :
    ...
    ResultState st0 st g0 g2 source sourceCapacity levels i level := by
...
end Project.ClobDepth.Frozen.FoundBranchFacts
