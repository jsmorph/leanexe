...
theorem initial_start_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hNext : ∀ (aux : List Value) (s : Scratch), aux.length = 19 →
    ...
    wp Project.Drone.«module» ((func23.drop 40).take 17 ++ rest) Q store
      { WordArrayPush.frame [] (List.replicate 20 (.i64 0)) (List.replicate 13 (.i64 0))
  ...
  exact hNext (List.replicate 19 (.i64 0))
...
theorem initial_entry (env : HostEnv Unit) (store : Store Unit) (P : Store Unit → List Value → Prop)
    (hNext : wp Project.Drone.«module» (emptyProgram 20 ++ func23.drop 40)
      (fun c => match c with
        | .Fallthrough final frame => P final (frame.values.take 2)
        | .Return final values => P final (values.take 2)
        | _ => False)
      store (WordArrayPush.frame [] (List.replicate 20 (.i64 0))
        (List.replicate 13 (.i64 0)) zeroPushScratch) env) :
    TerminatesWith env Project.Drone.«module» 23 store [] P := by
  refine TerminatesWith.of_wp_entry_for (f := func23Def) rfl ?_
  change wp Project.Drone.«module» func23 _ store { params := [], locals := List.replicate 48 (.i64 0) } env
  have hProgram : func23 = emptyProgram 20 ++ func23.drop 40 := rfl
  rw [hProgram]
