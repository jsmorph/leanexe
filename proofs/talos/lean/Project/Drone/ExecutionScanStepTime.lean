...
theorem scanStep_time_lt (env : HostEnv Unit) (initial : Store Unit)
  ...
  all_goals
    ...
    wp_scan_frame [hScratch, choiceValues, func13Def, hTime, hOwner]
    refine wp_iff_cons rfl ?_
    simp only [hTime, ne_eq, show (1 : UInt32) ≠ 0 by decide, not_false_eq_true, ↓reduceIte]
    wp_scan_frame [hScratch, choiceValues, func13Def, hTime, hOwner]
    refine wp_iff_cons rfl ?_
    simp only [ne_eq, show (1 : UInt32) ≠ 0 by decide, not_false_eq_true, ↓reduceIte]
    wp_scan_frame [hScratch, choiceValues, func13Def, hTime, hOwner]
    refine wp_call_tw hCall ?_
    rintro final returned ⟨hFinal, rfl⟩
    subst final
    wp_scan_frame [hScratch, choiceValues, func13Def, hTime, hOwner]
    scan_calls hCall [hScratch, choiceValues, func13Def, hTime, hOwner]
    try subst owner
    try simp only [UInt64.ofNat_add, show UInt64.ofNat 1 = 1 from rfl, UInt64.add_sub_cancel] at ⊢
    simp only [choose, hTime, true_or, ↓reduceIte, scanFrame,
      List.cons_append, List.nil_append, UInt64.ofNat_add, show UInt64.ofNat 1 = 1 from rfl] at hNext
    apply hNext
    simp only [List.length_set, hScratch]

#print axioms scanStep_time_lt
