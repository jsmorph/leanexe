...
theorem scanStep_time_eq (env : HostEnv Unit) (initial : Store Unit)
  ...
  by_cases hExcess : candidate.excess < incumbent.excess
  all_goals
    by_cases hOwner : owner = 0
    all_goals
      have hCall := hPred
      try simp only [hOwner] at hCall
      scan_calls hCall [hScratch, choiceValues, func13Def, hTime, hEq, hExcess, hOwner]
      try subst owner
      try simp only [UInt64.ofNat_add, show UInt64.ofNat 1 = 1 from rfl, UInt64.add_sub_cancel] at ⊢
      simp only [choose, hTime, hEq, hExcess, false_or, or_false, or_true, true_and, UInt64.lt_irrefl,
        ↓reduceIte, scanFrame, List.cons_append, List.nil_append, UInt64.ofNat_add, show UInt64.ofNat 1 = 1 from rfl] at hNext
      apply hNext
      simp only [List.length_set, hScratch]

#print axioms scanStep_time_eq
