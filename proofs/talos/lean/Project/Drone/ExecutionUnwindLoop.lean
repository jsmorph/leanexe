...
theorem unwind_loop_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
    ...
    (hAux : aux.length = 36) (hAux33 : aux[33]? = some (.i64 0))
    (hAux35 : aux[35]? = some (.i64 0)) (hCount : count ≤ index + 1) (hIndex : index < terrain.size) (hState : state < 45)
    ...
    (hNext : ∀ (final : Store Unit) (heap : Heap) (node : FreeNode) (output : Array UInt64)
      ...
      (nextAux : List Value) (nextScratch : Scratch), nextAux.length = 36 → nextAux[33]? = some (.i64 0) → nextAux[35]? = some (.i64 0) →
  ...
  · exact ⟨initialHeap, rowNode, row, count, index, state, false, out0, out1, aux, s, rfl,
      hAux, hAux33, hAux35, le_rfl, le_rfl, hIndex, hCount, hState, by simp, hHeap, hBudget,
  ...
  · rintro store frame ⟨heap, node, currentRow, fuel, currentIndex, currentState, tracked,
      currentOut0, currentOut1, currentAux, scratch, rfl, hLength, hCurrent33, hCurrent35, hFuelBound, hIndexBound, hCurrentIndex,
    ...
    | zero =>
      ...
      simpa only [unwindFrame, unwindParams, List.take_zero, List.drop_zero, List.nil_append] using
        hNext store heap node currentRow currentIndex currentState tracked currentOut0 currentOut1
          currentAux scratch hLength hCurrent33 hCurrent35 hCurrentHeap hCurrentBudget hCurrentRow hPreserve hFresh hOutputSize hExpected
    | succ fuel =>
      ...
      intro final nextHeap nextNode nextAux nextScratch hNextLength hNext33 hNext35 hFinalHeap hFinalBudget hFinalTerrain hFinalHistory
      ...
      · refine ⟨nextHeap, nextNode, (currentRow.push (speed currentState)).push
          ...
          rfl, hNextLength, hNext33, hNext35.trans hCurrent35, by omega, by omega, by omega, by omega, hNextState, ?_, hFinalHeap, ?_,
