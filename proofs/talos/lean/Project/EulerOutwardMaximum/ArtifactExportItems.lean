...
theorem export0_decoded :
    exportEntry { bytes := artifactBytes, pos := 426, limit := 548 } =
      .ok (Cache.raw.exports[0]!, { bytes := artifactBytes, pos := 435, limit := 548 }) := by cbv

theorem export1_decoded :
    exportEntry { bytes := artifactBytes, pos := 435, limit := 548 } =
...
theorem exports_tail1_decoded :
    Internal.vectorLoop exportEntry 10 { bytes := artifactBytes, pos := 435, limit := 548 } =
...
theorem exports_tail0_decoded :
    Internal.vectorLoop exportEntry 11 { bytes := artifactBytes, pos := 426, limit := 548 } =
...
theorem exports_vector_decoded :
    vector exportEntry { bytes := artifactBytes, pos := 425, limit := 548 } =
      .ok (Cache.raw.exports, { bytes := artifactBytes, pos := 548, limit := 548 }) := by
  refine vector_eq_of_parts (length := 11)
    (itemsStart := { bytes := artifactBytes, pos := 426, limit := 548 }) ?_ ?_ ?_
  · cbv
  · decide
