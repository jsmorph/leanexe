...
import Project.EulerOutwardMaximum.ArtifactParsedSections
...
theorem decode_eq_cache_parts : decode artifactBytes = .ok Cache.raw := by
  refine decode_eq_of_parts (versionStart := { bytes := artifactBytes, pos := 4, limit := 5260 })
    (sectionsStart := { bytes := artifactBytes, pos := 8, limit := 5260 }) (finish := { bytes := artifactBytes, pos := 5260, limit := 5260 }) ?_ ?_ ?_ ?_
  · cbv
  · cbv
