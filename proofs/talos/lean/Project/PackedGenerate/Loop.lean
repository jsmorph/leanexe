...
namespace Project.PackedGenerate.Loop
...
theorem generated_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (P : Locals → Prop) (hP : P frame)
    (hAdvance : ∀ next index (h : next.validIndex 3), P next →
      P (FixedArrayCopy.counterFrame next 3 index h))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hdone : ∀ final result,
      Ready 3 8 9 count count pointer result →
      result.get 1 = some (.i64 offset.toUInt64) → P result →
  ...
  apply program_spec 3 8 9 wordProgram «module» env initial frame pointer count
    ...
    (fun result => result.get 1 = some (.i64 offset.toUInt64) ∧ P result)
    (by decide) (by decide) hready ⟨hoffset, hP⟩ hfit hbound
  ...
  · intro current next index _ hready hoffset _ Q rest hnext
    simp only [wordProgram, List.cons_append, List.nil_append,
      wp_localGet_cons, Frame.withValues_get, hoffset.1, hready.2.1,
      wp_constI64_cons, wp_andI64_cons, wp_addI64_cons, word_value]
    exact hnext next hready hoffset
  · intro final result hready hState
    exact hdone final result hready hState.1 hState.2

#print axioms emitted_loop
#print axioms generated_loop_spec

end Project.PackedGenerate.Loop
