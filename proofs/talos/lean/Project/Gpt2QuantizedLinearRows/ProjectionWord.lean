import Project.Gpt2QuantizedLinearRows.ProjectionBias
...
theorem branch_parts (withBias : Bool) :
    branch withBias = dotPrefix withBias ++ scaleCode withBias ++
      if withBias then biasCode else [.localSet 67, .localGet 67] := by
  cases withBias <;> rfl
...
theorem branch_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hWidth : 0 < width) (hCount : rows * outputWidth < UInt64.size) (hIndex : index < rows * outputWidth)
  ...
  apply dotPrefix_spec env initial weightsPtr inputPtr valuePtr scalePtr outputPtr weights input
    weightOffset scaleOffset biasOffset width outputWidth rows index withBias frame
    hWeights hValues hWeightSize hWidth hCount hIndex hReady hState
  intro afterDot hReadyDot hStateDot
  apply scaleCode_spec env initial weightsPtr inputPtr valuePtr scalePtr outputPtr weights input
    weightOffset scaleOffset biasOffset width outputWidth rows index withBias _ afterDot
    hWeights hScales hScaleSize hCount hIndex hReadyDot hStateDot
  ...
  | false =>
    ...
    have hTyped : I64Values afterScale.locals := by
      rcases hStateScale with ⟨_, _, _, _, _, _, _, _, _, _, _, h⟩
      exact h
    ...
    let result := { afterScale with locals := afterScale.locals.set 56 (.i64 scaled.toUInt64) }
    have hReadyResult : PackedGenerateLoop.Ready 29 72 73 (rows * outputWidth) index outputPtr result := by
...
theorem outputWord_spec (env : HostEnv Unit) (initial : Store Unit)
  ...
  have hBranch (post : Assertion Unit)
      (hPost : ∀ result, PackedGenerateLoop.Ready 29 72 73 (rows * outputWidth) index outputPtr result →
        OutputState (parameters weightsPtr inputPtr weights input weightOffset scaleOffset biasOffset
          width outputWidth rows withBias) valuePtr scalePtr width outputWidth rows result →
    ...
    have h := branch_spec env initial weightsPtr inputPtr valuePtr scalePtr outputPtr weights input
      weightOffset scaleOffset biasOffset width outputWidth rows index withBias frame
      hWeights hValues hScales hWeightSize hScaleSize hBiasSize hWidth hCount hIndex hReady hState post []
