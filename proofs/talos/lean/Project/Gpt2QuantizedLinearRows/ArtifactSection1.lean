...
theorem types_section_decoded :
    sized (vector funcType) { bytes := artifactBytes, pos := 9, limit := 4741 } = .ok (Cache.raw.types, { bytes := artifactBytes, pos := 125, limit := 4741 }) := by
  refine sized_eq_of_parts (size := 115)
    (payload := { bytes := artifactBytes, pos := 10, limit := 4741 }) (finish := { bytes := artifactBytes, pos := 125, limit := 125 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
