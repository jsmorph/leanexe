import Project.Gpt2QuantizedLinearRows.ProjectionWord
...
theorem outputState_advance (params : List Wasm.Value) (valuePtr scalePtr : UInt64)
    ...
    (frame : Locals) (index : Nat) (hValid : frame.validIndex 29)
    ...
    OutputState params valuePtr scalePtr width outputWidth rows
      (FixedArrayCopy.counterFrame frame 29 index hValid) := by
  rcases hState with ⟨hFrameParams, hLength, hValueOwner, hScaleOwner, hValueCopyOwner, hValuePtr,
  ...
  simp (config := { maxDischargeDepth := 64 }) only [OutputState, FixedArrayCopy.counterFrame,
    ...
    Nat.reduceSub, Nat.reduceLT, Nat.reduceEqDiff, reduceIte, hValueOwner, hScaleOwner,
...
theorem outputLoop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hReady : PackedGenerateLoop.Ready 29 72 73 (rows * outputWidth) 0 outputPtr frame)
    ...
    (hDone : ∀ final result,
      PackedGenerateLoop.Ready 29 72 73 (rows * outputWidth) (rows * outputWidth) outputPtr result →
    ...
    wp «module» ((func8.drop 83).take 1 ++ rest) Q initial frame env := by
  ...
  · intro current next index hIndex hReady hState hWrites Q rest hNext
    exact outputWord_spec env current weightsPtr inputPtr valuePtr scalePtr outputPtr weights input
      ...
      (hValues.writesRange hWrites (by simpa only [quantized_values_size] using hValueSep))
      (hScales.writesRange hWrites (by simpa only [quantized_scales_size] using hScaleSep))
      hWeightSize hScaleSize hBiasSize hWidth hCount hIndex hReady hState Q rest hNext
  · intro final result hReady hState hOutput hWrites
    exact hDone final result hReady hState (by simpa only [linearRows_eq] using hOutput) hWrites
