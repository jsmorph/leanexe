...
theorem code0_decoded :
    code { bytes := artifactBytes, pos := 305, limit := 4741 } = .ok (Cache.raw.codes[0]!, { bytes := artifactBytes, pos := 394, limit := 4741 }) := by
  refine code_eq_of_parts (size := 88)
    (payload := { bytes := artifactBytes, pos := 306, limit := 4741 })
