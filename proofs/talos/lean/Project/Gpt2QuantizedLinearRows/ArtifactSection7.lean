...
theorem exports_section_decoded :
    sized (vector exportEntry) { bytes := artifactBytes, pos := 181, limit := 4741 } = .ok (Cache.raw.exports, { bytes := artifactBytes, pos := 301, limit := 4741 }) := by
  refine sized_eq_of_parts (size := 119)
    (payload := { bytes := artifactBytes, pos := 182, limit := 4741 }) (finish := { bytes := artifactBytes, pos := 301, limit := 301 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
