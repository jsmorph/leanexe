import Project.Gpt2QuantizedLinearRows.ProjectionRescale
...
def biasCode : Wasm.Program := (branch true).drop 131
...
theorem biasCode_spec (env : HostEnv Unit) (initial : Store Unit)
  ...
  have hBound := scale_index_bound biasOffset outputWidth index weights.size hBiasSize hOut
  ...
  refine PackedWordAccess.guard_spec «module» env initial _ weightsPtr weights
    (biasOffset + index % outputWidth * 4) 74 75 76
