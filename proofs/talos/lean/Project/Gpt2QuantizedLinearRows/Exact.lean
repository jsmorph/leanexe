import Project.Gpt2QuantizedLinearRows.Linear
...
def ExactSpecFor (module_ : Wasm.Module) : Prop :=
  ∀ (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    4 * rows ≤ 2^32 → 4 * (rows * outputWidth) ≤ 2^32 →
    (takeFirstFitFrom 0 (QuantizeRows.scaleNeed rows) heap.nodes = none →
      heap.top.toNat + 48 + (QuantizeRows.scaleNeed rows).toNat < 4294967296 ∧
      FixedArrayBump.requiredPages heap.top (QuantizeRows.scaleNeed rows) ≤ initial.memoryCap module_ 0) →
    (let next := heap.allocate (QuantizeRows.scaleNeed rows);
      takeFirstFitFrom 0 (QuantizeRows.byteNeed width rows) next.nodes = none →
        ...
        FixedArrayBump.requiredPages next.top (QuantizeRows.byteNeed width rows) ≤ initial.memoryCap module_ 0) →
...
theorem linearRows_exact_for : ExactSpecFor «module» := linearRows_exact

#print axioms linearRows_exact_for
