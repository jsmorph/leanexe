...
namespace Project.ClobMarket.Frozen.Correct

open Wasm Project.Common Project.Clob Project.ClobMarket
  Project.ClobMarket.Frozen.Model Project.ClobLimit.Frozen.InternalLoopInvariant
...
def SourceResultAt (st : Store Unit) (values : List Value)
    (result : Project.ClobLimit.Frozen.Model.OpResultL) : Prop :=
...
inductive OutcomeAt (initial final : Store Unit)
  ...
  | valid
      ...
      (data : Project.ClobLimit.Frozen.InternalLoopResult.OutputData)
      (hValues : values = [.i64 data.trades, .i64 data.book, .i64 0])
      (hOutput : Project.ClobLimit.Frozen.InternalLoopResult.OutputAt
        (Project.ClobLimit.Frozen.RunMatchCorrect.runMatchContext initial os
...
theorem func21_correct : CorrectSpec := by
  ...
  · refine TerminatesWith.mono
    ...
    have hContext :=
      Project.ClobLimit.Frozen.RunMatchCorrect.runMatchContext_result st os
...
end Project.ClobMarket.Frozen.Correct
