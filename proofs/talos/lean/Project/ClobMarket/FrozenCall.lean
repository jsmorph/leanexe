...
namespace Project.ClobMarket.Frozen.Call

open Wasm Project.Clob Project.ClobMarket Project.ClobMarket.Frozen.Model
...
theorem callProg_spec (env : HostEnv Unit) (st : Store Unit)
    ...
    (hCall : TerminatesWith (m := Project.ClobMarket.Frozen.«module») (id := 18)
      (initial := st) (env := env)
      (Project.ClobLimit.Frozen.RunMatchCorrect.runMatchArgs 0 book
        (unlimitedTakerL order)) P)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ (st' : Store Unit) (values : List Value), P st' values →
      wp Project.ClobMarket.Frozen.«module» rest Q st'
        { callFrame book order with values := values } env) :
    wp Project.ClobMarket.Frozen.«module» (Entry.callProg ++ rest) Q st
      (Price.priceFrame book order) env := by
  have hCall' : TerminatesWith (m := Project.ClobMarket.Frozen.«module»)
    ...
    simpa [Project.ClobLimit.Frozen.RunMatchCorrect.runMatchArgs] using hCall
...
end Project.ClobMarket.Frozen.Call
