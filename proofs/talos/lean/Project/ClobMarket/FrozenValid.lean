...
namespace Project.ClobMarket.Frozen.Valid

open Wasm Project.Common Project.Clob Project.ClobMarket
  Project.ClobMarket.Frozen.Model
  Project.ClobLimit.Frozen.InternalLoopInvariant
  Project.ClobMatchFuel.Frozen.AllocatorFrame Project.ClobPostOnly.Frozen.Model
...
def ValidSpec : Prop :=
  ∀ (env : HostEnv Unit) (st : Store Unit)
    ...
    TerminatesWith (m := Project.ClobMarket.Frozen.«module») (id := 21)
      (initial := st) (env := env) (Entry.marketArgs book order)
      (fun st' values =>
        ∃ data,
          values = [.i64 data.trades, .i64 data.book, .i64 0] ∧
          Project.ClobLimit.Frozen.InternalLoopResult.OutputAt
            (Project.ClobLimit.Frozen.RunMatchCorrect.runMatchContext st os
...
theorem func21_valid : ValidSpec := by
  ...
  let ctx := Project.ClobLimit.Frozen.RunMatchCorrect.runMatchContext st os taker
    g0 g2 limit
  have hRunMatch : TerminatesWith (m := Project.ClobMarket.Frozen.«module»)
      (id := 18) (initial := st) (env := env)
      (Project.ClobLimit.Frozen.RunMatchCorrect.runMatchArgs 0 book taker)
      (Project.ClobLimit.Frozen.InternalLoopResult.Postcondition ctx) := by
  ...
  · simp [Project.ClobMarket.Frozen.«module»]
  · change wp Project.ClobMarket.Frozen.«module» func21 _ st
    ...
    apply Call.callProg_spec env st book order
      (Project.ClobLimit.Frozen.InternalLoopResult.Postcondition ctx) hRunMatch
...
end Project.ClobMarket.Frozen.Valid
