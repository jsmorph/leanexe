...
namespace Project.ClobMarket.Frozen.ValidResult
...
def outputFrame (book : UInt64) (order : OrderL)
    (ctx : Context) (data : Project.ClobLimit.Frozen.InternalLoopResult.OutputData) :
    Locals :=
...
theorem validResultProg_spec (env : HostEnv Unit) (st : Store Unit)
    ...
    (values : List Value)
    (hValues : values =
      Project.ClobLimit.Frozen.InternalLoopResult.outputValues ctx data)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.ClobMarket.Frozen.«module» rest Q st
      (resultFrame book order ctx data) env) :
    wp Project.ClobMarket.Frozen.«module»
  ...
  simp [Project.ClobLimit.Frozen.InternalLoopResult.outputValues,
    Call.callFrame]
...
theorem resultProg_spec (env : HostEnv Unit) (st : Store Unit)
    ...
    wp Project.ClobMarket.Frozen.«module» (Entry.resultProg ++ rest) Q st
      (resultFrame book order ctx data) env := by
  simp only [Entry.resultProg, List.cons_append, List.nil_append]
  wp_run
  simpa [resultFrame, outputFrame] using hNext

end Project.ClobMarket.Frozen.ValidResult
