...
import Project.EulerOutwardFaceStep.ArtifactCodeVector
import Project.EulerOutwardFaceStep.ArtifactMetadata
...
theorem sections_from6 :
    sectionLoop 9063 6 Cache.raw { bytes := artifactBytes, pos := 9077, limit := 9077 } =
      .ok (Cache.raw, { bytes := artifactBytes, pos := 9077, limit := 9077 }) := by rfl
