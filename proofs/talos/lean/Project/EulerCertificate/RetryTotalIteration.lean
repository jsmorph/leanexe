import Project.EulerCertificate.RetryTotalValid
import Project.EulerCertificate.RetryTotalInvalid
...
theorem retry_total_iteration_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
  ...
  · obtain ⟨fuel, dt, hFrame, hSame, hReserved⟩ := hActive
    ...
    · subst fuel
      ...
      have hShape := AnnotationMatches.function_179_while_loop_0_guard_eq
      change some (retryLoop.take 7) = some (FuelGuard.program 0 25) at hShape
      rw [Option.some.inj hShape]
      apply FuelGuard.program_spec 0 25 module env store frame 0 0 hFrame.values
  ...
  · obtain ⟨result, hFrame, hResult, hReserved, hCapacity, hSeparated⟩ := hDone
    rw [← List.take_append_drop 7 retryLoop]
    apply retry_returned_guard_spec env store frame expected.status expected.dt result.root expected.boundary hFrame
