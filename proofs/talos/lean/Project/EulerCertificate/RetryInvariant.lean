import Project.EulerCertificate.RetryBranches
...
def RetryTotalDone (initial : Store Unit) (initialHeap : Heap) (need : UInt64)
  ...
  ∃ result : FreeNode,
    RetryReturnedAt frame expected.status expected.dt result.root expected.boundary ∧
...
theorem retry_loop_parts : retryLoop = retryLoop.take 7 ++
    ...
    [.localGet 28, .constI64 1, .eqI64,
      .iff 0 1 [.constI64 1] [.constI64 0] [] [.i64],
      .constI64 0, .eqI64, .eqz,
      .iff 0 0 retryTrial (retryFailureProgram 3 ++ retryZeroBoundary 88 ++ [.constI64 1, .localSet 25]), .br 0] := rfl
