import Project.EulerCertificate.AdvanceScan
...
open Project.EulerRiemann.Execution (Heap RetryStoreAt)

macro "certificate_advance_trial_peel" : tactic => `(tactic|
  repeat
    first
...
theorem advance_total_trial_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (n : Nat) (trials time alpha : UInt64) (boundary : Vector) (spare limit pageLimit : Nat)
    ...
    (hAlpha : frame.locals[24]? = some (.i64 alpha))
    ...
    (hNext : let dt := Time.proposal n time alpha
      let trial := Control.retry (dt.toNat + 1) n trials.toNat time dt alpha grid
      ∀ final finalHeap result,
        ...
        wp module rest Q final
          (advanceTrialFrame frame n trials time source.root alpha dt trial.dt result.root trial.boundary trial.status) env) :
    wp module (advanceTrialBody.take 96 ++ rest) Q initial frame env := by
  ...
  have hRetry := retry_pages_exact env initial heap source grid n (dt + 1) trials time dt alpha spare limit pageLimit
  ...
  unfold advanceTrialBody advanceWorkBody advanceLoop func184
  dsimp only
  certificate_advance_trial_peel
  ...
  certificate_advance_trial_peel
  simpa [advanceTrialFrame, hParams, dt, advanceParams, vectorValues, boundsValues] using
