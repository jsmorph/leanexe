import Project.EulerCertificate.OutputAppendData
...
theorem output_append_owned_spec (env : HostEnv Unit)
    ...
    (hNext : ∀ final : Store Unit,
      ...
      wp module rest Q final
        (counterFrame (resultFrame frame 55 (allocatedRoot heap.top need heap.nodes)) 56 right.size
          (by simpa only [Locals.validIndex, resultFrame_params, resultFrame_locals_length] using hCounter)) env) :
    wp module (outputAppendDataProgram ++ rest) Q
  ...
  apply output_append_data_spec env _ frame source.root upper.root
