import Project.EulerCertificate.RetryTotalLoop
import Project.EulerCertificate.RetryReturn
...
theorem retry_pages_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env module 179 initial
      ...
      (fun final values => ∃ finalHeap result,
        values = vectorValues expected.boundary ++ [.i64 result.root, .i64 result.root, .i64 expected.dt, .i64 expected.status] ∧
  ...
  let expected := Control.retry fuel.toNat n trials.toNat time dt alpha grid
  let entry := func179Def.toLocals [.i64 fuel, .i64 (UInt64.ofNat n), .i64 trials, .i64 time,
  ...
  refine TerminatesWith.of_wp_entry_for (f := func179Def) rfl ?_ (by decide)
  change wp module func179 _ initial entry env
  rw [retry_loop_shape, List.append_assoc]
  have hEntryShape : func179.take 4 = [.constI64 0, .localSet 8, .constI64 0, .localSet 25] := rfl
  rw [hEntryShape]
  wp_run [entry, func179Def, List.set, List.cons_append, List.nil_append,
    List.getElem?_cons_zero, List.getElem?_cons_succ,
    reduceIte, Nat.reduceAdd, Nat.reduceLT, Nat.reduceSub]
  change wp _ ([.block 0 0 [.loop 0 0 retryLoop]] ++ func179.drop 5) _ initial entry env
  ...
  · obtain ⟨result, hFrame, hResult, hReserved, hCapacity, hSeparated⟩ := hDone
    ...
    have hCleared : ({ resultFrame with values := [] } : Locals) = resultFrame := by
      cases resultFrame
      simp_all
    rw [hCleared]
    apply retry_return_spec env final resultFrame expected.status expected.dt result.root expected.boundary hFrame
    refine ⟨finalHeap, result, ?_, hStore, hFinalPages, hResult, hReserved, hCapacity, hSeparated⟩
    simp [expected, vectorValues, boundsValues]
  · let need := normalizedCapacity (UInt64.ofNat grid.size) 7
    ...
    have hExpected : expected = { status := 4, dt := lastDt, grid := #[], boundary := Vectors.zero } := by
      simpa only [Control.retry] using hSame.symm
    change Control.retry fuel.toNat n trials.toNat time dt alpha grid = { status := 4, dt := lastDt, grid := #[], boundary := Vectors.zero } at hExpected
    let params := resultFrame.params
    let saved := resultFrame.locals.take 115
    have hParams : params.length = 8 := by simp [params, hFrame.params]
    have hSaved : saved.length = 115 := by simp [saved, hFrame.locals]
    have hDtRead : params[4]? = some (.i64 lastDt) := by simp [params, hFrame.params]
    have hDoneRead : saved[17]? = some (.i64 0) := by
      simpa only [saved, List.getElem?_take_of_lt (by decide : 17 < 115), boolWord,
    ...
    apply retry_failure_program_spec env final finalHeap params saved hParams hSaved
      4 lastDt 0 0 0 0 0 0 hDtRead hStore.heapState hBump hStore.pages _ (retryZeroBoundary 101)
    intro previous current capacity next
    apply retry_zero_spec env (finalHeap.emptyStore final) _ 101 (Or.inr rfl)
      (by simp [retryFailureFrame, FixedArrayResult.finishFrame, FixedArrayFold.resultFrame,
    ...
    simp only [List.range_succ, List.range_zero, List.map_cons, List.map_nil,
      List.cons_append, List.nil_append, List.flatten_nil]
    wp_run [retryZeroFrame, retryFailureFrame, FixedArrayResult.finishFrame, FixedArrayFold.resultFrame,
    ...
    · simp [hExpected, allocatedNode, vectorValues, boundsValues, Vectors.zero]
