import Project.EulerCertificate.AdvanceTotalInvariant
import Project.EulerCertificate.AdvanceTotalTrial
import Project.EulerCertificate.ControlProgress
...
macro "certificate_advance_total_peel" : tactic => `(tactic|
  repeat
    first
    | wp_run [advanceTimeFrame, advanceScanFrame, advanceTrialFrame,
        advanceParams, advanceAccumulatedFrame, advanceReturnedFrame, advanceReturnTail, advanceContinuedFrame,
        vectorValues, boundsValues, List.range_succ, List.range_zero, List.flatMap_cons, List.flatMap_nil,
...
theorem advance_total_work_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
    ...
    (source : FreeNode) (grid : Array Project.EulerRiemann.Traversal.Cell) (boundary : Vector) (tracked : Bool)
    ...
    (hFrame : AdvanceFrameAt frame fuel n trials time source.root (if tracked then source.root else 0) 0 0 false boundary)
    (hSame : Control.advance fuel.toNat n trials.toNat time grid boundary = expected)
    ...
    (hNext : ∀ final resultFrame,
      advanceTotalInvariant initial initialHeap n trials startTime expected spare limit pageLimit final resultFrame →
  ...
  refine advance_scan_spec env store (advanceTimeFrame frame) n fuel trials time source.root boundary grid
    hFrame.time.params hFrame.time.locals hFrame.time.values hCurrent.owner.buffer.values _ _ ?_
  by_cases hScan : (OutwardMaximum.gridUpper grid).status = 0
  · have hScanned := hFrame.time.scan { status := 0, value := (OutwardMaximum.gridUpper grid).value }
    ...
    refine advance_total_trial_spec env store heap (advanceScanFrame (advanceTimeFrame frame) source.root
      { status := 0, value := (OutwardMaximum.gridUpper grid).value })
      fuel source grid n trials time (OutwardMaximum.gridUpper grid).value boundary callSpare limit pageLimit hScanned.params hScanned.locals
      hScanned.values (by simp [advanceScanFrame, advanceTimeFrame, advanceParams, frame, hLocals])
    ...
    have hTrialFrame := hScanned.trial (OutwardMaximum.gridUpper grid).value dt trial.dt result.root trial.boundary trial.status
    by_cases hTrialSuccess : trial.status = 0
    · have hNextSame : Control.advance (fuel - 1).toNat n trials.toNat (IEEE64.add time trial.dt) trial.grid
          (Vectors.add boundary trial.boundary) = expected := by
        simpa [hFuelNat, Control.advance, hTime, hScan, dt, trial, hTrialSuccess] using hSame
      have hTrialValid := (Control.retry_valid (dt.toNat + 1) n trials.toNat time dt (OutwardMaximum.gridUpper grid).value grid hTrialSuccess)
      ...
      have hNextIndexed := Control.retry_indexed (dt.toNat + 1) n trials.toNat time dt (OutwardMaximum.gridUpper grid).value grid hCurrent.indexed hTrialSuccess
      ...
      apply advance_continue_spec env final finalHeap
        (advanceScanFrame (advanceTimeFrame frame) source.root
          { status := 0, value := (OutwardMaximum.gridUpper grid).value }) fuel source grid
        n trials time (OutwardMaximum.gridUpper grid).value dt trial.dt result.root boundary trial.boundary tracked
      ...
      dsimp only [frame]
      certificate_advance_total_peel
      have hNextFrame := (hTrialFrame.accumulated trial.boundary).continued
        (IEEE64.add time trial.dt) result.root (Vectors.add boundary trial.boundary)
      simp only [advanceContinuedFrame, advanceAccumulatedFrame, advanceTrialFrame, advanceScanFrame, advanceTimeFrame, advanceParams,
        hTrialSuccess] at hNextFrame
      apply hNext
      · exact ⟨_, hReplacement.1, hReplacementPages, Or.inl ⟨fuel - 1, IEEE64.add time trial.dt, result, trial.grid, Vectors.add boundary trial.boundary, true,
    ...
    · have hExpected : expected = { status := trial.status, time, grid, boundary } := by
      ...
      certificate_advance_total_peel
      simp only [advanceReturnedFrame, advanceTrialFrame, advanceScanFrame, advanceTimeFrame, advanceParams] at hReturned
      apply hNext
      ...
      · rw [hReturned.measure, hFrame.measure]
        simp
  · have hExpected : expected = { status := 2, time, grid, boundary } := by
      simpa [hFuelNat, Control.advance, hTime, hScan] using hSame.symm
    have hScanned := hFrame.time.scan (OutwardMaximum.gridUpper grid)
    ...
    certificate_advance_total_peel
    simp only [advanceReturnedFrame, advanceScanFrame, advanceTimeFrame, advanceParams] at hReturned
    apply hNext
