import Project.EulerCertificate.RetryGuard
...
def retryRatioFrame (frame : Locals) (n : Nat) (dt alpha : UInt64) (result : Checked) : Locals :=
  let locals := frame.locals.set 21 (.i64 (UInt64.ofNat n))
  let locals := locals.set 22 (.i64 dt)
  let locals := locals.set 23 (.i64 alpha)
  let locals := locals.set 25 (.i64 result.value)
  let locals := locals.set 24 (.i64 result.status)
  let locals := locals.set 26 (.i64 result.status)
  let locals := locals.set 27 (.i64 result.value)
...
theorem retry_ratio_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerCertificate.«module» (retryTrial.take 16 ++ rest) Q store frame env := by
  ...
  unfold retryTrial retryLoop func179
  dsimp only
