import Project.EulerCertificate.RetryTotalStep
...
theorem retry_total_valid_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
  ...
  have hRatio : ratioFrame.locals[27]? = some (.i64 checked.value) := by
  ...
  · have hSameStep : (match Control.attempt n trials.toNat dt checked.value grid with
      ...
      cases hAttempt : Control.attempt n trials.toNat dt
          (OutwardCfl.gridRatioChecked n dt alpha).value grid <;>
        simpa only [hFuelNat, Control.retry, hValid, ite_true, checked,
          beq_iff_eq, hChecked, hAttempt] using hSame
    dsimp only [frame]
    certificate_retry_ratio_peel
