import Project.Gpt2CachedStep.LinearRows.FrozenDotStep
...
theorem emitted_dot : wordCode = wordCode.take 30 ++ RangeFoldLoop.program 45 46 dotStep ++ wordCode.drop 31 := rfl

def OutputState (weightsOwner inputOwner weightsPtr inputPtr : UInt64) (weights input : ByteArray)
...
theorem word_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hready : PackedGenerateLoop.Ready 12 43 44 (rows * outputWidth) index outputPtr frame)
    ...
    (hnext : ∀ result,
      PackedGenerateLoop.Ready 12 43 44 (rows * outputWidth) index outputPtr result →
  ...
  simp only [wordCode, outerBody, func21, List.getElem?_cons_zero, List.getElem?_cons_succ,
  ...
  apply RangeFoldLoop.program_spec_with_stack (values := [.i32 (PackedGenerateLoop.address outputPtr index)])
    (count := inputWidth) (P := fun inner => Accumulator weightsOwner inputOwner weightsPtr inputPtr weights input
  ...
  · intro inner next hinner hinnerReady hacc Q rest hnext
    exact dotStep_spec env initial weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset
  ...
  · intro result hinnerReady hacc
    ...
    refine wp_call_tw ((PackedWordRead.exact «module» 17 (some 17) rfl rfl
      env initial weightsOwner weightsPtr weights (biasOffset + index % outputWidth) hweights hbiasIndex).append_args
    ...
    apply Frame.of_withValues
      (P := fun result => PackedGenerateLoop.Ready 12 43 44 (rows * outputWidth) index outputPtr result ∧
        OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows result)
