import Project.Gpt2CachedStep.LinearRows.FrozenWord
...
open Project.Gpt2LinearRows (input_index_bound weight_index_bound bias_index_bound)

open Wasm Project.Common Project.ProofKit PackedMemory

set_option maxRecDepth 32768 in
theorem emitted_generation :
    (func21.drop 49).take 1 = PackedGenerateLoop.program 12 43 44 wordCode := rfl

theorem outputState_advance (weightsOwner inputOwner weightsPtr inputPtr : UInt64) (weights input : ByteArray)
    (weightOffset biasOffset inputWidth outputWidth rows : Nat)
    (frame : Locals) (index : Nat) (hvalid : frame.validIndex 12)
    (hstate : OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows frame) :
    OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows
      (FixedArrayCopy.counterFrame frame 12 index hvalid) := by
...
theorem generated_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hready : PackedGenerateLoop.Ready 12 43 44 (rows * outputWidth) 0 outputPtr frame)
    (hstate : OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows frame)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hdone : ∀ final result,
      PackedGenerateLoop.Ready 12 43 44 (rows * outputWidth) (rows * outputWidth) outputPtr result →
      OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows result →
    ...
    wp «module» ((func21.drop 49).take 1 ++ rest) Q initial frame env := by
  ...
  apply PackedGenerateLoop.program_spec (value := fun index =>
    ...
    (P := OutputState weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows)
  · decide
  · decide
  · exact hready
  · exact hstate
  ...
  · intro next index hvalid hstate
    exact outputState_advance weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset
  ...
  · intro current next index hindex hready hstate hwrites Q rest hnext
    ...
    apply word_spec env current weightsOwner inputOwner weightsPtr inputPtr outputPtr weights input weightOffset biasOffset
