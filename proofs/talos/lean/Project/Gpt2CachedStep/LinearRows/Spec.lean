import Project.Gpt2CachedStep.LinearRows.Entry

namespace Project.Gpt2CachedStep.LinearRows.Spec
...
theorem linearRows_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env «module» 21 initial
      [.i64 (UInt64.ofNat rows), .i64 (UInt64.ofNat outputWidth), .i64 (UInt64.ofNat inputWidth),
        ...
        .i64 (UInt64.ofNat input.size), .i64 inputPtr, .i64 inputOwner, .i64 (UInt64.ofNat weights.size), .i64 weightsPtr, .i64 weightsOwner]
      (fun final values =>
        values = [.i64 (UInt64.ofNat (4 * (rows * outputWidth))), .i64 (outputRoot base (rows * outputWidth) nodes), .i64 (outputRoot base (rows * outputWidth) nodes)] ∧
  ...
  change wp «module» func21 _ initial
    (entryFrame (parameters weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows)) env
  apply body_spec env initial weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows
  ...
  simpa [func21Def, Function.numParams, hvalues, -UInt64.ofNat_mul] using hpost

#print axioms linearRows_exact

end Project.Gpt2CachedStep.LinearRows.Spec
