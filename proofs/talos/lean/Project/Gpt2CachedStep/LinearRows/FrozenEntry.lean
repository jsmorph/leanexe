import Project.Gpt2CachedStep.LinearRows.FrozenEntryFrame
...
theorem body_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (Q : Assertion Unit)
    (hdone : ∀ final result,
      result.values = [.i64 (UInt64.ofNat (4 * (rows * outputWidth))), .i64 (outputRoot base (rows * outputWidth) nodes), .i64 (outputRoot base (rows * outputWidth) nodes)] →
    ...
    wp «module» func21 Q initial
      (entryFrame (parameters weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows)) env := by
  ...
  have hsplit : func21 = func21.take 18 ++ (func21.drop 18).take 12 ++
      (func21.drop 30).take 15 ++ func21.drop 45 := rfl
  rw [hsplit]
  ...
  apply PackedAllocation.program_spec «module» env initial
    (parameters weightsOwner inputOwner weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth rows)
    (saved (rows * outputWidth)) (List.replicate 8 (.i64 0)) 45 rfl
  ...
  have hsetup : func21.drop 45 = [.localGet 50, .localSet 44, .constI64 0, .localSet 12] ++
      (func21.drop 49).take 1 ++ func21.drop 50 := rfl
  rw [hsetup]
  simp only [List.append_assoc]
  ...
  apply generated_loop_spec env (bufferStore initial base (rows * outputWidth) allocations nodes)
    weightsOwner inputOwner weightsPtr inputPtr (outputRoot base (rows * outputWidth) nodes) weights input
  ...
  · dsimp [outputRoot]
    omega
  · exact ⟨rfl, rfl, rfl, rfl, by change 12 < 59; decide⟩
