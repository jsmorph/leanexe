import Project.Gpt2CachedStep.LayerNorm.FrozenMeans
...
def InversesBuiltState (params : List Wasm.Value) (meansPtr inversesPtr : UInt64)
    (rows : Nat) (frame : Locals) : Prop :=
  MeansBuiltState params meansPtr rows frame ∧
  frame.locals[32]? = some (.i64 inversesPtr) ∧ frame.locals[33]? = some (.i64 inversesPtr) ∧
  frame.locals[34]? = some (.i64 (UInt64.ofNat (4 * rows)))

set_option maxRecDepth 32768 in
theorem emitted_inverses_prefix : (func20.drop 49).take 49 = sizeCode 19 ++ PackedCapacity.program 70 72 ++
    PackedAllocation.program 72 ++ [.localGet 77, .localSet 71, .constI64 0, .localSet 20] ++
    (func20.drop 91).take 1 ++
    [.localGet 71, .localSet 41, .localGet 41, .localSet 42, .localGet 19, .localSet 43] := rfl

set_option maxRecDepth 32768 in
theorem inverses_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    (weightsOwner inputOwner weightsPtr inputPtr meansPtr : UInt64) (weights input : ByteArray)
    ...
    (hState : MeansBuiltState (parameters weightsOwner inputOwner weightsPtr inputPtr weights input
      scaleOffset biasOffset rows) meansPtr rows frame)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ final result,
      InversesBuiltState (parameters weightsOwner inputOwner weightsPtr inputPtr weights input scaleOffset biasOffset rows)
        meansPtr (allocatedRoot heap.top (temporaryNeed rows) heap.nodes) rows result →
      heap.PackedOutput initial final (temporaryNeed rows) (inverses input rows) →
      wp «module» rest Q final result env) :
    wp «module» ((func20.drop 49).take 49 ++ rest) Q initial frame env := by
  ...
  have hMeansSep := hMeansProtected.allocated_disjoint (temporaryNeed rows) hBound
  ...
  rw [emitted_inverses_prefix]
  simp only [List.append_assoc]
  apply sizeCode_spec env initial frame 19 rows hParamLength hLocals hValues
  ...
  let prepared := FixedArrayCapacity.capacityFrame (sizeFrame frame 19 rows) 72 (temporaryNeed rows)
  ...
  apply inversesLoop_spec env (heap.allocatePackedStore initial (temporaryNeed rows))
    weightsOwner inputOwner weightsPtr inputPtr meansPtr meansPtr (allocatedRoot heap.top (temporaryNeed rows) heap.nodes)
    weights input scaleOffset biasOffset rows _ hInputAllocated hInputSize hMeansAllocated
  ...
  · simp [PackedGenerateLoop.Ready,
    ...
    rfl
  · simp [hLocals] at hMeansOwner hMeansPointer hMeansSize
    simp (config := { maxDischargeDepth := 64 }) [InversesState,
      prepared,
      FixedArrayCapacity.capacityFrame, sizeFrame, hParams, parameters, hLocals, hMeansOwner, hMeansPointer, hMeansSize,
  ...
  have hOutput := heap.packedOutput initial final (temporaryNeed rows) (inverses input rows) hHeap
    (by rw [inverses_size]; exact hNeed) (fun h => (hBump h).1) hPages
    (by rw [inverses_size]; exact hWrites) hBytes
  rcases hState with ⟨hResultParams, hResultLocals, hResultMeansOwner, hResultMeansPtr, hResultMeansSize, hByteLength, hResultTyped⟩
  ...
  apply hNext
  · simp (config := { maxDischargeDepth := 64 }) only [InversesBuiltState, MeansBuiltState, parameters,
      hResultLocals, List.length_set, List.getElem?_set, Nat.reduceEqDiff, Nat.reduceLT, reduceIte,
      I64Values.set, hResultTyped, hResultMeansOwner, hResultMeansPtr, hResultMeansSize, and_self]
  · exact hOutput

#print axioms inverses_spec
