...
def oldReleaseCode : Wasm.Program :=
  [.localGet 126, .constI64 0, .neI64,
   .iff 0 0 [
     .localGet 28, .constI64 0, .neI64,
     .iff 0 0 [.localGet 28, .call 42] [],
     .localGet 25, .constI64 0, .neI64, .localGet 25, .localGet 28, .neI64, .and,
...
theorem oldRelease_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hOld : frame.get 126 = some (.i64 (if layer = 0 then 0 else 1)))
  ...
  · wp_packed_frame [hValues, hOld, hZero]
    refine wp_iff_cons rfl ?_
    simpa [wp_simp, oldReleaseHeap, oldReleaseStore, hZero, hFrame] using hNext (by simpa [oldReleaseHeap, oldReleaseStore, hZero] using hHeap)
  · have hInputOwned := hInput hZero
    ...
    wp_packed_frame [hValues, hUpdatesRead, hUpdatesNonzero]
    refine wp_iff_cons rfl ?_
    rw [ite_eq_left (by simp [hUpdatesNonzero])]
    ...
    wp_packed_frame [hValues, hInputRead, hUpdatesRead, hInputNonzero, hNe]
    refine wp_iff_cons rfl ?_
    rw [ite_eq_left (by simp [hNe, hInputNonzero])]
