import Project.Gpt2CachedStep.CachedHidden.TraversalState
...
theorem emitted_layerBody : layerBody =
    [.localGet 103, .localGet 104, .geUI64, .br_if 1] ++ (layerBody.drop 4).take 261 ++ [.br 0] := rfl
...
theorem layerLoop_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hState : LayerState (parameters weightsOwner weightsPtr cacheOwner cachePtr weights cache token position)
      embeddingNode.root embeddingNode.root 0 0 0 frame)
  ...
  apply wp_block_cons
  apply wp_loop_cons (Inv := Inv) (μ := PackedGenerateLoop.measure 103 12)
  ...
  · rintro current next ⟨index, hIndex, hTraversal⟩
    ...
    have hValues := hTraversal.state.2.2.1
    ...
    · subst index
      ...
      simpa [hState.2.2.1, hEmpty] using
    ...
    · have hLt : index < 12 := by omega
      ...
      have hResultEmpty : ({ result with values := [] } : Locals) = result :=
        Frame.ext _ _ rfl rfl hResult.state.2.2.1.symm
      constructor
