...
def jobCountFrame (count categories members : Nat) (wordsPointer incidencePointer internal : UInt64)
  ...
  jobFrame count categories wordsPointer incidencePointer internal state (jobCountSaved members (state.position + 1) saved) tail
...
theorem jobPrepareRow_exact (env : HostEnv Unit) (initial : Store Unit) (count categories members : Nat)
    ...
    (next : wp Project.Beck.«module» rest Q initial
      (jobReplicateFrame (jobParams count categories wordsPointer incidencePointer state)
    ...
    wp Project.Beck.«module» (jobEligible.take 16 ++ rest) Q initial
      (jobCountFrame count categories members wordsPointer incidencePointer internal state saved tail) env := by
