import Project.ClobMatchFuel.FrozenTradeAllocSearch
import Project.ClobMatchFuel.FrozenBookAllocFit

namespace Project.ClobMatchFuel.Frozen.TradeAllocFit
...
abbrev tradeAllocFitStore (st : Store Unit) (choice : FreeChoice) :
    Store Unit :=
  BookAllocFit.fixedArrayAllocFitStore st choice 4
...
private def fitInv (st0 : Store Unit) (base : Locals) (need : UInt64)
  ...
  fun st s =>
    (∃ capacity next : UInt64, ∃ visited remaining : List FreeNode,
      ...
      s = TradeAllocSearch.tradeAllocSearchFrame base need
        (previousRoot 0 visited)
        (freeHead (remaining ++ choice.node :: tail)) capacity next 0) ∨
    (st = tradeAllocFitStore st0 choice ∧
      s = TradeAllocSearch.tradeAllocSearchFrame base need choice.previous
...
theorem tradeAllocSearchProg_fit
    ...
    (hNext : wp «module» rest Q (tradeAllocFitStore st choice)
      (TradeAllocSearch.tradeAllocSearchFrame base need choice.previous
        choice.node.root choice.node.capacity choice.next choice.node.root) env) :
    wp «module» (TradeAllocSearch.tradeAllocSearchProg ++ rest) Q st
  ...
  simp only [TradeAllocSearch.tradeAllocSearchProg, List.cons_append,
    List.nil_append]
  apply wp_block_cons
  apply wp_loop_cons
      ...
      | nil =>
          ...
          | cons hp h32 hfit hrc hcapacity hnodeNext hsep htailRep =>
              ...
              · refine wp_iff_cons (c := 1) (vs := [])
                ...
                · simpa only [TradeAllocSearch.tradeAllocSearchFrame] using
                    hFinalFrame
                · unfold fitMeasure
              ...
              · refine wp_iff_cons (c := 0) (vs := [])
                ...
                · simpa only [TradeAllocSearch.tradeAllocSearchFrame,
...
end Project.ClobMatchFuel.Frozen.TradeAllocFit
