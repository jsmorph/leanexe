import Project.ClobMatchFuel.FrozenBookAllocSearch
import Project.ClobMatchFuel.FrozenBookAllocFitState

namespace Project.ClobMatchFuel.Frozen.BookAllocFit
...
private def fitInv (st0 : Store Unit) (base : Locals) (need : UInt64)
  ...
  fun st s =>
    (∃ capacity next : UInt64, ∃ visited remaining : List FreeNode,
      ...
      s = BookAllocSearch.bookAllocSearchFrame base need
        (previousRoot 0 visited)
        (freeHead (remaining ++ choice.node :: tail)) capacity next 0) ∨
    (st = bookAllocFitStore st0 choice ∧
      s = BookAllocSearch.bookAllocSearchFrame base need choice.previous
...
theorem bookAllocSearchProg_fit
    ...
    (hNext : wp «module» rest Q (bookAllocFitStore st choice)
      (BookAllocSearch.bookAllocSearchFrame base need choice.previous
        choice.node.root choice.node.capacity choice.next choice.node.root) env) :
    wp «module» (BookAllocSearch.bookAllocSearchProg ++ rest) Q st
  ...
  simp only [BookAllocSearch.bookAllocSearchProg, List.cons_append,
    List.nil_append]
  apply wp_block_cons
  apply wp_loop_cons
      ...
      | cons node remaining =>
          ...
          | cons hp h32 hfit hrc hcapacity hnodeNext hsep htailRep =>
              ...
              have hnext' : st1.mem.read64
                ...
                rw [← toUInt32_eq_ofNat]
                exact hnodeNext
              simp only [BookAllocSearch.bookAllocSearchBodyProg,
                BookAllocSearch.bookAllocSearchFrame, freeHead]
...
end Project.ClobMatchFuel.Frozen.BookAllocFit
