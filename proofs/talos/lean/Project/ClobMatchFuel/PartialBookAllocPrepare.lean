...
theorem partialBookAllocPrepareProg_spec
    ...
    (hValues : base.values = [])
    (hLengthLocal : base.locals[69]? = some (.i64 (UInt64.ofNat n)))
  ...
  have hLengthGet : base.locals[69] = .i64 (UInt64.ofNat n) := getElem_of_some hLengthLocal
  have hCapacityGet : base.locals[83] = .i64 capacity := getElem_of_some hCapacityLocal
  have hNextGet : base.locals[84] = .i64 next := getElem_of_some hNextLocal
  ...
  have hFinalFrame :
      { base with
        locals := (((base.locals.set 80 (.i64 (orderArrayBytesU n))).set
          85 (.i64 0)).set 81 (.i64 0)).set 82 (.i64 g1)
        values := [] } =
      PartialBookAllocSearch.bookAllocSearchFrame base
