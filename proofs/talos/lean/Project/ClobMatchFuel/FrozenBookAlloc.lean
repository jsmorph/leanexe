import Project.ClobMatchFuel.FrozenBookAllocPrepare

/-!
# Complete erased-book allocation

This module composes capacity preparation, first-fit search, and bump fallback
for the erased-book allocator.  Its theorem exposes the exact store and local
...
namespace Project.ClobMatchFuel.Frozen.BookAlloc

open Wasm Project.Common Project.Clob Project.Runtime Project.ClobMatchFuel
  Project.ClobMatchFuel.Frozen.Allocation

def bookAllocProg : Wasm.Program :=
  BookAllocPrepare.bookAllocPrepareProg ++
    BookAllocBump.bookAllocNoFitProg

set_option Elab.async false in
theorem bookAllocProg_spec
    ...
    (hFitDone : ∀ choice : FreeChoice,
      ...
      wp «module» rest Q (BookAllocFit.bookAllocFitStore st choice)
        (BookAllocSearch.bookAllocSearchFrame base (orderArrayBytesU n)
          choice.previous choice.node.root choice.node.capacity choice.next
          choice.node.root) env)
    (hBumpDone : ∀ previous : UInt64,
      wp «module» rest Q
        (BookAllocBump.bookAllocBumpStore st g0 (orderArrayBytesU n))
        (BookAllocSearch.bookAllocSearchFrame base (orderArrayBytesU n)
          previous 0 (g0 + 48 + orderArrayBytesU n)
          ((g0 + 48 + orderArrayBytesU n - 1) / 65536 + 1)
          (g0 + 48)) env) :
    wp «module» (bookAllocProg ++ rest) Q st base env := by
  have hNeed8 : 8 ≤ (orderArrayBytesU n).toNat := by
    ...
    omega
  unfold bookAllocProg BookAllocBump.bookAllocNoFitProg
  rw [List.append_assoc, List.append_assoc]
  apply BookAllocPrepare.bookAllocPrepareProg_spec env st base n
    (freeHead nodes) capacity next hParams hLocals hValues hLengthLocal
    hCapacityLocal hNextLocal hn hbytes hg1 Q
    (BookAllocSearch.bookAllocSearchProg ++
      BookAllocBump.bookAllocBumpProg ++ rest)
  ...
  | none =>
      have hNoFit : takeFirstFit (orderArrayBytesU n) nodes = none := by
        ...
        exact hProject.symm
      exact BookAllocBump.bookAllocNoFitProg_spec env st base g0
  ...
  | some choice =>
      apply BookAllocFit.bookAllocSearchProg_fit env st base
      ...
      exact BookAllocBump.bookAllocBumpProg_skip env
...
end Project.ClobMatchFuel.Frozen.BookAlloc
