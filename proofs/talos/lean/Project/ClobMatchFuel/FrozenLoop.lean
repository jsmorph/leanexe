...
/-!
# Generated match loop

The exported matcher places the guard and dispatcher in one zero-arity loop.
A trailing branch re-enters after every dispatcher outcome, including a
completed result.  The enclosing block receives either a completed state or a
running state whose fuel reached zero.
-/

namespace Project.ClobMatchFuel.Frozen.Loop

open Wasm Project.ClobMatchFuel Project.ClobMatchFuel.Frozen.LoopInvariant

def bodyProg : Wasm.Program :=
  LoopControl.loopGuardProg ++ (Iteration.dispatchProg ++ [.br 0])
  ...
  · intro st1 s1 hInvariant1
    ...
    · rcases hRunning with ⟨data, facts⟩
      ...
      · unfold bodyProg
        ...
        apply LoopIteration.dispatch_spec env ctx st1 s1 data facts hFuelZero
    ...
    · rcases hCompleted with ⟨data, facts⟩
      rcases facts.result with ⟨hBook, hTrades, hRemaining, hDoneFlag, hParams,
        hLocals, hValues⟩
...
end Project.ClobMatchFuel.Frozen.Loop
