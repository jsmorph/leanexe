import Project.ClobMatchFuel.FrozenAllocation
import Project.Runtime.FreeList
import Interpreter.Wasm.Wp.Block
import Interpreter.Wasm.Wp.Loop

namespace Project.ClobMatchFuel.Frozen.BookAllocSearch
...
def bookAllocSearchFrame (base : Locals)
  ...
  { base with
    locals := ((((((base.locals.set 67 (.i64 need)).set 68 (.i64 previous)).set
      69 (.i64 current)).set 70 (.i64 capacity)).set 71 (.i64 next)).set
      72 (.i64 result)) }
...
def bookAllocSearchProg : Wasm.Program :=
  [.block 0 0 [.loop 0 0 bookAllocSearchBodyProg]]
...
private def noFitInv (st0 : Store Unit) (base : Locals) (need : UInt64)
  ...
  fun st s =>
    ∃ previous capacity next : UInt64,
      ...
      s = bookAllocSearchFrame base need previous (freeHead remaining)
        capacity next 0
...
theorem bookAllocSearchProg_no_fit
    ...
    (hNext : ∀ previous capacity next : UInt64,
      wp «module» rest Q st
        (bookAllocSearchFrame base need previous 0 capacity next 0) env) :
    wp «module» (bookAllocSearchProg ++ rest) Q st
      (bookAllocSearchFrame base need 0 (freeHead nodes) capacity next 0) env := by
  simp only [bookAllocSearchProg, List.cons_append, List.nil_append]
  apply wp_block_cons
  ...
  · rintro st1 s1
    ...
    | nil =>
        simp only [bookAllocSearchBodyProg, bookAllocSearchFrame, freeHead]
        wp_run_with [hParams, hLocals, hValues]
        simpa only [bookAllocSearchFrame, hValues] using
...
end Project.ClobMatchFuel.Frozen.BookAllocSearch
