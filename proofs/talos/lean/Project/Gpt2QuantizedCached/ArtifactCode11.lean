...
theorem code11_decoded :
    code { bytes := artifactBytes, pos := 1459, limit := 28315 } = .ok (Cache.raw.codes[11]!, { bytes := artifactBytes, pos := 1529, limit := 28315 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 1460, limit := 28315 })
