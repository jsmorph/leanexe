...
theorem code1_decoded :
    code { bytes := artifactBytes, pos := 796, limit := 28315 } = .ok (Cache.raw.codes[1]!, { bytes := artifactBytes, pos := 811, limit := 28315 }) := by
  refine code_eq_of_parts (size := 14)
    (payload := { bytes := artifactBytes, pos := 797, limit := 28315 })
