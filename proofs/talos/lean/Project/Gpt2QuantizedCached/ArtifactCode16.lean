...
theorem code16_decoded :
    code { bytes := artifactBytes, pos := 1810, limit := 28017 } = .ok (Cache.raw.codes[16]!, { bytes := artifactBytes, pos := 1880, limit := 28017 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 1811, limit := 28017 })
