...
theorem code4_decoded :
    code { bytes := artifactBytes, pos := 954, limit := 28017 } = .ok (Cache.raw.codes[4]!, { bytes := artifactBytes, pos := 1060, limit := 28017 }) := by
  refine code_eq_of_parts (size := 105)
    (payload := { bytes := artifactBytes, pos := 955, limit := 28017 })
