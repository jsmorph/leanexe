...
namespace Project.Gpt2QuantizedCached.Coefficients
...
def validPrefix (input : ByteArray) (offset count : Nat) : Bool :=
  (List.range count).foldl (fun valid index =>
    valid && (!(input[offset + index]! == 128))) true
...
theorem source_eq (input : ByteArray) (offset count : Nat) :
    validCoefficients input offset count = validPrefix input offset count := by
...
theorem emitted_loop :
    func23 = func23.take 12 ++ RangeFoldLoop.program 14 15 stepCode ++ func23.drop 13 := rfl
...
theorem step_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hSize : offset + count ≤ input.size) (hIndex : index < count)
    (hReady : RangeFoldLoop.Ready 14 15 count index frame)
    (hAcc : Accumulator owner ptr input offset count index frame)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ result, RangeFoldLoop.Ready 14 15 count (index + 1) result →
  ...
  have hCounter : frame.locals[9]? = some (.i64 (UInt64.ofNat index)) := by
    simpa [Locals.get, hParams, hLength] using hReady.2.1
  have hStop : frame.locals[10]? = some (.i64 (UInt64.ofNat count)) := by
  ...
  · wp_packed_frame [hParams, hLength, hTotal, hCounter, hStop, hStride, hReady.1, hPrefix]
    ...
    apply PackedByteAccess.guard_spec «module» env initial _ ptr input (offset + index)
      17 19 [] hBytes (by omega)
    · simp [Locals.get, hLength]
    · simp [Locals.get, hLength]
    · simp [UInt64.ofNat_add]
    wp_packed_frame [hParams, hLength]
    have hEq : input[offset + index]!.toUInt64 = 128 ↔ input[offset + index]! = 128 :=
      UInt8.toUInt64_inj (b := 128)
    by_cases hByte : input[offset + index]! = 128
    all_goals
      refine wp_iff_cons rfl ?_
      first | rw [ite_eq_left (by simpa [hEq, hByte])] | rw [ite_eq_right (by simpa [hEq, hByte])]
    ...
    all_goals
      ...
      · simp only [RangeFoldLoop.Ready, Locals.get, hLength, List.length_set,
        ...
        exact ⟨rfl, trivial⟩
      · simp only [Accumulator, hLength, List.length_set, List.getElem?_set,
          Nat.reduceLT, Nat.reduceEqDiff, reduceIte, hStride, prefix_succ, hPrefix, hByte,
          Bool.true_and, Bool.false_eq_true, beq_self_eq_true, Bool.not_true, and_self]
        <;> simp [hByte]
...
theorem exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env «module» 23 initial
      ...
      (fun final values => final = initial ∧
        values = [.i64 (if validCoefficients input offset count then 1 else 0)]) := by
  ...
  change wp «module» func23 _ initial
    { params := [.i64 owner, .i64 ptr, .i64 (UInt64.ofNat input.size),
        .i64 (UInt64.ofNat offset), .i64 (UInt64.ofNat count)],
      locals := List.replicate 21 (.i64 0), values := [] } env
  ...
  · intro result hReady hAcc
    ...
    simp [func23Def, source_eq]

#print axioms exact
end Project.Gpt2QuantizedCached.Coefficients
