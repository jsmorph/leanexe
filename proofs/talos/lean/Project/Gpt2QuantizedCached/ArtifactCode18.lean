...
theorem code18_decoded :
    code { bytes := artifactBytes, pos := 1951, limit := 28017 } = .ok (Cache.raw.codes[18]!, { bytes := artifactBytes, pos := 2021, limit := 28017 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 1952, limit := 28017 })
