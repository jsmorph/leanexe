...
theorem code2_decoded :
    code { bytes := artifactBytes, pos := 811, limit := 28315 } = .ok (Cache.raw.codes[2]!, { bytes := artifactBytes, pos := 883, limit := 28315 }) := by
  refine code_eq_of_parts (size := 71)
    (payload := { bytes := artifactBytes, pos := 812, limit := 28315 })
