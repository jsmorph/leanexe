...
theorem code22_decoded :
    code { bytes := artifactBytes, pos := 2230, limit := 28315 } = .ok (Cache.raw.codes[22]!, { bytes := artifactBytes, pos := 2808, limit := 28315 }) := by
  refine code_eq_of_parts (size := 576)
    (payload := { bytes := artifactBytes, pos := 2232, limit := 28315 })
