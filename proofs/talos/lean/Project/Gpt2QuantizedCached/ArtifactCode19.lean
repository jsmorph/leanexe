...
theorem code19_decoded :
    code { bytes := artifactBytes, pos := 2021, limit := 28315 } = .ok (Cache.raw.codes[19]!, { bytes := artifactBytes, pos := 2091, limit := 28315 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 2022, limit := 28315 })
