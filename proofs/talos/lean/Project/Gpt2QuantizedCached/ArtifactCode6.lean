...
theorem code6_decoded :
    code { bytes := artifactBytes, pos := 1107, limit := 28017 } = .ok (Cache.raw.codes[6]!, { bytes := artifactBytes, pos := 1178, limit := 28017 }) := by
  refine code_eq_of_parts (size := 70)
    (payload := { bytes := artifactBytes, pos := 1108, limit := 28017 })
