...
theorem functionTypeIndices_section_decoded :
    sized (vector Leb.u32) { bytes := artifactBytes, pos := 535, limit := 28017 } = .ok (Cache.raw.functionTypeIndices, { bytes := artifactBytes, pos := 603, limit := 28017 }) := by
  refine sized_eq_of_parts (size := 67)
    (payload := { bytes := artifactBytes, pos := 536, limit := 28017 }) (finish := { bytes := artifactBytes, pos := 603, limit := 603 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
