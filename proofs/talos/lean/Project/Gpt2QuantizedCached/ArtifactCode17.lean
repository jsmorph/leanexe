...
theorem code17_decoded :
    code { bytes := artifactBytes, pos := 1880, limit := 28017 } = .ok (Cache.raw.codes[17]!, { bytes := artifactBytes, pos := 1951, limit := 28017 }) := by
  refine code_eq_of_parts (size := 70)
    (payload := { bytes := artifactBytes, pos := 1881, limit := 28017 })
