...
theorem memories_section_decoded :
    sized (vector memoryType) { bytes := artifactBytes, pos := 604, limit := 28315 } = .ok (Cache.raw.memories, { bytes := artifactBytes, pos := 608, limit := 28315 }) := by
  refine sized_eq_of_parts (size := 3)
    (payload := { bytes := artifactBytes, pos := 605, limit := 28315 }) (finish := { bytes := artifactBytes, pos := 608, limit := 608 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
