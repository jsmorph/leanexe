...
theorem code7_decoded :
    code { bytes := artifactBytes, pos := 1178, limit := 28017 } = .ok (Cache.raw.codes[7]!, { bytes := artifactBytes, pos := 1248, limit := 28017 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 1179, limit := 28017 })
