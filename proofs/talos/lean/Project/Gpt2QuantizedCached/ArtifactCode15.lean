...
theorem code15_decoded :
    code { bytes := artifactBytes, pos := 1740, limit := 28017 } = .ok (Cache.raw.codes[15]!, { bytes := artifactBytes, pos := 1810, limit := 28017 }) := by
  refine code_eq_of_parts (size := 69)
    (payload := { bytes := artifactBytes, pos := 1741, limit := 28017 })
