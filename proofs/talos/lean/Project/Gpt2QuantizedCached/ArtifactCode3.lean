...
theorem code3_decoded :
    code { bytes := artifactBytes, pos := 883, limit := 28315 } = .ok (Cache.raw.codes[3]!, { bytes := artifactBytes, pos := 954, limit := 28315 }) := by
  refine code_eq_of_parts (size := 70)
    (payload := { bytes := artifactBytes, pos := 884, limit := 28315 })
