...
theorem exports_section_decoded :
    sized (vector exportEntry) { bytes := artifactBytes, pos := 643, limit := 28017 } = .ok (Cache.raw.exports, { bytes := artifactBytes, pos := 780, limit := 28017 }) := by
  refine sized_eq_of_parts (size := 135)
    (payload := { bytes := artifactBytes, pos := 645, limit := 28017 }) (finish := { bytes := artifactBytes, pos := 780, limit := 780 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
