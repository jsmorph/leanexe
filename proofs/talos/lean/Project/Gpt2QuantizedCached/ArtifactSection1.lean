...
theorem types_section_decoded :
    sized (vector funcType) { bytes := artifactBytes, pos := 9, limit := 28017 } = .ok (Cache.raw.types, { bytes := artifactBytes, pos := 534, limit := 28017 }) := by
  refine sized_eq_of_parts (size := 523)
    (payload := { bytes := artifactBytes, pos := 11, limit := 28017 }) (finish := { bytes := artifactBytes, pos := 534, limit := 534 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
