...
theorem code9_decoded :
    code { bytes := artifactBytes, pos := 1318, limit := 28017 } = .ok (Cache.raw.codes[9]!, { bytes := artifactBytes, pos := 1389, limit := 28017 }) := by
  refine code_eq_of_parts (size := 70)
    (payload := { bytes := artifactBytes, pos := 1319, limit := 28017 })
