...
theorem emitted_layerBody : layerBody =
    [.localGet 118, .localGet 119, .geUI64, .br_if 1] ++ (layerBody.drop 4).take 137 ++ [.br 0] := rfl
...
theorem layerLoop_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    wp «module» ((func58.drop 57).take 1 ++ rest) Q initial frame env := by
  ...
  apply wp_block_cons
  apply wp_loop_cons (Inv := Inv) (μ := PackedGenerateLoop.measure 118 12)
