...
structure LayerFrame (params : List Value) (embedding hidden updates : UInt64)
  ...
  protectedEmbedding : frame.locals[80]? = some (.i64 (embedding))
  protectedUpdates : frame.locals[83]? = some (.i64 (0))
  ...
  counter : frame.locals[119]? = some (.i64 (UInt64.ofNat layer))
  limit : frame.locals[120]? = some (.i64 (12))
  step : frame.locals[121]? = some (.i64 (1))
  flag : frame.locals[143]? = some (.i64 (if layer = 0 then 0 else 1))
...
theorem embedding_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hLocals : frame.locals.length = 144) (hValues : frame.values = []) (hTyped : I64Values frame.locals)
