...
structure AppendedFrame (params : List Value) (embedding input updates hidden cache newUpdates : UInt64)
  ...
  inputCopy : frame.locals[23]? = some (.i64 input)
  updatesCopy : frame.locals[26]? = some (.i64 updates)
  updatesCopyAgain : frame.locals[30]? = some (.i64 updates)
...
theorem layerAppend_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    wp «module» (PackedAppend.program 121 ++ layerResultCode ++ rest) Q initial frame env := by
  have hParamLength : frame.params.length = 8 := by rw [hState.paramsEq, hParams]
  rw [List.append_assoc]
  apply PackedAppend.program_spec 121 «module» env initial heap frame updatesPtr cachePtr updates output.cache
  ...
  have hResultLength : result.locals.length = 141 := hPreserved.2.1.trans hState.length
  have hRead (index : Nat) (hi : index < 113 ∨ 126 ≤ index) :
  ...
  · constructor
    · constructor <;>
        simp (config := { maxDischargeDepth := 64 }) only [hResultParams, hResultLength,
          ...
          hRead 5 (by decide), hRead 6 (by decide), hRead 7 (by decide), hRead 134 (by decide), hRead 137 (by decide), hRead 15 (by decide), hRead 16 (by decide), hRead 17 (by decide), hRead 18 (by decide), hRead 19 (by decide), hRead 20 (by decide), hRead 21 (by decide), hRead 110 (by decide), hRead 111 (by decide), hRead 112 (by decide), hRead 134 (by decide),
          hState.toLayerFrame.embeddingOwner, hState.toLayerFrame.embeddingPtr, hState.toLayerFrame.embeddingSize, hState.toLayerFrame.protectedEmbedding, hState.toLayerFrame.protectedUpdates, hState.toLayerFrame.hiddenOwner, hState.toLayerFrame.hiddenPtr, hState.toLayerFrame.hiddenSize, hState.toLayerFrame.updatesOwner, hState.toLayerFrame.updatesPtr, hState.toLayerFrame.updatesSize, hState.toLayerFrame.status, hState.toLayerFrame.counter, hState.toLayerFrame.limit, hState.toLayerFrame.step]
    all_goals simp only [hResultLength, List.length_set, List.getElem?_set, Nat.reduceEqDiff, Nat.reduceLT, reduceIte,
      hRead 23 (by decide), hRead 26 (by decide), hRead 30 (by decide), hRead 49 (by decide),
      hState.hiddenOwnerCopy, hState.updatesOwnerCopy, hState.updatesOriginalCopy, hState.cacheOwner]
  · exact hOutput
