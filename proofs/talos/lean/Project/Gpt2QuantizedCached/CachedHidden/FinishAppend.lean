...
theorem finishAppend_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    wp «module» (PackedAppend.program 118 ++ finishSuccessResultCode ++ rest) Q initial frame env := by
  have hParamLength : frame.params.length = 8 := by rw [hState.paramsEq, hParams]
  rw [List.append_assoc]
  apply PackedAppend.program_spec 118 «module» env initial heap frame cachePtr updatesPtr cache updates
  ...
  have hRead (index : Nat) (hi : index < 110 ∨ 123 ≤ index) :
  ...
  wp_packed_frame [hResultParams, hParams, hResultLength, hReturned,
    hRead 99 (by decide), hRead 101 (by decide), hState.oldCacheSize, hState.updateSize,
    ← UInt64.ofNat_add]
  apply hNext
  · constructor <;>
      simp (config := { maxDischargeDepth := 64 }) only [hResultLength,
        ...
        hRead 5 (by decide), hRead 83 (by decide), hRead 84 (by decide),
        hRead 103 (by decide), hRead 104 (by decide), hRead 105 (by decide), hRead 106 (by decide),
