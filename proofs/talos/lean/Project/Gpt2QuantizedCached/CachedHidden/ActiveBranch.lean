...
theorem emitted_activeCode : activeCode = layerCallCode ++ PackedAppend.program 121 ++
...
theorem activeBranch_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hInputOwned : ∃ node, node.root = inputPtr ∧ heap.OwnsPacked initial node input)
    (hUpdatesOwned : updatesPtr = 0 ∨ ∃ node, node.root = updatesPtr ∧ heap.OwnsPacked initial node updates)
  ...
  have hSize : updates.size + (cachedBlock weights input cache layer position).cache.size ≤ 4294967296 :=
    (Nat.add_le_add hUpdatesSize (CachedBlock.cachedBlock_cache_le weights input cache layer position hInputSize)).trans
      (by omega)
  rw [← List.append_assoc (PackedAppend.program 121) layerResultCode]
  ...
  apply cacheRelease_spec env initial appendStore heap
    ...
    (by
      intro hZero
      obtain ⟨node, hRoot, hOwned⟩ := hInputOwned
      rw [← hRoot]
      exact (hMemory.cache.owned hZero).root_ne
        ((hMemory.cacheFresh hZero).owns_disjoint (hMemory.cache.owned hZero).buffer.rootBound hOwned))
    (by
      intro hZero
      rcases hUpdatesOwned with hRoot | ⟨node, hRoot, hOwned⟩
      · rw [hRoot]
        intro hNull
        have := (hMemory.cache.owned hZero).buffer.rootBound
        simp [hNull] at this
      · rw [← hRoot]
        exact (hMemory.cache.owned hZero).root_ne
          ((hMemory.cacheFresh hZero).owns_disjoint (hMemory.cache.owned hZero).buffer.rootBound hOwned))
