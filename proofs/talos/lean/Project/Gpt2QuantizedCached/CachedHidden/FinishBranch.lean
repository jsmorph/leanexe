...
theorem emitted_finishBranchFull : (func58.drop 60).take 48 =
    finishPrepareCode ++ failureTestCode 114 ++ [.iff 0 0 finishFailureCode finishSuccessCode] := rfl
...
theorem finishBranch_spec (env : HostEnv Unit) (original initial : Store Unit) (before heap : Heap)
    ...
    wp «module» ((func58.drop 60).take 48 ++ rest) Q initial frame env := by
  ...
  have hRead : prepared.get 114 = some (.i64 status) := by
    simpa [Locals.get, hPrepared.paramsEq, hParams, hPrepared.length] using hPrepared.status
  rw [← List.append_assoc (failureTestCode 114) [.iff 0 0 finishFailureCode finishSuccessCode]]
  apply failureGuard_spec env initial prepared 114 status hPrepared.values hRead
