...
structure CalledFrame (params : List Value) (embedding input updates hidden cache : UInt64)
  ...
  hiddenOwnerCopy : frame.locals[23]? = some (.i64 input)
  ...
  leftPtr : frame.locals[113]? = some (.i64 updates)
  leftSize : frame.locals[114]? = some (.i64 (UInt64.ofNat updateBytes))
  rightPtr : frame.locals[115]? = some (.i64 cache)
  rightSize : frame.locals[116]? = some (.i64 (UInt64.ofNat output.cache.size))
...
theorem layerCall_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
  ...
  · constructor
    ...
    all_goals simp only [hState.length, List.length_set, List.getElem?_set, Nat.reduceEqDiff, Nat.reduceLT, reduceIte,
      hState.hiddenOwnerCopy, hState.updatesOwnerCopy, hState.updatesOriginalCopy]
  · exact hCompletion
