...
theorem oldRelease_owned_spec (env : HostEnv Unit) (original initial : Store Unit) (before heap : Heap)
    ...
    (hState : StagedFrame params embedding inputNode.root oldUpdates.root
    ...
    (hNext : ∀ result,
      ...
      StagedFrame params embedding inputNode.root oldUpdates.root
        (statusRoot outputStatus hidden) update.root inputBytes.size oldUpdateBytes.size 0 layer
  ...
  have hRead82 : frame.get 138 = some (.i64 update.root) := by
  ...
  have hRead79 : frame.get 135 = some (.i64 (statusRoot outputStatus hidden)) := by
  ...
  apply releasePending_spec env original initial before heap params embedding inputNode.root oldUpdates.root
    ...
    oldUpdates oldUpdateBytes 26
    [(145, 0), (138, update.root), (142, embedding), (135, statusRoot outputStatus hidden)]
    hParams hMemory hState hOldUpdates hUpdatesFresh hHiddenUpdates hUpdateUpdates hRead26
  · intro entry hMem
  ...
  apply releasePending_spec env original (heap.releaseStore initial oldUpdates) before (heap.release oldUpdates)
    ...
    0 layer hiddenBytes updateBytes outputStatus after inputNode inputBytes 23
    [(26, oldUpdates.root), (145, 0), (138, update.root), (142, embedding), (135, statusRoot outputStatus hidden)]
    hParams hAfterMemory hAfter hAfterInput hInputFresh hHiddenInput hUpdateInput hAfter23
  · intro entry hMem
    simp only [List.mem_cons, List.not_mem_nil, or_false] at hMem
