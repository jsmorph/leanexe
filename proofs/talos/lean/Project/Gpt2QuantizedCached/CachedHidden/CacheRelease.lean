...
theorem cacheRelease_enabled {before heap : Heap} {original initial : Store Unit}
    ...
    (input oldUpdates : UInt64)
    (hInput : output.status = 0 → cache.root ≠ input)
    (hOldUpdates : output.status = 0 → cache.root ≠ oldUpdates) :
    PackedReleaseCounted.enabled (statusRoot output.status cache)
      [(79, statusRoot output.status hidden), (82, update.root),
        (38, oldUpdates), (34, oldUpdates), (31, input), (26, oldUpdates), (23, input)] = decide (output.status = 0) := by
  ...
  · have hCache := h.cache.owned hZero
    ...
    simp [PackedReleaseCounted.enabled, statusRoot, hZero, hNonzero, hHiddenNe, hUpdateNe, hInput hZero, hOldUpdates hZero]
...
theorem cacheRelease_spec (env : HostEnv Unit) (original initial : Store Unit) (before heap : Heap)
    ...
    (hInputNe : output.status = 0 → cache.root ≠ input)
    (hUpdatesNe : output.status = 0 → cache.root ≠ oldUpdates)
  ...
  have hEnabled := cacheRelease_enabled hMemory input oldUpdates hInputNe hUpdatesNe
  ...
  have hInput : frame.get 23 = some (.i64 input) := by
    simpa [Locals.get, hState.paramsEq, hParams, hState.length] using hState.hiddenOwner
  have hOldUpdates : frame.get 26 = some (.i64 oldUpdates) := by
    simpa [Locals.get, hState.paramsEq, hParams, hState.length] using hState.updatesOwner
  have hInputCopy : frame.get 31 = some (.i64 input) := by
    simpa [Locals.get, hState.paramsEq, hParams, hState.length] using hState.inputCopy
  have hUpdatesCopy : frame.get 34 = some (.i64 oldUpdates) := by
    simpa [Locals.get, hState.paramsEq, hParams, hState.length] using hState.updatesCopy
  have hUpdatesCopyAgain : frame.get 38 = some (.i64 oldUpdates) := by
    simpa [Locals.get, hState.paramsEq, hParams, hState.length] using hState.updatesCopyAgain
  ...
  apply selectedRelease_spec env initial heap params embedding input oldUpdates
    ...
    (statusRoot output.status cache) 57 87 [(79, statusRoot output.status hidden), (82, update.root),
        (38, oldUpdates), (34, oldUpdates), (31, input), (26, oldUpdates), (23, input)]
