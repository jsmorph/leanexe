...
def sizeCode : Wasm.Program :=
  [.localGet 11, .localSet 97, .localGet 10, .localSet 98] ++ CheckedNatMul.program 97 98 ++
    [.localSet 95, .constI64 4, .localSet 96] ++ CheckedNatMul.program 95 96 ++
    [.localSet 31, .localGet 31, .localSet 95]
...
theorem sizeCode_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals) (outputWidth rows : Nat)
    (hParams : frame.params.length = 13) (hLocals : frame.locals.length = 100)
    (hValues : frame.values = []) (hRows : frame.params[11]? = some (.i64 (UInt64.ofNat rows)))
    (hWidth : frame.params[10]? = some (.i64 (UInt64.ofNat outputWidth)))
  ...
  apply CheckedNatMul.program_spec 97 98 «module» env store _
  ...
  apply CheckedNatMul.program_spec 95 96 «module» env store _ (UInt64.ofNat (rows * outputWidth)) 4 []
  · rfl
  · simp [Locals.get, hParams, hLocals]
  · simp [Locals.get, hParams, hLocals]
