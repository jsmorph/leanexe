import Project.Gpt2QuantizedCached.GroupedProjection.ProjectionWord
...
theorem outputState_advance (params : List Wasm.Value) (valuePtr scalePtr : UInt64)
    (width outputWidth rows : Nat) (hParams : params.length = 13)
    (frame : Locals) (index : Nat) (hValid : frame.validIndex 32)
    ...
    OutputState params valuePtr scalePtr width outputWidth rows
      (FixedArrayCopy.counterFrame frame 32 index hValid) := by
...
theorem outputLoop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hReady : PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) 0 outputPtr frame)
    ...
    (hDone : ∀ final result,
      PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) (rows * outputWidth) outputPtr result →
  ...
  apply PackedGenerateLoop.program_spec
    ...
    (P := OutputState (parameters weightsOwner weightsPtr inputOwner inputPtr weights input weightOffset scaleOffset biasOffset
  ...
  · intro current next index hIndex hReady hState hWrites Q rest hNext
    exact outputWord_spec env current weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
