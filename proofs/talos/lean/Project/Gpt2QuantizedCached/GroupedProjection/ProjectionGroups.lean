import Project.Gpt2QuantizedCached.GroupedProjection.ProjectionGroup
...
theorem groupsLoop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hGroupReady : RangeFoldLoop.Ready 97 98 (width / 64) 0 frame)
    ...
    wp «module» (RangeFoldLoop.program 97 98 (groupStep withBias) ++ rest) Q initial
      { frame with values := [.i32 (PackedGenerateLoop.address outputPtr index)] } env := by
  let P := fun group (next : Locals) =>
    PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) index outputPtr { next with values := [] } ∧
    Accumulator weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr weights input weightOffset scaleOffset
  ...
  have hGroups64 : width / 64 < UInt64.size := by
    ...
    nlinarith
  apply RangeFoldLoop.program_spec_with_stack 97 98 (groupStep withBias) «module» env initial _
  ...
  · intro group next hGroup hGroupReady hP post tail hTail
    have hStep := group_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
...
def groupsCode (withBias : Bool) : Wasm.Program :=
  (branch withBias).take 12 ++ RangeFoldLoop.program 97 98 (groupStep withBias)
...
theorem groupsCode_spec (env : HostEnv Unit) (initial : Store Unit)
  ...
  cases withBias <;>
    simp only [groupsCode, branch, outputWord, outputBody, func8, Project.Gpt2QuantizedCached.func42,
  ...
  all_goals
    ...
    apply Frame.of_withValues
      (P := fun result => PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) index outputPtr result ∧
        RangeFoldLoop.Ready 97 98 (width / 64) 0 result ∧
        Accumulator weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr weights input weightOffset scaleOffset
          biasOffset width outputWidth rows index 0 _ result)
      (R := fun result => wp «module» (RangeFoldLoop.program 97 98 (groupStep _) ++ rest) Q initial result env) rfl
    case hNext =>
      intro result h
      exact groupsLoop_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
