import Project.Gpt2QuantizedCached.GroupedProjection.ProjectionState
...
theorem releaseCode_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hParamsLength : params.length = 13)
    (hState : OutputState params valueNode.root scaleNode.root width outputWidth rows frame)
    (hReady : PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) (rows * outputWidth) outputNode.root frame)
    (hEmpty : frame.values = [])
    (Q : Assertion Unit)
    (hDone : ∀ result,
      ...
      result.values = [.i64 (UInt64.ofNat (4 * (rows * outputWidth))), .i64 outputNode.root, .i64 outputNode.root] →
      Q (.Fallthrough (releasedStore heap initial valueNode scaleNode) result)) :
    wp «module» (func8.drop 100) Q initial frame env := by
  ...
  have hOutputPtr : frame.locals[83]? = some (.i64 outputNode.root) := by
  ...
  simp only [func8, Project.Gpt2QuantizedCached.func42, List.drop]
  ...
  wp_packed_frame [hParams, hParamsLength, hLocals, hValueOwner]
  refine wp_call_tw ((heap.releasePacked_exact env «module» 65 initial valueNode values
    (typeIdx := some 65) rfl rfl hHeap hValue).append_args rfl rfl rfl []) ?_
  ...
  wp_packed_frame [hParams, hParamsLength, hLocals, hScaleOwner, hValueOwner]
  refine wp_call_tw (((heap.release valueNode).releasePacked_exact env «module» 65
    (heap.releaseStore initial valueNode) scaleNode scales (typeIdx := some 65)
