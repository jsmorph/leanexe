import Project.Gpt2QuantizedCached.GroupedProjection.ProjectionBias
import Project.Gpt2QuantizedCached.GroupedProjection.ProjectionTotal
...
theorem branch_parts (withBias : Bool) :
    branch withBias = groupsCode withBias ++ totalCode withBias ++
      if withBias then biasCode else [.localSet 90, .localGet 90] := by
  cases withBias <;> rfl
...
theorem branch_spec (env : HostEnv Unit) (initial : Store Unit)
  ...
  apply groupsCode_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
  ...
  apply totalCode_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
  ...
  | false =>
    ...
    have hReadyResult : PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) index outputPtr result := by
    ...
    have hStateResult : OutputState (parameters weightsOwner weightsPtr inputOwner inputPtr weights input weightOffset scaleOffset biasOffset
  ...
  | true =>
    simp only [↓reduceIte]
    apply biasCode_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
...
theorem outputWord_parts : outputWord =
    [.localGet 12, .constI64 1, .eqI64, .iff 0 1 [.constI64 1] [.constI64 0] [] [.i64],
...
theorem outputWord_spec (env : HostEnv Unit) (initial : Store Unit)
  ...
  have hBranch (post : Assertion Unit)
      (hPost : ∀ result, PackedGenerateLoop.Ready 32 95 96 (rows * outputWidth) index outputPtr result →
        OutputState (parameters weightsOwner weightsPtr inputOwner inputPtr weights input weightOffset scaleOffset biasOffset
          width outputWidth rows withBias) valuePtr scalePtr width outputWidth rows result →
      ...
      wp «module» (branch withBias) post initial
        { frame with values := [.i32 (PackedGenerateLoop.address outputPtr index)] } env := by
    have h := branch_spec env initial weightsOwner weightsPtr inputOwner inputPtr valuePtr scalePtr outputPtr weights input
      weightOffset scaleOffset biasOffset width outputWidth rows index withBias frame
