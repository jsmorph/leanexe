...
theorem code14_decoded :
    code { bytes := artifactBytes, pos := 1669, limit := 28017 } = .ok (Cache.raw.codes[14]!, { bytes := artifactBytes, pos := 1740, limit := 28017 }) := by
  refine code_eq_of_parts (size := 70)
    (payload := { bytes := artifactBytes, pos := 1670, limit := 28017 })
