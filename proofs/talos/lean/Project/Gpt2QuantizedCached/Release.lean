...
namespace Project.Gpt2QuantizedCached.Release
...
theorem release_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env «module» 65 initial [.i64 root]
      (fun final values => values = [] ∧
        final = PackedRelease.store initial root head releases frees) :=
  PackedRelease.exact env «module» 65 initial root head releases frees
    (typeIdx := some 65) rfl rfl hRoot hRoot32 hFit
...
theorem release_owned (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env «module» 65 initial [.i64 node.root]
      (fun final values => values = [] ∧ final = heap.releaseStore initial node ∧
        (heap.release node).At final) :=
  heap.releasePacked_exact env «module» 65 initial node bytes
    (typeIdx := some 65) rfl rfl hHeap hOwner

#print axioms release_owned

end Project.Gpt2QuantizedCached.Release
