...
theorem code9_decoded :
    code { bytes := artifactBytes, pos := 1000, limit := 5720 } = .ok (Cache.raw.codes[9]!, { bytes := artifactBytes, pos := 1023, limit := 5720 }) := by
  refine code_eq_of_parts (size := 22)
    (payload := { bytes := artifactBytes, pos := 1001, limit := 5720 })
