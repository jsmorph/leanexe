...
theorem code4_decoded :
    code { bytes := artifactBytes, pos := 728, limit := 5720 } = .ok (Cache.raw.codes[4]!, { bytes := artifactBytes, pos := 770, limit := 5720 }) := by
  refine code_eq_of_parts (size := 41)
    (payload := { bytes := artifactBytes, pos := 729, limit := 5720 })
