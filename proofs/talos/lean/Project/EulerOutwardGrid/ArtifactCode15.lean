...
theorem code15_decoded :
    code { bytes := artifactBytes, pos := 1309, limit := 5720 } = .ok (Cache.raw.codes[15]!, { bytes := artifactBytes, pos := 1341, limit := 5720 }) := by
  refine code_eq_of_parts (size := 31)
    (payload := { bytes := artifactBytes, pos := 1310, limit := 5720 })
