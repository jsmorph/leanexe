import Project.EulerOutwardGrid.ScanLoop
import Project.EulerOutwardGrid.AnnotationMatches
...
def scanHead : Wasm.Program := func45.take 23
def scanTail : Wasm.Program := func45.drop 24

theorem scan_shape :
    func45 = scanHead ++ [.block 0 0 [.loop 0 0 scanLoop]] ++ scanTail := by
  calc
    func45 = AnnotationMatches.function_45_array_fold_0_program ++ func45.drop 28 :=
      AnnotationMatches.function_45_array_fold_0_tail_eq
    _ = scanHead ++ [.block 0 0 [.loop 0 0 scanLoop]] ++ scanTail := rfl

theorem scan_exact (env : HostEnv Unit) (initial : Store Unit)
    (pointer : UInt64) (grid : Array Traversal.Cell)
    (hGrid : Memory.GridAt initial pointer grid) :
    TerminatesWith env Project.EulerOutwardGrid.«module» 45 initial
      [.i64 pointer]
  ...
  change wp Project.EulerOutwardGrid.«module» func45 _ initial
    (func45Def.toLocals [.i64 pointer]) env
  rw [scan_shape]
  unfold scanHead func45
  dsimp only
  wp_run [func45Def, List.set, List.getElem?_cons_zero, List.getElem?_cons_succ,
  ...
  scan_peel
  change wp _ _ _ _
    (scanFrame pointer grid.size 0 ⟨0, 0⟩ { borrowed := pointer }) _
  nth_rw 1 [← scanPrefix_zero grid]
  apply scan_loop_spec env initial pointer grid hGrid 0 (Nat.zero_le _)
    { borrowed := pointer }
  intro scratch
  unfold func45
  dsimp only
