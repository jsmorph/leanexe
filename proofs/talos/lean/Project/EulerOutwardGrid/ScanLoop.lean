...
theorem scan_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNext : ∀ scratch : ScanScratch,
      wp Project.EulerOutwardGrid.«module» rest Q initial
        (scanFrame pointer grid.size grid.size (scanPrefix grid grid.size) scratch) env) :
    wp Project.EulerOutwardGrid.«module» ([.block 0 0 [.loop 0 0 scanLoop]] ++ rest) Q initial
      (scanFrame pointer grid.size index (scanPrefix grid index) scratch) env := by
  have hSize64 := hGrid.size_lt
  have hSizeNat := UInt64.toNat_ofNat_of_lt' hSize64
  simp only [List.cons_append, List.nil_append]
  apply wp_block_cons
  apply wp_loop_cons
    (Inv := scanInvariant initial pointer grid)
