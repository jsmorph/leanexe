...
theorem code0_decoded :
    code { bytes := artifactBytes, pos := 312, limit := 5409 } = .ok (Cache.raw.codes[0]!, { bytes := artifactBytes, pos := 401, limit := 5409 }) := by
  refine code_eq_of_parts (size := 88)
    (payload := { bytes := artifactBytes, pos := 313, limit := 5409 })
