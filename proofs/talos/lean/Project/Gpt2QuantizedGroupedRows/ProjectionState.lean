...
def groupBody (withBias : Bool) : Wasm.Program :=
  match ((branch withBias)[12]? : Option Wasm.Instruction) with
  | some (.block _ _ [.loop _ _ body _ _] _ _) => body
  | _ => []
...
theorem emitted_output : (func8.drop 99).take 1 = PackedGenerateLoop.program 30 93 94 outputWord := rfl

theorem emitted_groups (withBias : Bool) :
    branch withBias = (branch withBias).take 12 ++
      RangeFoldLoop.program 95 96 (groupStep withBias) ++ (branch withBias).drop 13 := by
  cases withBias <;> rfl
...
def Accumulator (weightsPtr inputPtr valuePtr scalePtr : UInt64) (weights input : ByteArray)
...
def StepState (weightsPtr inputPtr valuePtr scalePtr : UInt64) (weights input : ByteArray)
    (weightOffset scaleOffset biasOffset width outputWidth rows index group : Nat)
    (withBias : Bool) (frame : Locals) : Prop :=
  Accumulator weightsPtr inputPtr valuePtr scalePtr weights input weightOffset scaleOffset biasOffset
