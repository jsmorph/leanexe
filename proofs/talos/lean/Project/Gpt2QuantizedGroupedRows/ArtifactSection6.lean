...
theorem globals_section_decoded :
    sized (vector global) { bytes := artifactBytes, pos := 147, limit := 5441 } = .ok (Cache.raw.globals, { bytes := artifactBytes, pos := 180, limit := 5441 }) := by
  refine sized_eq_of_parts (size := 32)
    (payload := { bytes := artifactBytes, pos := 148, limit := 5441 }) (finish := { bytes := artifactBytes, pos := 180, limit := 180 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
