import Project.Gpt2QuantizedGroupedRows.ProjectionSize
import Project.Gpt2QuantizedGroupedRows.ProjectionLoop
...
theorem emitted_allocation : (func8.drop 68).take 32 = PackedCapacity.program 93 95 ++
    PackedAllocation.program 95 ++ [.localGet 100, .localSet 94, .constI64 0, .localSet 30] ++
...
theorem allocatedOutput_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hEmpty : frame.values = []) (hLengthLocal : frame.get 93 = some (.i64 (UInt64.ofNat (4 * (rows * outputWidth)))))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ final result,
      PackedGenerateLoop.Ready 30 93 94 (rows * outputWidth) (rows * outputWidth)
  ...
  apply PackedCapacity.program_spec 93 95 (UInt64.ofNat (4 * (rows * outputWidth))) «module» env initial frame
  · exact hLengthLocal
  · exact hEmpty
  · omega
  · simp [Locals.validIndex, hParamLength, hLocals]
  let prepared := FixedArrayCapacity.capacityFrame frame 95 (need outputWidth rows)
  ...
  apply PackedAllocation.program_spec_frame «module» env initial prepared 95 heap.top
