...
theorem memories_section_decoded :
    sized (vector memoryType) { bytes := artifactBytes, pos := 142, limit := 5441 } = .ok (Cache.raw.memories, { bytes := artifactBytes, pos := 146, limit := 5441 }) := by
  refine sized_eq_of_parts (size := 3)
    (payload := { bytes := artifactBytes, pos := 143, limit := 5441 }) (finish := { bytes := artifactBytes, pos := 146, limit := 146 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
