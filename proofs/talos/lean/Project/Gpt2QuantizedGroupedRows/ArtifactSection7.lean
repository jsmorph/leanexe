...
theorem exports_section_decoded :
    sized (vector exportEntry) { bytes := artifactBytes, pos := 181, limit := 5441 } = .ok (Cache.raw.exports, { bytes := artifactBytes, pos := 308, limit := 5441 }) := by
  refine sized_eq_of_parts (size := 126)
    (payload := { bytes := artifactBytes, pos := 182, limit := 5441 }) (finish := { bytes := artifactBytes, pos := 308, limit := 308 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
