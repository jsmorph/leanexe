...
theorem functionTypeIndices_section_decoded :
    sized (vector Leb.u32) { bytes := artifactBytes, pos := 126, limit := 5409 } = .ok (Cache.raw.functionTypeIndices, { bytes := artifactBytes, pos := 141, limit := 5409 }) := by
  refine sized_eq_of_parts (size := 14)
    (payload := { bytes := artifactBytes, pos := 127, limit := 5409 }) (finish := { bytes := artifactBytes, pos := 141, limit := 141 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
