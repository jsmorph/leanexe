...
theorem types_section_decoded :
    sized (vector funcType) { bytes := artifactBytes, pos := 9, limit := 5441 } = .ok (Cache.raw.types, { bytes := artifactBytes, pos := 125, limit := 5441 }) := by
  refine sized_eq_of_parts (size := 115)
    (payload := { bytes := artifactBytes, pos := 10, limit := 5441 }) (finish := { bytes := artifactBytes, pos := 125, limit := 125 })
    ?_ ?_ ?_ ?_ ?_
  · cbv
  · decide
  · cbv
