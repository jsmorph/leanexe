...
theorem inputCode_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hNext : ∀ final, QuantizeRows.Output heap initial final input 64 (rows * (width / 64)) →
      wp «module» rest Q final
        (inputFrame frame inputPtr input width rows (QuantizeRows.valueNode heap 64 (rows * (width / 64))).root
  ...
  have hRows := quantizeRows_exact env initial heap 0 inputPtr input 64 (rows * (width / 64))
