import Project.TinyGpt2Hidden.Layout
...
macro_rules
  | `(tactic| tiny_hidden_steps $target:ident $codeNamespace:ident ($transfer:term) ($ownerValue:term)
    ...
    let func53Def := Lean.mkIdent (target.getId ++ `func53Def)
    let func4Def := Lean.mkIdent (target.getId ++ `func4Def)
    let func54Def := Lean.mkIdent (target.getId ++ `func54Def)
    let func55Def := Lean.mkIdent (target.getId ++ `func55Def)
    let func56Def := Lean.mkIdent (target.getId ++ `func56Def)
    let func58Def := Lean.mkIdent (target.getId ++ `func58Def)
    let func5Def := Lean.mkIdent (target.getId ++ `func5Def)
    let func65Def := Lean.mkIdent (target.getId ++ `func65Def)
    let func72Def := Lean.mkIdent (target.getId ++ `func72Def)
    let func73Def := Lean.mkIdent (target.getId ++ `func73Def)
    let func74Def := Lean.mkIdent (target.getId ++ `func74Def)
    ...
    `(tactic|
      (
        proof_step =>
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 18 (by decide) (layout_norm1_exact $env $initial))
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 27 (by decide) (layout_query_exact $env $initial))
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 30 (by decide) (layout_key_exact $env $initial))
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 31 (by decide) (layout_value_exact $env $initial))
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 52 (by decide) (attentionRow_exact $env $initial ($position+1) query keys values))
            (f := $func52Def) rfl rfl rfl _ rfl ?_
          unfold $tail20
          generalize hattended : attentionRow ($position+1) query keys values = attended
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 53 (by decide) (layout_attention_exact $env $initial))
            (f := $func53Def) rfl rfl rfl _ rfl ?_
          unfold $tail21
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 54 (by decide) (layout_attentionBias_exact $env $initial))
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 55 (by decide) (layout_expand_exact $env $initial))
            (f := $func55Def) rfl rfl rfl _ rfl ?_
          unfold $tail39
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 58 (by decide) (layout_norm2_exact $env $initial))
            (f := $func58Def) rfl rfl rfl _ rfl ?_
          unfold $tail40
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 56 (by decide) (layout_expandBias_exact $env $initial))
            (f := $func56Def) rfl rfl rfl _ rfl ?_
          unfold $tail71
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 65 (by decide) (activate_exact $env $initial (addRows ⟨d0, d1, d2, d3⟩ lowBias)))
            (f := $func65Def) rfl rfl rfl _ rfl ?_
          unfold $tail83
          generalize hactLow : activate (addRows ⟨d0, d1, d2, d3⟩ lowBias) = actLow
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 65 (by decide) (activate_exact $env $initial (addRows ⟨d4, d5, d6, d7⟩ highBias)))
            (f := $func65Def) rfl rfl rfl _ rfl ?_
          unfold $tail84
          generalize hactHigh : activate (addRows ⟨d4, d5, d6, d7⟩ highBias) = actHigh
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 72 (by decide) (contractRow_exact $env $initial $ownerValue $pointer $weights ⟨actLow, actHigh⟩ $ha (by omega)))
            (f := $func72Def) rfl rfl rfl _ rfl ?_
          unfold $tail85
        ...
        proof_step =>
          ...
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
          refine wp_call_exact_append ($transfer 73 (by decide) (layout_normFinal_exact $env $initial))
            (f := $func73Def) rfl rfl rfl _ rfl ?_
          unfold $tail89
          wp_fixed_frame [$func74Def:term, rowResults, contextResults, wideResults, List.append_nil, List.cons_append, List.nil_append]
        proof_step =>
