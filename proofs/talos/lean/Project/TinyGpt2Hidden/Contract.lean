...
theorem contractRow_exact (env : HostEnv Unit) (initial : Store Unit)
  ...
  change wp Project.TinyGpt2Hidden.module func69 _ initial
    (func69Def.toLocals [.i64 owner, .i64 pointer,
      .i64 x.low.x0, .i64 x.low.x1, .i64 x.low.x2, .i64 x.low.x3,
      .i64 x.high.x0, .i64 x.high.x1, .i64 x.high.x2, .i64 x.high.x3]) env
  unfold func69
  wp_fixed_frame [func69Def, rowResults]
  refine wp_call_tw (ConstantFunction.exact _ env initial 67 1148 (some 67) rfl rfl) ?_
  ...
  subst st
  wp_fixed_frame [func69Def]
  refine wp_call_tw ((ConstantFunction.exact _ env initial 68 1180 (some 68) rfl rfl).append_args
    rfl rfl rfl _) ?_
