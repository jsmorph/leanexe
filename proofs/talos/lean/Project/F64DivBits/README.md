# Binary64 division primitive

The source entry LeanExe.Examples.Float64Bits.divBits accepts raw UInt64
binary64 words and returns the raw result of one WebAssembly f64.div.
The generated [Program.lean](Program.lean) is checked by [Spec.lean](Spec.lean).

The exact theorem divBits_exact quantifies over every input encoding,
terminates independently of fuel, and preserves the complete store.  It
returns the pure Talos IEEE64 div result.  The source and generated-WAT
numerical theorems prove a finite result with absolute error at most 2^-52
when both operands are finite, the denominator is nonzero, and the numerator magnitude is at most the denominator magnitude.  These are explicit theorem premises,
...
is the deterministic pinned Talos semantics.  The [frozen manifest](../../../../artifacts/f64_div_bits/b3d81061ed69ffb1a60f9c87fbc82bb2b7d6edbc224ecaef0fb71636b2c34c62/manifest.json) registers
...
```sh
tools/talos-proof.js check f64_div_bits
