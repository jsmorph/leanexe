...
/-!
# Reusable raw binary64 magnitude guards

LeanExe floating-point entry points accept raw `UInt64` words and clear the
sign bit before comparing their magnitude encoding with a positive binary64
constant.  This module proves the reusable half-unit instance of that guard
using only Talos's integer IEEE-754 model.  It never evaluates a native Lean
`Float`.
-/

namespace Project.ProofKit.F64Bounds

set_option exponentiation.threshold 4096
set_option maxRecDepth 8192

/-- Clear the sign bit of a raw binary64 word. -/
def f64AbsBits (bits : UInt64) : UInt64 :=
  bits &&& 0x7FFFFFFFFFFFFFFF

/-- Raw-bit guard recognizing binary64 encodings with magnitude at most one
half, including both signed zeros. -/
...
/-- Passing the raw-bit half-unit guard implies finiteness and the real
magnitude bound used by the binary64 numerical kernel theorems. -/
...
end Project.ProofKit.F64Bounds
