...
theorem abs_le_eight_of_lt_successor (word : UInt64)
  ...
  have hnext : |value 0x4020000000000001| = 8 + 8*arithmeticEpsilon := by
    norm_num [value, Wasm.IEEE64.scaledValue, Wasm.IEEE64.scaledMagnitude,
      Wasm.IEEE64.sign, Wasm.IEEE64.exponent, Wasm.IEEE64.fraction,
      UInt64.toNat_ofNat, arithmeticEpsilon]
  by_cases hb : absBits word ≤ 0x4020000000000000
  · have hm := abs_value_mono word 0x4020000000000000 (by
      simpa only [show absBits 0x4020000000000000 = 0x4020000000000000 by decide] using hb)
    simpa only [hone] using hm
  · have hm := abs_value_mono 0x4020000000000001 word (by
      rw [UInt64.le_iff_toNat_le]
      rw [UInt64.le_iff_toNat_le] at hb
      change 4620693217682128897 ≤ (absBits word).toNat
      change ¬(absBits word).toNat ≤ 4620693217682128896 at hb
      omega)
    rw [hnext] at hm
    linarith
