...
def Ready (indexLocal stopLocal count index : Nat) (frame : Locals) (values : List Value := []) : Prop :=
  frame.values = values ∧
...
theorem program_spec_with_stack (indexLocal stopLocal : Nat) (stepCode : Wasm.Program)
    (module_ : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    (frame : Locals) (values : List Value) (count : Nat) (P : Nat → Locals → Prop)
    (hcount : count < UInt64.size)
    (hready : Ready indexLocal stopLocal count 0 frame values) (hP : P 0 frame)
    (hstep : ∀ index next, index < count →
      Ready indexLocal stopLocal count index next values → P index next →
      ∀ (Q : Assertion Unit) rest,
      (∀ result, Ready indexLocal stopLocal count (index + 1) result values →
        P (index + 1) result → wp module_ rest Q initial result env) →
      wp module_ (stepCode ++ rest) Q initial next env)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hdone : ∀ result, Ready indexLocal stopLocal count count result values →
  ...
  let Inv : AssertionF Unit := fun current next => current = initial ∧
    ∃ index, index ≤ count ∧ Ready indexLocal stopLocal count index next values ∧ P index next
  ...
  · rintro current next ⟨rfl, index, hindex, hready, hP⟩
    ...
    · subst index
      ...
      simp [hentry]
      have hframeValues : ({ next with values := values } : Locals) = next :=
    ...
    · have hlt : index < count := by omega
      ...
      have hresultValues : ({ result with values := values } : Locals) = result :=
        Frame.ext _ _ rfl rfl hresult.1.symm
      change Inv current { result with values := values } ∧
        measure indexLocal count current { result with values := values } < measure indexLocal count current next
      rw [hresultValues]
...
theorem program_spec (indexLocal stopLocal : Nat) (stepCode : Wasm.Program)
    ...
    (hdone : ∀ result, Ready indexLocal stopLocal count count result →
      P count result → wp module_ rest Q initial result env) :
    wp module_ (program indexLocal stopLocal stepCode ++ rest) Q initial frame env := by
  exact program_spec_with_stack indexLocal stopLocal stepCode module_ env initial frame []
    count P hcount hready hP hstep Q rest hdone

#print axioms program_spec_with_stack
