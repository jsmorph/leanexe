import Project.ProofKit.PackedPushMemory
...
def program (scratch : Nat) : Wasm.Program :=
  [.localGet (scratch + 1), .constI64 1, .addI64, .localSet (scratch + 4)] ++
    ...
    PackedCopy.program scratch (scratch + 3) (scratch + 1) (scratch + 5) none ++
    [.localGet (scratch + 3), .localGet (scratch + 1), .addI64, .wrapI64,
     .localGet (scratch + 2), .wrapI64, .store8 0, .localGet (scratch + 3)]
...
theorem program_spec_available (scratch : Nat) (module_ : Wasm.Module) (env : HostEnv Unit)
  ...
  have hSourceSep := hProtected.allocated_disjoint (need bytes) hFit
  ...
  simp only [program, List.append_assoc, List.cons_append, List.nil_append,
    wp_localGet_cons, Frame.withValues_get, hLength, hValues, wp_constI64_cons,
    wp_addI64_cons, show (1 : UInt64) = UInt64.ofNat 1 from rfl, ← UInt64.ofNat_add]
  ...
  have hCounter : copying.validIndex (scratch + 5) := by
    ...
    omega
  apply PackedCopy.program_spec scratch (scratch + 3) (scratch + 1) (scratch + 5) none
    module_ env (heap.allocatePackedStore initial (need bytes)) copying source
    (allocatedRoot heap.top (need bytes) heap.nodes) 0 bytes hCounter rfl
    (by omega) (by omega) (by omega) (by intro _ h; cases h)
    ((hCopyingRead scratch (by omega)).trans hSource) hCopyingRoot
    ((hCopyingRead (scratch + 1) (by omega)).trans hLength) rfl hAllocatedBytes
    (by omega) (by omega) (by omega)
  intro copied hWrites hCopiedBytes
  ...
  have hDoneLength := (counterFrame_get_ne copying (scratch + 5) bytes.size (scratch + 1) hCounter (by omega)).trans
    ((hCopyingRead (scratch + 1) (by omega)).trans hLength)
  have hDoneByte := (counterFrame_get_ne copying (scratch + 5) bytes.size (scratch + 2) hCounter (by omega)).trans
    ((hCopyingRead (scratch + 2) (by omega)).trans hByte)
  let root := allocatedRoot heap.top (need bytes) heap.nodes
  let address := (root + UInt64.ofNat bytes.size).toUInt32
  have hFitEnd : root.toNat + (bytes.size + 1) ≤ 4294967296 :=
    (Nat.add_le_add_left hNeed root.toNat).trans hAddressBound
  have hAddress : address.toNat = root.toNat + bytes.size := by
    have h := PackedCopy.address_toNat root 0 (bytes.size + 1) bytes.size (by simpa using hFitEnd) (by omega)
    simpa [PackedCopy.address, address] using h
  have hMemory : root.toNat + (bytes.size + 1) ≤ copied.mem.pages * 65536 := by
    rw [hWrites.2.1]
    exact (Nat.add_le_add_left hNeed root.toNat).trans hMemoryBound
  have hStoreBound : ¬copied.mem.pages * 65536 < address.toNat + 1 := by omega
  have hNarrow : byte.toUInt64.toUInt32.toUInt8 = byte := by simp
  have hWrap (word : UInt64) : UInt32.ofNat (word.toNat % 2^32) = word.toUInt32 := by
    apply UInt32.toNat.inj
    simp
  simp only [wp_localGet_cons, Frame.withValues_get, hDoneRoot, hDoneLength, hDoneByte,
    counterFrame_values, wp_addI64_cons, wp_wrapI64_cons, wp_store8_cons,
    UInt32.add_zero, UInt32.toNat_zero, Nat.add_zero, hWrap, hStoreBound, ite_false, hNarrow]
  rw [ite_eq_right (by exact hStoreBound)]
  apply hNext _ _ rfl
  ...
  · have hCopyWrites : Memory.WritesRange (heap.allocatePackedStore initial (need bytes))
        copied root.toNat (root.toNat + bytes.size) := by
      simpa only [Nat.add_zero] using hWrites
    have hAllWrites : Memory.WritesRange (heap.allocatePackedStore initial (need bytes))
        { copied with mem := copied.mem.write8 address byte } root.toNat (root.toNat + (bytes.size + 1)) :=
      (hCopyWrites.mono (Nat.le_refl _) (by omega)).trans
        (PackedCopy.write8_range copied address byte _ _ (by omega) (by omega))
    exact heap.packedOutput initial _ (need bytes) (bytes.push byte) hHeap
