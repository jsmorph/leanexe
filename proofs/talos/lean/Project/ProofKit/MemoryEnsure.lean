...
theorem ensureProgram_spec_of_grow (module_ : Wasm.Module) (env : HostEnv Unit)
...
theorem ensureProgram_spec (module_ : Wasm.Module) (env : HostEnv Unit)
    ...
    (hNext : wp module_ rest Q (ensured store required) frame env) :
    wp module_ (ensureProgram pageLocal ++ rest) Q store frame env := by
  exact ensureProgram_spec_of_grow module_ env store frame pageLocal required
    hMemory32 hValues hLocal hCurrent hBound (fun _ => hCap) Q rest hNext
