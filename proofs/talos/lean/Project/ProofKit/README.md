...
| `Project.ProofKit.CopyAddress` | Execution of zero-offset and local-offset raw-word address expressions with an arbitrary stack tail. |
| `Project.ProofKit.OffsetArrayCopy` | Terminating raw-word copies with optional source and destination offsets, exact copied reads, and store preservation outside the copied interval. |
...
`ScalarConditional` covers a value-producing conditional whose decoded instruction retains explicit parameter and result-type lists.  Its expression and assignment theorems use the same descriptor evaluator and preserve those lists in the program statement.  They apply Talos's checked control-type equality at the conditional execution boundary.  `Stmt.typedIteAssignProgram_frame_spec` accepts the resulting `Locals` frame and its ordinary continuation.
