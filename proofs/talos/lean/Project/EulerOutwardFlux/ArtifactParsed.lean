...
import Project.EulerOutwardFlux.ArtifactParsedSections
...
theorem decode_eq_cache_parts : decode artifactBytes = .ok Cache.raw := by
  refine decode_eq_of_parts (versionStart := { bytes := artifactBytes, pos := 4, limit := 7175 })
    (sectionsStart := { bytes := artifactBytes, pos := 8, limit := 7175 }) (finish := { bytes := artifactBytes, pos := 7175, limit := 7175 }) ?_ ?_ ?_ ?_
  · cbv
  · cbv
