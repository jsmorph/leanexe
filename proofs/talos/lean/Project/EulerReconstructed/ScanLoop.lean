import Project.EulerReconstructed.ScanLoopShape

namespace Project.EulerReconstructed.Execution
open Wasm
open Project.EulerOutwardGrid.Execution (ScanScratch scanPrefix scanPrefix_succ)
...
theorem scan_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNext : ∀ scratch : ScanScratch,
      wp Project.EulerReconstructed.«module» rest Q initial
        (scanFrame owner pointer grid.size grid.size (scanPrefix grid grid.size) scratch) env) :
    wp Project.EulerReconstructed.«module» ([.block 0 0 [.loop 0 0 scanLoop]] ++ rest) Q initial
