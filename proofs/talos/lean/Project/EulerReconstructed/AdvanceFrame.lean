import Project.EulerReconstructed.AdvanceLoopShape
...
structure AdvanceFrameAt (frame : Locals) (fuel : UInt64) (n : Nat)
    (trials time source tracker outputTime outputRoot : UInt64) (done : Bool) : Prop where
  params : frame.params = [.i64 fuel, .i64 (UInt64.ofNat n), .i64 trials, .i64 time, .i64 source, .i64 source]
...
def advanceContinuedFrame (frame : Locals) (fuel : UInt64) (n : Nat)
    (trials nextTime result : UInt64) : Locals :=
  let locals := frame.locals.set 34 (.i64 (UInt64.ofNat n))
  let locals := locals.set 35 (.i64 trials)
  let locals := locals.set 36 (.i64 nextTime)
  let locals := locals.set 37 (.i64 result)
  let locals := locals.set 38 (.i64 result)
  let locals := locals.set 39 (.i64 (UInt64.ofNat n))
  let locals := locals.set 40 (.i64 trials)
  let locals := locals.set 41 (.i64 nextTime)
  let locals := locals.set 42 (.i64 result)
  let locals := locals.set 43 (.i64 result)
  let locals := locals.set 44 (.i64 result)
  let locals := locals.set 0 (.i64 result)
  { params := [.i64 (fuel - 1), .i64 (UInt64.ofNat n), .i64 trials,
      .i64 nextTime, .i64 result, .i64 result]
    locals
    values := [] }
...
theorem AdvanceFrameAt.time {frame : Locals} {fuel : UInt64} {n : Nat}
    ...
    AdvanceFrameAt (advanceTimeFrame frame) fuel n trials time source tracker outputTime outputRoot done := by
  cases h
  constructor <;> simp_all [advanceTimeFrame]

theorem AdvanceFrameAt.scan {frame : Locals} {fuel : UInt64} {n : Nat}
    {trials time source tracker outputTime outputRoot : UInt64} {done : Bool}
    (h : AdvanceFrameAt frame fuel n trials time source tracker outputTime outputRoot done) (stats : Checked) :
    AdvanceFrameAt (advanceScanFrame frame source stats)
      fuel n trials time source tracker outputTime outputRoot done := by
  cases h
...
theorem AdvanceFrameAt.trial {frame : Locals} {fuel : UInt64} {n : Nat}
    ...
    AdvanceFrameAt (advanceTrialFrame frame n trials time source alpha dt trialDt result status)
      fuel n trials time source tracker outputTime outputRoot done := by
  cases h
...
theorem AdvanceFrameAt.finish {frame : Locals} {fuel : UInt64} {n : Nat}
    {trials time source tracker outputTime outputRoot : UInt64} {done : Bool}
    (h : AdvanceFrameAt frame fuel n trials time source tracker outputTime outputRoot done) :
    AdvanceFrameAt (advanceFinishedFrame frame time source) fuel n trials time source tracker time source true := by
  cases h
...
theorem AdvanceFrameAt.continued {frame : Locals} {fuel : UInt64} {n : Nat}
    ...
    AdvanceFrameAt (advanceContinuedFrame frame fuel n trials nextTime result)
      (fuel - 1) n trials nextTime result result outputTime outputRoot done := by
  cases h
