import Project.EulerReconstructed.ArtifactTranslation

namespace Project.EulerReconstructed.HostInitial
open Wasm

def ResetSpecFor (m : Wasm.Module) : Prop :=
  ∀ env : HostEnv Unit,
    TerminatesWith env m 150 (m.initialStore (α := Unit)) []
...
theorem reset_store_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env module 150 initial []
  ...
  have hReset : { initial with globals := { globals :=
    ...
    rw [← hGlobals]
  refine TerminatesWith.of_wp_entry_for (f := func150Def) rfl ?_ (by decide)
  change wp module func150 _ initial (func150Def.toLocals []) env
  wp_run [func150, func150Def, hGlobals, List.set, hReset]
  simp
...
end Project.EulerReconstructed.HostInitial
