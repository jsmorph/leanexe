...
open Project.EulerRiemann.Execution (boolWord allocatedStore allocatedRoot countedStore)
...
theorem sweep_entry_spec (env : HostEnv Unit) (store : Store Unit) (n : Nat)
    ...
    (grid : Array Project.EulerRiemann.Traversal.Cell) (hGrid : Memory.GridAt store source grid)
    ...
    wp Project.EulerReconstructed.«module» (func121.take 6 ++ rest) Q store
...
theorem sweep_capacity_spec (env : HostEnv Unit) (store : Store Unit) (n : Nat)
    ...
    wp Project.EulerReconstructed.«module» ((func121.drop 6).take 18 ++ rest) Q store
      (sweepAllocationFrame (sweepParameters n fuel axis ratio owner source)
...
theorem sweep_install_spec (env : HostEnv Unit) (store : Store Unit) (n : Nat)
    ...
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerReconstructed.«module» rest Q (writeLength store target (UInt64.ofNat count))
    ...
    wp Project.EulerReconstructed.«module» ((func121.drop 39).take 8 ++ rest) Q store
