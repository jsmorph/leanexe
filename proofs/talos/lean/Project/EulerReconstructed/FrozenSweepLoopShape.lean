import Project.EulerReconstructed.FrozenUpdateCell
...
def sweepLoop : Wasm.Program :=
  match (func121[47]? : Option Wasm.Instruction) with
  | some (.block _ _ [.loop _ _ body _ _] _ _) => body
  | _ => []

theorem sweep_loop_shape :
    func121[47]? = some (.block 0 0 [.loop 0 0 sweepLoop]) := rfl

structure SweepScratch where
  cell : Project.EulerRiemann.Frozen.Traversal.Cell := ⟨0, ⟨0, 0, 0, 0⟩, 0, 0⟩
  result : Project.EulerRiemann.Frozen.Traversal.Cell := ⟨0, ⟨0, 0, 0, 0⟩, 0, 0⟩
  n : UInt64 := 0
  fuel : UInt64 := 0
  axis : UInt64 := 0
  ratio : UInt64 := 0
...
def sweepFrame (n : Nat) (fuel : UInt64) (axis : Bool) (ratio owner source target : UInt64)
    (count index : Nat) (scratch : SweepScratch) (allocation : List Wasm.Value) : Locals :=
  { params := [.i64 (UInt64.ofNat n), .i64 fuel, .i64 (boolWord axis), .i64 ratio,
    ...
    locals := [
      ...
      .i64 scratch.n, .i64 scratch.fuel, .i64 scratch.axis, .i64 scratch.ratio, .i64 scratch.owner,
      .i64 scratch.pointer,
      .i64 (UInt64.ofNat scratch.cell.index), .i64 scratch.cell.state.density,
      .i64 scratch.cell.state.mx, .i64 scratch.cell.state.my, .i64 scratch.cell.state.energy,
      .i64 scratch.cell.pressure, .i64 scratch.cell.status,
...
def sweepInvariant (initial : Store Unit) (n : Nat) (fuel : UInt64) (axis : Bool)
    (ratio owner source target : UInt64) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell)
  ...
  fun current frame =>
    ...
    ∃ (index : Nat) (scratch : SweepScratch),
      index ≤ grid.size ∧
      Memory.PrefixAt current target (Traversal.sweep n fuel.toNat axis ratio grid) (7 * index) ∧
      frame = sweepFrame n fuel axis ratio owner source target grid.size index scratch allocation

def sweepMeasure (count : Nat) (_ : Store Unit) (frame : Locals) : Nat :=
  match frame.get 46 with
  | some (.i64 index) => count - index.toNat + 1
  | _ => 0

theorem sweepMeasure_frame (count index n : Nat) (initial : Store Unit)
    (fuel : UInt64) (axis : Bool) (ratio owner source target : UInt64) (scratch : SweepScratch)
    (allocation : List Wasm.Value) (hAllocation : allocation.length = 8)
    (hi : index < UInt64.size) :
    sweepMeasure count initial
      (sweepFrame n fuel axis ratio owner source target count index scratch allocation) =
      count - index + 1 := by
