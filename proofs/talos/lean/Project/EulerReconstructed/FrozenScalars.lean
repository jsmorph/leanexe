import Project.EulerReconstructed.FrozenFaceRegion
import Project.EulerReconstructed.FrozenReconstructionRegion
import Project.EulerReconstructed.FrozenGridRegion
import Project.EulerReconstructed.FrozenCflRegion
...
theorem reconstruct_exact (env : HostEnv Unit) (initial : Store Unit) (fuel : UInt64)
    ...
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 75 initial
      (stateValues right ++ stateValues center ++ stateValues left ++ [.i64 fuel])
      (fun final values => final = initial ∧ values =
        facesValues (ModelAgreement.faces (Project.EulerRiemann.Frozen.Reconstruction.reconstruct fuel.toNat left center right))) := by
  rw [ModelAgreement.reconstruct]
  exact Project.FunctionRegion.terminatesWith ReconstructionRegion.shift 40
...
theorem face_step_exact (env : HostEnv Unit) (initial : Store Unit) (ratio : UInt64)
    ...
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 105 initial
      (arguments ratio center leftOuter leftInner rightInner rightOuter)
      (fun final values => final = initial ∧ values =
        cellValues (faceStepCheckedBits ratio center leftOuter leftInner rightInner rightOuter)) := by
  rw [ModelAgreement.faceStep]
  exact Project.FunctionRegion.terminatesWith FaceRegion.shift 70
...
theorem rejectedCell_exact (env : HostEnv Unit) (initial : Store Unit) :
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 103 initial []
...
theorem scanCell_exact (env : HostEnv Unit) (initial : Store Unit)
    (acc : Project.ProofKit.F64Outward.Checked) (cell : Project.EulerRiemann.Frozen.Traversal.Cell) :
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 45 initial
      ...
      (fun final values => final = initial ∧ values =
        checkedValues (Project.EulerRiemann.Frozen.OutwardMaximum.scanCell acc cell)) := by
  rw [ModelAgreement.scanCell]
  exact Project.FunctionRegion.terminatesWith GridRegion.shift 44
    (by norm_num [GridRegion.domain])
    (Project.EulerOutwardGrid.Execution.scanCell_exact env initial acc (ModelAgreement.cell cell))

theorem gridRatioChecked_exact (env : HostEnv Unit) (initial : Store Unit)
    (n dt alpha : UInt64) :
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 53 initial
      [.i64 alpha, .i64 dt, .i64 n]
      (fun final values => final = initial ∧ values = checkedValues
        (Project.EulerRiemann.Frozen.OutwardCfl.gridRatioChecked n.toNat dt alpha)) := by
  rw [ModelAgreement.gridRatio]
  exact Project.FunctionRegion.terminatesWith CflRegion.shift 15
