import Project.EulerReconstructed.FrozenExecutionRetryTotal
import Project.EulerReconstructed.FrozenGridScan
...
def advanceLoop : Wasm.Program :=
  match (func129[4]? : Option Wasm.Instruction) with
...
def advanceContinueBody : Wasm.Program :=
  match (advanceTrialBody[70]? : Option Wasm.Instruction) with
  | some (.iff _ _ body _ _ _) => body
  | _ => []
...
theorem advance_loop_shape : func129 =
    func129.take 4 ++ [.block 0 0 [.loop 0 0 advanceLoop]] ++ func129.drop 5 := rfl

theorem advance_work_shape : advanceWorkBody = advanceWorkBody.take 13 ++
    [.localGet 17, .constI64 0, .eqI64,
...
theorem advance_trial_shape : advanceTrialBody = advanceTrialBody.take 60 ++
    [.localGet 36, .constI64 0, .eqI64,
      .iff 0 1 [.constI64 1] [.constI64 0] [] [.i64], .constI64 1, .eqI64,
      .iff 0 1 [.constI64 1] [.constI64 0] [] [.i64], .constI64 0, .eqI64, .eqz,
...
def advanceTrialFrame (frame : Locals) (n : Nat) (trials time source alpha dt trialDt result : UInt64)
  ...
  let locals := locals.set 46 (.i64 dt)
  let locals := locals.set 47 (.i64 1)
  let locals := locals.set 48 (.i64 (dt + 1))
  let locals := locals.set 18 (.i64 (dt + 1))
  let locals := locals.set 19 (.i64 (UInt64.ofNat n))
  let locals := locals.set 20 (.i64 trials)
  let locals := locals.set 21 (.i64 time)
  let locals := locals.set 22 (.i64 dt)
  let locals := locals.set 23 (.i64 alpha)
  let locals := locals.set 24 (.i64 source)
  let locals := locals.set 25 (.i64 source)
  let locals := locals.set 29 (.i64 result)
  let locals := locals.set 28 (.i64 result)
  let locals := locals.set 27 (.i64 trialDt)
  let locals := locals.set 26 (.i64 status)
  let locals := locals.set 30 (.i64 status)
  let locals := locals.set 31 (.i64 trialDt)
  let locals := locals.set 32 (.i64 result)
  let locals := locals.set 33 (.i64 result)
  { frame with locals, values := [] }
