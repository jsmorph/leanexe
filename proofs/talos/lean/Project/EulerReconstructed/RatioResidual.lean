...
noncomputable def spacingErrorBound (n : Nat) : ℝ :=
  enclosureWidth (Wasm.IEEE64.div 0x3FF0000000000000 (Project.EulerRiemann.Time.smallNaturalBits n))
...
theorem spacing_error (n : Nat) (hn : n ≤ 800) (hs : (spacingLower n).status = 0) :
  ...
  have he := F64Outward.div_error false 0x3FF0000000000000
    (Project.EulerRiemann.Time.smallNaturalBits n) hs
  have hword : Project.EulerRiemann.Time.smallNaturalBits 1 = 0x3FF0000000000000 := by decide +kernel
  have hone : value 0x3FF0000000000000 = (1 : ℝ) := by
    simpa only [hword, Nat.cast_one] using Project.EulerRiemann.Time.smallNaturalBits_value 1 (by decide)
  simpa only [spacingLower, spacingErrorBound, hone,
    Project.EulerRiemann.Time.smallNaturalBits_value n hn] using he
