...
import Project.EulerRiemann.FrozenNumericsLineGeometry

namespace Project.EulerReconstructed.Frozen.Conservation
open Project.Euler2DCellStep.Sweep
open Project.EulerRiemann.Frozen.Conservation (clamp lineState orient_twice)
open Project.EulerRiemann.Frozen.Numerics (stateWords cellWords)
...
theorem line_face_accepted {n : Nat} (hn : 0 < n) (fuel : Nat) (axis : Bool) (ratio : UInt64)
    (grid : Grid n n) (h : Accepted (Numerics.outputs fuel ratio axis grid))
    (line : Fin n) (k : Nat) (hk : k < n) :
    (Project.EulerRiemann.Frozen.OutwardNumerics.faceRowStep ratio
...
theorem line_next_words {n : Nat} (hn : 0 < n) (fuel : Nat) (axis : Bool) (ratio : UInt64)
  ...
  have hc : clamp hn k = (⟨k, hk⟩ : Fin n) :=
    Project.EulerRiemann.Frozen.Conservation.clamp_current hn ⟨k, hk⟩
