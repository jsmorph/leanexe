import Project.EulerReconstructed.FrozenRetryTotalLoop
...
theorem retry_pages_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    (source : FreeNode) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell) (n : Nat) (fuel trials time dt alpha : UInt64)
  ...
  · let need := normalizedCapacity (UInt64.ofNat grid.size) 7
    ...
    have hCurrentCap : limit ≤ final.memoryCap module 0 * 65536 := by
      change limit ≤ final.memoryCap Project.EulerRiemann.Frozen.«module» 0 * 65536
      rw [hStore.cap]
      exact hCap
