import Project.EulerReconstructed.FrozenExecutionSweep
import Project.EulerRiemann.FrozenHeapGrid
...
theorem sweep_owned (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    (source : FreeNode) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell) (n : Nat) (fuel : UInt64) (axis : Bool) (ratio : UInt64)
    ...
    TerminatesWith env Project.EulerReconstructed.Frozen.«module» 121 initial
      ...
      (fun final values => values = [.i64 node.root, .i64 node.root] ∧
        ...
        (heap.allocate need).Owns final node (Traversal.sweep n fuel.toNat axis ratio grid) ∧
  ...
  have hSep : gridFreeSeparated source.root grid.size heap.nodes := by
    ...
    omega
  have hCall := sweep_exact env initial n fuel axis ratio source.root source.root heap.top heap.allocations
  ...
  · exact (hTotal.memoryCap Project.EulerReconstructed.Frozen.«module» 0).trans
      (heap.allocateStore_memoryCap initial need Project.EulerReconstructed.Frozen.«module» 0)
