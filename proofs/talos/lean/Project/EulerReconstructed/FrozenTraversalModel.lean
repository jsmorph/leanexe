import Project.EulerReconstructed.FrozenNumericsModel
import Project.EulerRiemann.FrozenTraversalModel

namespace Project.EulerReconstructed.Frozen.Traversal
open Project.Euler2DCellStep.Sweep
open Project.EulerRiemann.Frozen.Traversal (Cell Indexed asGrid neighborIndex neighborIndex_eq linearIndex_lt)
...
theorem neighborIndex_x (n : Nat) (j i : Fin n) (forward : Bool) :
  ...
  cases forward <;> simp only [neighborIndex_eq, Project.EulerRiemann.Frozen.Geometry.neighbor_x
...
theorem neighborIndex_y (n : Nat) (j i : Fin n) (forward : Bool) :
  ...
  cases forward <;> simp only [neighborIndex_eq, Project.EulerRiemann.Frozen.Geometry.neighbor_y
...
theorem sweep_accepted_iff (n fuel : Nat) (axis : Bool) (ratio : UInt64)
    ...
    Project.EulerRiemann.Frozen.Traversal.accepted (sweep n fuel axis ratio grid) = true ↔
      Accepted (Numerics.outputs fuel ratio axis (asGrid n grid)) := by
  simp only [Project.EulerRiemann.Frozen.Traversal.accepted, Array.all_eq_true, beq_iff_eq]
  constructor
  · intro ha j i
    have hi : j.val * n + i.val < grid.size := by
      rw [h.1]
