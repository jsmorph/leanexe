import Project.EulerReconstructed.FrozenScanLoopShape
...
macro "reconstructed_scan_peel" : tactic => `(tactic|
  repeat
    first
    | wp_run [scanFrame, Project.EulerOutwardGrid.Execution.scanFrame, Memory.cellWords, Array.getD, List.set,
...
theorem scan_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNext : ∀ scratch : ScanScratch,
      wp Project.EulerReconstructed.Frozen.«module» rest Q initial
        (scanFrame owner pointer grid.size grid.size (scanPrefix grid grid.size) scratch) env) :
    wp Project.EulerReconstructed.Frozen.«module» ([.block 0 0 [.loop 0 0 scanLoop]] ++ rest) Q initial
  ...
  · rintro current frame ⟨hCurrent, i, sc, hi, hFrame⟩
    ...
    · have hEncoded : ¬ UInt64.ofNat grid.size ≤ UInt64.ofNat i := by
      ...
      refine ⟨?_, ?_⟩
      · refine ⟨rfl, i + 1,
          ⟨ModelAgreement.cell grid[i], scanPrefix grid i, 0, 1⟩, by omega, ?_⟩
    ...
    · have heq : i = grid.size := by omega
      have hEncoded : UInt64.ofNat grid.size ≤ UInt64.ofNat i := by
        simp [heq]
      reconstructed_scan_peel
      simpa [scanFrame, Project.EulerOutwardGrid.Execution.scanFrame, heq] using hNext sc
