...
theorem step_pages_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    (source : FreeNode) (grid : Array Project.EulerRiemann.Traversal.Cell) (n : Nat) (fuel ratio : UInt64)
    (pageLimit : Nat)
    ...
    (hSecond : let need := normalizedCapacity (UInt64.ofNat grid.size) 7
      ...
      takeFirstFitFrom 0 need afterFirst.nodes = none →
        ...
        bumpPages afterFirst.top need ≤ initial.memoryCap Project.EulerReconstructed.«module» 0)
    ...
    let result := stepAllocation heap need (Project.EulerRiemann.Traversal.accepted (Traversal.sweep n fuel.toNat false ratio grid))
    TerminatesWith env Project.EulerReconstructed.«module» 124 initial
      ...
      (fun final values => values = [.i64 result.2.root, .i64 result.2.root] ∧
        ...
        final.memoryCap Project.EulerReconstructed.«module» 0 = initial.memoryCap Project.EulerReconstructed.«module» 0 ∧
  ...
  change wp Project.EulerReconstructed.«module» func124 _ initial
  ...
  have hFrame1 (saved : FreeNode) (savedGrid : Array Project.EulerRiemann.Traversal.Cell)
  ...
  subst sameStore
  cases hAccepted : Project.EulerRiemann.Traversal.accepted middle with
  | false =>
    reconstructed_step_peel
  ...
  | true =>
    ...
    have hFrame2 (saved : FreeNode) (savedGrid : Array Project.EulerRiemann.Traversal.Cell)
        (hSaved : heap.Owns initial saved savedGrid) :
    ...
    have hFinalFrame (saved : FreeNode) (savedGrid : Array Project.EulerRiemann.Traversal.Cell)
        (hSaved : heap.Owns initial saved savedGrid) :
