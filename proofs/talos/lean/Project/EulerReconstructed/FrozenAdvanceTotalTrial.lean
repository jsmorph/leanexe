import Project.EulerReconstructed.FrozenAdvanceScan
import Project.EulerReconstructed.FrozenExecutionRetryTotal
import Project.EulerRiemann.FrozenProposalFuel
...
open Project.EulerRiemann.Frozen.Execution (Heap RetryStoreAt)

macro "reconstructed_advance_trial_peel" : tactic => `(tactic|
  repeat
    first
...
theorem advance_total_trial_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (n : Nat) (trials time alpha : UInt64) (spare limit pageLimit : Nat)
    ...
    (hNext : let dt := Time.proposal n time alpha
      ...
      ∀ final finalHeap result,
        ...
        wp module rest Q final
          (advanceTrialFrame frame n trials time source.root alpha dt trial.dt result.root trial.status) env) :
    wp module (advanceTrialBody.take 60 ++ rest) Q initial frame env := by
  ...
  unfold advanceTrialBody advanceWorkBody advanceLoop func129
  dsimp only
  reconstructed_advance_trial_peel
