...
theorem retry_accept_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerReconstructed.«module» (retryAcceptBody ++ rest) Q store
...
theorem retry_reject_spec (env : HostEnv Unit) (store : Store Unit) (heap : Heap)
    ...
    (result : FreeNode) (grid : Array Project.EulerRiemann.Traversal.Cell)
    (hParams : frame.params = [.i64 fuel, .i64 (UInt64.ofNat n), .i64 trials, .i64 time,
      .i64 dt, .i64 alpha, .i64 source, .i64 source])
    ...
    (hNext : wp Project.EulerReconstructed.«module» rest Q (heap.releaseStore store result)
    ...
    wp Project.EulerReconstructed.«module» (retryRejectBody ++ rest) Q store
