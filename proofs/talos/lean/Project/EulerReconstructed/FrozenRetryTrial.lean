import Project.EulerReconstructed.FrozenRetryCflReject
import Project.EulerReconstructed.FrozenStepReserve
...
open Project.EulerRiemann.Frozen.Execution (boolWord Heap stepAllocation)
      ...
      ∀ final, result.1.At final → result.1.Owns final result.2 (Traversal.step n trials.toNat ratio grid) →
        ...
        wp Project.EulerReconstructed.Frozen.«module» rest Q final
          (retryTrialFrame frame n trials ratio source.root result.2.root
            (Project.EulerRiemann.Frozen.Traversal.accepted (Traversal.step n trials.toNat ratio grid))) env) :
    wp Project.EulerReconstructed.Frozen.«module» (retryStepBody.take 30 ++ rest) Q initial frame env := by
