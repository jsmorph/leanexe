import Project.EulerReconstructed.FrozenRetryShape
import Project.EulerReconstructed.FrozenHeapAllocate
...
theorem retry_failure_prepare_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    wp Project.EulerReconstructed.Frozen.«module» ([.constI64 status, .localSet 9, .localGet 4, .localSet 10] ++
  ...
  change wp Project.EulerReconstructed.Frozen.«module» (FixedArrayCapacity.constantProgram 0 7 79 ++ rest) Q store
    (FixedArraySearch.frame params (retryFailureSaved saved status dt) []
      need previous current capacity next result) env
  apply FixedArrayCapacity.constantProgram_spec 0 7 79 Project.EulerReconstructed.Frozen.«module» env store _ rfl
...
theorem retry_failure_program_spec (env : HostEnv Unit) (store : Store Unit) (heap : Heap)
    ...
    (hBump : takeFirstFitFrom 0 8 heap.nodes = none →
      heap.top.toNat + 48 + 8 ≤ 4294967296 ∧ bumpPages heap.top 8 ≤ store.memoryCap Project.EulerReconstructed.Frozen.«module» 0)
    (hPages : store.mem.pages ≤ 65536) (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous current capacity next,
      wp Project.EulerReconstructed.Frozen.«module» rest Q (heap.emptyStore store)
        (retryFailureFrame params saved status dt previous current capacity next
          (allocatedRoot heap.top 8 heap.nodes)) env) :
    wp Project.EulerReconstructed.Frozen.«module» (retryFailureProgram status ++ rest) Q store
      (FixedArraySearch.frame params saved [] need previous current capacity next result) env := by
  have hStart : params.length + (retryFailureSaved saved status dt).length = 79 := by
    simp [retryFailureSaved, hParams, hSaved]
  ...
  change wp Project.EulerReconstructed.Frozen.«module» (resultProgram 84 69 ++ lengthStoreProgram 69 0 ++
    finishProgram 69 11 12 ++ rest) Q (heap.allocateStore store 8) base env
  simp only [List.append_assoc]
  apply resultProgram_spec 84 69 Project.EulerReconstructed.Frozen.«module» env _ base root rfl hRoot (by omega) hValid69
  have hInstalledRoot := resultFrame_get_result base 69 root (by omega) hValid69
  apply lengthStore_spec Project.EulerReconstructed.Frozen.«module» env _ _ root 0 69 rfl hInstalledRoot hAddress
  apply finishProgram_spec Project.EulerReconstructed.Frozen.«module» env _ _ root 69 11 12 rfl hInstalledRoot
