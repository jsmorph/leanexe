import Project.EulerReconstructed.FrozenRetryRatio
...
def retryCflRejectedFrame (frame : Locals) (fuel : UInt64) (n : Nat)
  ...
  let locals := frame.locals.set 44 (.i64 (UInt64.ofNat n))
  let locals := locals.set 45 (.i64 trials)
  let locals := locals.set 46 (.i64 time)
  let locals := locals.set 47 (.i64 half)
  let locals := locals.set 48 (.i64 alpha)
  let locals := locals.set 49 (.i64 source)
  let locals := locals.set 50 (.i64 source)
  let locals := locals.set 51 (.i64 (UInt64.ofNat n))
  let locals := locals.set 52 (.i64 trials)
  let locals := locals.set 53 (.i64 time)
  let locals := locals.set 54 (.i64 half)
  let locals := locals.set 55 (.i64 alpha)
  let locals := locals.set 56 (.i64 source)
  let locals := locals.set 57 (.i64 source)
  let locals := locals.set 58 (.i64 0)
...
theorem retry_cfl_reject_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerReconstructed.Frozen.«module» (retryCflRejectBody ++ rest) Q store frame env := by
  ...
  unfold retryCflRejectBody retryTrial retryLoop func125
  dsimp only
  repeat
    first
