import Project.EulerReconstructed.NumericalSafety
import Project.EulerReconstructed.TraceBalance
import Project.EulerReconstructed.ControlTrace
import Project.EulerRiemann.OutwardMaximumCfl

namespace Project.EulerReconstructed.Numerics
open CodeLib.IEEE64
open Project.Euler2DCellStep.Sweep (orient)
open Project.EulerRiemann.Traversal (Cell accepted initialCells initialCells_getElem initialCell_safe)
open Project.EulerRiemann.Control (CellsSafe)
open Project.EulerRiemann.Hyperbolicity (CellsHyperbolic cells_hyperbolic)
...
theorem step_facts (n fuel : Nat) (dt : UInt64) (grid next : Array Cell)
  ...
  have hr : value dt * (n : ℝ) ≤ value (Conservation.traceRatio n dt grid) :=
    (Project.EulerRiemann.OutwardCfl.grid_ratio_accepted n dt _ h.2.1).2.2.2.2.2.1
  ...
  · intro cell hc axis i
    exact Project.EulerRiemann.OutwardMaximum.grid_characteristic_courant grid n dt h.1 h.2.1 cell hc axis i
