import Project.EulerReconstructed.FrozenExecutionSolve
...
theorem solve_initial_exact (env : HostEnv Unit) (n : Nat) (trials : UInt64) (hn : 2 ≤ n ∧ n ≤ 800) :
    TerminatesWith env module 148 (module.initialStore (α := Unit)) [.i64 trials, .i64 (UInt64.ofNat n)]
      (fun final values => ∃ (finalHeap : Heap) (result : FreeNode),
        values = [.i64 result.root] ∧ finalHeap.At final ∧
        finalHeap.OwnsWords final result (Control.solve n trials.toNat) ∧ final.mem.pages ≤ 8192) := by
