...
open Project.EulerRiemann.Frozen.Execution (cellValues boolWord)

set_option maxRecDepth 32768
set_option maxHeartbeats 1000000

macro "reconstructed_sweep_peel" : tactic => `(tactic|
  repeat
    first
...
macro "reconstructed_sweep_write_peel" h:Lean.Parser.Tactic.simpLemma : tactic => `(tactic|
...
theorem sweep_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    (n : Nat) (fuel : UInt64) (axis : Bool) (ratio owner source target : UInt64)
    (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell) (hn : 2 ≤ n ∧ n ≤ 800)
    (hIndexed : Project.EulerRiemann.Frozen.Traversal.Indexed n grid)
    ...
    (hPrefix : Memory.PrefixAt initial target (Traversal.sweep n fuel.toNat axis ratio grid) (7 * index))
    ...
    (hNext : ∀ (final : Store Unit) (scratch : SweepScratch),
      ...
      Memory.GridAt final target (Traversal.sweep n fuel.toNat axis ratio grid) →
      Memory.WritesGrid initial final target grid.size →
      wp Project.EulerReconstructed.Frozen.«module» rest Q final
        (sweepFrame n fuel axis ratio owner source target grid.size grid.size scratch allocation) env) :
    wp Project.EulerReconstructed.Frozen.«module» ([.block 0 0 [.loop 0 0 sweepLoop]] ++ rest) Q initial
      (sweepFrame n fuel axis ratio owner source target grid.size index scratch allocation) env := by
  have hSize64 := hGrid.size_lt
  have hSizeNat := UInt64.toNat_ofNat_of_lt' hSize64
  have hOutSize := Traversal.sweep_size n fuel.toNat axis ratio grid
  simp only [List.cons_append, List.nil_append]
  apply wp_block_cons
  apply wp_loop_cons
    (Inv := sweepInvariant initial n fuel axis ratio owner source target grid allocation)
  ...
  · rintro current frame ⟨hCurrentGrid, hWrites, i, sc, hi, hPrefix, hFrame⟩
    ...
    · have hEncoded : ¬ UInt64.ofNat grid.size ≤ UInt64.ofNat i := by
      ...
      have hOutIndex : i < (Traversal.sweep n fuel.toNat axis ratio grid).size := by omega
      ...
      reconstructed_sweep_peel
      refine wp_call_tw
        (updateCell_exact env current n fuel axis ratio owner source grid grid[i]
          hn hIndexed.1 hCellIndex hCurrentGrid) ?_
      rintro final values ⟨hFinal, rfl⟩
      subst final
      have hOutCell : (Traversal.sweep n fuel.toNat axis ratio grid)[i] =
          Traversal.updateCell n fuel.toNat axis ratio grid grid[i] := by
      ...
      have hSource := hCurrentGrid.writeCell_disjoint
        (cell := Traversal.updateCell n fuel.toNat axis ratio grid grid[i]) hFit hlt hDisjoint
      have hFrame := hWrites.trans
        (Memory.writeCell_frame (Traversal.updateCell n fuel.toNat axis ratio grid grid[i]) hFit hlt)
      let written0 := Memory.writeField current target i 0
        (UInt64.ofNat (Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).index)
      let written1 := Memory.writeField written0 target i 1
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).state.density)
      let written2 := Memory.writeField written1 target i 2
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).state.mx)
      let written3 := Memory.writeField written2 target i 3
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).state.my)
      let written4 := Memory.writeField written3 target i 4
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).state.energy)
      let written5 := Memory.writeField written4 target i 5
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).pressure)
      let written6 := Memory.writeField written5 target i 6
        ((Traversal.updateCell n fuel.toNat axis ratio grid grid[i]).status)
      reconstructed_sweep_write_peel hAllocation
      ...
      · exact ⟨hSource, hFrame, i + 1,
          ⟨grid[i], Traversal.updateCell n fuel.toNat axis ratio grid grid[i],
            UInt64.ofNat n, fuel, boolWord axis, ratio, owner, source⟩,
    ...
    · have heq : i = grid.size := by omega
      ...
      have hComplete : Memory.GridAt current target (Traversal.sweep n fuel.toNat axis ratio grid) := by
