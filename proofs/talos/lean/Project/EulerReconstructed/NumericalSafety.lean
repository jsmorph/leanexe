...
namespace Project.EulerReconstructed.Numerics
open CodeLib.IEEE64
...
open Project.EulerRiemann.Hyperbolicity (StateHyperbolic CellsHyperbolic state_hyperbolic cells_hyperbolic)
open Project.EulerRiemann.Traversal (Cell accepted)
open Project.EulerRiemann.OutwardNumerics (reconstructed_step_parts reconstructed_step_characteristic_courant)
...
noncomputable def ReconstructionFacts (fuel : Nat) (left center right : State) : Prop :=
  ...
  out.status = 0 ∧ StateSafe out.left ∧ StateSafe out.right ∧
    ...
    Project.EulerReconstruction.Spec.Accuracy fuel left center right out ∧
...
theorem reconstruction_facts (fuel : Nat) (left center right : State)
  ...
  have hs := Project.EulerRiemann.Reconstruction.reconstruct_safe fuel left center right h
  exact ⟨h, hs.1, hs.2, state_hyperbolic _ hs.1, state_hyperbolic _ hs.2,
    Project.EulerRiemann.Reconstruction.reconstruct_accuracy fuel left center right h,
    Project.EulerRiemann.Reconstruction.reconstruct_factor fuel left center right⟩
...
theorem stencil_facts (n fuel : Nat) (dt ratio : UInt64) (input : Stencil)
    ...
    (ha : (Project.EulerRiemann.OutwardNumerics.reconstructedStepCheckedBits fuel ratio
...
end Project.EulerReconstructed.Numerics
