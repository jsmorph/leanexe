import Project.EulerReconstructed.FrozenAdvanceTotalFrame
import Project.EulerReconstructed.FrozenAdvanceContinue
...
def AdvanceActive (initial : Store Unit) (initialHeap : Heap) (n : Nat) (trials startTime : UInt64)
  ...
  ∃ (fuel time : UInt64) (source : FreeNode) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell)
      (tracked : Bool),
    AdvanceFrameAt frame fuel n trials time source.root (if tracked then source.root else 0) 0 0 false ∧
    Control.advance fuel.toNat n trials.toNat time grid = expected ∧
