import Project.EulerReconstructed.FrozenAdvanceTotalInvariant
...
open Project.EulerRiemann.Frozen.Control (Result)

macro "reconstructed_advance_total_peel" : tactic => `(tactic|
  repeat
    first
...
theorem advance_total_work_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
    ...
    (source : FreeNode) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell) (tracked : Bool)
    ...
    (hFrame : AdvanceFrameAt frame fuel n trials time source.root (if tracked then source.root else 0) 0 0 false)
    (hSame : Control.advance fuel.toNat n trials.toNat time grid = expected)
  ...
  refine advance_scan_spec env store (advanceTimeFrame frame) n fuel trials time source.root grid
  ...
  · have hScanned := hFrame.time.scan { status := 0, value := (OutwardMaximum.gridUpper grid).value }
    have hCurrentCap : limit ≤ store.memoryCap module 0 * 65536 := by
      change limit ≤ store.memoryCap Project.EulerRiemann.Frozen.«module» 0 * 65536
      rw [hStore.cap]
      exact hCap
    ...
    dsimp only [frame]
    reconstructed_advance_total_peel
    ...
    refine advance_total_trial_spec env store heap (advanceScanFrame (advanceTimeFrame frame) source.root
      ...
      fuel source grid n trials time (OutwardMaximum.gridUpper grid).value callSpare limit pageLimit hScanned.params hScanned.locals
    ...
    have hTrialFrame := hScanned.trial (OutwardMaximum.gridUpper grid).value dt trial.dt result.root trial.status
    by_cases hTrialSuccess : trial.status = 0
    · have hNextSame : Control.advance (fuel - 1).toNat n trials.toNat (IEEE64.add time trial.dt) trial.grid = expected := by
        simpa [hFuelNat, Control.advance, hTime, hScan, dt, trial, hTrialSuccess] using hSame
      have hTrialValid := (Control.retry_success (dt.toNat + 1) n trials.toNat time dt (OutwardMaximum.gridUpper grid).value grid hTrialSuccess).1
      ...
      apply advance_continue_spec env final finalHeap
        ...
        n trials time (OutwardMaximum.gridUpper grid).value dt trial.dt result.root tracked
