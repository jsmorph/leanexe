import Project.EulerReconstructed.FrozenRetryInvariant
...
open Project.EulerRiemann.Frozen.Control (Attempt)
...
theorem retry_total_step_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
    (source : FreeNode) (grid : Array Project.EulerRiemann.Frozen.Traversal.Cell) (n : Nat) (fuel trials time dt alpha ratio : UInt64)
    ...
    (hSame : (if Project.EulerRiemann.Frozen.Traversal.accepted (Traversal.step n trials.toNat ratio grid)
  ...
  cases hAccepted : Project.EulerRiemann.Frozen.Traversal.accepted (Traversal.step n trials.toNat ratio grid) with
  | true =>
