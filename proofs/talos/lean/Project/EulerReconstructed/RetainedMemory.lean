...
import Project.EulerRiemann.ExecutionInitialCells
...
theorem initial_cells_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env Project.EulerReconstructed.«module» 141 initial [.i64 (UInt64.ofNat n)]
  ...
  exact Project.EulerRiemann.Execution.initial_cells_exact env initial heap n limit pageLimit
...
theorem output_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env Project.EulerReconstructed.«module» 144 initial
  ...
  Project.FunctionRegion.terminatesWith RiemannRegion.shift 99
    (by norm_num [RiemannRegion.domain])
    (Project.EulerRiemann.Execution.output_exact env initial heap source grid n pageLimit time status
...
theorem release_owned (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env Project.EulerReconstructed.«module» 152 initial [.i64 node.root]
  ...
  Project.FunctionRegion.terminatesWith RiemannRegion.shift 107
    (by norm_num [RiemannRegion.domain])
    (Project.EulerRiemann.Execution.release_owned env initial heap node grid hHeap hOwner)
