...
import Project.EulerReconstruction.ArtifactCodeVector
import Project.EulerReconstruction.ArtifactMetadata
...
theorem sections_from6 :
    sectionLoop 5605 6 Cache.raw { bytes := artifactBytes, pos := 5619, limit := 5619 } =
      .ok (Cache.raw, { bytes := artifactBytes, pos := 5619, limit := 5619 }) := by rfl
