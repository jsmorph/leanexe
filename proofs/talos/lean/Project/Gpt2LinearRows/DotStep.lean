...
def outerBody : Wasm.Program :=
  match (func1[49]? : Option Wasm.Instruction) with
...
def parameters (weightsPtr inputPtr : UInt64) (weights input : ByteArray)
  ...
  [.i64 weightsPtr, .i64 (UInt64.ofNat weights.size),
...
def Accumulator (weightsPtr inputPtr : UInt64) (weights input : ByteArray)
...
theorem dotStep_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hready : RangeFoldLoop.Ready 43 44 inputWidth index frame values)
    (hacc : Accumulator weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth
      rows row column index original frame)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hnext : ∀ result, RangeFoldLoop.Ready 43 44 inputWidth (index + 1) result values →
      Accumulator weightsPtr inputPtr weights input weightOffset biasOffset inputWidth outputWidth
  ...
  simp only [dotStep, outerBody, func1, List.getElem?_cons_zero, List.getElem?_cons_succ,
  ...
  refine wp_call_tw ((PackedWordRead.exact «module» 0 (some 0) rfl rfl
    env initial 0 inputPtr input (row * inputWidth + index) hinput hinputIndex).append_args
  ...
  refine wp_call_tw ((PackedWordRead.exact «module» 0 (some 0) rfl rfl
    env final 0 weightsPtr weights (weightOffset + index * outputWidth + column) hweights hweightIndex).append_args
      rfl rfl rfl values) ?_
  rintro final' resultValues ⟨result, rfl, rfl, rfl⟩
  wp_packed_frame [hparams, hlength, hrow, hcol, htotal, hcounter, hstop, hstride, hready.1, hinNonzero, houtNonzero]
