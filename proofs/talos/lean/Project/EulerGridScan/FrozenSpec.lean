import Project.EulerGridScan.FrozenExecution

namespace Project.EulerGridScan.Frozen.Spec
open Wasm
open Project.EulerGridScan.Frozen.Execution
...
noncomputable def AcceptedSafety (input : Array UInt64) : Prop :=
  ...
  result.status = 0 →
    ...
    ∀ index < input.size / 3, (Project.EulerGridStep.Frozen.Scan.stateAt input index).status = 0 ∧
      CodeLib.IEEE64.value (Project.EulerGridStep.Frozen.Scan.stateAt input index).speed ≤
...
theorem maxSpeedCheckedBits_exact : ExactSpecFor Project.EulerGridScan.Frozen.«module» := by
...
theorem maxSpeedCheckedBits_wat_safe : SafeSpecFor Project.EulerGridScan.Frozen.«module» := by
  intro env initial pointer input hArray
  refine TerminatesWith.mono (maxSpeedCheckedBits_exact env initial pointer input hArray) ?_
  rintro final values ⟨rfl, rfl⟩
  exact ⟨rfl, rfl, Project.EulerGridStep.Frozen.Scan.maxSpeed_safe input⟩

#print axioms maxSpeedCheckedBits_exact
#print axioms maxSpeedCheckedBits_wat_safe
end Project.EulerGridScan.Frozen.Spec
