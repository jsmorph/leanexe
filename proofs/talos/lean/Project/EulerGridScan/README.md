...
The historical frozen scan binary has 3,292 bytes and SHA-256
...
duplication. Current compiler output computes both returned projections in
one loop, whose proof retains the complete result pair. This scan factoring left the then-current 11,222-byte grid-step
...
remaining scan. [LoopShape.lean](LoopShape.lean) identifies the generated
loop directly from the entry. [LoopFrame.lean](LoopFrame.lean) records
the scratch slots and preserved caller locals. [Loop.lean](Loop.lean)
...
[Execution.lean](Execution.lean) composes the loop with the exact header
...
records the historical connection from their decoded and validated module
to the earlier execution model. Frozen identity and translation have not
been revalidated against the current source model in this I/O branch work.
...
the three standard logical axioms. The separate full grid-step source proof is recorded in its linked checkpoint.
