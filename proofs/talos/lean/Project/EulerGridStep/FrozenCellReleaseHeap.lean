import Project.EulerGridStep.FrozenCellReleaseCall
import Project.EulerGridStep.FrozenReleaseHeap
...
theorem cell_release_call_heap {m : Wasm.Module} (layout : Layout m)
    ...
    TerminatesWith env m 40 initial [.i64 (roots count)]
      (fun final values => values = [] ∧
        ReleaseResult initial final (roots count) (fieldRequest output.size) (free.headD 0)
          (cellPrefix output index cell count) ∧
        BufferState final output.size (cellKept roots output index cell (count - 1))
          (roots count :: free) allocs (releases + 1) (frees + 1) ∧
  ...
  have hCall := release_owned_buffer_state_heap layout env initial (roots count) allocs releases frees
  ...
  exact ⟨hValues, by simpa only [cellPrefix_size] using hResult,
    by simpa only [cellPrefix_size] using hFinal, hHeap⟩

#print axioms cell_release_call_heap
