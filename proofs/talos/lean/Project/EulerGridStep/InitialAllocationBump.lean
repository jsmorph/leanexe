...
def initialFreshBumpFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { base with
    locals := ((((((base.locals.set 46 (.i64 0)).set 42 (.i64 0)).set 43 (.i64 0)).set
      44 (.i64 (heapTop + 48 + capacity))).set
      45 (.i64 ((heapTop + 48 + capacity - 1) / 65536 + 1))).set
      46 (.i64 (heapTop + 48))), values := [] }

def initialFreshAllocFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { initialFreshBumpFrame base heapTop capacity with
    locals := (initialFreshBumpFrame base heapTop capacity).locals.set 36 (.i64 (heapTop + 48)) }
...
theorem initial_allocation_bump_spec
  ...
  · rintro st1 s1 ⟨hSt, hFrame⟩
    ...
    change wp module_ (allocationHeaderProgram 48 43 ++ []) _ (initialFreshBumpedStore st heapTop capacity)
      (initialFreshBumpFrame frame heapTop capacity) env
    apply allocation_header_program_spec 48 43 module_ env (initialFreshBumpedStore st heapTop capacity)
