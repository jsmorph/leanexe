...
def initialFreshSearchFrame (base : Locals) : Locals :=
  { base with locals := ((base.locals.set 51 (.i64 0)).set 47 (.i64 0)).set 48 (.i64 0) }

def initialFreshBumpFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { base with
    locals := ((((((base.locals.set 51 (.i64 0)).set 47 (.i64 0)).set 48 (.i64 0)).set
      49 (.i64 (heapTop + 48 + capacity))).set
      50 (.i64 ((heapTop + 48 + capacity - 1) / 65536 + 1))).set
      51 (.i64 (heapTop + 48))), values := [] }

def initialFreshAllocFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { initialFreshBumpFrame base heapTop capacity with
    locals := (initialFreshBumpFrame base heapTop capacity).locals.set 41 (.i64 (heapTop + 48)) }
...
theorem initial_allocation_bump_spec
    ...
    (hCapacityLocal : frame.locals[46]? = some (.i64 capacity))
  ...
  have hCapacityGet : frame.locals[46] = .i64 capacity := by
  ...
  · rintro st1 s1 ⟨hSt, hFrame⟩
    ...
    change wp module_ (allocationHeaderProgram 53 48 ++ []) _ (initialFreshBumpedStore st heapTop capacity)
      (initialFreshBumpFrame frame heapTop capacity) env
    apply allocation_header_program_spec 53 48 module_ env (initialFreshBumpedStore st heapTop capacity)
