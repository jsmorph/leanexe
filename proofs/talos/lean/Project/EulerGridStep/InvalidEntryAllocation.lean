...
def invalidEntryFreshSearchFrame (base : Locals) : Locals :=
  { base with locals := ((base.locals.set 44 (.i64 0)).set 40 (.i64 0)).set 41 (.i64 0) }

def invalidEntryFreshBumpFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { base with
    locals := ((((((base.locals.set 44 (.i64 0)).set 40 (.i64 0)).set 41 (.i64 0)).set
      42 (.i64 (heapTop + 48 + capacity))).set
      43 (.i64 ((heapTop + 48 + capacity - 1) / 65536 + 1))).set
      44 (.i64 (heapTop + 48))), values := [] }
...
def invalidEntryFreshAllocFrame (base : Locals) (heapTop capacity : UInt64) : Locals :=
  { invalidEntryFreshBumpFrame base heapTop capacity with
    locals := (invalidEntryFreshBumpFrame base heapTop capacity).locals.set 35 (.i64 (heapTop + 48)) }
...
theorem invalid_entry_allocation_bump_spec
    ...
    (hCapacityLocal : frame.locals[39]? = some (.i64 capacity))
  ...
  have hCapacityGet : frame.locals[39] = .i64 capacity := by
  ...
  · rintro st1 s1 ⟨hSt, hFrame⟩
    ...
    change wp module_ (allocationHeaderProgram 46 41 ++ []) _ (invalidEntryFreshBumpedStore st heapTop capacity)
      (invalidEntryFreshBumpFrame frame heapTop capacity) env
    apply allocation_header_program_spec 46 41 module_ env (invalidEntryFreshBumpedStore st heapTop capacity)
