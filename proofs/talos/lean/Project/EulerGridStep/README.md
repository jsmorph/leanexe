...
The complete current specification passed on 2026-09-25 against the final I/O merge model.  Its outer loop stages the next accumulator before testing the conjunction release guard and keeps the initial root in local 44.  The execution and safety proofs use the standard logical axioms.  The [model](Model.lean), [payload correspondence](Payload.lean), and [output safety](Outputs.lean) connect the cell recurrence to all six returned fields.  The [separate maximum-speed scan](../EulerGridScan/README.md) bounds rounded computed speeds and supports adaptive time steps.
...
| [Grid setup](GridSetup.lean), [loop frame](GridLoopFrame.lean), and [return staging](GridFinish.lean) | The emitted 47-local entry function and its release guards. |
...
The historical package contains 8,866 bytes with SHA-256 `bc546b72e740ec6e953dc3c01e88a44c19fd914c109c64a33e8d8edcabfe2297`.  [Embedded bytes](ArtifactBytes.lean), [decoding](ArtifactDecode.lean), [validation](ArtifactValidation.lean), and [translation](ArtifactTranslation.lean) establish its binary identity and execution model.  Its `Frozen` proof modules retain the allocation behavior of that binary.  The independent aggregate artifact check passed for this package on 2026-09-25.
