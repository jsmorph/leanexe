import Project.EulerGridStep.FrozenArenaAdvance
import Project.EulerGridStep.FrozenAdvanceFirstPreserved
import Project.EulerGridStep.FrozenAdvanceLaterProtected
import Project.EulerGridStep.FrozenAdvanceFirstRejectedPreserved
import Project.EulerGridStep.FrozenAdvanceLaterRejectedProtected
...
theorem advanceAt_first_preserved_outcome {m : Wasm.Module} (layout : Layout m)
  ...
  · simpa only [arenaAdvanceRoot, ArenaAdvanceState, hAccepted, ite_true, Nat.zero_add,
      ...
      advanceAt_first_preserved layout env initial ratio inputUnused pointer unused allocs releases frees base input output
  ...
  · simpa only [arenaAdvanceRoot, ArenaAdvanceState, hAccepted, ite_false, Nat.zero_add,
      show UInt64.ofNat 0 = 0 from rfl] using
      advanceAt_first_rejected_preserved layout env initial ratio inputUnused pointer unused allocs releases frees base input output
        hInput hPositive hAccepted hSize hBuffers hHeap hBudget hSeparate

/-- Complete later-cell control covers either outcome while retaining the bounded arena. -/
theorem advanceAt_later_protected_outcome {m : Wasm.Module} (layout : Layout m)
    (env : HostEnv Unit) (initial : Store Unit)
    (ratio inputUnused pointer unused allocs releases frees : UInt64)
  ...
  · simpa only [arenaAdvanceRoot, ArenaAdvanceState, hAccepted, ite_true, Nat.zero_add,
      show UInt64.ofNat 0 = 0 from rfl] using
      advanceAt_later_protected layout env initial ratio inputUnused pointer unused allocs releases frees base input output
        index hInput hPositive hi hAccepted hSize hState initialOutput hProtected hSeparate
  · simpa only [arenaAdvanceRoot, ArenaAdvanceState, hAccepted, ite_false, Nat.zero_add,
      show UInt64.ofNat 0 = 0 from rfl] using
      advanceAt_later_rejected_protected layout env initial ratio inputUnused pointer unused allocs releases frees base input output
        index hInput hPositive hi hAccepted hSize hState initialOutput hProtected hSeparate

#print axioms advanceAt_first_preserved_outcome
#print axioms advanceAt_later_protected_outcome
