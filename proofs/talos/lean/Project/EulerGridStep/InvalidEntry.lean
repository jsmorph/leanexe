...
def invalidEntryResultFrame (frame : Locals) (heap : UInt64) : Locals :=
  let allocated := invalidEntryFreshAllocFrame (FixedArrayCapacity.capacityFrame frame 46 16) heap 16
  { allocated with
    locals := (((allocated.locals.set 43 (.i64 1)).set 1 (.i64 (heap + 48))).set
      38 (.i64 (heap + 48))).set 39 (.i64 (heap + 48))
    values := [] }

theorem invalid_entry_shape : gridInvalidBody = FixedArrayCapacity.constantProgram 1 1 46 ++
...
theorem invalid_entry_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hParams : frame.params.length = 2) (hLocals : frame.locals.length = 52) (hValues : frame.values = [])
    (hFit : heap.toNat + 48 + 16 ≤ initial.mem.pages * 65536) (hPages : initial.mem.pages ≤ 65536)
  ...
  apply FixedArrayCapacity.constantProgram_spec 1 1 46 m env initial frame hValues (by omega)
    (by simp [Locals.validIndex, hParams, hLocals])
  change wp m (invalidEntryAllocationRegion ++ (gridInvalidBody.drop 35 ++ rest)) Q initial
    (FixedArrayCapacity.capacityFrame frame 46 16) env
  apply invalid_entry_allocation_bump_spec m env initial (FixedArrayCapacity.capacityFrame frame 46 16) heap 16 allocs
