import Project.EulerGridStep.FrozenAdvanceExecution
import Project.EulerGridStep.FrozenFreshWriterHeap
import Project.EulerGridStep.FrozenLaterArenaState
...
theorem advanceAt_first_preserved {m : Wasm.Module} (layout : Layout m)
  ...
  have hRun := hWriter.mono (fun final values hPost => by
    ...
    show values = [.i64 (arenaRoot base output.size 6), .i64 (arenaRoot base output.size 6)] ∧
      ...
      final.mem.pages = initial.mem.pages ∧ UInt64Array.At final pointer input ∧
      UInt64Array.At final (arenaRoot base output.size 0) output ∧
      (⟨arenaRoot base output.size 0, output⟩ : LiveBuffer).At final output.size
    refine ⟨hValues, ⟨?_, ?_, ?_⟩, hPages, hInputFinal, hPreserved.2.2, hPreserved⟩
...
#print axioms advanceAt_first_preserved
