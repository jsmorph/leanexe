import Project.EulerGridStep.FrozenCopyLoop
import Project.EulerGridStep.FrozenFieldTailModel

namespace Project.EulerGridStep.Frozen.Execution
open Wasm
...
def copyUpdateStoreReturn (offset : Nat) : Wasm.Program :=
  [.localGet (offset + 4), .localGet (offset + 1), .constI64 1, .mulI64, .constI64 1, .addI64,
   .constI64 8, .mulI64, .addI64, .wrapI64, .localGet (offset + 6), .store64 0, .localGet (offset + 4)]

def copyUpdateProgram (offset : Nat) : Wasm.Program :=
  [.localGet (offset + 4), .wrapI64, .localGet (offset + 2), .store64 0] ++
    prefixProgram offset (offset + 4) (offset + 3) (offset + 5) ++ copyUpdateStoreReturn offset

/-- The complete generated tail after allocation: initialize, copy, update and return. -/
theorem copy_update_spec (offset : Nat) (m : Wasm.Module) (env : HostEnv Unit)
    (initial : Store Unit) (frame : Locals) (source target : UInt64)
    (input : Array UInt64) (index : Nat) (value : UInt64)
    (hCounter : frame.validIndex (offset + 5)) (hValues : frame.values = [])
    (hSource : frame.get offset = some (.i64 source))
    (hTarget : frame.get (offset + 4) = some (.i64 target))
    (hIndex : frame.get (offset + 1) = some (.i64 (UInt64.ofNat index)))
    (hLength : frame.get (offset + 2) = some (.i64 (UInt64.ofNat input.size)))
    (hCount : frame.get (offset + 3) = some (.i64 (UInt64.ofNat input.size)))
    (hValue : frame.get (offset + 6) = some (.i64 value))
    ...
    (hNext : ∀ final, FieldWriteState initial source target input index value final →
      wp m rest Q final
        { counterFrame frame (offset + 5) input.size hCounter with values := [.i64 target] } env) :
    wp m (copyUpdateProgram offset ++ rest) Q initial frame env := by
  ...
  change wp m (prefixProgram offset (offset + 4) (offset + 3) (offset + 5) ++ ((copyUpdateStoreReturn offset) ++ rest)) Q
    (writeLength initial target input.size) { params := frame.params, locals := frame.locals } env
  rw [copyFrame_ofParts frame hValues]
  apply copy_loop_spec offset (offset + 4) (offset + 3) (offset + 5) m env (writeLength initial target input.size) frame
    source target input (payloadSnapshot initial target input.size) hCounter
    (by omega) (by omega) (by omega) hValues hSource hTarget hCount
    ...
    (by simp [payloadSnapshot]) hSeparate Q ((copyUpdateStoreReturn offset) ++ rest)
  intro current output hCopy
  have hArray := copyState_complete _ _ _ _ _ _ hCopy
  have hTargetGet := (counterFrame_get_ne frame (offset + 5) input.size (offset + 4) hCounter (by omega)).trans hTarget
  have hIndexGet := (counterFrame_get_ne frame (offset + 5) input.size (offset + 1) hCounter (by omega)).trans hIndex
  have hValueGet := (counterFrame_get_ne frame (offset + 5) input.size (offset + 6) hCounter (by omega)).trans hValue
  ...
  simp only [copyUpdateStoreReturn, List.cons_append, List.nil_append, wp_localGet_cons,
...
#print axioms copy_update_spec
end Project.EulerGridStep.Frozen.Execution
