import Project.EulerGridStep.ReusedCellState
...
theorem writeCell_reused_framed {m : Wasm.Module} (layout : Layout m)
    ...
    (hState : ReusedCellState initial heapTop roots output index cell 0 allocs releases frees)
    ...
    (hWriteFrame : ∀ field < 6, ∀ current final, P current →
      FieldResult (reusedChoice roots allocs output.size field) current final
    ...
    TerminatesWith env m 34 initial
      ...
      (fun final values => values = [.i64 (roots 6), .i64 (roots 6)] ∧
        ...
        final.globals.globals[0]? = some (.i64 heapTop) ∧
        final.mem.pages = initial.mem.pages ∧ P final) := by
  let W := fun field current =>
    ReusedCellState current heapTop roots output index cell field allocs releases frees ∧
      P current ∧ current.mem.pages = initial.mem.pages
  let R := fun count current => ReleasingCellState current heapTop
  ...
  have hRun := writer_accepted_sequence layout env initial unused roots index cell W R hAccepted
    ⟨hState, hP, rfl⟩ (by
      ...
      apply (reused_cell_field_call layout env current (writerCallUnused unused roots field)
        heapTop allocs releases frees roots output index field cell hField hi hCurrent hSlots).mono
      ...
      apply releasing_cell_start current heapTop roots output index cell
        (allocs + 6) releases frees initial.mem.pages P
      · simpa only [show reusePool roots 6 = [] from rfl,
          show UInt64.ofNat 6 = 6 from rfl] using hCurrent.buffers
      · exact hCurrent.heap
      · exact hPages
      · exact hObserved) (by
      intro count hLo hHi current hCurrent
      exact releasing_cell_call layout env current heapTop
