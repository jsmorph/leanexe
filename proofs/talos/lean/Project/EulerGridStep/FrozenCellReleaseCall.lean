import Project.EulerGridStep.FrozenWriterCopies

namespace Project.EulerGridStep.Frozen.Execution
open Wasm

def cellKept (roots : Nat → UInt64) (output : Array UInt64) (index : Nat)
    (cell : Project.EulerCellStep.Model.CheckedCell) (count : Nat) : List LiveBuffer :=
  ⟨roots 6, cellPrefix output index cell 6⟩ :: cellLive roots output index cell count

/-- Releasing one intermediate preserves the completed result and all earlier live prefixes. -/
theorem cell_release_call {m : Wasm.Module} (layout : Layout m)
    ...
    TerminatesWith env m 40 initial [.i64 (roots count)]
      (fun final values => values = [] ∧
        BufferState final output.size (cellKept roots output index cell (count - 1))
          (roots count :: free) allocs (releases + 1) (frees + 1)) := by
  ...
  apply hCall.mono
  rintro final values ⟨hValues, _, hFinal⟩
  exact ⟨hValues, by simpa only [cellPrefix_size] using hFinal⟩

#print axioms cell_release_call
