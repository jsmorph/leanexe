...
def gridFinishFrame (ratio pointer : UInt64) (base cells index : Nat) (output : Array UInt64)
  ...
  { frame with locals := ((((((frame.locals.set 33 (.i64 root)).set 34 (.i64 root)).set
      35 (.i64 (UInt64.ofNat index))).set 36 (.i64 root)).set 37 (.i64 root)).set
      38 (.i64 root)).set 39 (.i64 root), values := [] }
...
theorem grid_finish_spec {m : Wasm.Module} (layout : Layout m)
    ...
    wp m (gridValidBody.drop 95 ++ rest) Q initial
