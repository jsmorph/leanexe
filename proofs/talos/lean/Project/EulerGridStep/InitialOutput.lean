...
def initialOutputFrame (frame : Locals) (heap : UInt64) (count : Nat) : Locals :=
  ...
  { allocated with
    locals := allocated.locals.set 37 (.i64 (UInt64.ofNat count))
    values := [] }
...
theorem initial_output_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hValues : frame.values = []) (hCount : frame.locals[35]? = some (.i64 (UInt64.ofNat count)))
    (hZero : frame.locals[38]? = some (.i64 0))
