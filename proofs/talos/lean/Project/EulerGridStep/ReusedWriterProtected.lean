import Project.EulerGridStep.ReusedWriterFramed
...
theorem writeCell_reused_protected {m : Wasm.Module} (layout : Layout m)
  ...
  apply writeCell_reused_framed layout env initial unused heapTop allocs releases frees roots output index
    cell hAccepted hi hState hSlots (fun current => UInt64Array.At current observedRoot observed ∧ protectedBuffer.At current output.size)
  ...
  · intro field hField current final hOld hResult
    ...
    · apply hResult.preserves_array observedRoot observed hOld.1
      simpa only [cellPrefix_size, reusedChoice, FieldAllocation.root]
