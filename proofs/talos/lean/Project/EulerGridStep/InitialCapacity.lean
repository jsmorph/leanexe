...
def initialCapacityFrame (frame : Locals) (length : UInt64) : Locals :=
  { frame with
    locals := frame.locals.set 46 (.i64 (FixedArrayCapacity.normalizedCapacity length 1)),
    values := [] }
...
theorem initial_capacity_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hValues : frame.values = []) (hLength : frame.locals[40]? = some (.i64 length))
  ...
  have hLengthGet : frame.locals[40] = .i64 length := by
    have h := hLength
