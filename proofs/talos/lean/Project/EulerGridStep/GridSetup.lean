...
def gridSetupFrame (frame : Locals) (root : UInt64) : Locals :=
  { frame with
    locals := (((((((frame.locals.set 5 (.i64 root)).set 6 (.i64 root)).set 7 (.i64 0)).set
      28 (.i64 root)).set 8 (.i64 root)).set 9 (.i64 root)).set 10 (.i64 0)).set 47 (.i64 0)
    values := [] }

/-- Exact sixteen-instruction handoff from zero fill to the outer loop. -/
theorem grid_setup_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hValues : frame.values = []) (hRoot : frame.locals[41]? = some (.i64 root))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp m rest Q initial (gridSetupFrame frame root) env) :
    wp m ((gridValidBody.drop 78).take 16 ++ rest) Q initial frame env := by
...
def gridInitialScratch (ratio : UInt64) (base length : Nat) : GridScratch :=
  ...
  { l2 := ratio
    ...
    l8 := root
    l42 := UInt64.ofNat count
    l43 := root
    l44 := UInt64.ofNat count
    l46 := UInt64.ofNat cells
    l48 := FixedArrayCapacity.normalizedCapacity (UInt64.ofNat count) 1
    l51 := heap + 48 + fieldRequest count
    l52 := (heap + 48 + fieldRequest count - 1) / 65536 + 1
    l53 := root }
