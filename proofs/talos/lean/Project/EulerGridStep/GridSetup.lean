...
def gridSetupFrame (frame : Locals) (root : UInt64) : Locals :=
  { frame with
    locals := ((((((frame.locals.set 5 (.i64 root)).set 6 (.i64 root)).set 7 (.i64 0)).set
      8 (.i64 root)).set 9 (.i64 root)).set 10 (.i64 0)).set 44 (.i64 root)
    values := [] }
...
theorem grid_setup_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hValues : frame.values = []) (hRoot : frame.locals[38]? = some (.i64 root))
...
def gridInitialScratch (ratio : UInt64) (base length : Nat) : GridScratch :=
  ...
  { l2 := ratio
    ...
    l40 := root
    l41 := UInt64.ofNat count
    l43 := UInt64.ofNat cells
    l45 := FixedArrayCapacity.normalizedCapacity (UInt64.ofNat count) 1
    l48 := heap + 48 + fieldRequest count
    l49 := (heap + 48 + fieldRequest count - 1) / 65536 + 1
    l50 := root }
