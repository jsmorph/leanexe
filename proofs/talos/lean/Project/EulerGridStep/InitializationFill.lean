...
theorem initial_fill_program_shape : (gridValidBody.drop 71).take 7 =
    [.localGet 43, .wrapI64, .localGet 42, .store64 0] ++ fillProgram 43 42 44 45 := rfl
...
theorem initial_fill_spec (m : Wasm.Module) (env : HostEnv Unit)
    ...
    (hCount : frame.get 42 = some (.i64 (UInt64.ofNat count)))
    (hValue : frame.get 45 = some (.i64 0))
    ...
    (hNext : ∀ final, InitializedArray initial final target count →
      wp m rest Q final (counterFrame frame 44 count hCounter) env) :
  ...
  change wp m (fillProgram 43 42 44 45 ++ rest) Q (writeLength initial target count)
    { params := frame.params, locals := frame.locals } env
  rw [copyFrame_ofParts frame hValues]
  apply fill_loop_spec 43 42 44 45 m env (writeLength initial target count) frame target 0
