...
theorem initial_fill_program_shape : (gridValidBody.drop 71).take 7 =
    [.localGet 38, .wrapI64, .localGet 37, .store64 0] ++ fillProgram 38 37 39 40 := rfl
...
theorem initial_fill_spec (m : Wasm.Module) (env : HostEnv Unit)
    ...
    (hCounter : frame.validIndex 39) (hValues : frame.values = [])
    (hTarget : frame.get 38 = some (.i64 target))
    (hCount : frame.get 37 = some (.i64 (UInt64.ofNat count)))
    (hValue : frame.get 40 = some (.i64 0))
    ...
    (hNext : ∀ final, InitializedArray initial final target count →
      wp m rest Q final (counterFrame frame 39 count hCounter) env) :
  ...
  change wp m (fillProgram 38 37 39 40 ++ rest) Q (writeLength initial target count)
    { params := frame.params, locals := frame.locals } env
  rw [copyFrame_ofParts frame hValues]
  apply fill_loop_spec 38 37 39 40 m env (writeLength initial target count) frame target 0
