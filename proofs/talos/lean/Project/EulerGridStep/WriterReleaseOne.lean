...
/-- The writer's guarded temporary release. -/
def writerReleaseOne (pointerLocal : Nat) : Wasm.Program :=
  [.localGet pointerLocal, .constI64 0, .eqI64, .eqz,
    .iff 0 1 [.localGet pointerLocal, .localGet 65, .eqI64, .eqz] [.const 0] [] [.i32],
...
theorem writer_release_one_spec (m : Wasm.Module) (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNonzero : pointer ≠ 0) {retained : UInt64}
    (hRetained : frame.get 65 = some (.i64 retained)) (hDistinct : pointer ≠ retained)
  ...
  simp only [wp_localGet_cons, copyFrame_get_withValues, hPointer, hRetained,
    wp_eqI64_cons, wp_eqz_cons, hDistinct, ite_false, ite_true, wp_nil,
    List.take, List.drop, List.nil_append]
  apply wp_iff_cons rfl
  rw [ite_eq_left (by decide : (1 : UInt32) ≠ 0)]
  ...
  rw [wp_nil]
  simpa only [List.take_zero, List.drop_zero, List.nil_append, List.append,
