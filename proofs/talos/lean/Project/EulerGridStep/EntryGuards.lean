...
def gridEntryFrame (ratio pointer : UInt64) : Locals :=
  { params := [.i64 ratio, .i64 pointer], locals := List.replicate 49 (.i64 0), values := [] }
...
def gridGuardFrame (ratio pointer : UInt64) (length : Nat) : Locals :=
  ...
  let ls := if Project.EulerConservative.Model.positiveBits ratio then
    let ls := ls.set 37 (.i64 pointer)
    if length = 0 then ls else ((ls.set 39 (.i64 pointer)).set 37 (.i64 (UInt64.ofNat length))).set 38 (.i64 3)
    else ls
