...
def initialDimensionsFrame (frame : Locals) (pointer : UInt64) (length : Nat) : Locals :=
  ...
  let ls := frame.locals.set 42 (.i64 pointer)
  let ls := ls.set 40 (.i64 (UInt64.ofNat length))
  let ls := ls.set 41 (.i64 3)
  let ls := ls.set 2 (.i64 cellsWord)
  let ls := ls.set 40 (.i64 1)
  let ls := ls.set 43 (.i64 6)
  let ls := ls.set 44 (.i64 cellsWord)
  let ls := ls.set 41 (.i64 (UInt64.ofNat (6 * cells)))
  let ls := ls.set 42 (.i64 countWord)
  let ls := ls.set 3 (.i64 countWord)
  let ls := ls.set 40 (.i64 countWord)
  { frame with
    locals := ls.set 43 (.i64 0)
    values := [] }
