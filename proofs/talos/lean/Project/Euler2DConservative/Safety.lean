...
/-- The sixteen rounded intermediates, in source evaluation order. -/
def intermediateWords (rho momentum transverse energy : UInt64) : List UInt64 :=
  ...
  [velocity, transport, transverseVelocity, transverseTransport, kineticSum, halfKinetic, internal, pressure, pressureOverDensity,
    radicand, soundSpeed, speed, momentumFlux, transverseFlux, enthalpy, energyFlux]
...
theorem accepted_inputGuard (rho momentum transverse energy : UInt64)
    (h : (Model.sideCheckedBits rho momentum transverse energy).status = 0) :
    Model.stateGuard rho momentum transverse energy = true := by
...
theorem accepted_intermediates_finite (rho momentum transverse energy : UInt64)
    (h : (Model.sideCheckedBits rho momentum transverse energy).status = 0) :
    ∀ word ∈ intermediateWords rho momentum transverse energy, CodeLib.IEEE64.Finite word := by
  unfold Model.sideCheckedBits at h
  split at h
  · dsimp only at h
    split at h
    · rename_i hfirst
      ...
      · rename_i hsecond
        ...
        · rename_i hthird
          ...
          obtain ⟨hfirst1, hks⟩ := Bool.and_eq_true_iff.mp hfirst2
          obtain ⟨hfirst0, htt⟩ := Bool.and_eq_true_iff.mp hfirst1
          obtain ⟨hfirst00, hut⟩ := Bool.and_eq_true_iff.mp hfirst0
          obtain ⟨hu, ht⟩ := Bool.and_eq_true_iff.mp hfirst00
          ...
          obtain ⟨hthird22, htFlux⟩ := Bool.and_eq_true_iff.mp hthird3
          obtain ⟨hthird2, hmFlux⟩ := Bool.and_eq_true_iff.mp hthird22
          obtain ⟨hc, hs⟩ := Bool.and_eq_true_iff.mp hthird2
          simp only [intermediateWords, List.forall_mem_cons]
          exact ⟨finite_of_guard hu, finite_of_guard ht, finite_of_guard hut,
            finite_of_guard htt, finite_of_guard hks, finite_of_guard hk,
            ...
            finite_of_guard hmFlux, finite_of_guard htFlux, finite_of_guard he, finite_of_guard heFlux,
...
theorem accepted_admissible (rho momentum transverse energy : UInt64)
    (h : (Model.sideCheckedBits rho momentum transverse energy).status = 0) :
    Guard.Admissible
      (Guard.decodedState rho momentum transverse energy) :=
  Guard.stateGuard_admissible rho momentum transverse energy (accepted_inputGuard rho momentum transverse energy h)
