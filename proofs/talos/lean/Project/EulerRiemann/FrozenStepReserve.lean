import Project.EulerRiemann.FrozenExecutionStep
import Project.EulerRiemann.FrozenHeapReserve
...
theorem step_pages_reserved (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
  ...
  apply (step_pages_exact env initial heap source grid n ratio pageLimit hn hIndexed hHeap hOwner hPages hPageLimit
...
theorem step_reserved (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 80 initial
      ...
      (fun final values => values = [.i64 result.2.root, .i64 result.2.root] ∧
        ...
        ∀ (saved : FreeNode) (savedGrid : Array Traversal.Cell), heap.Owns initial saved savedGrid →
          result.1.Owns final saved savedGrid ∧ regionsDisjoint saved.region result.2.region) := by
  exact step_pages_reserved env initial heap source grid n ratio spare limit 65536
