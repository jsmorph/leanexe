import Project.EulerRiemann.FrozenRetryFrame
import Project.EulerRiemann.FrozenAnnotationMatches
...
theorem retry_validity_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerRiemann.Frozen.«module» ((retryLoop.drop 7).take 8 ++ rest) Q store frame env := by
...
theorem retry_active_guard_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerRiemann.Frozen.«module» (retryLoop.take 7 ++ rest) Q store frame env := by
