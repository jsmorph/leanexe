import Project.EulerRiemann.FrozenAllocationBump
import Project.ClobMatchFuel.FrozenBookAllocFitState

namespace Project.EulerRiemann.Frozen.Execution
open Wasm Project.Runtime Project.Clob Project.ClobMatchFuel.Frozen.BookAllocFit
...
theorem fitUnlink_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (unlinkStore store choice)
      (allocationFrame params saved need choice.previous choice.node.root choice.node.capacity
        choice.next result) env) :
    wp Project.EulerRiemann.Frozen.«module» (fitUnlink ++ rest) Q store
      (allocationFrame params saved need choice.previous choice.node.root choice.node.capacity
...
theorem sweep_fit_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (fixedArrayAllocFitStore store choice 7)
      (allocationFrame params saved need choice.previous choice.node.root choice.node.capacity
        choice.next choice.node.root) env) :
    wp Project.EulerRiemann.Frozen.«module» (sweepFit ++ rest) Q store
      (allocationFrame params saved need choice.previous choice.node.root choice.node.capacity
        choice.next result) env := by
