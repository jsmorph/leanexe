import Project.EulerRiemann.FrozenAdvanceTotalLoop
...
theorem advance_pages_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
  ...
  have hInv : advanceTotalInvariant initial heap n time expected spare limit pageLimit initial entry :=
  ...
  change wp module func85 _ initial entry env
  ...
  apply advance_total_loop_spec env initial heap n time expected spare limit pageLimit initial entry
...
theorem advance_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    TerminatesWith env module 85 initial
      ...
      (fun final values => ∃ finalHeap result tracked,
        ...
        AdvanceCurrent initial heap n time expected.time final finalHeap result expected.grid tracked ∧
        finalHeap.Reserved (gridCapacity n) (spare + 1) limit) := by
  apply (advance_pages_exact env initial heap source grid n fuel time spare limit 65536
