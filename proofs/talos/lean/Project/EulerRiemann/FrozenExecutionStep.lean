...
import Project.EulerRiemann.FrozenSweepPageBound
...
theorem step_pages_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (hFirst : takeFirstFitFrom 0 (normalizedCapacity (UInt64.ofNat grid.size) 7) heap.nodes = none →
      ...
      bumpPages heap.top (normalizedCapacity (UInt64.ofNat grid.size) 7) ≤
        initial.memoryCap Project.EulerRiemann.Frozen.«module» 0)
    (hSecond : let need := normalizedCapacity (UInt64.ofNat grid.size) 7
      ...
      takeFirstFitFrom 0 need afterFirst.nodes = none →
        ...
        bumpPages afterFirst.top need ≤ initial.memoryCap Project.EulerRiemann.Frozen.«module» 0)
    ...
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 80 initial
  ...
  change wp Project.EulerRiemann.Frozen.«module» func80 _ initial
    (func80Def.toLocals [.i64 (UInt64.ofNat n), .i64 ratio, .i64 source.root, .i64 source.root]) env
  unfold func80
  step_peel
  refine wp_call_tw (sweep_pages_owned env initial heap source grid n false ratio pageLimit
  ...
  | true =>
    ...
    have hSecondCall := sweep_pages_owned env firstStore afterFirst first middle n true ratio pageLimit
...
theorem step_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
  ...
  have hNeed : (normalizedCapacity (UInt64.ofNat grid.size) 7).toNat = 8 + grid.size * 56 := by
    simpa only [hWord] using sweep_capacity_toNat (UInt64.ofNat grid.size) (by omega)
  exact step_pages_exact env initial heap source grid n ratio 65536 hn hIndexed hHeap hOwner
