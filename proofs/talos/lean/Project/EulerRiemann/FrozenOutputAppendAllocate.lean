import Project.EulerRiemann.FrozenOutputAppendOwned
import Project.EulerRiemann.FrozenArrayAllocationExecute
...
def outputAppendAllocationResult (params saved tail : List Wasm.Value)
    ...
    (hStart : params.length + saved.length = 47) : Locals :=
...
theorem output_append_allocate_spec (header : Bool) (env : HostEnv Unit)
    (store : Store Unit) (heap : Heap) (params saved tail : List Wasm.Value)
    (hParams : params.length = 5) (hSaved : saved.length = 42) (hTail : tail.length = 4)
    (hStart : params.length + saved.length = 47)
    ...
    (hUpper : ({ params := params, locals := saved, values := [] } : Locals).get 37 =
      some (.i64 upper.root))
    (hTotalCount : ({ params := params, locals := saved, values := [] } : Locals).get 40 =
      some (.i64 (UInt64.ofNat (left.size + right.size))))
    (hLeftCount : ({ params := params, locals := saved, values := [] } : Locals).get 41 =
      some (.i64 (UInt64.ofNat left.size)))
    (hRightCount : ({ params := params, locals := saved, values := [] } : Locals).get 42 =
    ...
    wp module (FixedArrayAllocate.program 47 1 ++ outputAppendDataProgram header ++ rest) Q store
      (FixedArraySearch.frame params saved tail need previous current capacity next result) env := by
  rw [List.append_assoc]
  apply heap_array_allocation_program_spec env store heap params saved tail 47 hStart
  ...
  have hCounter : frame.validIndex 44 := by simp [Locals.validIndex, hFrameParams, hFrameLocals]
  have hTarget : frame.get 52 = some (.i64 (allocatedRoot heap.top need heap.nodes)) := by
  ...
  apply output_append_owned_spec header env store heap frame source upper need left right
  ...
  · exact (FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 37 (by omega)).trans hUpper
  · exact hTarget
  · exact (FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 40 (by omega)).trans hTotalCount
  · exact (FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 41 (by omega)).trans hLeftCount
  · exact (FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 42 (by omega)).trans hRightCount
