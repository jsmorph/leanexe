import Project.EulerRiemann.FrozenAllocationSearchNone
import Project.ProofKit.FreeListMemory

namespace Project.EulerRiemann.Frozen.Execution
open Wasm Project.Runtime Project.ClobMatchFuel.Frozen.BookAllocFit Project.ProofKit.Memory
...
theorem search_fit_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (fixedArrayAllocFitStore initial choice 7)
      (allocationFrame params saved need choice.previous choice.node.root choice.node.capacity
        choice.next choice.node.root) env) :
    wp Project.EulerRiemann.Frozen.«module» ([.block 0 0 [.loop 0 0 sweepSearch]] ++ rest) Q initial
