...
theorem evaluate_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 69 initial
  ...
  change wp Project.EulerRiemann.Frozen.«module» func69 _ initial
    (func69Def.toLocals [.i64 ratio,
  ...
  subst final
  wp_run [func69Def, cellStepValues, List.set,
    List.getElem?_cons_zero, List.getElem?_cons_succ,
    reduceIte, Nat.reduceAdd, Nat.reduceLT, Nat.reduceSub]
  simp [evaluateValues, Numerics.evaluate, inputsValues, stateValues]

macro "update_cell_peel" : tactic => `(tactic|
  wp_run [func76Def, cellValues, inputsValues, stateValues, evaluateValues,
...
theorem updateCell_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 76 initial
  ...
  let out := Numerics.evaluate ratio input
  refine TerminatesWith.of_wp_entry_for (f := func76Def) rfl ?_ (by decide)
  change wp Project.EulerRiemann.Frozen.«module» func76 _ initial
    (func76Def.toLocals [.i64 (UInt64.ofNat n), .i64 (boolWord axis), .i64 ratio,
