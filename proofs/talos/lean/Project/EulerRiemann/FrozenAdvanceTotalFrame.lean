import Project.EulerRiemann.FrozenAdvanceGuard
...
theorem AdvanceFrameAt.returned {frame : Locals} {fuel : UInt64} {n : Nat}
    {time source tracker outputTime outputRoot : UInt64}
...
theorem AdvanceFrameAt.return_status {frame : Locals} {fuel : UInt64} {n : Nat}
    ...
    (h : AdvanceFrameAt frame fuel n time source tracker outputTime outputRoot done) (status : UInt64) :
...
theorem advance_returned_guard_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
  ...
  change some (advanceLoop.take 7) = some (FuelGuard.program 0 10) at hShape
  rw [Option.some.inj hShape]
  apply FuelGuard.program_spec 0 10 module env store frame fuel 1 h.values
...
def advanceReturnTail : Wasm.Program :=
  [.localSet 6, .localGet 2, .localSet 7, .localGet 3, .localSet 8,
    .localGet 4, .localSet 9, .constI64 1, .localSet 10]
...
theorem advance_trial_failure_shape :
    advanceTrialFailureBody = [.localGet 33] ++ advanceReturnTail := rfl

theorem advance_return_tail_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    (fuel : UInt64) (n : Nat) (time source status : UInt64)
    (hParams : frame.params = [.i64 fuel, .i64 (UInt64.ofNat n), .i64 time, .i64 source, .i64 source])
    (hLocals : frame.locals.length = 45) (hValues : frame.values = [.i64 status])
