import Project.EulerRiemann.FrozenExecutionSolve
...
theorem solve_initial_exact (env : HostEnv Unit) (n : Nat) (hn : 2 ≤ n ∧ n ≤ 800) :
    TerminatesWith env module 103 (module.initialStore (α := Unit)) [.i64 (UInt64.ofNat n)]
      (fun final values => ∃ (finalHeap : Heap) (result : FreeNode),
        values = [.i64 result.root] ∧ finalHeap.At final ∧
        finalHeap.OwnsWords final result (Control.solve n) ∧ final.mem.pages ≤ 8192) := by
  apply solve_exact env _ initialHeap n 536870912 8192 hn initial_heap_at initial_heap_below
