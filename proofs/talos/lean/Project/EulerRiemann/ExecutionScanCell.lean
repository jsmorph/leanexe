...
theorem scanCell_exact (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env Project.EulerRiemann.«module» 31 initial
  ...
  have yCall := side_exact env initial
    cell.state.density cell.state.my cell.state.mx cell.state.energy
  change TerminatesWith env Project.EulerRiemann.«module» 22 initial _
    (fun final values => final = initial ∧
  ...
  change wp Project.EulerRiemann.«module» func31 _ initial
    (func31Def.toLocals [.i64 acc.status, .i64 acc.alpha, .i64 (UInt64.ofNat cell.index),
      .i64 cell.state.density, .i64 cell.state.mx, .i64 cell.state.my,
      .i64 cell.state.energy, .i64 cell.pressure, .i64 cell.status]) env
  unfold func31
  wp_run [func31Def, List.set, List.getElem?_cons_zero, List.getElem?_cons_succ,
