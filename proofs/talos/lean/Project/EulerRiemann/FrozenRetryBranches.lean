import Project.EulerRiemann.FrozenRetryTrial
...
theorem retry_accept_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerRiemann.Frozen.«module» (retryAcceptBody ++ rest) Q store
...
theorem retry_reject_spec (env : HostEnv Unit) (store : Store Unit) (heap : Heap)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (heap.releaseStore store result)
      (retryRejectedFrame (retryTrialFrame frame n ratio source result.root false)
        fuel n time dt source (heap.frees + 1)) env) :
    wp Project.EulerRiemann.Frozen.«module» (retryRejectBody ++ rest) Q store
