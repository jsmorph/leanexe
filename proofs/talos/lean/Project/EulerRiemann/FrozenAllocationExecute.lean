import Project.EulerRiemann.FrozenAllocationSearchFit

namespace Project.EulerRiemann.Frozen.Execution
open Wasm Project.Runtime Project.ClobMatchFuel.Frozen.BookAllocFit
...
theorem allocationInit_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    wp Project.EulerRiemann.Frozen.«module» (allocationInit ++ rest) Q store
...
theorem allocationCount_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (countedStore store count) frame env) :
    wp Project.EulerRiemann.Frozen.«module» (allocationCount ++ rest) Q store frame env := by
  obtain ⟨hGlobalLength, hGlobalRead⟩ := List.getElem_of_getElem? hGlobal
...
theorem allocationCore_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hBump : takeFirstFitFrom 0 need nodes = none →
      ...
      bumpPages base need ≤ store.memoryCap Project.EulerRiemann.Frozen.«module» 0)
    (hPages : store.mem.pages ≤ 65536) (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous current capacity next : UInt64,
      wp Project.EulerRiemann.Frozen.«module» rest Q (allocatedStore store base need nodes)
        (allocationFrame params saved need previous current capacity next
          (allocatedRoot base need nodes)) env) :
    wp Project.EulerRiemann.Frozen.«module» (allocationCore ++ rest) Q store
...
theorem allocation_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hPages : store.mem.pages ≤ 65536) (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous current capacity next : UInt64,
      wp Project.EulerRiemann.Frozen.«module» rest Q
        (countedStore (allocatedStore store base need nodes) count)
        (allocationFrame params saved need previous current capacity next
          (allocatedRoot base need nodes)) env) :
    wp Project.EulerRiemann.Frozen.«module» ((func77.drop 24).take 15 ++ rest) Q store
      (allocationFrame params saved need previous current capacity next result) env := by
