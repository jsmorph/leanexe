import Project.EulerRiemann.FrozenScanLoopShape
...
theorem scan_loop_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hNext : ∀ scratch : ScanScratch,
      wp Project.EulerRiemann.Frozen.«module» rest Q initial
        (scanFrame owner pointer grid.size grid.size (scanPrefix grid grid.size) scratch) env) :
    wp Project.EulerRiemann.Frozen.«module» ([.block 0 0 [.loop 0 0 scanLoop]] ++ rest) Q initial
