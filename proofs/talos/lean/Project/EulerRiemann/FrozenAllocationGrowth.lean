...
theorem sweep_growth_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hCap : required ≤ store.memoryCap Project.EulerRiemann.Frozen.«module» 0)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (ensured store required) frame env) :
    wp Project.EulerRiemann.Frozen.«module» ((sweepBump.drop 17).take 5 ++ rest) Q store frame env := by
