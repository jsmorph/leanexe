import Project.EulerRiemann.FrozenRetryLoopShape
...
theorem retry_trial_pages_spec (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    ...
    (n : Nat) (time dt : UInt64) (spare limit pageLimit : Nat)
    ...
    (hCap : limit ≤ initial.memoryCap Project.EulerRiemann.Frozen.«module» 0 * 65536)
    ...
    (hNext : let ratio := IEEE64.div dt (Time.spacing n)
      ...
      ∀ final, result.1.At final → result.1.Owns final result.2 (Traversal.step n ratio grid) →
        ...
        final.memoryCap Project.EulerRiemann.Frozen.«module» 0 = initial.memoryCap Project.EulerRiemann.Frozen.«module» 0 →
        ...
        wp Project.EulerRiemann.Frozen.«module» rest Q final
          (retryTrialFrame frame n ratio source.root result.2.root
            (Traversal.accepted (Traversal.step n ratio grid))) env) :
    wp Project.EulerRiemann.Frozen.«module» (retryTrial.take 35 ++ rest) Q initial frame env := by
  ...
  subst current
  retry_trial_peel
  refine wp_call_tw (step_pages_reserved env initial heap source grid n
    (IEEE64.div dt (Time.spacing n)) spare limit pageLimit hn hIndexed hHeap hOwner hPages hPageLimit
      ...
      ∀ final, result.1.At final → result.1.Owns final result.2 (Traversal.step n ratio grid) →
        ...
        wp Project.EulerRiemann.Frozen.«module» rest Q final
          (retryTrialFrame frame n ratio source.root result.2.root
            (Traversal.accepted (Traversal.step n ratio grid))) env) :
    wp Project.EulerRiemann.Frozen.«module» (retryTrial.take 35 ++ rest) Q initial frame env := by
  exact retry_trial_pages_spec env initial heap frame fuel source grid n time dt spare limit 65536
    hParams hLocals hValues hn hIndexed hHeap hOwner hPages (Nat.le_refl _) hReserve hLimit hCap
    hLimit.le Q rest hNext
