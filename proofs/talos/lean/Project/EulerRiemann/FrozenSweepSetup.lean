...
import Project.EulerRiemann.FrozenSweepLoopShape
...
theorem sweep_entry_spec (env : HostEnv Unit) (store : Store Unit) (n : Nat) (axis : Bool)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q store
      (allocationFrame (sweepParameters n axis ratio owner source) (sweepSaved source grid.size)
        0 0 0 0 0 0) env) :
    wp Project.EulerRiemann.Frozen.«module» (func77.take 6 ++ rest) Q store
...
theorem sweep_install_spec (env : HostEnv Unit) (store : Store Unit) (n : Nat) (axis : Bool)
    ...
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (writeLength store target (UInt64.ofNat count))
    ...
    wp Project.EulerRiemann.Frozen.«module» ((func77.drop 39).take 8 ++ rest) Q store
