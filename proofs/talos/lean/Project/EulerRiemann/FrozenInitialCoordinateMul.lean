...
def quadrupleProgram : Wasm.Program :=
  [.constI64 4, .localSet 43, .localGet 0, .localSet 44] ++
    Project.ProofKit.CheckedNatMul.program 43 44 ++ [.localSet 41]

def quadrupleFrame (frame : Locals) (n : Nat) (tail : List Value) : Locals :=
  { frame with
    locals := ((frame.locals.set 41 (.i64 4)).set 42 (.i64 (UInt64.ofNat n))).set 39
      (.i64 (UInt64.ofNat (4 * n)))
    values := tail }

theorem quadruple_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q store
      (quadrupleFrame frame n tail) env) :
    wp Project.EulerRiemann.Frozen.«module» (quadrupleProgram ++ rest) Q store frame env := by
  ...
  have hFit : (4 : UInt64).toNat * (UInt64.ofNat n).toNat < UInt64.size := by
    ...
    omega
  have hMul : wp Project.EulerRiemann.Frozen.«module»
    ...
    apply Project.ProofKit.CheckedNatMul.program_spec 43 44 _ env store staged 4
      (UInt64.ofNat n) tail rfl
    · simp [staged, Locals.get, hParams, hLocals]
    · simp [staged, Locals.get, hParams, hLocals]
    · exact hFit
    · simpa [wp_simp, staged, quadrupleFrame, hParams, hLocals,
        UInt64.ofNat_mul] using hNext
  unfold quadrupleProgram
  rw [List.append_assoc]
...
def initialQuadrupleFrame (frame : Locals) (n : Nat) (tail : List Value) : Locals :=
  quadrupleFrame frame n (.i64 5 :: tail)
...
theorem initial_quadruple_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    wp Project.EulerRiemann.Frozen.«module» (func94.take 10 ++ rest) Q store frame env := by
  ...
  apply quadruple_spec env store { frame with values := .i64 5 :: tail }
    n (.i64 5 :: tail) hn hParams hLocals rfl
  · exact hN
  · exact hNext

#print axioms initial_mul_region
#print axioms quadruple_spec
