import Project.EulerRiemann.FrozenExecutionRun
import Project.EulerRiemann.FrozenExecutionOutput
import Project.EulerRiemann.FrozenControlTime
...
macro "solve_peel" : tactic => `(tactic|
  wp_run [func103, func103Def, List.cons_append, List.nil_append, List.append_nil,
...
theorem solve_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
    (n limit pageLimit : Nat) (hn : 2 ≤ n ∧ n ≤ 800)
    ...
    TerminatesWith env module 103 initial [.i64 (UInt64.ofNat n)]
      (fun final values => ∃ (finalHeap : Heap) (result : FreeNode),
        values = [.i64 result.root] ∧ finalHeap.At final ∧
        finalHeap.OwnsWords final result (Control.solve n) ∧ final.mem.pages ≤ pageLimit) := by
  ...
  have hRun := run_exact env initial heap n 0 runLimit pageLimit hn hHeap hBelow (by rfl)
  ...
  have hSize : (Control.run n).grid.size = n * n := (Control.run_indexed n hn).1
  refine TerminatesWith.of_wp_entry_for (f := func103Def) rfl ?_ (by decide)
  change wp module func103 _ initial (func103Def.toLocals [.i64 (UInt64.ofNat n)]) env
  solve_peel
  ...
  have hTop : currentHeap.top.toNat ≤ runLimit := by
    ...
    omega
  have hBudget : OutputBudget current currentHeap (outputBytes (Control.run n).grid.size) pageLimit := by
    rw [hSize]
    refine ⟨by omega, by omega, hCurrentPages, hPageLimit, ?_⟩
    rw [hCurrent.cap]
    exact hCap
  have hOutput := output_exact env current currentHeap source (Control.run n).grid n pageLimit
    (Control.run n).time (Control.run n).status hCurrent.heapState hOwner
