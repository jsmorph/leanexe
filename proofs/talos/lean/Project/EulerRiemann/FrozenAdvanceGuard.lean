import Project.EulerRiemann.FrozenAdvanceFrame
...
theorem advance_active_guard_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q store frame env) :
    wp Project.EulerRiemann.Frozen.«module» (advanceLoop.take 7 ++ rest) Q store frame env := by
...
theorem advance_time_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q store (advanceTimeFrame frame) env) :
    wp Project.EulerRiemann.Frozen.«module» ((advanceLoop.drop 7).take 2 ++ rest) Q store frame env := by
...
theorem advance_finish_spec (env : HostEnv Unit) (store : Store Unit) (frame : Locals)
    ...
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q store (advanceFinishedFrame frame time source) env) :
    wp Project.EulerRiemann.Frozen.«module» (advanceFinishBody ++ rest) Q store frame env := by
