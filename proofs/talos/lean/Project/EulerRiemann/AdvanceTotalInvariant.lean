...
theorem advanceTotalInvariant.values {initial : Store Unit} {initialHeap : Heap} {n : Nat}
    {startTime : UInt64} {expected : Control.Result} {spare limit : Nat} {store : Store Unit} {frame : Locals}
    (h : advanceTotalInvariant initial initialHeap n startTime expected spare limit store frame) :
    frame.values = [] := by
