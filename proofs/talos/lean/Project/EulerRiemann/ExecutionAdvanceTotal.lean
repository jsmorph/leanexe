...
theorem advance_exact (env : HostEnv Unit) (initial : Store Unit) (heap : Heap)
  ...
  have hInv : advanceTotalInvariant initial heap n time expected spare limit initial entry :=
  ...
  apply advance_total_loop_spec env initial heap n time expected spare limit initial entry
    hn hLimit hCap hInv
