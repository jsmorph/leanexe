...
theorem advance_total_iteration_spec (env : HostEnv Unit) (initial : Store Unit) (initialHeap : Heap)
    ...
    wp module advanceLoop
      (advanceTotalIterationPost initial initialHeap n startTime expected spare limit (advanceMeasure frame))
      store frame env := by
  ...
  · obtain ⟨fuel, time, source, grid, tracked, hFrame, hSame, hEnough, hCurrent, hReserve⟩ := hActive
    ...
    · subst time
      ...
      have hReserved : heap.Reserved (gridCapacity n) (spare + 1) limit := by
        apply hReserve.mono
        cases tracked <;> simp
      change advanceTotalInvariant _ _ _ _ _ _ _ _ _ ∧ _
      refine ⟨⟨heap, hStore, Or.inr ⟨source, tracked, ?_, ?_, hReserved⟩⟩, ?_⟩
    ...
    · dsimp only [frame]
      advance_total_peel
      refine advance_total_work_spec env initial initialHeap n startTime expected spare limit
        store heap frame fuel time source grid tracked hn hLimit hCap hStore hFrame hSame hEnough
  ...
  · obtain ⟨source, tracked, hFrame, hCurrent, hReserved⟩ := hDone
    ...
    exact ⟨heap, hStore, source, tracked, hFrame, hCurrent, hReserved⟩
