...
def fluxValues (rhoL momentumL transverseL energyL rhoR momentumR transverseR energyR : UInt64) : List Wasm.Value :=
  let out := Numerics.fluxCheckedBits rhoL momentumL transverseL energyL rhoR momentumR transverseR energyR
  [.i64 out.alpha, .i64 out.energy, .i64 out.transverse, .i64 out.momentum, .i64 out.mass, .i64 out.status]

macro "riemann_flux_guard" checkCond:term "reject" rejection:term : tactic =>
  `(tactic|
    (by_cases hcheck : $checkCond
     case neg =>
       ...
       subst values
       dynamic_peel
       simp_all +zetaDelta [fluxValues, Numerics.fluxCheckedBits, Project.Euler2DDynamicFlux.Model.rejectedFlux, -UInt64.not_le]
     all_goals dynamic_peel))

theorem rejectedFlux_exact (env : HostEnv Unit) (initial : Store Unit) :
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 53 initial []
      (fun final values => final = initial ∧
        values = [.i64 0, .i64 0, .i64 0, .i64 0, .i64 0, .i64 1]) := by
  refine TerminatesWith.of_wp_entry_for (f := func53Def) rfl ?_ (by decide)
  change wp Project.EulerRiemann.Frozen.«module» func53 _ initial (func53Def.toLocals []) env
  unfold func53
  wp_run
...
theorem flux_exact
    ...
    TerminatesWith env Project.EulerRiemann.Frozen.«module» 54 initial [.i64 energyR, .i64 transverseR, .i64 momentumR, .i64 rhoR, .i64 energyL, .i64 transverseL, .i64 momentumL, .i64 rhoL]
  ...
  let left := Numerics.sideCheckedBits rhoL momentumL transverseL energyL
  have leftCall := side_exact env initial rhoL momentumL transverseL energyL
  change TerminatesWith env Project.EulerRiemann.Frozen.«module» 22 initial [.i64 energyL, .i64 transverseL, .i64 momentumL, .i64 rhoL]
  ...
  let right := Numerics.sideCheckedBits rhoR momentumR transverseR energyR
  have rightCall := side_exact env initial rhoR momentumR transverseR energyR
  change TerminatesWith env Project.EulerRiemann.Frozen.«module» 22 initial [.i64 energyR, .i64 transverseR, .i64 momentumR, .i64 rhoR]
  ...
  refine TerminatesWith.of_wp_entry_for (f := func54Def)
    rfl ?_ (by decide)
  change wp Project.EulerRiemann.Frozen.«module» func54 _ initial (func54Def.toLocals [.i64 rhoL, .i64 momentumL, .i64 transverseL, .i64 energyL, .i64 rhoR, .i64 momentumR, .i64 transverseR, .i64 energyR]) env
  unfold func54
  ...
  subst st
  subst values
  riemann_flux_guard (left.status = 0) reject (rejectedFlux_exact env initial)
  refine wp_call_tw rightCall ?_
  rintro st values ⟨hst, hvalues⟩
  subst st
  subst values
  riemann_flux_guard (right.status = 0) reject (rejectedFlux_exact env initial)
  ...
  all_goals
    ...
    change TerminatesWith env Project.EulerRiemann.Frozen.«module» 46 initial [.i64 rhoR, .i64 rhoL, .i64 right.massFlux, .i64 left.massFlux, .i64 alpha]
    ...
    change TerminatesWith env Project.EulerRiemann.Frozen.«module» 46 initial [.i64 momentumR, .i64 momentumL, .i64 right.momentumFlux, .i64 left.momentumFlux, .i64 alpha]
    ...
    change TerminatesWith env Project.EulerRiemann.Frozen.«module» 46 initial [.i64 transverseR, .i64 transverseL, .i64 right.transverseFlux, .i64 left.transverseFlux, .i64 alpha]
    ...
    change TerminatesWith env Project.EulerRiemann.Frozen.«module» 46 initial [.i64 energyR, .i64 energyL, .i64 right.energyFlux, .i64 left.energyFlux, .i64 alpha]
    ...
    subst values
    dynamic_peel
    riemann_flux_guard (mass.status = 0) reject (rejectedFlux_exact env initial)
    riemann_flux_guard (momentum.status = 0) reject (rejectedFlux_exact env initial)
    riemann_flux_guard (transverse.status = 0) reject (rejectedFlux_exact env initial)
    riemann_flux_guard (energy.status = 0) reject (rejectedFlux_exact env initial)
    simp_all +zetaDelta [fluxValues, Numerics.fluxCheckedBits, -UInt64.not_le]
