import Project.EulerRiemann.FrozenAllocationHeader
...
theorem bumpPrefix_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    wp Project.EulerRiemann.Frozen.«module» (bumpPrefix ++ rest) Q store
...
theorem bumpInstall_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hGlobal : store.globals.globals[0]? = some (.i64 base))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q
    ...
    wp Project.EulerRiemann.Frozen.«module» (bumpInstall ++ rest) Q store
...
theorem sweep_bump_spec (env : HostEnv Unit) (store : Store Unit)
    ...
    (hCap : bumpPages base need ≤ store.memoryCap Project.EulerRiemann.Frozen.«module» 0)
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : wp Project.EulerRiemann.Frozen.«module» rest Q (bumpStore store base need)
      (allocationFrame params saved need previous current (base + 48 + need)
        ((base + 48 + need - 1) / 65536 + 1) (base + 48)) env) :
    wp Project.EulerRiemann.Frozen.«module» (sweepBump ++ rest) Q store
      (allocationFrame params saved need previous current capacity next result) env := by
  rw [sweep_bump_parts]
