...
def inferenceHiddenPrefix : Wasm.Program :=
  [.constI64 0, .localSet 5, .localGet 0, .localSet 6,
   ...
   .localGet 9, .localGet 10, .localGet 11, .call 71,
...
theorem inference_prefix_shape : func75.take 52 =
...
theorem inference_prefix_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    wp module (func75.take 52 ++ rest) Q initial
      (func75Def.toLocals (inferenceParams pointer t0 t1 t2 t3)) env := by
  rw [inference_prefix_shape]
  simp only [inferenceHiddenPrefix, List.cons_append, List.nil_append]
  wp_fixed_frame [func75Def, inferenceParams, List.map, ValueType.zero, List.replicate]
