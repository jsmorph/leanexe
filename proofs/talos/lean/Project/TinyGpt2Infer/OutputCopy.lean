...
theorem output_copy_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hCounter : frame.validIndex 53) (hValues : frame.values = [])
    (hSource : frame.get 48 = some (.i64 source))
    (hTarget : frame.get 52 = some (.i64 target))
    (hCount : frame.get 50 = some (.i64 (UInt64.ofNat input.size)))
    (hLength : frame.get 49 = some (.i64 (UInt64.ofNat input.size)))
    ...
    (hNext : ∀ final : Store Unit,
      ...
      wp module rest Q final (FixedArrayCopy.counterFrame frame 53 input.size hCounter) env) :
    wp module ((outputBody.drop 88).take 15 ++ rest) Q initial frame env := by
  rw [output_copy_shape]
  exact UInt64Array.pushCopy_spec 48 52 50 49 53 54 module env initial frame source target input value
