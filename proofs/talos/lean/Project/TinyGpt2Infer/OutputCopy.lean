...
theorem output_copy_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hSource : frame.get 51 = some (.i64 source))
    (hTarget : frame.get 55 = some (.i64 target))
    (hCount : frame.get 53 = some (.i64 (UInt64.ofNat input.size)))
    (hLength : frame.get 52 = some (.i64 (UInt64.ofNat input.size)))
    ...
    (hNext : ∀ final : Store Unit,
      ...
      wp module rest Q final (FixedArrayCopy.counterFrame frame 56 input.size hCounter) env) :
    wp module ((outputBody.drop 88).take 15 ++ rest) Q initial frame env := by
  rw [output_copy_shape]
  exact UInt64Array.pushCopy_spec 51 55 53 52 56 57 module env initial frame source target input value
