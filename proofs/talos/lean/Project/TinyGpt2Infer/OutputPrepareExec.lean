...
theorem output_prepare_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hLength : (FixedArraySearch.frame params saved tail (UInt64.ofNat (capacity (count + 1)))
      previous current capacity' next result).get 56 = some (.i64 (UInt64.ofNat (count + 1))))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous' : UInt64, wp module rest Q
      (OutputMemory.prepare initial start (count + 1) allocations)
      (resultFrame (outputAllocationFrame params saved tail start count previous') 57
  ...
  have hRoot : allocatedFrame.get 67 = some (.i64 (node start (count + 1)).root) := by
  ...
  have hLength' : allocatedFrame.get 56 = some (.i64 (UInt64.ofNat (count + 1))) := by
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 56 (by omega)] at hLength
    change (FixedArraySearch.frame _ _ _ _ _ _ _ _ _).get 56 = _
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 56 (by omega)]
    exact hLength
  have hLower : allocatedFrame.params.length ≤ 57 := by simpa only [allocatedFrame, outputAllocationFrame,
    FixedArraySearch.frame, hParams] using (show 5 ≤ 57 by decide)
  have hValid : allocatedFrame.validIndex 57 := by
    simp only [allocatedFrame, outputAllocationFrame, FixedArraySearch.frame, Locals.validIndex,
      List.length_append, List.length_cons, List.length_nil]
    omega
  change wp module ((ScalarTransition.Stmt.assign 57 (.get 67)).program 72 ++
    FixedArrayResult.lengthStoreLocalProgram 57 56 ++ rest) Q _ allocatedFrame env
  apply ScalarTransition.Expr.assign_frame_spec (.get 67) 72 57 allocatedFrame
  ...
  apply FixedArrayResult.lengthStoreLocal_spec module env _ _ (node start (count + 1)).root
    (UInt64.ofNat (count + 1)) 57 56
    (resultFrame_get_result allocatedFrame 57 _ hLower hValid)
    ((resultFrame_get_ne allocatedFrame 57 56 _ hLower (by decide)).trans hLength')
