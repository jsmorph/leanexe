...
theorem output_prepare_spec (env : HostEnv Unit) (initial : Store Unit)
    ...
    (hLength : (FixedArraySearch.frame params saved tail (UInt64.ofNat (capacity (count + 1)))
      previous current capacity' next result).get 54 = some (.i64 (UInt64.ofNat (count + 1))))
    (Q : Assertion Unit) (rest : Wasm.Program)
    (hNext : ∀ previous' : UInt64, wp module rest Q
      (OutputMemory.prepare initial start (count + 1) allocations)
      (resultFrame (outputAllocationFrame params saved tail start count previous') 55
  ...
  have hRoot : allocatedFrame.get 65 = some (.i64 (node start (count + 1)).root) := by
  ...
  have hLength' : allocatedFrame.get 54 = some (.i64 (UInt64.ofNat (count + 1))) := by
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 54 (by omega)] at hLength
    change (FixedArraySearch.frame _ _ _ _ _ _ _ _ _).get 54 = _
    rw [FixedArraySearch.frame_get_before _ _ _ _ _ _ _ _ _ 54 (by omega)]
    exact hLength
  have hLower : allocatedFrame.params.length ≤ 55 := by simpa only [allocatedFrame, outputAllocationFrame,
    FixedArraySearch.frame, hParams] using (show 5 ≤ 55 by decide)
  have hValid : allocatedFrame.validIndex 55 := by
    simp only [allocatedFrame, outputAllocationFrame, FixedArraySearch.frame, Locals.validIndex,
      List.length_append, List.length_cons, List.length_nil]
    omega
  change wp module ((ScalarTransition.Stmt.assign 55 (.get 65)).program 70 ++
    FixedArrayResult.lengthStoreLocalProgram 55 54 ++ rest) Q _ allocatedFrame env
  apply ScalarTransition.Expr.assign_frame_spec (.get 65) 70 55 allocatedFrame
  ...
  apply FixedArrayResult.lengthStoreLocal_spec module env _ _ (node start (count + 1)).root
    (UInt64.ofNat (count + 1)) 55 54
    (resultFrame_get_result allocatedFrame 55 _ hLower hValid)
    ((resultFrame_get_ne allocatedFrame 55 54 _ hLower (by decide)).trans hLength')
