import Project.TinyGpt2Infer.OutputGuard
...
structure OutputProgress (initial : Store Unit) (pointer empty : UInt64)
  ...
  locals : OutputLoopLocals pointer empty x (node start count).root count frame
...
theorem output_loop_spec (env : HostEnv Unit) (initial : Store Unit) (frame : Locals)
    ...
    (hNext : ∀ (final : Store Unit) (finalFrame : Locals),
      OutputProgress initial pointer empty weights x start 256 final finalFrame →
      wp module rest Q final finalFrame env) :
    wp module ([.block 0 0 [.loop 0 0 outputBody]] ++ rest) Q initial frame env := by
  let progress := OutputProgress initial pointer empty weights x start
  ...
  · intro current currentFrame hInv
    ...
    apply output_guard_spec env current currentFrame pointer empty (node start index).root x index
    ...
    · intro hContinue
      ...
      have hCurrentCap : current.memoryCap module 0 = initial.memoryCap module 0 := by
        rw [hProgress.store]
        rfl
      apply output_iteration_spec env current currentFrame pointer empty weights x start index
