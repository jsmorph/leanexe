import Project.ClobPostOnly.FrozenFindBest

/-!
# The internal `findBest` wrapper

Function 13 derives the loop fuel from the represented order-array length.
It passes the source taker and initial empty result to function 12.  Its two
results use the same option ABI as the standalone `findBest` export.
-/

namespace Project.ClobPostOnly.Frozen.FindBestWrapper

open Wasm Project.Common Project.Clob Project.ClobPostOnly
  Project.ClobFindBest.Frozen.Model Project.ClobPostOnly.Frozen.FindBest

set_option maxHeartbeats 64000000

/-- The internal wrapper supplies the source fuel and returns its option ABI. -/
theorem func13_spec (env : HostEnv Unit) (st : Store Unit) (ptr : UInt64)
    (os : List OrderL) (taker : OrderL)
    (hlen : os.length < 4294967296) (hInput : OrdersAt st ptr os) :
    TerminatesWith (m := «module») (id := 13) (initial := st) (env := env)
  ...
  have hnowrap : ¬UInt64.ofNat os.length + 1 < UInt64.ofNat os.length := by
    ...
    omega
  apply TerminatesWith.of_wp_entry_for (f := func13Def)
  · simp [«module»]
  · change wp «module» func13 _ st
    ...
    norm_num
    rw [hadd]
    refine wp_call_tw (func12_spec env st ptr os taker hlen hInput) ?_
    rintro st' vs ⟨rfl, rfl⟩
    wp_run
    simp [func13Def, optionVals]

end Project.ClobPostOnly.Frozen.FindBestWrapper
