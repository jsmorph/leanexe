...
def appendTradeStoreFrame (ptr g0 : UInt64) (order : OrderL)
  ...
  { params := [.i64 ptr, .i64 order.oid, .i64 order.otrader,
    ...
    locals := [.i64 0, .i64 ptr, .i64 order.oid, .i64 order.otrader, .i64 order.oside, .i64 order.oprice,
      ...
      order.oid, .i64 8, .i64 0, .i64 0, .i64 ((g0 + 48 + orderArrayBytesU (n + 1)) + 48 +
      8), .i64 (((g0 + 48 + orderArrayBytesU (n + 1)) + 48 + 8 - 1) / 65536 + 1), .i64 0,
      .i64 (orderArrayBytesU (n + 1)), .i64 0, .i64 0, .i64 (g0 + 48 + orderArrayBytesU (n
...
theorem appendTradeStoreProg_spec (env : HostEnv Unit) (st0 st6 : Store Unit)
  ...
  · rw [← htradePtr]
    ...
    simpa only [FreshTradeArrayAt, FreshFixedArrayAt, emptyFixedArrayMem,
      fixedArrayMem, fixedArrayHeaderMem,
