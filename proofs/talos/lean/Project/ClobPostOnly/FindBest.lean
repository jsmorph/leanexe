...
iteration to copy the result and set the done flag.  This artifact uses the
same invariant at function 12 with eligibility calls to function 10.
-/

namespace Project.ClobPostOnly.FindBest

open Wasm Project.Common Project.Clob Project.ClobPostOnly
  Project.ClobFindBest.Model Project.ClobPostOnly.SearchHelpers

set_option maxHeartbeats 64000000
set_option maxRecDepth 100000

macro "po_read_candidate_tail " hHead:ident hHeadB:ident hlt:ident
...
private theorem bestPrefixL_some_lt (os : List OrderL) (taker : OrderL)
...
private def fbFrame (fuel ptr : UInt64) (taker : OrderL) (k : Nat)
...
private def fbInv (st0 : Store Unit) (ptr : UInt64) (os : List OrderL)
  ...
  fun st s =>
    ...
    ∃ fuel rTag rPayload done
      ...
      s = fbFrame fuel ptr taker k (bestPrefixL os taker k)
...
private def fbMeasure (_ : Store Unit) (s : Locals) : Nat :=
  match s.params, s.locals with
  | .i64 fuel :: _, _ :: _ :: .i64 done :: _ =>
      2 * fuel.toNat + (if done = 0 then 1 else 0)
  | _, _ => 0

private theorem fbInv_some_step (st : Store Unit) (ptr : UInt64)
    ...
    fbInv st ptr os taker st
      (fbFrame (UInt64.ofNat (os.length + 1 - (k + 1))) ptr taker (k + 1)
...
/-- The generated fuel loop returns the source search result and preserves the
store. -/
    ...
    · rintro st2 s ⟨rfl, k, hk, fuel, rTag, rPayload, done,
      ...
      · simp [fbFrame]
        ...
        · have hklt : k < os.length := Nat.lt_of_le_of_ne hk hkend
          ...
          | none =>
              ...
              refine wp_call_tw (func10_spec env st2 taker maker) ?_
              ...
              · simp only [boolWord, if_pos helig]
                ...
                norm_num
                refine ⟨hb5, ?_⟩
                refine wp_call_tw (func10_spec env st3 taker maker) ?_
                ...
                · rw [hfuel_next, hkadd]
                  ...
                  refine ⟨rfl, k + 1, by omega,
                    ...
                    Or.inl ⟨rfl, rfl⟩⟩
                  simp only [fbFrame, hnext, optionTag, optionPayload, maker]
                · simp only [fbMeasure]
          ...
          | some j =>
              ...
              refine wp_call_tw (func10_spec env st2 taker candidate) ?_
              rintro st3 vs ⟨rfl, rfl⟩
              rw [hcandidate]
              by_cases helig : eligibleL taker os[k]!
              · simp only [boolWord, if_pos helig]
                ...
                · refine wp_iff_cons rfl ?_
                  ...
                  · refine wp_iff_cons rfl ?_
                    ...
                    rw [if_neg (by simp [optionTag])]
                    norm_num
                    po_read_candidate_tail hHead hHeadB hlt hb1 hb2 hb3 hb4 hb5
                    refine wp_call_tw (func10_spec env st3 taker candidate) ?_
                    rintro st4 vs ⟨rfl, rfl⟩
                    rw [hcandidate]
                    simp only [boolWord, if_pos helig]
                    norm_num
                    ...
                    constructor
                    · rw [hfuel_next, hkadd]
                      simpa only [fbFrame, candidate, optionTag, optionPayload] using
                        (fbInv_some_step st4 ptr os taker k k j candidate
                          (by omega) hnext)
                    · simp only [fbMeasure]
                      rw [hfuel_next, toNat_ofNat_lt, toNat_ofNat_lt]
                      · omega
                  ...
                  · refine wp_iff_cons rfl ?_
                    ...
                    · rw [hfuel_next, hkadd]
                      simpa only [fbFrame, candidate, optionTag, optionPayload] using
                        (fbInv_some_step st4 ptr os taker k j j candidate
                          (by omega) hnext)
                    · simp only [fbMeasure]
                      rw [hfuel_next, toNat_ofNat_lt, toNat_ofNat_lt]
                      · omega
                ...
                · refine wp_iff_cons rfl ?_
                  ...
                  · refine wp_iff_cons rfl ?_
                    ...
                    · rw [hfuel_next, hkadd]
                      simpa only [fbFrame, candidate, optionTag, optionPayload] using
                        (fbInv_some_step st4 ptr os taker k k k candidate
                          (by omega) hnext)
                    · simp only [fbMeasure]
                      rw [hfuel_next, toNat_ofNat_lt, toNat_ofNat_lt]
                      · omega
                  ...
                  · refine wp_iff_cons rfl ?_
                    ...
                    · rw [hfuel_next, hkadd]
                      simpa only [fbFrame, candidate, optionTag, optionPayload] using
                        (fbInv_some_step st4 ptr os taker k j k candidate
                          (by omega) hnext)
                    · simp only [fbMeasure]
                      rw [hfuel_next, toNat_ofNat_lt, toNat_ofNat_lt]
                      · omega
      ...
      · simp [fbFrame]
        ...
        · rw [if_neg (by simp [hfz])]
          ...
          rw [if_neg (by simp)]
          wp_run
          simp [func12Def, optionVals, findBestL_eq_prefix]
