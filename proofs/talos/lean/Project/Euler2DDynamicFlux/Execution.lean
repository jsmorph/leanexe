...
theorem fluxCheckedBits_exact_in_module {m : Wasm.Module} (layout : Layout m)
  ...
  change TerminatesWith env m 5 initial [.i64 energyL, .i64 transverseL, .i64 momentumL, .i64 rhoL]
  ...
  change TerminatesWith env m 5 initial [.i64 energyR, .i64 transverseR, .i64 momentumR, .i64 rhoR]
  ...
  refine TerminatesWith.of_wp_entry_for (f := func17Def)
    (by simpa [layout.noImports] using layout.flux) ?_ (by simp [layout.noImports])
  change wp m func17 _ initial (func17Def.toLocals [.i64 rhoL, .i64 momentumL, .i64 transverseL, .i64 energyL, .i64 rhoR, .i64 momentumR, .i64 transverseR, .i64 energyR]) env
  unfold func17
  wp_run [func17Def, List.set, List.getElem?_cons_zero, List.getElem?_cons_succ,
  ...
  · dynamic_peel
    ...
    have massCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial right.speed left.massFlux right.massFlux rhoL rhoR
    ...
    have momentumCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial right.speed left.momentumFlux right.momentumFlux momentumL momentumR
    ...
    have transverseCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial right.speed left.transverseFlux right.transverseFlux transverseL transverseR
    change TerminatesWith env m 9 initial [.i64 transverseR, .i64 transverseL, .i64 right.transverseFlux, .i64 left.transverseFlux, .i64 right.speed]
    ...
    have energyCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial right.speed left.energyFlux right.energyFlux energyL energyR
  ...
  · dynamic_peel
    ...
    have massCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial left.speed left.massFlux right.massFlux rhoL rhoR
    ...
    have momentumCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial left.speed left.momentumFlux right.momentumFlux momentumL momentumR
    ...
    have transverseCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial left.speed left.transverseFlux right.transverseFlux transverseL transverseR
    change TerminatesWith env m 9 initial [.i64 transverseR, .i64 transverseL, .i64 right.transverseFlux, .i64 left.transverseFlux, .i64 left.speed]
      (fun final values => final = initial ∧ values = [.i64 transverse.value, .i64 transverse.status]) at transverseCall
    refine wp_call_tw transverseCall ?_
    rintro st values ⟨hst, hvalues⟩
    subst st
    ...
    have energyCall := Project.EulerDynamicFlux.Execution.componentCheckedBits_exact_in_module_core
      layout.components env initial left.speed left.energyFlux right.energyFlux energyL energyR
