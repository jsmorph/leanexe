import Project.ClobFindBest.FrozenSelectTag
...
theorem select_payload_spec (env : HostEnv Unit) (st : Store Unit)
    ...
    (hNext : ∀ next : List Value, next.length = 44 →
      next[30]? = some (.i64 (optionPayload (bestStepL os taker k best))) →
      next[0]? = scratch[0]? →
      next[1]? = scratch[1]? →
      next[2]? = scratch[2]? →
      next[3]? = scratch[3]? →
    ...
    wp module ((stepCode.drop 31).take 5 ++ rest) Q st
...
#print axioms select_payload_spec
