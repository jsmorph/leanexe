...
namespace Project.ClobLimit.Frozen.InternalLoopProgress
...
def fullState (data : RunningData) (taker : OrderL) (i : Nat) :
    Project.ClobMatchFuel.Frozen.Model.MatchStateL :=
...
def partialState (data : RunningData) (taker : OrderL) (i : Nat) :
    Project.ClobMatchFuel.Frozen.Model.MatchStateL :=
  { book := Project.ClobMatchFuel.Frozen.Model.setQtyL data.orders i
...
theorem stopped_source (ctx : Context) (st : Store Unit) (s : Locals)
  ...
  · rw [hFuelNat] at hSource
    rw [Project.ClobMatchFuel.Frozen.Model.matchFuelL_succ_zero fuel ctx.taker
  ...
  · by_cases hRemaining : data.remaining = 0
    · rw [hFuelNat] at hSource
      ...
      exact hSource
    · rw [hFuelNat] at hSource
      rw [Project.ClobMatchFuel.Frozen.Model.matchFuelL_succ_none fuel ctx.taker
...
theorem partial_source (ctx : Context) (st : Store Unit) (s : Locals)
  ...
  have hSource := facts.source
  rw [hFuelNat] at hSource
  rw [Project.ClobMatchFuel.Frozen.Model.matchFuelL_succ_partial fuel ctx.taker
...
theorem full_source (ctx : Context) (st : Store Unit) (s : Locals)
    ...
    (hFind : findBestL data.orders ctx.taker = some i)
    (hQty : data.orders[i]!.oqty ≤ data.remaining) :
    ctx.result = Project.ClobMatchFuel.Frozen.Model.matchFuelL
  ...
  have hSource := facts.source
  rw [hFuelNat] at hSource
  rw [Project.ClobMatchFuel.Frozen.Model.matchFuelL_succ_full
...
end Project.ClobLimit.Frozen.InternalLoopProgress
