...
namespace Project.ClobLimit.Frozen.InternalLoopAdvance

open Wasm Project.Clob Project.ClobFindBest.Frozen.Model Project.ClobLimit
  Project.ClobLimit.Frozen.InternalFullBranch
  Project.ClobLimit.Frozen.InternalLoopInvariant
  Project.ClobLimit.Frozen.InternalLoopProgress
...
def nextData (data : RunningData) (taker : OrderL) (i : Nat)
  ...
  { steps := data.steps + 1
    ...
    tradeValues := data.tradeValues ++
      [Project.ClobMatchFuel.Frozen.Model.fillTradeL taker data.orders[i]!
        data.orders[i]!.oqty] }
...
theorem of_full (ctx : Context) (st : Store Unit) (s : Locals)
    ...
    (hTradesOwned : OwnedTradeArrayAt st1 newTrades newTradesCapacity
      (data.tradeValues ++
        [Project.ClobMatchFuel.Frozen.Model.fillTradeL ctx.taker data.orders[i]!
          data.orders[i]!.oqty]))
  ...
  have hTradeLimit : data.tradeValues.length + 1 ≤ ctx.tradeLimit := by
    ...
    have hFuelPositive : 0 < data.fuel.toNat := by
      ...
      simpa using hZero
    omega
  have hBudget := Project.ClobMatchFuel.Frozen.Budget.spend_step hFuel (by
    calc
      g0Final.toNat ≤ data.g0.toNat + 96 +
          orderArrayBytes (data.orders.length - 1) +
          tradeArrayBytes (data.tradeValues.length + 1) := hHeapUpper
      _ ≤ data.g0.toNat + Project.ClobMatchFuel.Frozen.Budget.stepBytes
          ctx.bookLimit ctx.tradeLimit := by
        simpa [Nat.add_assoc] using Nat.add_le_add_left
          (Project.ClobMatchFuel.Frozen.Budget.fullStepBytes_le facts.bookLength
  ...
  · simp only [nextData]
    calc
      ...
      _ = data.steps + 1 + (data.fuel - 1).toNat := by
        rw [Project.ClobMatchFuel.Frozen.Budget.fuel_sub_one_toNat data.fuel hFuel]
        have hFuelPositive : 0 < data.fuel.toNat := by
          by_contra h
          have hZero : data.fuel.toNat = 0 := by omega
          apply hFuel
...
end Project.ClobLimit.Frozen.InternalLoopAdvance
