...
namespace Project.ClobLimit.InternalLoopBounds
...
theorem of_running (ctx : Context) (st : Store Unit) (s : Locals)
  ...
  have hTradeLength : data.tradeValues.length + 1 ≤ ctx.tradeLimit := by
    ...
    omega
  have hAvailable := Project.ClobMatchFuel.Budget.one_step_available hFuel
    facts.budget
  have hAvailable' := hAvailable
  unfold Project.ClobMatchFuel.Budget.stepBytes orderArrayBytes tradeArrayBytes
  ...
  have hPartialTradeFit32 : data.g0.toNat + 96 +
    ...
    unfold orderArrayBytes tradeArrayBytes fixedArrayBytes
    omega
  have hFullBookTop := Project.ClobMatchFuel.Budget.allocationTop_toNat data.g0
  ...
  have hTradeTop := Project.ClobMatchFuel.Budget.allocationTop_toNat data.g0
  ...
  have hPartialTradeFit32After :
    ...
    omega
  have hFullTradeTop := Project.ClobMatchFuel.Budget.allocationTop_toNat
    (data.g0 + 48 + orderArrayBytesU (data.orders.length - 1))
    (tradeArrayBytesU (data.tradeValues.length + 1))
    (tradeArrayBytes (data.tradeValues.length + 1)) hTradeNeed
    (by simpa [hTradeNeed] using hFullTradeFit32After)
  have hPartialTradeTop := Project.ClobMatchFuel.Budget.allocationTop_toNat
  ...
  · rw [hFullNeed]
    unfold Project.ClobMatchFuel.Budget.stepBytes orderArrayBytes
  ...
  · calc
      data.g0.toNat + 96 + orderArrayBytes (data.orders.length - 1) +
          tradeArrayBytes (data.tradeValues.length + 1) ≤
          data.g0.toNat + Project.ClobMatchFuel.Budget.stepBytes
            ctx.bookLimit ctx.tradeLimit := by
        simpa [Nat.add_assoc] using Nat.add_le_add_left
          (Project.ClobMatchFuel.Budget.fullStepBytes_le hBookLength
  ...
  · calc
      data.g0.toNat + 96 + orderArrayBytes data.orders.length +
          ...
          data.g0.toNat + Project.ClobMatchFuel.Budget.stepBytes
            ctx.bookLimit ctx.tradeLimit := by
        simpa [Nat.add_assoc] using Nat.add_le_add_left
          (Project.ClobMatchFuel.Budget.partialStepBytes_le hBookLength
            hTradeLength) data.g0.toNat
      _ ≤ ctx.limit := hAvailable

end Project.ClobLimit.InternalLoopBounds
