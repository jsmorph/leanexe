...
import Project.ClobMatchFuel.FrozenBudget
...
namespace Project.ClobLimit.Frozen.InternalLoopInvariant
...
structure Context where
  initialFuel : UInt64
  taker : OrderL
  initialState : Project.ClobMatchFuel.Frozen.Model.MatchStateL
  initialG0 : UInt64
  initialG2 : UInt64
  initialMem : Mem
  initialPages : Nat
...
def Context.result (ctx : Context) : Project.ClobMatchFuel.Frozen.Model.MatchStateL :=
  Project.ClobMatchFuel.Frozen.Model.matchFuelL ctx.initialFuel.toNat ctx.taker
    ctx.initialState
...
structure RunningFacts (ctx : Context) (st : Store Unit) (s : Locals)
  ...
  source : ctx.result =
    Project.ClobMatchFuel.Frozen.Model.matchFuelL data.fuel.toNat ctx.taker
      data.sourceState
...
end Project.ClobLimit.Frozen.InternalLoopInvariant
