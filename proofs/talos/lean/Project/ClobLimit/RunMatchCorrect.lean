...
theorem runMatchContext_result
  ...
  unfold runMatchContext Context.result Context.base
    Project.ClobMatchFuel.LoopInvariant.Context.result Project.ClobLimit.Model.runMatchL
  rw [hFuel]

def RunMatchSpec : Prop :=
  ∀ (env : HostEnv Unit) (st : Store Unit)
    (bookOwner book bookCapacity g0 g2 g4 g5 : UInt64)
    ...
    TerminatesWith (m := «module») (id := 18) (initial := st) (env := env)
      ...
      (MatchOutput.Postcondition
        (runMatchContext st os taker g0 g2 g4 g5 limit))

set_option Elab.async false in
theorem func18_correct : RunMatchSpec := by
  intro env st bookOwner book bookCapacity g0 g2 g4 g5 os taker limit hLength
  ...
  have hInternal : TerminatesWith (m := «module») (id := 17)
      (initial := stA) (env := env)
      (MatchEntry.internalArgs (UInt64.ofNat (os.length + 1)) taker
        bookOwner book (g0 + 48) (g0 + 104) taker.oqty)
      (fun st' values => MatchOutput.Postcondition ctx st' values) := by
    ...
    · simpa [ctx, runMatchContext, stA, RunMatchAllocations.allocationsStore, RunMatchEmptyAlloc.allocStore,
        List.getElem?_set] using hg4
    · simpa [ctx, runMatchContext, stA, RunMatchAllocations.allocationsStore, RunMatchEmptyAlloc.allocStore,
        List.getElem?_set] using hg5
    · exact hAlloc.pages
    · rw [hAlloc.pages]
      exact hPages
    · exact hAddressLimit
  ...
  · change wp «module» func18 _ st
    ...
    apply RunMatchCall.callProg_spec env stA
      ...
      (MatchOutput.Postcondition ctx) hInternal
    ...
    · intro final hFinalValues
      ...
      simp [func18Def, Function.numParams, hFinalValues,
        MatchOutput.outputValues, runMatchArgs, ctx]
