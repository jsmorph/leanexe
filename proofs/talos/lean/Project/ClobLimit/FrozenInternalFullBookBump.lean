...
/-!
# Internal full-book bump allocation

The full-fill replacement-book allocator reaches this body after its free-list
search returns no reusable node.  The body extends global 0 and writes one
stride-five fixed-array header.  Its result frame records the generated scratch
locals.
-/

namespace Project.ClobLimit.Frozen.InternalFullBookBump
...
def allocFrame (base : Locals) (need previous current capacity next result :
    UInt64) : Locals :=
  { base with
    locals := ((((((base.locals.set 55 (.i64 need)).set 56
      (.i64 previous)).set 57 (.i64 current)).set 58
      (.i64 capacity)).set 59 (.i64 next)).set 60 (.i64 result)) }

def fullBookSearchBodyProg : Wasm.Program :=
  [
  .localGet 68,
  .constI64 0,
  .eqI64,
...
def fullBookSearchProg : Wasm.Program :=
  [.block 0 0 [.loop 0 0 fullBookSearchBodyProg]]

def fullBookSearchInitProg : Wasm.Program :=
  [
  .constI64 0,
  .localSet 71,
  .constI64 0,
...
def fullBookBumpProg : Wasm.Program :=
  [
  .localGet 71,
  .constI64 0,
  .eqI64,
...
def fullBookNoFitProg : Wasm.Program :=
  fullBookSearchInitProg ++ fullBookSearchProg ++ fullBookBumpProg

set_option Elab.async false in
theorem fullBookSearchProg_empty
    ...
    wp «module» (fullBookSearchProg ++ rest) Q st
      (allocFrame base need previous 0 capacity next 0) env := by
  simp only [fullBookSearchProg, List.cons_append, List.nil_append]
  apply wp_block_cons
  apply wp_loop_cons
    (Inv := fun st' s =>
...
theorem fullBookBumpProg_spec
    ...
    wp «module» (fullBookBumpProg ++ rest) Q st
      (allocFrame base need previous 0 capacity next 0) env := by
  simp only [fullBookBumpProg, List.cons_append, List.nil_append, allocFrame]
  wp_run_with [hParams, hLocals, hValues]
  ...
  have hFinalFrame :
      { params := base.params,
        locals := ((((((((base.locals.set 55 (.i64 need)).set 56
          (.i64 previous)).set 57 (.i64 0)).set 58
          (.i64 capacity)).set 59 (.i64 next)).set 60 (.i64 0)).set 58
          (.i64 (g0 + 48 + need))).set 59
          (.i64 ((g0 + 48 + need - 1) / 65536 + 1))).set 60
          (.i64 (g0 + 48)) } =
        allocFrame base need previous 0 (g0 + 48 + need)
...
set_option Elab.async false in
theorem fullBookNoFitProg_spec
    (env : HostEnv Unit) (st : Store Unit) (base : Locals)
    ...
    (hNext : wp «module» rest Q
      ...
      (allocFrame base need 0 0 (g0 + 48 + need)
        ((g0 + 48 + need - 1) / 65536 + 1) (g0 + 48)) env) :
    wp «module» (fullBookNoFitProg ++ rest) Q st
      (allocFrame base need previous current capacity next result) env := by
  have hBump := fullBookBumpProg_spec env st base g0 need 0 capacity next
    hParams hLocals hValues hNeed8 hTop hFit32 hFit hPages hg0 Q rest hNext
  have hSearch := fullBookSearchProg_empty env st base need 0 capacity next
    hParams hLocals hValues Q (fullBookBumpProg ++ rest) hBump
  unfold fullBookNoFitProg
  rw [List.append_assoc]
  simp only [fullBookSearchInitProg, List.cons_append, List.nil_append,
    allocFrame]
  wp_run_with [hParams, hLocals, hValues]
  simp only [hg1]
  have hOverwrite :
      ((((((((base.locals.set 55 (.i64 need)).set 56
        (.i64 previous)).set 57 (.i64 current)).set 58
        (.i64 capacity)).set 59 (.i64 next)).set 60 (.i64 0)).set 56
        (.i64 0)).set 57 (.i64 0)) =
      (((((base.locals.set 55 (.i64 need)).set 56 (.i64 0)).set 57
        (.i64 0)).set 58 (.i64 capacity)).set 59 (.i64 next)).set 60
        (.i64 0) := by
    apply List.ext_getElem?
    intro i
    by_cases h60 : 57 = i
...
end Project.ClobLimit.Frozen.InternalFullBookBump
