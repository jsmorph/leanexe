...
import Project.ClobLimit.HeapCorrect
import Project.ClobLimit.HeapResidualFacts
import Project.ClobLimit.Model

namespace Project.ClobLimit.HeapRunMatch
...
def runMatchContext (st : Store Unit) (os : List OrderL) (taker : OrderL)
    (g0 g2 g4 g5 : UInt64) (limit : Nat) : Context :=
...
theorem runMatchContext_result
    ...
    (g0 g2 g4 g5 : UInt64) (limit : Nat)
    (hLength : os.length < 4294967296) :
    (runMatchContext st os taker g0 g2 g4 g5 limit).result =
...
structure OutputData extends LoopResult.OutputData where
  bookOwner : Value
  tradesOwner : Value

def outputValues (ctx : Context) (data : OutputData) : List Value :=
  HeapResult.outputValues ctx data.toOutputData data.bookOwner data.tradesOwner

abbrev OutputAt (ctx : Context) (st : Store Unit) (data : OutputData) :=
  LoopResult.OutputAt ctx st data.toOutputData

def RawPostcondition (ctx : Context) (st : Store Unit) (values : List Value) : Prop :=
  ∃ data bookOwner tradesOwner,
    values = HeapResult.outputValues ctx data bookOwner tradesOwner ∧ LoopResult.OutputAt ctx st data
  ...
  ∀ (env : HostEnv Unit) (st : Store Unit)
    ...
    g0.toNat + 112 + (os.length + 1) *
      Project.ClobMatchFuel.Budget.stepBytes os.length (os.length + 1) ≤
        limit →
    TerminatesWith (m := Project.ClobLimit.«module») (id := 18) (initial := st) (env := env)
      (runMatchArgs 0 book taker)
      (Postcondition
        (runMatchContext st os taker g0 g2 g4 g5 limit))
...
theorem func18_correct : RunMatchSpec := by
  ...
  let ctx := runMatchContext st os taker g0 g2 g4 g5 limit
  ...
  have hInternal : TerminatesWith (m := Project.ClobLimit.«module») (id := 17)
      (initial := stA) (env := env)
      (InternalEarlyExit.internalArgs (UInt64.ofNat (os.length + 1)) taker
        0 book (g0 + 48) (g0 + 104) taker.oqty)
      (fun st' values => RawPostcondition ctx st' values) := by
    change TerminatesWith env Project.ClobLimit.«module» 17 stA
      (HeapEntry.args ctx book (g0 + 48) (g0 + 104)) (RawPostcondition ctx)
    apply HeapCorrect.spec env stA ctx book bookCapacity (g0 + 48) (g0 + 104) 8 (g0 + 112) []
    · exact hBook48
    · exact hBook32
    · exact hBookCapacity
    · rw [hHeapNat]; omega
    · simp [FreeListSeparatedFromFixedArray]
    · rw [hTradesNat]; omega
    · change (g0 + 104).toNat + fixedArrayBytes 0 4 < 4294967296
      rw [hTradesNat]; simp [fixedArrayBytes]; omega
    ...
    · simpa [stA, ctx, runMatchContext, RunMatchAllocations.allocationsStore,
        RunMatchEmptyAlloc.allocStore, List.getElem?_set] using hg4
    · simpa [stA, ctx, runMatchContext, RunMatchAllocations.allocationsStore,
        RunMatchEmptyAlloc.allocStore, List.getElem?_set] using hg5
    · exact hAlloc.pages
    · rw [hAlloc.pages]; exact hPages
    · exact hAddressLimit
    · rw [hAlloc.pages]; exact hMemoryLimit
    · change (g0 + 112).toNat + (UInt64.ofNat (os.length + 1)).toNat *
  ...
  · change wp Project.ClobLimit.«module» func18 _ st
      (RunMatchPrepare.entryFrame 0 book taker) env
    rw [RunMatchEntry.func18_decomposition]
    apply RunMatchPrepare.prepareProg_spec env st 0 book taker os
      hLength hBook.2
    apply RunMatchAllocations.allocationsProg_spec env st 0 book g0 g2
    ...
    apply RunMatchCall.callProg_spec env stA
      (RunMatchAllocations.finalFrame 0 book taker os g0)
      (UInt64.ofNat (os.length + 1)) taker 0 book (g0 + 48)
      (g0 + 104) taker.oqty hCallLocals
      (RawPostcondition ctx) hInternal
    intro st' values hPost
    rcases hPost with ⟨data, bookOwner, tradesOwner, hValues, hOutput⟩
    simp only [RunMatchEntry.resultProg, List.cons_append, List.nil_append]
    simp (config := { maxSteps := 10000000 }) [wp_simp, hCallLocals.params,
      hCallLocals.locals, hValues, HeapResult.outputValues]
    refine ⟨⟨data, bookOwner, tradesOwner⟩, ?_, hOutput⟩
    simp [func18Def, Function.numParams, outputValues, HeapResult.outputValues, runMatchArgs, ctx]

#print axioms func18_correct
end Project.ClobLimit.HeapRunMatch
