...
/-!
# Internal match-loop step bounds

One remaining budget step bounds the replacement book and appended trade
allocations.  The derived facts cover both full-fill and partial-fill book
sizes.  Branch composition consumes this record without repeating arithmetic.
-/

namespace Project.ClobLimit.Frozen.InternalLoopBounds

open Wasm Project.Common Project.Clob Project.ClobLimit
  Project.ClobLimit.Frozen.InternalLoopInvariant
  Project.ClobMatchFuel.Frozen.Allocation
...
theorem of_running (ctx : Context) (st : Store Unit) (s : Locals)
  ...
  have hTradeLength : data.tradeValues.length + 1 ≤ ctx.tradeLimit := by
    ...
    omega
  have hAvailable := Project.ClobMatchFuel.Frozen.Budget.one_step_available hFuel
    facts.budget
  have hAvailable' := hAvailable
  unfold Project.ClobMatchFuel.Frozen.Budget.stepBytes orderArrayBytes tradeArrayBytes
    fixedArrayBytes at hAvailable'
  ...
  have hPartialTradeFit32 : data.g0.toNat + 96 +
    ...
    unfold orderArrayBytes tradeArrayBytes fixedArrayBytes
    omega
  have hFullBookTop := Project.ClobMatchFuel.Frozen.Budget.allocationTop_toNat data.g0
  ...
  have hPartialBookTop :=
    Project.ClobMatchFuel.Frozen.Budget.allocationTop_toNat data.g0
  ...
  have hTradeTop := Project.ClobMatchFuel.Frozen.Budget.allocationTop_toNat data.g0
  ...
  have hPartialTradeFit32After :
    ...
    omega
  have hFullTradeTop := Project.ClobMatchFuel.Frozen.Budget.allocationTop_toNat
    (data.g0 + 48 + orderArrayBytesU (data.orders.length - 1))
    (tradeArrayBytesU (data.tradeValues.length + 1))
    (tradeArrayBytes (data.tradeValues.length + 1)) hTradeNeed
    (by simpa [hTradeNeed] using hFullTradeFit32After)
  have hPartialTradeTop := Project.ClobMatchFuel.Frozen.Budget.allocationTop_toNat
    (data.g0 + 48 + orderArrayBytesU data.orders.length)
    (tradeArrayBytesU (data.tradeValues.length + 1))
    (tradeArrayBytes (data.tradeValues.length + 1)) hTradeNeed
    (by simpa [hTradeNeed] using hPartialTradeFit32After)
  have hFit : data.g0.toNat +
      Project.ClobMatchFuel.Frozen.Budget.stepBytes ctx.bookLimit ctx.tradeLimit ≤
  ...
  · rw [hFullNeed]
    unfold Project.ClobMatchFuel.Frozen.Budget.stepBytes orderArrayBytes
      tradeArrayBytes fixedArrayBytes at hFit
    unfold orderArrayBytes fixedArrayBytes
    omega
  · rw [hPartialNeed]
    unfold Project.ClobMatchFuel.Frozen.Budget.stepBytes orderArrayBytes
  ...
  · calc
      data.g0.toNat + 96 + orderArrayBytes (data.orders.length - 1) +
          tradeArrayBytes (data.tradeValues.length + 1) ≤
          data.g0.toNat + Project.ClobMatchFuel.Frozen.Budget.stepBytes
            ctx.bookLimit ctx.tradeLimit := by
        simpa [Nat.add_assoc] using Nat.add_le_add_left
          (Project.ClobMatchFuel.Frozen.Budget.fullStepBytes_le hBookLength
            hTradeLength) data.g0.toNat
  ...
  · calc
      data.g0.toNat + 96 + orderArrayBytes data.orders.length +
          ...
          data.g0.toNat + Project.ClobMatchFuel.Frozen.Budget.stepBytes
            ctx.bookLimit ctx.tradeLimit := by
        simpa [Nat.add_assoc] using Nat.add_le_add_left
          (Project.ClobMatchFuel.Frozen.Budget.partialStepBytes_le hBookLength
            hTradeLength) data.g0.toNat
      _ ≤ ctx.limit := hAvailable

end Project.ClobLimit.Frozen.InternalLoopBounds
