...
def resultFrame (book : UInt64) (order : OrderL)
    (ctx : Context) (data : HeapRunMatch.OutputData) : Locals :=
  { params := [.i64 book, .i64 order.oid, .i64 order.otrader,
    ...
    locals := [.i64 0, .i64 book, .i64 order.oid, .i64 order.otrader,
      ...
      .i64 order.oside, .i64 order.oprice, .i64 order.oqty,
      data.bookOwner, .i64 data.book, data.tradesOwner,
      .i64 data.trades, .i64 ctx.result.remaining, data.bookOwner,
      .i64 data.book, data.tradesOwner, .i64 data.trades,
      .i64 ctx.result.remaining] ++ List.replicate 30 (.i64 0)
    values := [] }
...
theorem validResultStoreProg_spec
    ...
    (data : HeapRunMatch.OutputData) (values : List Value)
    (hValues : values = HeapRunMatch.outputValues ctx data)
  ...
  simp only [LimitEntry.validResultStoreProg, List.cons_append,
    List.nil_append]
  simp only [hValues, HeapRunMatch.outputValues, HeapResult.outputValues]
  wp_run
