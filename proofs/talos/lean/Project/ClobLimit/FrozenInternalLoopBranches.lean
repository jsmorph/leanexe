  ...
  · intro st1 s1 hResult hBookOwned hNewBookOwned hTradesOwned hNewTradesOwned
    ...
    have hNewBook32 :
        (data.g0 + 48).toNat + fixedArrayBytes
          (Project.ClobMatchFuel.Frozen.Model.setQtyL data.orders i
            (data.orders[i]!.oqty - data.remaining)).length 5 <
          4294967296 := by
      rw [Project.ClobMatchFuel.Frozen.Model.setQtyL_length, hBookRoot]
      have hFit := bounds.partialBookFit32
      rw [hBookNeed] at hFit
      exact hFit
    have hNewBookCapacity : fixedArrayBytes
      ...
      rw [Project.ClobMatchFuel.Frozen.Model.setQtyL_length, hBookNeed]
    have hNewBookBelow :
        (data.g0 + 48).toNat +
            (orderArrayBytesU data.orders.length).toNat ≤
    ...
    have hNewTrades32 :
        (data.g0 + 48 + orderArrayBytesU data.orders.length + 48).toNat +
            fixedArrayBytes (data.tradeValues ++
              [Project.ClobMatchFuel.Frozen.Model.fillTradeL ctx.taker
...
end Project.ClobLimit.Frozen.InternalLoopBranches
