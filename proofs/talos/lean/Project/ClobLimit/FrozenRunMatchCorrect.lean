...
namespace Project.ClobLimit.Frozen.RunMatchCorrect
...
theorem runMatchContext_result
    ...
    (runMatchContext st os taker g0 g2 limit).result =
      Project.ClobLimit.Frozen.Model.runMatchL os taker := by
  have hFuel : (UInt64.ofNat (os.length + 1)).toNat = os.length + 1 := by u64_omega
  unfold runMatchContext Context.result Project.ClobLimit.Frozen.Model.runMatchL
  rw [hFuel]
  ...
  have hInternal : TerminatesWith (m := «module») (id := 17)
    ...
    · change (g0 + 112).toNat +
          (UInt64.ofNat (os.length + 1)).toNat *
            Project.ClobMatchFuel.Frozen.Budget.stepBytes os.length
...
end Project.ClobLimit.Frozen.RunMatchCorrect
