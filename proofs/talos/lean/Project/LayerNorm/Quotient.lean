...
theorem quotient_error_bounded (c d a r centerError denominatorError : ℝ)
    (hc0 : 0 ≤ centerError) (he0 : 0 ≤ denominatorError)
    (hr : 1/1000 ≤ r) (hd : r/2 ≤ d)
    (ha : |a/r| ≤ 2) (hc : |c-a| ≤ centerError*arithmeticEpsilon)
    (he : |d-r| ≤ denominatorError*arithmeticEpsilon*r) :
    |c/d-a/r| ≤ (2000*centerError+4*denominatorError)*arithmeticEpsilon := by
  ...
  have hterm : |(a/r)*(r-d)| ≤ 2*(denominatorError*arithmeticEpsilon*r) := by
  ...
  have hscale := mul_le_mul_of_nonneg_left hr
    (by positivity [F64ArithmeticBounds.epsilon_pos] : 0 ≤ 1000*centerError*arithmeticEpsilon)
  have hdscale := mul_le_mul_of_nonneg_left hd
    (by positivity [F64ArithmeticBounds.epsilon_pos] :
      0 ≤ (2000*centerError+4*denominatorError)*arithmeticEpsilon)
  nlinarith only [hn, hscale, hdscale]
...
theorem quotient_error (c d a r : ℝ) (hr : 1/1000 ≤ r) (hd : r/2 ≤ d)
  ...
  exact (quotient_error_bounded c d a r 32 200000001 (by norm_num) (by norm_num)
    hr hd ha hc he).trans (by norm_num [arithmeticEpsilon])

theorem normalized_error_bounded (x : Fin 4 → UInt64) (bound : ℝ)
    ...
    Finite q ∧ |value q| ≤ 3 ∧
      |value q - Real.normalized (1/100000) (fun j => value (x j)) i| ≤
        (40000000*bound^2+16000*bound+4000007)*arithmeticEpsilon := by
  let c := centeredWords x
  let d := denominator (c 0) (c 1) (c 2) (c 3)
  let r := Real.deviation (1/100000) (fun j => value (x j))
  let a := Real.centered (fun j => value (x j)) i
  have hc := centered_error_bounded x bound hb (by linarith) hf hx i
  have hd := denominator_error_bounded x bound hb hbmax hf hx
  have hr : 1/1000 ≤ r := by
  ...
  have hq := quotient_error_bounded (value (c i)) (value d) a r (8*bound)
    (10000000*bound^2+1000001) (by linarith) (by positivity)
    hr hd.2.2.1 ha hc.2.2 hd.2.2.2
  have hb2 : bound^2 ≤ 256 := by nlinarith
  have hbudget : (40000000*bound^2+16000*bound+4000007)*arithmeticEpsilon ≤ 1 := by
    norm_num [arithmeticEpsilon]
    nlinarith
  have bq : |value (c i)/value d| ≤ 3 := by
    apply (F64ArithmeticBounds.magnitude_of_error _ _ _ 2 hq ha).trans
  ...
  have he : |value (Wasm.IEEE64.div (c i) d)-a/r| ≤
      (40000000*bound^2+16000*bound+4000007)*arithmeticEpsilon :=
    (abs_sub_le _ _ _).trans ((add_le_add ho.2 hq).trans_eq (by ring))
  refine ⟨ho.1, ?_, he⟩
  exact (F64ArithmeticBounds.magnitude_of_error _ _ _ 2 he ha).trans (by linarith only [hbudget])
...
theorem normalized_error (x : Fin 4 → UInt64)
  ...
  have h := normalized_error_bounded x 4 (by norm_num) (by norm_num) hf hx i
  exact ⟨h.1, h.2.1, h.2.2.trans (by norm_num [arithmeticEpsilon])⟩
