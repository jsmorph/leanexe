...
| 0.5 | Restructure the emitter per [the emitter plan](../../docs/emitter.md): `emit : IR.Module → Wasm.Module` with serialization split off, validated by byte-identical output for all twenty cases; add the per-module `rfl` round-trip gate. | Byte-identical regeneration plus the new gate on all cases. |
