...
The exact 28,315-byte package passes decoding, validation, execution-model
equality, session transfer, and the independent declaration audit.  The
combined `artifact_gpt2_128_exact` theorem includes grammar membership and
`CoreValid`.  Full numerical propagation remains in progress.
...
All forty-six exact-artifact manifests identify this Talos revision.  The
quantized projection uses the extended scalar instruction profile.  The
earlier forty-three manifests now identify that verifier.  The 2026-09-24
aggregate artifact check passed all forty-six packages.  The source artifact tool fetches its
...
The 2026-09-24 conformance run passed twenty-six official execution files in Wasmtime and 6,996 assertions in Talos, with four configured skips and no failures.  The same gate checked fifteen pinned invalid or malformed modules against their exact artifact decoder or validator errors.  Text-origin invalid modules have encoder-added custom sections removed before classification, while raw malformed binary modules remain unchanged.
