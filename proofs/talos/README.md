...
The [packed construction loop](lean/Project/PackedGenerate/Spec.lean)
proves that the emitted loop writes the bytes of `makeWords` for every
word count and offset when the output fits in the current memory and the
32-bit address range.  It applies the shared
...
output range.  The `packed_generate` registration remains incomplete:
allocation and the public return still need composition with this loop.
