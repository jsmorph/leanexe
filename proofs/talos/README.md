...
| `euler_rusanov_step` | [`LeanExe.Examples.EulerRusanovStep`](../../LeanExe/Examples/EulerRusanovStep.lean) | `sodQuarterStepCheckedBits` | `EulerRusanovStep.Model.sodQuarterStepCheckedBitsModel_exact`; generated-WAT theorem pending | The no-argument case makes three guarded flux calls and six exactly associated binary64 conservative updates, returning status plus two three-word cells.  The pure model proves the seven expected words.  Its generated-WAT execution theorem, exact-byte package, and raw-data publication remain incomplete, so it is not imported by the completed aggregate. |
