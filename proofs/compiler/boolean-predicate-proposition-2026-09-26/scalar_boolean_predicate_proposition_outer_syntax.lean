...
run_elab do
          ...
          for compound in [false, true] do
            let call (argument : Lean.Expr) :=
              let guard : PropositionGuard := if compound then
                ⟨.canonical (.junction 1 .conjunction (.compare .lt (.bvar 0) (literalExpr 7))
                  (.compare .eq (toWord (.app (.bvar 2) argument)) (literalExpr 1))), rfl⟩
                else ⟨.canonical (.compare .le (.bvar 0) (literalExpr 1)), rfl⟩
              toWord (form.proposition negations guard
                (.predicate (negations + 1) 2 (BooleanGuardNegation.expr 1 argument))
                (.predicate 0 2 (booleanLiteralExpr true))).expr
  ...
  Lean.logInfo m!"{comparisons} native/outer Boolean-input predicate proposition syntax comparisons and {rejected} invalid-input tests passed"
