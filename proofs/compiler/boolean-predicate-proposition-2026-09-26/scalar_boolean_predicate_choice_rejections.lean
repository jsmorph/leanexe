    ...
    for form in [BooleanChoiceForm.ordinary, .dependent ⟨`yesProof, `noProof, .default, .default⟩] do
      for flag in [false, true] do
        let make (yes no : BooleanLocal) :=
          (form.local 0 false (.literal 0 flag) (.literal 0 true) yes no).expr
        unless (check (make yes no)).isSome do throwError "positive predicate choice control rejected"
        controls := controls + 1
        for argument in [literalExpr 0, Lean.Expr.bvar index, .const `unsupportedArgument [], .bvar wordIndex] do
          ...
          for body in [make invalid no, make yes invalid] do
            unless (check body).isNone do throwError "invalid active/inactive predicate choice branch accepted"
            rejected := rejected + 1
    for flag in [false, true] do
      let condition := booleanRelationCondition false (booleanLiteralExpr flag) (booleanLiteralExpr true)
      let evidence := booleanRelationEvidence false (booleanLiteralExpr flag) (booleanLiteralExpr true)
      ...
      for body in [make (.bvar 0) liftedNo, make liftedYes (.bvar 0)] do
        unless (check body).isNone do throwError "predicate choice condition proof used as a Boolean value"
        rejected := rejected + 1
  unless controls == 12 && rejected == 108 do throwError "unexpected counts {controls}, {rejected}"
  Lean.logInfo m!"{controls} predicate choice admission controls and {rejected} branch/proof rejection tests passed"
