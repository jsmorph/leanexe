          ...
          for compound in [false, true] do
            let call (argument : Lean.Expr) :=
              let guard : PropositionGuard := if compound then
                ⟨.canonical (.junction 1 .conjunction (.compare .lt (.bvar 2) (literalExpr 7))
                  (.compare .eq (toWord (.app (.bvar 0) argument)) (literalExpr 1))), rfl⟩
                else ⟨.canonical (.compare .le (.bvar 2) (literalExpr 1)), rfl⟩
              ...
              for flag in [false, true] do
                ...
                for (x, y) in inputs do
                  ...
                  let f := fun b => GuardNegation.denote negations
                    (if (if compound then !(x < 7 && (base b).toUInt64 == 1) else x ≤ 1)
                ...
                for body in invalid do
                  ...
                  rejected := rejected + 1
  unless comparisons == 3584 && rejected == 3840 do
    throwError "unexpected counts {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/Boolean-input predicate proposition syntax comparisons and {rejected} invalid-input tests passed"
