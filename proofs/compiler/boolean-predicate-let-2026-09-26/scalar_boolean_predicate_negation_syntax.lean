        ...
        for nondep in [false, true] do
          for flag in [false, true] do
            ...
            for (x, y) in inputs do
              let expected := (GuardNegation.denote negations ((x != 0) && x == y)).toUInt64 +
                (GuardNegation.denote negations (flag && x == y)).toUInt64
            ...
            for body in invalid do
              ...
              rejected := rejected + 1
  unless comparisons == 896 && rejected == 896 && controls == 64 do
    throwError "unexpected counts {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/Boolean-input predicate negation syntax comparisons and {rejected} invalid-input tests and {controls} admission controls passed"
