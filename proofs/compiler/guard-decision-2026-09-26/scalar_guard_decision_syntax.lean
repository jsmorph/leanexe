  ...
  for op in [Junction.conjunction, .disjunction] do
    for n in [0, 1, 2, 5] do
      for (l, r) in [(left, right),
        ...
        unless guardDecision? guard evidence && (reannotatedGuard? guard.condition evidence).isSome do
          throwError "compound annotation decision rejected"
        let some func := extractScalarFunc `compound (some "entry") functionType (body guard.condition evidence) |
        ...
        for decision in invalid do
          unless !(guardDecision? guard decision) &&
              (extractScalarFunc `badCompound (some "entry") functionType (body guard.condition decision)).isNone do
            throwError "invalid compound decision accepted"
          rejected := rejected + 1
  unless comparisons == 336 && rejected == 144 do throwError "unexpected counts: {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/compound-syntax IR comparisons and {rejected} invalid-input rejections passed"
