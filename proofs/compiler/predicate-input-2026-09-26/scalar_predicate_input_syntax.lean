      ...
      for depth in [0, 1, 3] do
        ...
        for negations in [0, 1, 2] do
          for nondep in [false, true] do
            ...
            let source := helper wt.expr bt.expr wt.expr helperBody.expr (toWord calls.expr)
            let some func := extractScalarFunc `predicateInputSyntax (some "entry") functionType (wrap source) |
            ...
            let invalid : List Lean.Expr :=
              [helper wt.expr bt.expr word helperBody.expr (toWord calls.expr),
               helper word bt.expr wt.expr helperBody.expr (toWord calls.expr),
               helper wt.expr bt.expr (ResultType.identity wt).expr helperBody.expr (toWord calls.expr),
               helper (.app (.const ``Id [.zero]) (.const ``Nat [])) bt.expr
                 (.app (.const ``Id [.zero]) (.const ``Nat [])) helperBody.expr (toWord calls.expr),
               helper wt.expr word wt.expr helperBody.expr (toWord calls.expr),
            ...
            for body in invalid do
              ...
              rejected := rejected + 1
  unless comparisons == 2016 && rejected == 2448 do
    throwError "unexpected counts {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/predicate-input syntax comparisons and {rejected} invalid-input tests passed"
