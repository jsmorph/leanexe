...
run_elab do
          ...
          for flag in [false, true] do
            ...
            let invalid : List Lean.Expr :=
              [helper boolean bt.expr word helperBody.expr continuation,
               ...
               helper boolean bt.expr boolean helperBody.expr
                 (Step.yieldDirect (toWord (.app (.bvar 0) (.app (.bvar 0) literal)))),
               helper boolean bt.expr boolean helperBody.expr
  ...
  Lean.logInfo m!"{comparisons} native/Boolean-input step predicate syntax comparisons and {rejected} invalid-input tests passed"
