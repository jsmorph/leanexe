...
def word (x y : UInt64) : UInt64 :=
  (let value : Id UInt64 := x + y; (value : UInt64) == 0).toUInt64

def nested (x y : UInt64) : UInt64 :=
  (let flag : Id (Id Bool) := x == 0
   let value : Id (Id UInt64) := if (flag : Bool) then x + y else x - y
   let next : Id Bool := (value : UInt64) != 0
...
def helper (x y : UInt64) : UInt64 :=
  ...
  let f := fun flag : Bool =>
    (let saved : Id Bool := flag
     let value : Id UInt64 := if (saved : Bool) then x + y else x - y
     if _h : (saved : Bool) = outer then (value : UInt64) == x else (value : UInt64) != y).toUInt64
  f (y != 0)
...
def range (count seed : UInt64) : UInt64 := Id.run do
  ...
  for i in [:count.toNat] do
    let flag ← pure (let word : Id UInt64 := a + UInt64.ofNat i
                    let stop : Id (Id Bool) := (word : UInt64) % 7 == 0
                    (stop : Bool))
