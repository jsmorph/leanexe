#!/usr/bin/env node
// Review extraction of the branch compiler-proof corpus. GPU execution is omitted.
// Invocation: node compiler-review.js PINNED_CHECKOUT
'use strict';
const fs = require('node:fs');
const path = require('node:path');
const {spawnSync} = require('node:child_process');
const repo = path.resolve(process.argv[2]);
fs.mkdirSync(path.join(repo, 'build/wgsl'), {recursive:true});
...
try {
  ...
  const summary = {status:'pass', sourceCases:cases.length, executionReplayed:false,
    rejectedCases, independentMatrixProof:'pass', statementExecutionProofs:cases.length,
