...
\maketitle
\begin{abstract}
LeanExe compiles checked Lean declarations to WebAssembly through a restricted first-order extraction path.  The LeanExe subset is specified here by runtime typing and layout rules together with an implementation-indexed relation from checked declarations to WebAssembly execution.  The report separates kernel typing, static specialization, extraction acceptance, ownership analysis, binary validity, and artifact behavior.  It defines the internal and public type domains, proves two layout propositions, and states the numeric, collection, memory, and fixed-adapter semantics, including binary32 arithmetic and packed word storage.  It also states the proof obligation for checking a compilation against its Lean algorithm.  A source audit identifies incomplete runtime export-name rejection and a child-mask width obligation.  The reference IR evaluator has a narrower comparison domain than compiled execution.  Existing emitter, decoder, validator, packed-memory, and arithmetic theorems support their named boundaries.  An independent source operational semantics, mechanized acceptance judgments, and a general source-to-WebAssembly refinement theorem remain open.
\end{abstract}

\section{Scope and evidence}

The LeanExe subset is specified here by runtime typing and layout rules together with an implementation-indexed relation from checked declarations to WebAssembly execution.  An implementation-indexed relation is the graph of named compiler functions at a fixed source revision, composed with a stated target execution relation.  The independently stated typing rules describe runtime values and first-order code.  Shape-sensitive extraction premises still refer to compiler recognizers, so the report's acceptance definition depends on the implementation.

The source for this account is the specification checkpoint \longcode{df70a0a56dd909ce2a4685faba5498bdcaf01e81} of LeanExe~\cite{checkpoint}.  The compiler root pins Lean 4.34.0-rc2 at commit \longcode{6a10ac8c22beadecabdbb0919c2b50214762f91d}.  The proof workspace uses Talos, a Lean WebAssembly execution model and proof system, at revision \longcode{87e3aa5e8f6e6f3b3eb5e7e4c5aba43071002d47}.  This revision compares the earlier report checkpoint \longcode{2f3ec33f6e98ad98ac94df277c91e5319763cf9d} with the new checkpoint.  It adds binary32 operations and precision conversions, packed word access and construction, the revised evaluation and ownership rules, and the corresponding proof boundaries.  Repository citations identify the new checkpoint throughout.

The evidence consists of source inspection, the mathematical arguments in this report, and existing theorem declarations cited with their exact scope.  Report preparation ran no Lean build, compiler, artifact verifier, or runtime reproducer.  In particular, the two implementation concerns in Section~\ref{sec:audit} have source evidence and remain untested as compiled examples.  The language documents contain the broader source-operation inventory~\cite{typetheory,specification}.  Their floating-point inventory predates the new binary32 proofs.  For those operations this report uses the compiler definitions and theorem declarations at the fixed checkpoint~\cite{types,fp32proofs}.
...
Floating-point intrinsics & Add, subtract, multiply, and divide have type $\ty{Uw}\times\ty{Uw}\to\ty{Uw}$, and square root has type $\ty{Uw}\to\ty{Uw}$, for $w\in\{32,64\}$.\\
Precision conversion & Binary32-to-binary64 promotion has type $\ty{U32}\to\ty{U64}$.  Binary64-to-binary32 demotion has type $\ty{U64}\to\ty{U32}$.\\
...
Array map and modify & Body context extends $\Delta$ by $x:\tau$.  The map body has result type $\upsilon$, and the map operation returns $\ty{Array}\;\upsilon$.  Modify body has type $\tau$.\\
...
Packed words & A four-byte little-endian read has type $\ty{Bytes}\times\ty{Nat64}\to\ty{U32}$.  Generation takes a \code{Nat64} count and a direct body $i:\ty{Nat64}\vdash b:\ty{U32}$, returning \code{Bytes}.\\
...
Runtime intrinsics & Counter reads return \code{U64}.  Explicit release takes an accepted array or recursive root and returns \code{U64}, subject to \code{validateModuleReleases} (Section~\ref{sec:allocator}).\\
\bottomrule
\end{longtable}
...
For bytes $b$, the payload relation requires $n=|b|$ and $\mu[p:p+n]=b$.  Owner zero permits borrowed storage.  A nonzero owner identifies an allocation that contains the visible byte range and remains live while that range is used.  For an array $a:\ty{Array}\;\tau$, the header and elements satisfy
\[
...
The floating-point intrinsics retain binary32 encodings in \code{U32} and binary64 encodings in \code{U64}.  For width $w\in\{32,64\}$, the four binary operations and square root reinterpret words, execute the corresponding \code{f32} or \code{f64} instruction, and reinterpret the result.  The scalar ABI carries both widths in \code{i64}.  The binary32 sequence wraps to \code{i32} before reinterpretation and zero-extends the result.  Each arithmetic instruction rounds at its own position in the sequence.  The source type domain continues to represent floating-point values through integer words~\cite{fp32source,binary}.

The compiler recognizes \code{LeanExe.Float32} and \code{LeanExe.Float64} operation names.  It also recognizes the five \code{Wasm.IEEE64} operations from Talos.  The binary64 aliases use the same emitted instructions, allowing a source algorithm to name the arithmetic functions used by its execution model~\cite{types}.  Binary32 promotion to binary64 uses \code{f64.promote\_f32}, and binary64 demotion uses \code{f32.demote\_f64}.  Finite promotion preserves the value exactly.  Demotion applies binary32 rounding~\cite{wasm,binary}.

For width $w$, write $a_f=\operatorname{reinterpret}_{f_w}(a)$ and $b_f=\operatorname{reinterpret}_{f_w}(b)$.  For $p\in\{+,-,\times,/\}$, let $\operatorname{Wasm}_{p,w}(a_f,b_f)$ be the set of results permitted by the WebAssembly operation.  Define
\[
\operatorname{Fp}_{p,w}(a,b,r)
\iff
r\in\{\operatorname{reinterpret}_{i_w}(z)\mid
 z\in\operatorname{Wasm}_{p,w}(a_f,b_f)\}.
\]
Square root and precision conversion have corresponding unary relations.  WebAssembly permits a set of NaN results in applicable cases~\cite{wasm}.  A statement about every conforming engine must account for that relation.  Exact-word theorems in the pinned Talos model identify its deterministic arithmetic result.  Section~\ref{sec:sourcecorrespondence} states the completed binary32 source-model equalities and the remaining engine-level premise.
...
Accumulator replacement stages all new slots before releasing selected previous roots and copying the staged slots.  The first accumulator value is excluded from this release rule because source aliases may retain it.  Later release selection depends on fresh-owner and liveness analyses.  The emitted multi-slot assignment retains a shared fold computation when any result slot remains live.  Id binds materialize a fold result before a continuation selects its fields, and array map evaluates its body once per visited element before storing the result fields.  Nat-tail recursion materializes a used supported let binding once.  Explicit releases execute even when their counter results are unused.  For-in cleanup stages the next accumulator and stop condition before releasing eligible temporaries, preserving the returned owner slots~\cite{extract,storage,values,binary}.
...
\subsection{Packed word storage}

For a represented byte array $b$ and byte offset $o$ with $o+4\le |b|$, \code{LeanExe.Packed.getUInt32LE!} returns
\[
\sum_{j=0}^{3} b_{o+j}2^{8j}.
\]
The offset may be unaligned.  The emitter checks $o\le |b|$ and $4\le |b|-o$ before loading four bytes.  It traps when either check fails.  Address validity also requires the memory and nonwrapping premises stated above.  The pure Lean definition uses a panic at an invalid offset, so a successful-read source correspondence theorem uses the valid-offset premise~\cite{packedsource,binary}.

For count $n$ and a direct generator body $f$, \code{generateUInt32LE} computes the byte count $4n$ with checked Nat multiplication, allocates one byte array, and writes $f(0),\ldots,f(n-1)$ as four-byte little-endian words.  Empty generation evaluates no generator body.  The generated allocation and sequential stores implement the byte sequence produced by the Lean definition's repeated byte pushes.  Allocation requires a represented free list or enough space for a new block, and successful execution requires the resulting addresses to fit memory~\cite{packedsource,extract,binary}.  Section~\ref{sec:sourcecorrespondence} identifies the checked read and construction theorems.
...
Explicit source release requires \code{validateModuleReleases}.  An accepted release consumes a direct fresh allocation, a helper root identified as fresh, or a statically owner-zero array at its supported final-use position.  The direct-allocation cases include array map and concatenation.  An intervening fresh array with no owner fields cannot retain an earlier root, and the validator preserves the earlier root's release eligibility in that case~\cite{release}.  Automatic cleanup additionally uses result-root, borrowing, and liveness analyses.  The source definitions of the runtime counters and release all return zero, while generated execution reads and updates allocator state.  A source refinement statement that observes those operations needs their extended semantics.

For an exported heap result, extraction materializes the internal owner-bearing value before projecting its public slots.  Fields of a structured result therefore use the same evaluated owners and payloads~\cite{extract}.
...
The inspected layout predicates provide no general 64-slot bound for the ownership mask.  \code{heapChildMaskFromType} constructs a natural-number mask with a set bit at each owner position.  \code{Binary.i64Const} reduces a mask constant modulo $2^{64}$.  \code{coreReleaseInstrs} scans object slots and tests the stored mask through \code{i64.shr\_u}, whose shift count is modulo 64~\cite{values,binary,wasm}.  Consequently a general ownership proof needs a bound connecting the accepted layout to mask representation, or a verified wider representation.  This is a source-level proof obligation and potential implementation defect.  A compiled reproducer remains open.
...
\subsection{Per-compilation source correspondence}
\label{sec:sourcecorrespondence}

A per-compilation proof can establish that a selected generated module computes a named Lean function.  Fix a Lean function $F:A\to B$, the generated Talos module $W$, its selected entry and host environment, an input representation $\operatorname{Input}_A(s,a,x)$ relating store $s$ and argument slots $a$ to $x:A$, an output representation $\operatorname{Output}_B(s',r,y)$, and resource and ownership premises $P(s,x)$.  The required statement has the form
\[
\begin{split}
\forall s,a,x.\quad &P(s,x)\land\operatorname{Input}_A(s,a,x)\ \Longrightarrow\\
&\operatorname{TerminatesWith}_{W}(s,a,\lambda s',r.\,
  \operatorname{Output}_B(s',r,F(x))\land\operatorname{Preserves}(s,s',x)).
\end{split}
\]
Here \code{TerminatesWith} is the Talos total-execution judgment, and \code{Preserves} states the theorem's memory, input-lifetime, or allocator conclusions.  The method checks one proof for each selected compilation, with its own module identity and premises.  A separate exact-byte package can connect $W$ to a serialized binary through decoding, validation, and translation equality.  This separates source agreement from binary identity while allowing both in one result.

The checkpoint contains five unrestricted binary32 equalities:
\[
\begin{aligned}
\code{LeanExe.Float32.}p\code{Bits}(a,b)&=\code{Wasm.IEEE32.}p(a,b),
&&p\in\{\code{add},\code{sub},\code{mul},\code{div}\},\\
\code{LeanExe.Float32.sqrtBits}(a)&=\code{Wasm.IEEE32.sqrt}(a).
\end{aligned}
\]
Their declarations are \longcode{Project.ProofKit.F32Add.add\_eq}, \longcode{F32Sub.sub\_eq}, \longcode{F32Mul.mul\_eq}, \longcode{F32Div.div\_eq}, and \longcode{F32Sqrt.sqrt\_eq}, where the last four share the \code{Project.ProofKit} prefix~\cite{fp32proofs}.  They quantify over all input words, including zeros, subnormals, infinities, and NaNs.  The pinned Lean definitions of these operations reduce to \code{Float32.Model}.  The proofs identify that logical model's words with the deterministic Talos model.  Both models canonicalize their arithmetic NaN results.  The emitted WebAssembly instructions retain the standard's permitted NaN outcomes, so an all-engine exact-word claim needs a further premise or proof concerning exceptional results.  The binary32/binary64 precision conversions have implementation and test evidence at this checkpoint~\cite{fp32tests}.  Their source-model correspondence proofs remain open.

\longcode{Project.PackedRead.Spec.readWord\_exact} proves that the generated read returns the Lean function's word and preserves the store under byte-array representation and valid-offset premises~\cite{packedproofs}.  \longcode{Project.PackedGenerate.Spec.makeWords\_with\_reuse\_exact} proves exact source bytes for the compiled \code{makeWords} example, including first-fit free-block reuse and new-block allocation.  Its count, free-list, address, and page-capacity hypotheses state the permitted memory states.  \longcode{Project.ProofKit.PackedGenerateLoop.program\_spec} supplies the reusable sequential generator proof with a premise for the generator body.  Allocation-preservation lemmas prove that disjoint input bytes remain represented through allocation and output writes~\cite{packedproofs}.  Arbitrary compiled generator bodies still need their own execution proofs.

A completed four-byte transformer example, \longcode{Project.TinyGpt2Infer.Spec.infer\_exact}, proves the full generated inference result against the named algorithm \code{Project.TinyGpt2.infer}, with all 256 raw-word logits, preserved weights, and unchanged page count under its heap and input premises~\cite{tinyproof}.  The pretrained 124-million-parameter GPT-2 implementation uses binary32 and packed storage.  At this checkpoint its full generated-module/source correspondence remains open.  Its arithmetic and packed-memory lemmas provide reusable components for that proof.  The complete-model execution tests and the checked component theorems have distinct scopes~\cite{gpt2plan}.
...
A reference-evaluator comparison concerns its stated input and evaluator domain.  An exact-artifact theorem concerns its embedded binary and behavioral premises.  The implemented acceptance graph, the target execution relation, and the named artifact proofs each identify a concrete subject.  A general compiler-correctness theorem would prove those connections for every accepted input.  The per-compilation method proves the required connection for each selected module and Lean algorithm under explicit premises.

\begin{thebibliography}{99}
\small
\bibitem{checkpoint} LeanExe contributors. \emph{LeanExe source checkpoint}. Commit df70a0a56dd909ce2a4685faba5498bdcaf01e81, 2026. \url{https://github.com/jsmorph/leanexe/tree/df70a0a56dd909ce2a4685faba5498bdcaf01e81}.
\bibitem{typetheory} LeanExe contributors. \emph{LeanExe runtime type documentation}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/docs/leanexe-type-theory.md}.
\bibitem{specification} LeanExe contributors. \emph{Formal Specification of LeanExe Compilation and Execution}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/docs/leanexe-formal-specification.md}.
...
\bibitem{types} LeanExe contributors. \emph{Type and layout recognition}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Types.lean}.
\bibitem{extract} LeanExe contributors. \emph{Expression and module extraction}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Core.lean}.
\bibitem{report} LeanExe contributors. \emph{Declaration reports and classification}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Report.lean}.
\bibitem{patterns} LeanExe contributors. \emph{Pattern and monad recognition}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Patterns.lean}.
\bibitem{structural} LeanExe contributors. \emph{Structural recursion recognition}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/StructuralRec.lean}.
\bibitem{demand} LeanExe contributors. \emph{Demand analysis and structural equality}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Demand.lean}.
\bibitem{storage} LeanExe contributors. \emph{Storage and parameter lowering}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Storage.lean}.
\bibitem{values} LeanExe contributors. \emph{Value layouts, owner masks, and liveness}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Values.lean}.
\bibitem{release} LeanExe contributors. \emph{Explicit release checking}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/ReleaseCheck.lean}.
\bibitem{binary} LeanExe contributors. \emph{Production WebAssembly lowering and binary emission}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Wasm/Binary.lean}.
\bibitem{ir} LeanExe contributors. \emph{IR syntax and reference evaluation}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/IR/Core.lean}.
\bibitem{ireval} LeanExe contributors. \emph{Reference-evaluator entry restriction}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Extract/Eval.lean}.
\bibitem{wasm} WebAssembly Community Group. \emph{WebAssembly Core Specification}. Numeric semantics and validation. \url{https://webassembly.github.io/spec/core/exec/numerics.html}; \url{https://webassembly.github.io/spec/core/valid/instructions.html}. Accessed 18 September 2026.
\bibitem{certificates} LeanExe contributors. \emph{Scalar emitter certificates}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Wasm/ScalarCertificate.lean}.
\bibitem{artifact} LeanExe contributors. \emph{Artifact Verification Format and Talos proof inventory}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/docs/artifact-format.md}; \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/proofs/talos/README.md}. Theorem sources: \longcode{Project/Artifact/Binary/Proof/Decode.lean} and \code{Validate.lean}, namespace \code{Wasm.Binary.Proof}.
\bibitem{fp32source} LeanExe contributors. \emph{Binary32 source operations}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Float32.lean}.
\bibitem{fp32tests} LeanExe contributors. \emph{Binary32 arithmetic and conversion tests}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/test/f32_bits.js}.
\bibitem{packedsource} LeanExe contributors. \emph{Packed word source operations}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/LeanExe/Packed.lean}.
\bibitem{fp32proofs} LeanExe contributors. \emph{Binary32 source correspondence proofs}. Directory \longcode{proofs/talos/lean/Project/ProofKit}, files \code{F32Source.lean}, \code{F32Add.lean}, \code{F32Sub.lean}, \code{F32Mul.lean}, \code{F32Div.lean}, and \code{F32Sqrt.lean}. \url{https://github.com/jsmorph/leanexe/tree/df70a0a56dd909ce2a4685faba5498bdcaf01e81/proofs/talos/lean/Project/ProofKit}.
\bibitem{packedproofs} LeanExe contributors. \emph{Packed read, construction, and memory proofs}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/proofs/talos/lean/Project/PackedRead/Spec.lean}; \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/proofs/talos/lean/Project/PackedGenerate/Spec.lean}; ProofKit files \code{PackedGenerateLoop.lean} and \code{PackedAllocationMemory.lean} at the same checkpoint.
\bibitem{tinyproof} LeanExe contributors. \emph{Four-byte transformer inference theorem}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/proofs/talos/lean/Project/TinyGpt2Infer/Inference.lean}.
\bibitem{gpt2plan} LeanExe contributors. \emph{Pretrained GPT-2 implementation and proof plan}. \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/plans/gpt2-124m.md}; \url{https://github.com/jsmorph/leanexe/blob/df70a0a56dd909ce2a4685faba5498bdcaf01e81/data/gpt2-124m/README.md}.
\end{thebibliography}
\end{document}
