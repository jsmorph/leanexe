...
The source for this account is the specification checkpoint \code{2f3ec33f6e98ad98ac94df277c91e5319763cf9d} of LeanExe~\cite{checkpoint}.  The compiler root pins Lean 4.34.0-rc2 at commit \code{6a10ac8c22beadecabdbb0919c2b50214762f91d}.  The proof workspace pins Talos revision \code{87e3aa5e8f6e6f3b3eb5e7e4c5aba43071002d47}.  Source comparison at report preparation found no changes to the two specification documents or the inspected extraction, IR, and binary-emission files between that checkpoint and \code{7c7277d1e1ab8bc4ee5b4a014a8b975abb8b8054}.  The citations identify the fixed checkpoint throughout.
...
The implementation predicates are \code{valueLayout?}, \code{arrayElementLayout?}, \code{supportedPublicArrayElementType}, \code{supportedParamAbiType}, and \code{supportedResultAbiType}~\cite{types}.  Internal array-element layout and internal value layout admit the same type constructors.  Public arrays contain internal element layouts in memory, even though the public array value crosses the entry interface as one pointer.
...
\code{specializeInlineValue} substitutes static inputs and retains runtime binders.  Bounded normalization reduces class-method projections and selected transparent applications.  \code{blocksTransparentSpecialization} protects the primitive, matcher, recursor, and monad families that have dedicated lowering.  A generic source helper can therefore have accepted concrete inline uses while its unspecialized type has no retained runtime signature.
...
The five binary64 operations retain raw \code{U64} arguments and results.  For $p\in\{+,-,\times,/\}$, define
\[
\operatorname{Fp}_p(a,b,r)
\iff
r\in\operatorname{reinterpret}_{i64}
 \bigl(\operatorname{Wasm}_{p,64}(operatorname{reinterpret}_{f64}(a),
                                \operatorname{reinterpret}_{f64}(b))\bigr).
\]
...
\]
The fixed command modes use their corresponding environment-extraction functions.  \code{compileEnvironmentWithEntryModeDetailed} checks the entry, collects the reachable namespace-rooted declarations, normalizes with its fixed fuel, and collects synthetic helpers.  It extracts once with empty fresh-result summaries, derives those summaries, validates explicit releases, and extracts again using the summaries~\cite{extract}.  Report classification and complete extraction therefore have different result objects.
...
The separation of these obligations prevents two unsupported inferences: successful execution of the reference evaluator does not establish every compiled outcome, and an exact-artifact behavioral theorem does not establish correctness of every compilation.  The implemented acceptance graph, the target execution relation, and the named artifact proofs each identify a concrete subject.  A complete compiler-correctness result requires proved connections between those subjects under explicit premises.
