...
This corrected evidence record names the declaration's namespace, `LeanExe.Extract.Core`.  The preceding record used the file-derived prefix `Extract.Types`, which is not its Lean namespace.  The report uses the unqualified declaration name and its source-file citation.
