...
\end{equation}
For a finite binary32 value $y$, define $M(y)=2^{149}|y|$, a nonnegative integer that expresses its magnitude in units of $2^{-149}$.  The quotient-range premise for an integer exponent $b$ with $24\leq b\leq275$ is
\begin{equation}
 M(x)2^{149}\leq M(s)2^b,
 \qquad\text{equivalently}\qquad |x/s|\leq2^{b-149}.
 \label{eq:divisionrange}
\end{equation}
Under this premise, \code{F32DivisionBounds.div\_real\_error} bounds the division error by
\begin{equation}
 \varepsilon_{\rm div}(b)=\frac{2^{b-24}}{2^{149}}=2^{b-173}.
 \label{eq:divisionepsilon}
\end{equation}
Thus $b=156$ asserts $|x/s|\leq128$ and gives $\varepsilon_{\rm div}(156)=2^{-17}$.  In Equation~\eqref{eq:reconstruct}, the half-unit term comes from nearest-even rounding, the maximum term covers clipping, and $\varepsilon_{\rm div}(b)$ covers the initial binary32 division.  The quotient-range assumption is explicit in \proofref{ProofKit/QuantizedScalarError.lean}{\code{QuantizedScalarError.reconstruction}}.  The proof accounts for rounded scale selection and finite division.
