...
\maketitle
\begin{abstract}
LeanExe aims to execute an algorithm written in Lean with a checked proof that the emitted program computes the source result.  This work applies that goal to pretrained GPT-2 inference, where floating-point arithmetic, packed tensors, cached state, and memory management must agree across a complete sequence of token calls.  We present a checked proof connecting a WebAssembly artifact of 19,083 bytes to a Lean specification of cached GPT-2 with 124,439,808 parameters and a 128-token context.  In the semantics of the WebAssembly interpreter and proof library Talos, written in Lean, the decoded artifact terminates and returns exactly the cache and logit bytes of the Lean token-step recurrence for every weight array of the required length and every valid token sequence of length at most 128.  The proof establishes binary decoding, validation, and equality with the execution model, then composes initialization, packed input representation, all twelve transformer blocks, vocabulary projection, allocation sufficiency, and buffer releases.  Our Talos extensions define binary32 and binary64 arithmetic through integer operations and explicit rounding.  Arithmetic lemmas connect Lean's logical Float32 operations to Talos for all input words.  The standalone WebAssembly runtime Wasmtime executes the module with arithmetic NaN canonicalization enabled.  Tests against CPU PyTorch compare 6,432,896 logits across all supported context lengths, with a maximum observed absolute difference of 0.001450, and produce text completions.  The final theorem uses three standard logical axioms.  Applying the formal result to native execution relies on the interpretation of the formal semantics, the host's byte I/O, and Wasmtime.  Automatic checks of the command-line artifact's identity and the combined theorem's axiom policy remain open.
\end{abstract}

\section{Goal, motivation, and outcome}

\subsection{Verified execution of a Lean algorithm}

LeanExe's overall goal is to compile an algorithm written in Lean into a standalone program and check a proof that the emitted program computes the specified result.  The Lean definition serves as both compiler input and algorithm specification.  The intended proof connects a particular program's emitted instructions and memory operations to that definition~\cite{subset}.  This development applies that method to a pretrained language model that can generate text from a command-line prompt.

GPT-2 requires the verification method to handle a complete stateful numerical computation.  Each token step combines FP32 matrix products, normalization, attention over stored keys and values, activation routines, and vocabulary projection.  The implementation also allocates and releases buffers while retaining the weights and cache for the next call~\cite{cached,entry}.  The work was undertaken to establish that these computations and state changes compose into successful execution of the Lean algorithm throughout a 128-token session.  Runtime-supplied weights make the result applicable to every weight array of the required size, so changing their values does not require a new execution proof.

The required agreement is exact equality of the cache and logit bytes specified by the Lean source.  That source fixes the floating-point operation order and numerical routines, making their word-level behavior part of the algorithm.  A proof for the compiled artifact must therefore connect scalar arithmetic, tensor layout, control flow, and memory management.  Numerical comparisons with PyTorch provide separate evidence about the implementation's behavior on a pretrained checkpoint and recorded inputs.  The formal proof covers the source-defined computation throughout the stated input domain.

\subsection{GPT-2 inference and the artifact execution theorem}

The development produced a command-line GPT-2 implementation and a checked proof from its frozen WebAssembly binary to the Lean cached-inference algorithm.  The command loads pretrained weights, accepts a prompt, and generates text~\cite{guide}.  The proof covers the inference calls from encoded tokens and weight bytes to the resulting cache and logits.  The host supplies tokenization, sampling, and text decoding.  The proof includes the binary-to-execution-model connection, while its application to the running command retains the host and runtime assumptions stated below.
...
The implementation loads the pretrained GPT-2 checkpoint as external data.  The theorem quantifies over arbitrary bytes of the required weight length, including exceptional FP32 encodings.  The command-line demonstration separately checks the identity of the pretrained checkpoint and rejects a non-finite returned logit before sampling~\cite{driver}.  Those host checks belong to the demonstration's behavior.
