...
The proof carries memory capacity of 65,536 WebAssembly pages and a page-count bound of 65,536, with 65,536 bytes per page.  The $2.5$ GiB expression above bounds the charged heap top.  The formal page-count bound remains 4 GiB.  The measured linear-memory size in Section~\ref{sec:tests} is a separate observation.  Host allocation success within the modeled capacity and sufficient native execution resources belong to the runtime assumptions.
...
The canonical-mode run took 100.2 seconds and reached 1,107,361,792 bytes of WASM linear memory.  Its allocation counters recorded 33,025 allocations and 33,023 frees, leaving the weights and current cache.  The allocator reuses whole freed blocks.  The final cache contains $128\times73{,}728=9{,}437{,}184$ payload bytes.  The test also checks the four rejected-input conditions, cache reset and replay, and byte-exact agreement between cached and full-prefix WASM logits for the nine-token story prompt~\cite{tests,testcode}.  These timings describe the recorded local runs and support execution feasibility.  They provide no cross-system speed comparison.
...
From a prepared repository checkout, the WASM and PyTorch one-liners are:
