...
This report uses the drone branch for the planner and its proofs, and the current main branch for the account of LeanExe's compiler-correctness work.  The cited repository revisions distinguish these subjects.  The drone execution theorem concerns its generated WASM model.  A theorem identifying a decoded drone binary with that model remains a separate verification step.  The general compiler theorem on main covers the language admitted by \path{compile-arithmetic}; the heap-using drone program follows the per-program execution-proof route~\cite{drone-record,compiler-correctness}.
...
For $n>1$, all $T_i$ are positive.  The cumulative intervals cover $[0,t_f]$.  \path{WholeFlight.compute_normalized_cover} expresses any flight time as $t_i+sT_i$ for some segment and $s\in[0,1]$.  \path{Corridor.height_on_segment} identifies the corridor at the actual horizontal point, and \path{compute_spatial_segment} proves clearance there.  This connection ensures that the clearance theorem refers to the spatial terrain below the vehicle, even though horizontal progress $q(s)$ differs from normalized time $s$.
...
\begin{align*}
 ...
 P&\leq\operatorname{memoryCap}(M,0).
\end{align*}
The general predicate also bounds $P$ by 65,536.  Thus 64 MiB is a limit on total linear memory, including the caller's existing heap.  The required headroom is a conservative allocation budget within that limit.
...
The development record documents a composition error caught during proof construction: top-level \textup{\texttt{compute}} passes the terrain with owner zero, while earlier history and unwind lemmas expected an owned terrain pointer.  The corrected frames use the borrowed-input convention.  Separate preservation lemmas then carry the terrain through allocations and releases.  The distinction is operational: a nonzero data pointer and a zero ownership marker describe a readable caller-owned array~\cite{drone-record}.

Small call-preparation and return lemmas also reduced elaboration boundaries during the proof.  For example, the history append call was separated from parameter preparation, and the final nested branch needed an explicit last-instruction boundary.  These changes retained the execution statements and made the intermediate frame equalities explicit.  The principal audits use only \path{propext}, \path{Classical.choice}, and \path{Quot.sound}, as applicable.
...
The pinned setup uses Lean 4.34.0-rc2 and the repository's Talos dependency.  The repository runner enforces the local resource policy and serializes Lean work.  The proof build checks the whole-flight source result and the compiled specification in the same dependency graph.  Its printed axiom dependencies allow review of the final result's logical assumptions.  Main's compiler theorem has its own \texttt{tools/arithmetic-check.js proof} command on that branch.  The report reads that theorem and its retained integration evidence without changing the drone checkout to the main compiler.
...
\begin{tabular}{lrr}
\toprule
Terrain & Traversal ticks & Sampled altitude excess\\
\midrule
Flat & 90,240 & 0\\
Broad plateau & 90,240 & 300\\
Rounded hill & 90,240 & 850\\
Repeated ridges & 90,240 & 800\\
\bottomrule
\end{tabular}
...
Finite motion primitives and dynamic programming are established planning methods~\cite{lavalle}.  This development fixes a small primitive family so that exact word arithmetic, continuous polynomial safety, and compiled execution can be connected in one proof chain.  Its contribution is that connection for the implemented planner and its returned array.  The report supplies the equations, executable acceptance tests, invariants, memory assumptions, and named theorem endpoints needed to assess the claim.

\appendix
\section{Claim-to-theorem guide}

All drone names in Table~\ref{tab:theorems} are relative to \path{Project.Drone}.  The cited proof directory contains their statements and dependencies~\cite{drone-proofs}.  Compiler results use the separate \path{Project.Compiler.ArithmeticModule} namespace on main~\cite{compiler-source}.
...
General compiler admission and correctness & \path{Project.Compiler.ArithmeticModule.compileEnvironment_correct}\\
Successful compilation implies correctness & \path{Project.Compiler.ArithmeticModule.compileEnvironment_sound}\\
\bottomrule
\end{longtable}
\normalsize
