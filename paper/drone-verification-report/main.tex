...
\section{End-to-end verification for practical applications}
\label{sec:context}

LeanExe's goal is end-to-end verification of software for practical applications: a checked connection from the required behavior to the instructions that execute it.  The drone example computes a flight path from terrain elevations.  Its proofs cover the mathematical path and the program that constructs its waypoint array, including integer arithmetic, allocation, copying, and release.

\begin{figure}[!ht]
\centering
\includegraphics[width=.98\linewidth]{figures/ridges.pdf}
\caption{A compiled run over repeated ridges.  The gray region shows terrain, the dashed line gives the required clearance floor, and the blue curve shows drone elevation.  Dots mark the returned waypoints.  The curve follows the planner's prescribed motion between them.}
\label{fig:ridges}
\end{figure}

Figure~\ref{fig:ridges} uses the output of the WebAssembly planner executed in Wasmtime on a 15-station terrain profile.  The route covers 1,400 distance units in $107\tfrac{3}{7}$ seconds.  The planner minimizes travel time over its finite altitude and speed choices, then minimizes altitude excess above the clearance floor.  The selected path stays high across the valleys while satisfying the model's speed and acceleration limits.  Native Lean returns the same waypoint words for this input.

The drone proofs establish continuous clearance and motion bounds for an ideal point mass and exact execution of the planner in the generated WebAssembly model under stated memory assumptions.  Relating a decoded drone binary to that model remains a separate proof obligation.  The compiler theorem now on main provides a source-to-binary connection for its admitted scalar and bounded-loop language.  A physical deployment would also require models of terrain uncertainty, vehicle dynamics, and tracking error.
...
Figures~\ref{fig:ridges}, \ref{fig:flat-plateau}, and~\ref{fig:hill} show four additional runs of the compiled planner.  Each input contains 15 stations spanning 1,400 distance units.  The figures use the returned altitude and speed words, evaluated with the prescribed continuous primitives at 101 normalized times per segment.  Markers identify the returned waypoints.  The filled region is the piecewise-linear terrain, and the dashed curve is the required corridor.  The plotted horizontal coordinate follows the primitive's progress function.
...
\caption{Fresh WASM output on a rounded hill.  The planner selects a path above the corridor where doing so preserves traversal time.  This plot shares its axis limits with the other 15-station runs, including the repeated-ridges run in Figure~\ref{fig:ridges}.}
\label{fig:hill}
\end{figure}
