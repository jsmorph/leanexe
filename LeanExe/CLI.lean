...
def usage : String :=
  String.intercalate "\n" [
    ...
    "  lean-wasm compile-wasi-api --module <module> --entry <name> --out <path>",
...
def dispatch : List String → IO UInt32
  ...
  | ["compile-wasi-api", "--module", moduleName, "--entry", entryName, "--out", out] =>
      compileBytesResult
        (commandContext "compile-wasi-api"
          [("module", moduleName), ("entry", entryName), ("output", out)])
        out
        (LeanExe.Extract.Core.WasiCompilation.compile moduleName entryName)
        LeanExe.Wasm.Binary.CoreWasm.Wasi.moduleBytes
