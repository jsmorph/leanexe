...
def isWasiMonad (expr : Expr) : Bool :=
  match appFnArgs expr with
  | (.const ``LeanExe.Wasi.Action _, []) => true
  | (.const ``BaseIO _, []) => true
  | _ => false

def wasiPayload? (expr : Expr) : Option Expr :=
  match expr.consumeMData with
  | .app monad payload => if isWasiMonad monad then some payload else none
  | _ => none

def resultTypeAtom? (env : Environment) (expr : Expr) : Option Ty :=
  match wasiPayload? expr with
  | some payload => typeAtom? env payload
  | none => typeAtom? env expr

def wasiPrimitiveName (name : Name) : Bool :=
  LeanExe.Wasi.primitives.any (fun p => p.sourceName == name)
...
def supportedFunction? (env : Environment) (info : ConstantInfo) : Option Signature :=
  if info.isUnsafe || info.isPartial || (info.value?.isNone && !wasiPrimitiveName info.name) then
    none
  else
...
def compilerPrimitiveName (name : Name) : Bool :=
  (f64BinaryPrimitive? name).isSome || f64SqrtPrimitiveName name ||
    ...
    packedPrimitiveName name || wasiPrimitiveName name
...
def blocksTransparentSpecialization (name : Name) : Bool :=
  ...
  wasiPrimitiveName name || name == ``ite || name == ``dite || name == ``WellFounded.fix ||
