...
def knownExternal? (name : Name) : Option Classification :=
  ...
  if effectRoots.contains root then
    ...
    some { status := "implemented", reason := "pure Id do-notation plumbing erased by the generic compiler fragment" }
      ...
      ``Array.append, ``Array.map, ``Array.find?, ``Array.findIdx?, ``Array.any, ``Array.all, ``Array.foldl, ``Array.insertIdx, ``Array.insertIdx!, ``Array.insertIdxIfInBounds, ``Array.modify,
