...
def knownExternal? (name : Name) : Option Classification :=
  ...
  if effectRoots.contains root then
    ...
    some { status := "implemented", reason := "supported first-order do-notation, loop, and functor applications" }
      ...
      ``Array.append, ``Array.map, ``Array.find?, ``Array.findIdx?, ``Array.any, ``Array.all, ``Array.filter, ``Array.foldl, ``Array.foldlM, ``Array.foldr, ``Array.insertIdx, ``Array.insertIdx!, ``Array.insertIdxIfInBounds, ``Array.modify,
    ...
    some { status := "implemented", reason := "internal Option primitive in the generic compiler fragment" }
      ...
      ``Except.isOk].contains name then
  ...
  else if [``Nat.succ, ``Nat.pred, ``Nat.beq, ``Nat.blt, ``Nat.ble].contains name then
