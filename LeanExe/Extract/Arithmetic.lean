  ...
  if info.isUnsafe || info.isPartial then
    ...
    if reservedExportNames.contains exportName then
      ...
      let some func := extractScalarFunc entry (some exportName) info.type source
        | .error "unsupported scalar source: expected UInt64 arguments and result, supported let/local-function bindings, standard Id do operations, arithmetic, comparisons, or one unit-step range loop with a UInt64 accumulator"
