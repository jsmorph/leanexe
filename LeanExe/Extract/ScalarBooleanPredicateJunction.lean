...
theorem extractBooleanLocalWith_none_of_predicate_absent
    {locals : List ScalarBinding} {value : BooleanLocal} {index : Nat}
    (member : index ∈ value.functions)
    (absent : locals[index]?.bind ScalarBinding.predicateFunction? = none)
    (compile : (operand : Lean.Expr) → operand ∈ value.operands → Option LeanExe.IR.Expr) :
    extractBooleanLocalWith locals value compile = none := by
  cases found : extractBooleanLocalWith locals value compile with
  | none => rfl
  | some condition =>
    obtain ⟨function, present⟩ := scalarPredicateFunction_lookup
      ((extractBooleanLocalWith_variables found).functions index member)
    have lookup := congrArg (fun binding => binding.bind ScalarBinding.predicateFunction?) present
    simp [absent, ScalarBinding.predicateFunction?] at lookup
