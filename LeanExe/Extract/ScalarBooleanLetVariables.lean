...
/-- A word-bound slot cannot be requested as a Boolean flag. -/
def booleanWordLetLookup (indices : List Nat) (noLocal : 0 ∉ indices)
    (outer : (index : Nat) → index ∈ booleanLetVariables indices → Option LeanExe.IR.Expr) :
    (index : Nat) → index ∈ indices → Option LeanExe.IR.Expr
  | 0, member => False.elim (noLocal member)
  | index + 1, member => outer index ((mem_booleanLetVariables indices index).mpr member)

theorem booleanWordLetLookup_accepts (indices : List Nat) (noLocal : 0 ∉ indices)
    (outer : (index : Nat) → index ∈ booleanLetVariables indices → Option LeanExe.IR.Expr)
    (total : ∀ index member, ∃ target, outer index member = some target) :
    ∀ index member, ∃ target, booleanWordLetLookup indices noLocal outer index member = some target := by
  intro index member
  cases index with
  | zero => exact False.elim (noLocal member)
  | succ index => exact total index _

theorem booleanWordLetLookup_external (indices : List Nat) (noLocal : 0 ∉ indices)
    (outer : (index : Nat) → index ∈ booleanLetVariables indices → Option LeanExe.IR.Expr)
    (total : ∀ index member, ∃ target, booleanWordLetLookup indices noLocal outer index member = some target) :
    ∀ index member, ∃ target, outer index member = some target := by
  intro index member
  exact total (index + 1) ((mem_booleanLetVariables indices index).mp member)

theorem booleanWordLetLookup_correct (indices : List Nat) (noLocal : 0 ∉ indices)
    (outer : (index : Nat) → index ∈ booleanLetVariables indices → Option LeanExe.IR.Expr)
    (booleans : Nat → Bool) (store : LeanExe.IR.ScalarStore)
    (outerMeanings : ∀ index member expression, outer index member = some expression →
      expression.ScalarEval store (booleans index).toUInt64 store) :
    ∀ index member expression, booleanWordLetLookup indices noLocal outer index member = some expression →
      expression.ScalarEval store (booleanLetBooleans false booleans index).toUInt64 store := by
  intro index member expression found
  cases index with
  | zero => exact False.elim (noLocal member)
  | succ index => exact outerMeanings index _ expression found

theorem booleanWordLetLookup_holds (P : LeanExe.IR.Expr → Prop) (indices : List Nat) (noLocal : 0 ∉ indices)
    (outer : (index : Nat) → index ∈ booleanLetVariables indices → Option LeanExe.IR.Expr)
    (outerHolds : ∀ index member expression, outer index member = some expression → P expression) :
    ∀ index member expression, booleanWordLetLookup indices noLocal outer index member = some expression → P expression := by
  intro index member expression found
  cases index with
  | zero => exact False.elim (noLocal member)
  | succ index => exact outerHolds index _ expression found
