...
inductive BooleanAction where
  ...
  | pure (body : BooleanAction)
  | run (body : BooleanAction)
...
def leaf : BooleanAction → BooleanLocal
  ...
  | .pure body | .run body | .metadata _ body => body.leaf

def expr : BooleanAction → Lean.Expr
  | .value expression => expression.expr
  | .pure body => BooleanIdentity.pure body.expr
  | .run body => BooleanIdentity.run body.expr
...
theorem operands_size (action : BooleanAction) {operand : Lean.Expr}
  ...
  | pure body ih => have h := ih member; simp_all [leaf, expr, BooleanIdentity.pure]; omega
  | run body ih => have h := ih member; simp_all [leaf, expr, BooleanIdentity.run]; omega
