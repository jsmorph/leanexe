...
inductive Eval : Lean.Expr → List Scalar.Value → UInt64 → Prop where
  | range (count : EvalWith countExpr values stop) (initial : EvalWith initialExpr values start)
      ...
      Eval (Range.call countExpr initialExpr indexName accumulatorName indexBi accumulatorBi body)
...
theorem Supported.evaluates {types : List Scalar.BindingKind} {expr : Lean.Expr}
  ...
  | range count initial step =>
    ...
    exact ⟨iterate f stop.toNat 0 start, .range hstop hstart
