...
structure PropositionGuard where
  value : Guard
  nonboolean : value.hasBooleanCondition = false
  deriving Repr
...
theorem not_boolean_condition (guard : PropositionGuard) (expression : Lean.Expr) :
    guard.condition ≠ .app (.app (.app (.const ``Eq [.succ .zero]) (.const ``Bool [])) expression)
      (.const ``Bool.true []) := by
  rcases guard with ⟨value, nonboolean⟩
  cases value with
  | literal value =>
    cases value with
    | proposition n value =>
      cases n with
      | zero => cases value <;> simp [condition, Guard.condition, GuardLiteral.condition,
          GuardNegation.condition, GuardLiteral.propositionExpr]
      | succ n => simp [condition, Guard.condition, GuardLiteral.condition, GuardNegation.condition]
    | boolean m n value =>
      cases m with
      | zero => simp [Guard.hasBooleanCondition] at nonboolean
      | succ m => simp [condition, Guard.condition, GuardLiteral.condition, GuardNegation.condition]
  | compare op a b =>
    cases op <;> simp [Guard.hasBooleanCondition] at nonboolean
    all_goals simp [condition, Guard.condition, Comparison.condition]
  | junction n op a b =>
    cases n with
    | zero => cases op <;> simp [condition, Guard.condition, GuardNegation.condition, Junction.condition]
    | succ n => simp [condition, Guard.condition, GuardNegation.condition]
  | boolean m n op a b =>
    cases m with
    | zero => simp [Guard.hasBooleanCondition] at nonboolean
    | succ m => simp [condition, Guard.condition, GuardNegation.condition]
