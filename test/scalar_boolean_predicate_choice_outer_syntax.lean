...
run_elab do
          ...
          for unequal in [false, true] do
            let call (argument : Lean.Expr) := toWord
              (form.local negations unequal
                (.predicate negations 2 argument) (.literal 0 false)
  ...
  Lean.logInfo m!"{comparisons} native/outer Boolean-input predicate choice syntax comparisons and {rejected} invalid-input tests passed"
