...
async function main(){
 const {cell,side,flux}=await import('../tools/euler-2d-oracle.mjs');
 const reference=words=>{try{const q=words.map(value),out=cell(q[0],q.slice(1,5),q.slice(5,9),q.slice(9));return[0n,...[...out.state,out.p,out.alpha,out.courant].map(bits)];}catch{return[1n,0n,0n,0n,0n,0n,0n,0n];}};
 ...
 for(const [op,count]of [['sub',5],['div',3],['mul',12],['add',5],['sqrt',1]])assert.equal((wat.match(new RegExp('f64\\.'+op+'\\b','g'))||[]).length,count,'WAT '+op);
 ...
 cases.push(['updated state fails sufficient domain after valid inputs and CFL',[bits(.25),...near,...near,...advecting],1]);
 ...
 for(const badState of [[0n,0n,0n,bits(2.5)],[one,0n,0n,one],[one,one+1n,0n,bits(2.5)],[one,0n,(one+1n)|sign,bits(2.5)],[1n,1n,1n,2n]])for(let slot=0;slot<3;slot++){const q=[ratio,...a,...a,...a];q.splice(1+4*slot,4,...badState);cases.push(['bad state '+slot,q,1]);}
