...
def metadata : Manifest :=
  { schemaVersion := 1,
    ...
    dimensions := { rows := 1, cols := 1, inner := 1 },
    bindings := { group := 0, a := 0, b := 1, c := 2 },
    buffers := { a := { elements := 1, bytes := 4 },
                 b := { elements := 1, bytes := 4 },
                 c := { elements := 1, bytes := 4 } },
    workgroupSize := [8, 8, 1],
    dispatchWorkgroups := [1, 1, 1] }
...
def tokenHints : List String := ["const", "M", ":", "u32", "=", "1u", ";", "const", "N", ":", "u32", "=", "1u", ";", "const", "K", ":", "u32", "=",
 "1u", ";", "@", "group", "(", "0", ")", "@", "binding", "(", "0", ")", "var", "<", "storage", ",", "read", ">", "a",
