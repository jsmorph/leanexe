...
def metadata : Manifest :=
  { schemaVersion := 1,
    ...
    dimensions := { rows := 1, cols := 256, inner := 4 },
    bindings := { group := 0, a := 0, b := 1, c := 2 },
    buffers := { a := { elements := 4, bytes := 16 },
                 b := { elements := 1024, bytes := 4096 },
                 c := { elements := 256, bytes := 1024 } },
    workgroupSize := [8, 8, 1],
    dispatchWorkgroups := [32, 1, 1] }
...
def tokenHints : List String := ["const", "M", ":", "u32", "=", "1u", ";", "const", "N", ":", "u32", "=", "256u", ";", "const", "K", ":", "u32", "=",
 "4u", ";", "@", "group", "(", "0", ")", "@", "binding", "(", "0", ")", "var", "<", "storage", ",", "read", ">", "a",
