...
example : ProfileTyped [] [] [] addPair .nat64 :=
  ⟨.split (.pair (.nat (by decide)) (.nat (by decide)))
    (.natBin .add (.var rfl) (.var rfl)), rfl⟩
...
example : run [] 30 (initial (.letE overflow (.nat 7))) = .overflow .add largest 1 := by rfl
...
example : ProfileProgramTyped [] program signatures :=
  ⟨⟨.nil, signaturesWellFormed_iff.mp rfl,
    .cons (.split (.var rfl) (.natBin .add (.var rfl) (.var rfl))) .nil⟩, rfl⟩
