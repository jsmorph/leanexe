...
run_elab do
  ...
  for binder in [Lean.BinderInfo.default, .implicit] do
    for depth in [0, 2] do
      ...
      for negations in [0, 1, 2, 3] do
        for form in [BooleanEqualityForm.equality, .decision] do
          for unequal in [false, true] do
            let call (argument : Lean.Expr) := toWord
              (form.local negations unequal
                (.predicate negations 0 argument)
                (.junction (negations + 1) .disjunction
                  (.predicate 0 0 (BooleanGuardNegation.expr 1 argument))
                  (.literal 0 false))).expr
                ...
                for (count, seed) in inputs do
                  let expected := forIn (m := Id) [:count.toNat] seed fun i a =>
                    ...
                    let f := fun b => GuardNegation.denote negations
                      (form.denote unequal (GuardNegation.denote negations (base b))
                        (GuardNegation.denote (negations + 1) (base (!b) || false)))
  ...
  Lean.logInfo m!"{comparisons} native/Boolean-input step predicate equality syntax comparisons and {rejected} invalid-input tests and {controls} admission controls passed"
