      ...
      for negations in [0, 1, 2, 3] do
        for nesting in [1, 2, 4] do
          let call (argument : Lean.Expr) := toWord
            (BooleanGuardNegation.expr negations (.app (.bvar 0)
              ((List.range nesting).foldl (fun value _ =>
                BooleanGuardNegation.expr (negations + 1) (.app (.bvar 0) value)) argument)))
          for nondep in [false, true] do
            for flag in [false, true] do
              ...
              for body in invalid do
                ...
                rejected := rejected + 1
  unless comparisons == 4608 && rejected == 2688 && controls == 192 do
    throwError "unexpected counts {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/Boolean-input step predicate nested-call syntax comparisons and {rejected} invalid-input tests and {controls} admission controls passed"
