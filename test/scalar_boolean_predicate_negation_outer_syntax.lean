    ...
    for depth in [0, 2] do
      ...
      for negations in [0, 1, 2, 3] do
        let call (argument : Lean.Expr) := toWord
          (BooleanGuardNegation.expr negations (.app (.bvar 2) argument))
          ...
          for flag in [false, true] do
            ...
            for (count, seed) in inputs do
              let expected := forIn (m := Id) [:count.toNat] seed fun i a =>
                let f := fun b => GuardNegation.denote negations (b && seed != 0)
            ...
            for body in invalid do
              ...
              rejected := rejected + 1
  unless comparisons == 1536 && rejected == 1024 do
    throwError "unexpected counts {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/outer Boolean-input predicate negation syntax comparisons and {rejected} invalid-input tests passed"
