...
run_elab do
    ...
    for depth in [0, 2] do
      ...
      for negations in [0, 1, 2, 3] do
        for form in [BooleanChoiceForm.ordinary, .dependent ⟨`yesProof, `noProof, binder, binder⟩] do
          for compound in [false, true] do
            let call (argument : Lean.Expr) :=
              let guard : PropositionGuard := if compound then
                ⟨.canonical (.junction 1 .conjunction (.compare .lt (.bvar 1) (literalExpr 7))
                  (.compare .eq (toWord (.app (.bvar 0) argument)) (literalExpr 1))), rfl⟩
                else ⟨.canonical (.compare .le (.bvar 1) (literalExpr 1)), rfl⟩
              toWord (form.proposition negations guard
              ...
              for flag in [false, true] do
                ...
                for (count, seed) in inputs do
                  let expected := forIn (m := Id) [:count.toNat] seed fun i a =>
                    let base := fun b => b && a != seed
                    let f := fun b => GuardNegation.denote negations
                      (if (if compound then !(a < 7 && (base b).toUInt64 == 1) else a ≤ 1)
  ...
  Lean.logInfo m!"{comparisons} native/Boolean-input step predicate proposition syntax comparisons and {rejected} invalid-input tests and {controls} admission controls passed"
