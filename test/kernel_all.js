...
for(const name of ["sort","universe","levels","graph","binding","infer","pi","check",
  "substitution","application","conversion","let","export","proofs"]){
  const result=runChecked([process.execPath,`test/kernel_${name}.js`],{encoding:"utf8"});
  process.stdout.write(result.stdout);
}
for(const fixture of ["composition","let"]){
  const result=runChecked([process.execPath,"test/kernel_export.js",fixture],{encoding:"utf8"});
  process.stdout.write(result.stdout);
}
