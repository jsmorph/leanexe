...
run_elab do
  ...
  let value (mode : Nat) (condition evidence trueDomain falseDomain : Lean.Expr) : Lean.Expr :=
    if mode == 0 then .app (.app (.const ``Decidable.decide []) condition) evidence
    else if mode == 1 then
      .app (.app (.app (.app (.app (.const ``ite [.succ .zero]) boolType) condition) evidence) yes) no
    else
      .app (.app (.app (.app (.app (.const ``dite [.succ .zero]) boolType) condition) evidence)
        (.lam `proof trueDomain yes .default)) (.lam `proof falseDomain no .default)
  let body (mode : Nat) (condition evidence trueDomain falseDomain : Lean.Expr) :=
    let flag := Lean.Expr.bvar 0
    let flagCondition := Lean.Expr.app (.app (.app (.const ``Eq [.succ .zero]) boolType) flag) yes
    let flagEvidence := Lean.Expr.app (.app (.const ``instDecidableEqBool []) flag) yes
    let choice := Lean.Expr.app (.app (.app (.app (.app (.const ``ite [.succ .zero]) word)
      flagCondition) flagEvidence) (.bvar 2)) (.bvar 1)
    Lean.Expr.lam `x word (.lam `y word
      (.letE `flag boolType (value mode condition evidence trueDomain falseDomain) choice false) .default) .default
  ...
  for op in [Junction.conjunction, .disjunction] do
    for n in [0, 1, 2, 5] do
      for (l, r) in [(left, right),
        ...
        unless guardDecision? guard evidence && (propositionGuard? guard.condition evidence).isSome do
          throwError "compound annotation decision rejected"
        for mode in [0, 1, 2] do
          let some func := extractScalarFunc `compound (some "entry") functionType (body mode guard.condition evidence guard.condition (.app (.const ``Not []) guard.condition)) |
          ...
          for decision in invalid do
            unless !(guardDecision? guard decision) &&
                (extractScalarFunc `badCompound (some "entry") functionType (body mode guard.condition decision guard.condition (.app (.const ``Not []) guard.condition))).isNone do
          ...
          if mode == 2 then
            ...
            for (trueDomain, falseDomain) in
              ...
              unless (extractScalarFunc `badProofDomain (some "entry") functionType
                    (body mode guard.condition evidence trueDomain falseDomain)).isNone do
                throwError "wrong dependent proof domain accepted"
              rejected := rejected + 1
  unless comparisons == 1008 && rejected == 552 do throwError "unexpected counts: {comparisons}, {rejected}"
  Lean.logInfo m!"{comparisons} native/saved-decision-syntax IR comparisons and {rejected} invalid-input rejections passed"
