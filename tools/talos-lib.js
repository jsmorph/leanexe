...
function checkPrerequisites() {
  ...
  const manifest = JSON.parse(
    fs.readFileSync(path.join(proofRoot, "lake-manifest.json"), "utf8"),
  );
  const codeLib = manifest.packages?.find((item) => item.name === "CodeLib");
  if (!codeLib || codeLib.type !== "git" || !/^[0-9a-f]{40}$/.test(codeLib.rev)) {
    throw new Error("proof Lake manifest has no immutable CodeLib revision");
  }
  ...
  if (head.error || head.status !== 0) {
    throw new Error(`could not inspect materialized CodeLib revision: ${head.error?.message || head.stderr}`);
  }
  if (head.stdout.trim() !== codeLib.rev) {
    throw new Error(
      `materialized CodeLib revision is ${head.stdout.trim()}, expected ${codeLib.rev}; ` +
      "run the pinned proof-workspace Lake update",
    );
  }
...
function normalizeExpandedProgram(program, caseName) {
  const headerPrefix = [
    "/-",
  ...
  const generatedImport = generatedImports.find((candidate) =>
    program.startsWith(`${headerPrefix}\n${candidate}\n`));
  if (!generatedImport) {
    throw new Error(`${caseName}: Talos generated an unrecognized Program.lean header`);
  }
  for (const token of ["watFunctionBody%", "watModuleMetadata%"]) {
    if (program.includes(token)) {
      throw new Error(`${caseName}: Talos generated compact, WAT-dependent ${token} syntax`);
    }
  }

  const watPath = `../rust/build/${caseName}/program.wat`;
  const guard = [
    "-- Compile-time fidelity check: errors if WAT and generated Lean AST differ.",
    "#guard_msgs (drop info) in",
    `#eval Wasm.checkWatFidelity ${JSON.stringify(watPath)} «module»`,
  ].join("\n");
  const marker = "Wasm.checkWatFidelity";
  const markerCount = program.split(marker).length - 1;
  if (markerCount !== 1 || !program.includes(guard)) {
    throw new Error(
      `${caseName}: Talos generated an unrecognized or repeated WAT fidelity guard`,
    );
  }
  const guardedBlock = `\n${guard}\n`;
  if (program.split(guardedBlock).length !== 2) {
    throw new Error(`${caseName}: Talos fidelity guard is not a standalone block`);
  }
  const normalized = program
    .replace(guardedBlock, "")
    .replace(generatedImport, "import Project.TalosPrelude");
  if (normalized.includes(marker) || normalized.includes(watPath)) {
    throw new Error(`${caseName}: normalized Program.lean still depends on temporary WAT`);
  }
  if (generatedImports.some((candidate) => normalized.includes(candidate))) {
    throw new Error(`${caseName}: normalized Program.lean still imports the broad generator surface`);
  }
  return normalized;
}
...
function prepareCase(item, wasmTools, programMode) {
  ...
  try {
    ...
    runLimited(
      ...
      "10m",
      verifier,
      ["emit", "--force-emit", "--expanded", item.name],
      temporaryRoot,
    );
