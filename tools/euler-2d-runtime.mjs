...
export function verifyEvents(events){
 ...
 for(let eventIndex=1;eventIndex<events.length;eventIndex++){
  ...
  if(event.kind==='frame'){
   ...
   savedFrames.push(event);continue;
  }
  if(event.kind==='step'){
  ...
  assert.equal(event.kind,'final');assert.equal(eventIndex,events.length-1);assert.equal(t,end);assert.equal(frameIndex,frames);
  ...
  return {scenario,n,steps,retries,t,history,frames:savedFrames,final:event,
 ...
 throw Error('missing final event');
}
export function run2D(n=192,frames=21,scenario='four-quadrants'){
 const paths=artifacts.map(a=>{const p=path.join(root,'proofs/artifacts',a.case,a.sha256,'program.wasm'),bytes=fs.readFileSync(p);assert.equal(bytes.length,a.bytes);assert.equal(crypto.createHash('sha256').update(bytes).digest('hex'),a.sha256);return p;});
