...
function proofSource(emitted) {
  emitted=path.resolve(emitted);
  ...
  const code=program.replace("import Project.TalosPrelude",
...
`;
  return {code,names,ids};
}
async function check(emitted,attempt) {
  attempt=path.resolve(attempt);fs.mkdirSync(attempt,{recursive:true});
  const {code,names,ids}=proofSource(emitted);
  const file=path.join(attempt,"Helpers.lean");
  fs.writeFileSync(file,code,{flag:"wx"});
...
module.exports={check,proofSource};
