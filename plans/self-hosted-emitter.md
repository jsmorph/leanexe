...
The image codec items are covered by `LeanExe.Wasm.ImageTest`; reduced native/image byte equality is covered by `LeanExe.Wasm.ImageIntegrationTest`; and `test/selfhost_emitter.js` retains the complete two-stage fixed-point and public-error checks.  During this implementation session, focused Lean builds ran directly with the pinned Lean 4.31.0 toolchain under the user's explicit exception for the repository runner.

The direct verification pass also completed the core execution, WASI, reference-counting, `ByteArray`, `AsciiString`, `IntMap`, JSON, self-emission, and source-process suites; 340 standard Lean comparisons, 62 IR comparisons, and 56 validator fuzz cases passed.  The exact Node 24.13.0 and `wasm-tools` 1.251.0 version checks now pass, a pinned-Node bootstrap rebuild preserves the fixed point and complete corpus results above, and all nine WAT round trips are byte-identical.  The 2026-09-02 aggregate artifact-proof gate passed all twenty packages, and semantic conformance classified all fifteen invalid modules, produced 3,853 Talos passes with the six documented imported-memory failures, and passed all twenty-five files under Wasmtime.  Both maintained drivers launched Lean directly under the owner's session-only exception instead of enforcing the normal `tools/leanrun` resource scope.  The refreshed release record consumes their matching content receipts and leaves the policy-conforming cold-checkout gate as the sole release blocker.
