...
axioms.  Complete flux, cell-update, speed-scan, neighbor, memory-input,
update-callback, time-guard, and cell-initializer execution also pass
against the regenerated module with standard-only audits.  Sweep
allocation and execution, buffer release, acceptance, complete timestep
execution and reservation, spacing, and CFL proposal also pass against
the regenerated module.  The existing successful-retry and
successful-advance execution proofs also pass with their source-success
premises.  Both retry failure sequences now have exact execution proofs,
including empty-array allocation, all pointer assignments, held-grid
preservation, and the reduced memory reservation.  The shared allocator
...
heap, reservation, and sufficient-fuel premises, and now preserves the
supplied physical page bound.  Complete run execution composes
initialization and advancement with the exact source result and those
resource guarantees.  Its reservation is at most 319,523,176 bytes above
the entry heap top, including three full-grid spares.  Complete output
execution now passes.  The complete solve entry now composes both calls
and establishes the 512 MiB bound from the module's initial state.
The remaining execution inventory below records results for
the preserved 21,386-byte module and must be re-established for the
regenerated bytes.

The regenerated output packer's five capacities and allocations, both
map bodies, both copy pairs, and four header stores now match shared
checked programs.  Arbitrary-stride allocation state and execution
...
LTG.  The [complete packer theorem](../proofs/talos/lean/Project/EulerRiemann/ExecutionOutput.lean)
now composes both maps, both concatenations, the header, all three
releases, and both returned ABI pointers.  It proves exact Output.pack
words with owned storage and a preserved physical page bound.  The
allocation budget is 48 bytes per cell plus 344 bytes, at most
30,720,344 bytes for the requested maximum grid.

The [public specification](../proofs/talos/lean/Project/EulerRiemann/Spec.lean)
now proves complete terminating execution from Module.initialStore for
runtime sizes two through eight hundred, with exact represented output
and at most 512 MiB of linear memory.  Its success theorem connects a
...
That earlier theorem requires source retry success.  The
[total retry execution theorem](../proofs/talos/lean/Project/EulerRiemann/ExecutionRetryTotal.lean)
now proves all return branches without that premise.  It includes invalid
advancement and exhausted-fuel allocation, preserves every original grid,
and charges at most one allocation reservation.  Its seven supporting
modules pass with standard-only audits.  The complete entry proof must
establish the heap and reservation premises.
...
The [total advance function](../proofs/talos/lean/Project/EulerRiemann/ExecutionAdvanceTotal.lean)
now removes the source-success premise.  Its checked iteration covers
time completion, scan failure, failed retry, and accepted-grid replacement.
A shared binary64 division-result bound proves that encoding retry fuel
cannot overflow, including exceptional floating-point results.  The
final reservation accounts for the empty array retained after retry
failure.  The source run supplies the sufficient-fuel premise.  The
complete entry must establish the remaining heap and memory premises.
...
The [append payload](../proofs/talos/lean/Project/EulerRiemann/InitialAppendCopy.lean)
composes the shared prefix and offset-copy theorems against the emitted
regions.  It reconstructs the concatenated width-seven grid and preserves
...
complete initialCells caller now composes checked multiplication,
initial-cell evaluation, singleton allocation and writes, growth,
singleton release, and return.  It proves termination and exact source
output for every supported size under its entry heap, free-list,
reservation, and page-cap premises.  Shared constant-index and
consecutive-field store theorems extend the array-field-access LTG entry.
