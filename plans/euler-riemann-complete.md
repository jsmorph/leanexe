...
the entry heap top, including three full-grid spares.  Output and the
entry memory bound remain open.  The remaining execution inventory below records results for
...
support its one-word arrays.  The completed maps, concatenations, and
full packer remain open.
