...
private def allocatedFrame
    (inputPtr x result heapTop : UInt64) : Locals :=
  { params := [.i64 inputPtr],
    locals := [.i64 x, .i64 result, .i64 0, .i64 0,
      .i64 (heapTop + 48), .i64 0, .i64 0, .i64 0, .i64 16,
      .i64 0, .i64 0, .i64 (heapTop + 48 + 16),
      .i64 ((heapTop + 48 + 16 - 1) / 65536 + 1),
      .i64 (heapTop + 48)],
    values := [] }
...
theorem artifact_behavior :
  ...
  · obtain ⟨x, hInputEq⟩ := Array.size_eq_one_iff.mp hSingleton
    ...
    rw [hCapacityNat] at hFitMemory16
    ...
    have hPayloadStoreAddress :
        UInt32.ofNat ((heapTop.toNat + 48 + 8) % 4294967296) =
          (heapTop + 48 + 8).toUInt32 := by
      simpa [hRootInnerMod] using hPayloadAddress'
    ...
    change wp LeanExeGen.GeneratedRbade8cb1a4e3a423.«module»
      (Project.ProofKit.FixedArrayAllocator.region 1 ++ _) _ st'
      (allocationFrame inputPtr x (factorCount x)) env
    apply Project.ProofKit.FixedArrayAllocator.region_spec
      LeanExeGen.GeneratedRbade8cb1a4e3a423.«module» env st'
      (allocationFrame inputPtr x (factorCount x))
      heapTop 16 1 allocs
    · rfl
    · rfl
    · rfl
    ...
    · have hAllocFrame :
          Project.ProofKit.FixedArrayAllocator.allocFrame
              (allocationFrame inputPtr x (factorCount x)) heapTop 16 =
            allocatedFrame inputPtr x (factorCount x) heapTop := by
        simp [Project.ProofKit.FixedArrayAllocator.allocFrame,
          allocationFrame, allocatedFrame]
      rw [hAllocFrame]
      unfold Project.ProofKit.FixedArrayAllocator.allocStore
        Project.ProofKit.FixedArrayAllocator.headerMem allocatedFrame
      wp_run
      simp
      have hRootStoreAddressNat :
          (heapTop.toNat + 48) % 4294967296 =
            (heapTop + 48).toUInt32.toNat := by
        rw [← hRootAddress']
        simp
      have hPayloadStoreAddressNat :
          (heapTop.toNat + 48 + 8) % 4294967296 =
            (heapTop + 48 + 8).toUInt32.toNat := by
        rw [← hPayloadStoreAddress]
        simp
      refine ⟨?_, ?_, ?_⟩
      · rw [hRootStoreAddressNat, hRootAddressNat']
        omega
      · rw [hPayloadStoreAddressNat, hPayloadAddressNat']
        omega
      · refine ⟨heapTop + 48, ?_, ?_⟩
        ...
        · rw [hExpected]
          ...
          · simp only [Wasm.Mem.write64_pages]
            rw [hBump.rootToNat]
            simpa using hFitMemory16
          · rw [hRootAddress', hPayloadStoreAddress]
            word_reads
          · rw [hPayloadStoreAddress]
            word_reads
