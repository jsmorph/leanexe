...
private theorem helper_correct (env : HostEnv Unit) (initial : Store Unit)
  ...
  wp_run
  wp_block_loop invariant (factorInv initial (factorCount n))
    decreasing factorMeasure
  ...
  · obtain ⟨x, hInputEq⟩ := Array.size_eq_one_iff.mp hSingleton
    ...
    change wp LeanExeGen.GeneratedRbade8cb1a4e3a423.«module»
      (Project.ProofKit.FixedArrayAllocator.region 1 ++
        Project.ProofKit.FixedArraySingleton.resultSuffix ++ []) _ st'
