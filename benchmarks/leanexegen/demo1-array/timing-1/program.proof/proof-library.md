...
Use `root_toUInt32` and `top_toUInt32` when generated stores express an allocator address through `UInt32.ofNat`.  These lemmas expose the same modulo-`2^32` address used by Talos.  `root_toNat`, `top_toNat`, `top_not_lt_base`, `headerOffsets`, and `bump_no_grow` remain available separately when a proof needs only one fact.
