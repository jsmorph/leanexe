...
private theorem func0_correct (env : HostEnv Unit) (initial : Store Unit)
    ...
    TerminatesWith env LeanExeGen.GeneratedRbade8cb1a4e3a423.«module» 0
      ...
      (fun final results => final = initial ∧ results = [.i64 (factorCount n)]) := by
  ...
  · rintro st s ⟨rfl, fuel, rem, divisor, count, result, flag,
    ...
    · subst flag
      rcases hactive with
        ⟨hdivisor, hdivisorShape, hminFac, hcount, hbudget⟩
      ...
      · have hrem : rem.toNat ≤ 1 := by
        ...
        have hresult : count = factorCount n := by
          ...
          simpa [factorCount] using hcount
        have hremU : ¬ (1 : UInt64) < rem := by
          rw [UInt64.lt_iff_toNat_lt]
          change ¬ 1 < rem.toNat
          omega
        wp_peel
        simp [hfuel, hremU, factorFrame, factorMeasure, hresult]
        wp_peel
        simp [hremU]
        wp_peel
      · have hfuelPos : 0 < fuel.toNat := by
        ...
        · have hremU : ¬ rem ≤ (1 : UInt64) := by
          ...
          · have hquotU : rem / divisor < divisor := by
            ...
            have hresult := count_after_prime count (factorCount n) hprime hcount
            wp_peel
          ...
          · have hquotU : ¬ rem / divisor < divisor := by
            ...
            · have hmodNat : rem.toNat % divisor.toNat ≠ 0 := by
              ...
              · subst divisor
                ...
                · refine ⟨rfl, fuel - 1, rem, 3, count, result, 0,
                  ...
                  · intro h
                    ...
                    omega
                  · intro h
                    change rem.toNat + 2 ≤ fuel.toNat - 1 + 3
                    have hbound := hbudget hremGt
              ...
              · have hdivisorOdd : Odd divisor.toNat :=
                    hdivisorShape.resolve_left htwo
                obtain ⟨k, hk⟩ := hdivisorOdd
                have hminPrime : Nat.Prime rem.toNat.minFac :=
                  Nat.minFac_prime (by omega)
                have hminOdd : Odd rem.toNat.minFac :=
                  hminPrime.odd_of_ne_two (by omega)
                obtain ⟨j, hj⟩ := hminOdd
                have hdivisorAdd :
                    (divisor + 2).toNat = divisor.toNat + 2 := by
                  rw [UInt64.toNat_add]
                  change (divisor.toNat + 2) % 2 ^ 64 = divisor.toNat + 2
                  rw [Nat.mod_eq_of_lt]
                  have hremBound := rem.toNat_lt
                  have hminBound := Nat.minFac_le (by omega : 0 < rem.toNat)
                  omega
                have hnextOdd : Odd (divisor + 2).toNat := by
                  refine ⟨k + 1, ?_⟩
                  rw [hdivisorAdd]
                  omega
                have hnextLe : divisor.toNat + 2 ≤ rem.toNat.minFac := by
                  omega
                wp_peel
                simp [hmod]
                wp_peel
                ...
                · refine ⟨rfl, fuel - 1, rem, divisor + 2, count, result, 0,
                  ...
                  refine ⟨by omega, Or.inr ⟨k + 1, by omega⟩, ?_, hcount, ?_⟩
                  · intro h
                    exact hnextLe
                  · intro h
                    have hbound := hbudget hremGt
...
private theorem func1_correct (env : HostEnv Unit) (initial : Store Unit)
  ...
  · have hnU : n ≤ (1 : UInt64) := by
      rw [UInt64.le_iff_toNat_le]
      simpa using hn
    have hcount : factorCount n = 0 := by
      unfold factorCount
      rw [primeFactorsList_nil_of_le_one hn]
      rfl
  ...
  · have hnU : ¬n ≤ (1 : UInt64) := by
    ...
    wp_run
    simp
    apply Wasm.wp_call_tw (func0_correct env initial n)
...
private theorem expected_eq_self_of_size_ne_one (input : Array UInt64)
  ...
  | nil => simp [LeanExeGen.GeneratedRbade8cb1a4e3a423.FormalSpec.expected, hList]
  | cons x xs =>
      cases xs with
      | nil =>
          exfalso
          ...
          simpa using hLength
      | cons y ys =>
          simp [LeanExeGen.GeneratedRbade8cb1a4e3a423.FormalSpec.expected, hList]

private def allocationFrame (inputPtr x result : UInt64) : Locals :=
  { params := [.i64 inputPtr],
    locals := [
      .i64 x, .i64 result, .i64 0, .i64 0, .i64 inputPtr, .i64 0,
      .i64 0, .i64 0, .i64 16, .i64 0, .i64 0, .i64 0, .i64 0,
      .i64 0],
    values := [] }
...
theorem artifact_behavior :
  ...
  simp
  constructor
  · rw [← Project.ProofKit.Memory.toUInt32_toNat inputPtr]
    exact hLengthBound
  · rw [hInputAddress, hLength]
    by_cases hSize : input.size = 1
    · obtain ⟨x, rfl⟩ := Array.size_eq_one_iff.mp hSize
      have hEncoded : UInt64.ofNat (#[x] : Array UInt64).size = 1 := by rfl
      ...
      have hBump := Project.ProofKit.Allocation.bumpFacts heapTop 16
        initial.mem.pages hFitMemory16 hPages
      ...
      · rw [← Project.ProofKit.Memory.toUInt32_toNat inputPtr]
        exact hLengthBound
      · simp [hInputAddress, hLength]
        wp_peel
        constructor
        · have hValueAddressNat := congrArg UInt32.toNat hValueAddress
          rw [← hValueAddressNat] at hValueBound
          simpa using hValueBound
        · rw [hValueAddress, hValue]
          simp
          apply Wasm.wp_call_tw (func1_correct env initial x)
          rintro st' vs ⟨hStore, hValues⟩
          subst st'
          subst vs
          wp_run
          simp
          ...
          apply Wasm.wp_loop_cons
            (Inv := fun st s =>
              st = initial ∧ s = allocationFrame inputPtr x (factorCount x))
            (μ := fun _ _ => 0)
          · exact ⟨rfl, rfl⟩
          · rintro st s ⟨hStore, hFrame⟩
            subst st
            subst s
            wp_run
            simp [allocationFrame, hGlobals]
            wp_peel
            simp [hGlobals, hBump.noOverflow]
            wp_peel
            rw [show LeanExeGen.GeneratedRbade8cb1a4e3a423.«module».memIs64 =
              false by rfl]
            simp [hBump.noGrow]
            wp_peel
            simp [hGlobals]
            have hRootToNat := hBump.rootToNat
            have hAllocatorFit32 := hBump.fit32
            change heapTop.toNat + 48 + 16 ≤ 4294967296 at hAllocatorFit32
            change heapTop.toNat + 48 + 16 ≤ initial.mem.pages * 65536 at hFitMemory16
            obtain ⟨hHeader40, hHeader32, hHeader24, hHeader16,
              hHeader8⟩ := hBump.headerOffsets
            have hRootAddress :
                UInt32.ofNat ((heapTop.toNat + 48) % 4294967296) =
                  (heapTop + 48).toUInt32 := by
              rw [Project.ProofKit.Memory.toUInt32_eq_ofNat, hRootToNat]
            have hRootInnerMod :
                (heapTop.toNat + 48) % 18446744073709551616 =
                  heapTop.toNat + 48 :=
            ...
            have hPayloadAddress :
              ...
              rw [Project.ProofKit.Memory.toUInt32_eq_ofNat,
                UInt64.toNat_add, hRootToNat]
              simp only [show (8 : UInt64).toNat = 8 by rfl]
              rw [hRootInnerMod, Nat.mod_eq_of_lt (by omega)]
              apply UInt32.toNat.inj
              simp
            have hRootAddressNat :
                (UInt32.ofNat
                  ((heapTop.toNat + 48) % 4294967296)).toNat =
                    heapTop.toNat + 48 := by
            ...
            have hPayloadAddressNat :
                (UInt32.ofNat
                  (((heapTop.toNat + 48) % 18446744073709551616 + 8) %
                    4294967296)).toNat = heapTop.toNat + 56 := by
              simp [hRootInnerMod, Nat.mod_eq_of_lt (by omega :
                heapTop.toNat + 56 < 4294967296)]
            refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
            · rw [Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hHeader40, Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hHeader32, Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hHeader24, Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hHeader16, Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hHeader8, Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [Nat.mod_eq_of_lt (by omega)]
              omega
            · rw [hRootInnerMod, Nat.mod_eq_of_lt (by omega)]
            ...
            · change Project.ProofKit.UInt64Array.At _ (heapTop + 48)
              ...
              · simp only [Wasm.Mem.write64_pages]
                ...
                omega
              · rw [← hRootAddress]
                rw [Project.ProofKit.Memory.read64_write64_disjoint _ _ _ _
                  (by rw [hRootAddressNat, hPayloadAddressNat]; omega)]
    ...
    · have hEncoded : UInt64.ofNat input.size ≠ 1 := by
      ...
      wp_peel
      wp_peel
      rw [expected_eq_self_of_size_ne_one input hSize]
