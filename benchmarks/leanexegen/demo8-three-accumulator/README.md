...
A residual normalizer next unfolded the generated specification and reduced the singleton equation in the deterministic starter.  The untouched 67-line proof passed in 219.561 seconds, 29.9 percent below the initial run, with no Codex edit.  Its isolated Demo 7 screen did not pass the timing gate, so the accepted package was retained as an example pending a change to the execution path.

Leanexegen now checks a starter expected to be complete before starting Codex.  A successful full check accepts the proof directly, while an ordinary Lean proof failure starts the normal artifact-proof session; runner and later acceptance failures still stop the run.  Demo 8 passed this path in 212.727 seconds with zero Codex time and the same 67-line proof, a 32.1 percent reduction from its initial run.

`current-1` preserves the initial complete package, while `complete-starter-1`, `normalized-starter-1`, and `direct-starter-1` preserve the later packages, journals, telemetry, recipes, checked annotation modules, and exact artifact proofs.  Separate `tools/leanexegen verify -s` runs accepted all four packages over the same artifact digest.  The [timing record](timings.json) records their monotonic intervals, proof structure, and comparison results.
