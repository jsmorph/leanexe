...
The journals identified three metadata and generation changes.  The archived Demo 11 catalog predates the current capacity and allocation entries, which expose `FixedArrayAllocatorWindow.constantCapacityRegion_spec_withTail`.  A catalog test now protects that declaration in both generated index records.  The annotation generator now emits the equality between `FixedArrayFold.forwardSetupFrame` over a generated continuing frame and the initialized continuing frame.  Extractor version six reports the checked length dispatch, expected size, input local, and compiler encoding.
