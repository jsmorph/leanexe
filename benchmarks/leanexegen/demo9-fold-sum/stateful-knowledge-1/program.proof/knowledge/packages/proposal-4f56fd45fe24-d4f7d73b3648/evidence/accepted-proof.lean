...
private theorem singletonPost_conseq
    (module_ : Wasm.Module) (env : Wasm.HostEnv Unit)
    (inputPtr root value : UInt64) (input : Array UInt64)
    (hExpected : FormalSpec.expected input = #[value]) :
    FixedArrayFold.singletonResultPost 6 root value ⇛
      FixedArrayEqNode.branchPost module_ env
        AnnotationMatches.function_0_length_dispatch_0_suffix_program
        (publicPost inputPtr input) := by
  intro continuation hPost
  cases continuation <;>
    simp only [FixedArrayFold.singletonResultPost] at hPost
  case Fallthrough final frame =>
    rcases hPost with ⟨hReturn, hOutput⟩
    simp only [FixedArrayEqNode.branchPost]
    have hReturn' : ({ frame with values := [] } : Wasm.Locals).get 6 =
        some (.i64 root) := by
      simpa only [Wasm.Locals.get] using hReturn
    simp only [AnnotationMatches.function_0_length_dispatch_0_suffix_program,
      wp_localGet_cons, hReturn', Wasm.wp_nil, publicPost]
    refine ⟨root, rfl, ?_⟩
    rw [hExpected]
    exact hOutput
  all_goals contradiction

private theorem xorSingleton_spec
    (module_ : Wasm.Module) (env : Wasm.HostEnv Unit) (st : Wasm.Store Unit)
    (inputPtr root : UInt64) (input : Array UInt64)
    (item scratch done staged release : UInt64)
    (hPayloadBound :
    ...
    (hOutput : UInt64Array.At
      ...
      root #[ArrayFold.foldPrefix input UInt64.xor 0 input.size]) :
    Wasm.wp module_
      AnnotationMatches.function_0_array_fold_0_singleton_result_program
      (FixedArrayFold.singletonResultPost 6 root
        (ArrayFold.foldPrefix input UInt64.xor 0 input.size)) st
      (xorFrame inputPtr root input input.size item scratch done staged release)
      env := by
  let frame := xorFrame inputPtr root input input.size item scratch done staged release
  let value := ArrayFold.foldPrefix input UInt64.xor 0 input.size
  have hParams : frame.params.length = 1 := by
    simp only [frame, xorFrame,
      AnnotationMatches.function_0_array_fold_0_continuing_frame_params]
    rfl
  have hLocals : frame.locals.length = 20 := by
    simp only [frame, xorFrame,
      AnnotationMatches.function_0_array_fold_0_continuing_frame_locals_length]
  have hValues : frame.values = [] := by
    simp only [frame, xorFrame,
      AnnotationMatches.function_0_array_fold_0_continuing_frame_values]
  have hAccumulator : frame.get 1 = some (.i64 value) := by
    simp only [frame, value, xorFrame,
      AnnotationMatches.function_0_array_fold_0_continuing_frame_get_1]
  have hRoot : frame.get 7 = some (.i64 root) := by
    simp only [frame, xorFrame,
  ...
  have hRootLower : frame.params.length ≤ 7 := by omega
  have hRootValid : frame.validIndex 7 := by
    simpa [Wasm.Locals.validIndex, hParams, hLocals]
  have hResult : (FixedArrayFold.resultFrame frame 10 value).get 10 =
      some (.i64 value) :=
    FixedArrayFold.resultFrame_get_result frame 10 value
      hResultLower hResultValid
  have hRootAfter : (FixedArrayFold.resultFrame frame 10 value).get 7 =
      some (.i64 root) :=
    FixedArrayFold.resultFrame_get_of_ne frame 10 7 value (.i64 root)
      hResultLower hRootLower hRootValid (by decide) hRoot
  change Wasm.wp module_
    (FixedArrayFold.singletonResultProgram 1 10 7 4 6)
    (FixedArrayFold.singletonResultPost 6 root value) st frame env
  ...
  · simp [Wasm.Locals.validIndex, FixedArrayFold.resultFrame_params,
      FixedArrayFold.resultFrame_locals_length, hParams, hLocals]
  · rw [FixedArrayFold.resultFrame_params]
    omega
  · simp [Wasm.Locals.validIndex, FixedArrayFold.resultFrame_params,
      FixedArrayFold.resultFrame_locals_length, hParams, hLocals]
            ...
            · rintro st frame hInv
              ...
              · subst index
                ...
                · have hExpected : FormalSpec.expected input =
                  ...
                  have hCompact := xorSingleton_spec module env lengthSt
                    inputPtr root input item scratch done staged release
                    hPayloadBound
                    (hSingleton
                      (ArrayFold.foldPrefix input UInt64.xor 0 input.size))
                  have hSuffix : Wasm.wp module
                    ...
                    apply Wasm.wp.conseq
                      (Q := FixedArrayFold.singletonResultPost 6 root
                        (ArrayFold.foldPrefix input UInt64.xor 0 input.size))
                    · exact singletonPost_conseq module env inputPtr root
                        (ArrayFold.foldPrefix input UInt64.xor 0 input.size)
                        input hExpected
                    · exact hCompact
                  simp only [Wasm.wp_nil, List.take_zero, List.drop_zero,
                    List.nil_append, setupFrame,
                    FixedArrayFold.forwardSetupFrame_values, xorFrame,
                    AnnotationMatches.function_0_array_fold_0_continuing_frame_values]
