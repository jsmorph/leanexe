...
A generated `terminates_with_of_post_test` theorem composes the exact scalar function entry with `postTestProgram_spec`.  Its premises expose the generated initial compact state, the application invariant, the measure, and one body-and-condition transition, while its conclusion states the external scalar call.  Use the lower-level `terminates_with_of_loop` theorem when a proof needs a different loop theorem or post-loop composition.
