...
| 5 | Arrays with richer elements | Extend `Array` beyond `Array UInt64` to supported scalar, structure, and small-inductive element types | One-cell scalar arrays are specified and tested; structure and small-inductive arrays have fixed-width literal, replication, read, safe-read, default-read, write, modification, push, pop, append, extract, insertion, erasure, swapping, reversal, and returned-pointer support; the remaining mapping operation still needs multi-slot lowering |
