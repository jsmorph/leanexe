...
The `clob_depth` artifact is registered outside the aggregate gate while its proof is in progress.  `ClobDepth.Representation` states the stride-two level layout and owned fixed-array predicate, and `ClobDepth.Properties` proves side filtering, first-price append order, unique prices, exact per-price modular aggregation, and its bounded natural-number interpretation.  The remaining artifact proof must verify level update or append, the per-side order fold, both owned output arrays, allocator counters, page preservation, and the input memory frame.
