...
| Data types | Structures and user-defined inductives are useful for local values and returned values, but structure and inductive parameters are missing.  `Option` and `Except` still use special internal paths that cannot cross the public ABI. | Supported structures, small inductives, `Option`, and `Except` can appear in local values, parameters, and results according to one documented representation. |
