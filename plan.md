...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix and composition prove the aligned stride-five capacity, empty free-list scan, bump allocation, semantic allocation store, and exact result frame.  The first copy loop stores the smaller length and proves every word before the selected maker; the next proof boundary copies the shifted suffix and reconstructs `os.eraseIdx i`.
