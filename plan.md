...
| Memory model | The current memory model has arena allocation, copy-on-write arrays, internal `ByteArray` elements with owner-pointer-length layout, nested internal arrays with owner-pointer layout, products, fixed-width structure and tagged arrays for literals, replication, reads, default reads, writes, modifications, mapping, filtering, pushes, pops, appends, extracts, insertions, erasures, swaps, reversals, and returned pointers.  It now supports pointer-length `ByteArray` results, arena-owned `ByteArray.mk` from `Array UInt8`, arena-owned `ByteArray.push`, arena-owned `ByteArray.append`, arena-owned proof-indexed `ByteArray.set`, reference-counted release through recursive children, `ByteArray` owners, and nested-array owners, and conservative compiler-emitted release for owned helper-call results whose callees have no heap-bearing parameters.  It still lacks aliasing rules for faster updates, helper-result ownership summaries for callees that receive heap values, and a public ABI for nested heap graphs. | The compiler has explicit layouts for supported arrays and slices, a documented ownership model for returned buffers, and correctness tests for reads, writes, copies, aliases, and returned memory. |
