...
| ---- | ----------- | ------------------ |
| Control flow | The extractor accepts a restricted tail-recursion shape over `Nat` fuel, including local step lets before the tail call or tail-call `if`; list-shaped structural recursion over internal recursive inductives; scalar-accumulator `ByteArray.foldl`; scalar-accumulator `Array.foldl`; `ByteArray.findIdx?`; and array scans through `Array.find?`, `Array.findIdx?`, `Array.any`, `Array.all`, and `Array.filter` over fixed-width arrays.  Ordinary Go-style loops, early returns, breaks, structural recursion over branching data, and recursive descent through arrays are absent. | Lean programs can express common loops over counters, byte buffers, arrays, and simple inductives in accepted source forms, with exact rejection for unsupported recursion. |
