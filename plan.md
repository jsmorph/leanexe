...
| Memory model | The current memory model has arena allocation and copy-on-write arrays for one-cell scalar element types, plus fixed-width structure and tagged arrays for literals, reads, writes, and returned pointers.  It lacks slices, full multi-slot copy operations, owned byte outputs, and aliasing rules for faster updates. | The compiler has explicit layouts for supported arrays and slices, a documented ownership model for returned buffers, and correctness tests for reads, writes, copies, aliases, and returned memory. |
