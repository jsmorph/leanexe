...
The `clob_depth` artifact is registered outside the aggregate gate while its proof is in progress.  `ClobDepth.Representation` states the stride-two level layout and owned fixed-array predicate, and `ClobDepth.Properties` proves side filtering, first-price append order, unique prices, exact per-price modular aggregation, and its bounded natural-number interpretation.  `ClobDepth.MissingBranch.missingProg_spec` composes the missing scan frame through bump allocation, copying, appended stores, exact result locals, and owned source and result arrays; the remaining work must prove the found replacement path, compose both outcomes as function 3, prove the per-side fold, and compose both owned export results with page and input-memory frames.
