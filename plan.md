...
| 6 | Internal recursive inductives | Add self-recursive data values and monomorphic recursive instances with arena pointers at strict boundaries | Constructors, matches, branch values, helper values, a fuel-recursive list traversal, a direct structural traversal, and ordinary `List UInt64` construction and traversal pass Wasm tests; public recursive ABI, mutual recursion, recursive descent through arrays, standard `List` APIs, and GC remain planned |
...
| Functions | Higher-order arguments and closures are rejected.  This blocks callbacks, visitors, adapters, and ordinary library functions such as `List.map`, `List.filter`, `List.foldl`, `List.any`, and `List.find?` until direct-lambda specialization exists. | The first Go-level milestone can remain first-order, but the plan should keep a later escape-restricted function-value design open. |
