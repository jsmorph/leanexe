...
The `depth` step completed on 2026-07-18.  `ClobDepth.Representation` states the stride-two level layout and owned fixed-array predicate, and `ClobDepth.Properties` proves side filtering, first-price append order, unique prices, exact per-price modular aggregation, and its bounded natural-number interpretation.  Both level-update branches compose as `Func3.func3_spec` with one `addLevelL` result, the per-side fold carries an invariant with exact allocator globals and a single heap budget through every level-update call, and `ClobDepth.Func7.func7_terminates` returns both owned side arrays with the exact source `depthL` contents, the preserved orders array, exact globals, page equality, and the below-heap byte frame.  The registry marks the case complete and the focused proof gate passes against a regenerated model.
