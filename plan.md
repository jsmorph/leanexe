...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix and composition prove the aligned stride-five capacity, empty free-list scan, bump allocation, semantic allocation store, and exact result frame.  The next proof boundaries initialize the smaller book and prove its prefix and shifted-suffix copy loops.
