...
The `clob_limit` case is registered outside the aggregate artifact gate while its proof is in progress.  Its byte-pinned input contains exported function 21, `runMatch` at function 18, the recursive matcher at function 17, and the shared runtime suite at functions 22 through 25.  `ClobLimit.Model.limitL` states the invalid, fully filled, and residual-order branches and inherits the source matcher's natural-number conservation theorem.  The validity subsystem at functions 0 through 6 has an exact artifact theorem and is definitionally equal to the completed `postOnly` subsystem.  The exported invalid branch has an input-generic theorem covering the borrowed book, fresh empty trade array, allocator counters, page count, and below-heap memory frame.  A reusable function-region theorem transports exact interpreter behavior across finite renamings of closed direct-call regions.  A six-function certificate uses it to transfer the owner-aware `matchFuel` search loop and wrapper specifications to limit functions 13 and 14, replacing a third copy of the generated loop proof.  Function 17 has exact termination theorems for zero fuel, zero remaining quantity, and a missing maker, including all five owner-and-pointer results and an unchanged store.  Its separately compiled iteration-control theorem covers the remaining check, owner-aware search, selected-maker quantity read, and full-versus-partial dispatch without elaborating either allocation body.  The full-book, partial-book, and shared trade allocator layouts have exact empty-free-list theorems over the common bump-store semantics, including their complete generated scans and numeric scratch layouts.  The partial-fill book prefix proves all five maker reads, the reduced maker quantity, the source length, and the replacement-book bounds guard in the exact Limit frame.  Its separately compiled control theorem computes the flat-book width and enters an opaque one-result update branch through the shared module-parameterized continuation rule.  The allocator prefix proves the aligned stride-five byte capacity and initializes the exact free-list scan frame.  A separate composition proves the complete empty-list allocation through the shared bump store and exact result frame.  The post-allocation loop proves the counter and length stores, complete payload copy, reconstructed target book, preserved source book, fresh target header, and outside-payload memory frame.  A Limit-specific instruction adapter reuses the shared five-store semantics to prove the updated maker quantity, retained header, outside-payload frame, and returned target pointer.  The complete partial-book update composes these boundaries while retaining both owned books, exact globals and pages, and the allocator-to-final memory frame.  The following trade prefix records the replacement book, reads the maker identifier and price, and computes the old flat-word count and appended trade length.  Its allocator setup proves the aligned stride-four capacity and exact initial search frame, and the empty-list composition returns the semantic bump store and target pointer.  The trade copy loop proves the extended length, old-word prefix, source preservation, fresh target header, exact counter update, and outside-payload frame.  A four-store adapter reuses the shared append semantics to prove the represented extended trade array, retained header, outside-payload frame, and returned target pointer.  The complete partial-trade update composes allocation, copying, and append stores while retaining the old book, replacement book, old trades, new trades, exact globals and pages, and the allocator-to-final memory frame.  The final partial-fill assignments record the new trade pointer, zero remaining quantity, and completion in the exact recursive-result locals.  Their composition with trade preparation proves the complete continuation after replacement-book construction while retaining all four arrays and the exact allocator state.  The complete partial-fill branch composes both bump allocations, states the semantic replacement book and appended trades, preserves the two source arrays, and proves the final heap pointer, zero free-list head, counter increment by two, unchanged pages, and byte equality below the old heap top.  The generated artifact reproduces byte-for-byte, but the case does not count among the seventeen completed artifacts.

The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix and composition prove the aligned stride-five capacity, empty free-list scan, bump allocation, semantic allocation store, and exact result frame.  The complete book update stores the smaller length, copies the unchanged prefix and shifted suffix, reconstructs `os.eraseIdx i`, retains both owned books and exact allocator state, and returns the replacement pointer.  The complete trade update reuses the shared stride-four allocation and append theorem, retains all four owned arrays and exact allocator state, and records both the semantic full-fill trade and reduced remaining quantity.  The book-and-trade composition carries both bump allocations through the generated nested result branches, preserves bytes below the old heap top, and states the final heap pointer and counter.  The complete full-fill branch applies the recursive transition, copies the semantic replacement state into the loop parameters, and decrements fuel while retaining both source arrays and all allocator facts.  Exact loop-guard and five-result epilogue theorems cover running, completed, and zero-fuel states.  The next proof boundary connects the completed full- and partial-fill branches to iteration dispatch and a terminating loop invariant.
