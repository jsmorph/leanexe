...
Current progress: the first implementation slice for monomorphic, nonrecursive user inductives now supports constructors, generated matcher extraction, proof-erased constructor fields, nullary enums, local helper values, branch-selected values, and exported tagged results.  `Option` and `Except` now use source-identified internal representations, so `Except Unit α` works for supported payloads and neither built-in sum shares an anonymous runtime shape.  Public `Option` and `Except` results now use the same tagged multi-value ABI as user-defined inductive results.  The remaining consolidation work is to admit structured entry parameters.
...
| Data types | Structures, user-defined inductives, `Option`, and `Except` are useful for local values and returned values, but structure and inductive parameters are missing. | Supported structures, small inductives, `Option`, and `Except` can appear in local values, parameters, and results according to one documented representation. |
...
| Errors | Go programs rely on explicit error returns.  The nearest Lean form is `Except`; exported `Except` results now work, but `Except` parameters and broader error payload types remain limited by the current ABI. | Exported functions can return tagged success or error values with source-defined error types and host-decodable result layout. |
