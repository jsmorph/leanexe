...
The ownership topic should move to observability before more release semantics.  Add a diagnostic command, tentatively `lean-wasm ownership-report --module M --entry E`, that prints each extracted function's result type, result owner slots, helper-result fresh-owner offsets, compiler-inserted releases, returned owner slots kept live, fold accumulator release offsets, and explicit `LeanExe.Runtime.release` sites.  Useful snapshot cases are `byteArrayResultDropsOwnedTempStats`, `u64ListTailValue`, `JsonTreeCommand.makeTree`, and one fold-accumulator release case.  Include structured monadic-result demand cases such as `Except UInt64 ByteArray` and `Option ByteOutputState`, because counter-oriented tests still show duplicate heap-result demand even though ordinary output comparisons match standard Lean.
