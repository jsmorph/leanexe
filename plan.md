...
| Documentation and tools | The current documents distinguish fifteen completed proofs from sixteen checked artifacts.  The compiler and proof workspaces pin Lean 4.31.0, Wasmtime defaults to 44.0.0, repository instructions constrain every Lean process, and CLI failures use tested statuses and contextual stderr records. | Node and `wasm-tools` remain unpinned, Wasmtime downloads lack checksum verification, and older handwritten proof files retain linter warnings. |
