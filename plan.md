...
| ---- | ----------- | ------------------ |
| Control flow | The extractor accepts a restricted tail-recursion shape over `Nat` fuel, scalar-accumulator `ByteArray.foldl`, scalar-accumulator `Array.foldl`, `ByteArray.findIdx?`, and `Array.findIdx?` over fixed-width arrays.  Ordinary Go-style loops, early returns, breaks, and structural recursion over data are absent. | Lean programs can express common loops over counters, byte buffers, arrays, and simple inductives in accepted source forms, with exact rejection for unsupported recursion. |
