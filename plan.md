...
| Memory model | The current memory model has arena allocation, copy-on-write arrays, internal `ByteArray` elements with owner-pointer-length layout, nested internal arrays with owner-pointer layout, products, fixed-width structure and tagged arrays for literals, replication, reads, default reads, writes, modifications, mapping, filtering, pushes, pops, appends, extracts, insertions, erasures, swaps, reversals, and returned pointers.  It now supports pointer-length `ByteArray` results, arena-owned `ByteArray.mk` from `Array UInt8`, arena-owned `ByteArray.push`, arena-owned `ByteArray.append`, arena-owned proof-indexed `ByteArray.set`, reference-counted release through recursive children, `ByteArray` owners, nested-array owners, and compiler-emitted release for helper-call array and byte-array owner slots proven fresh by IR ownership summaries, including callees that receive heap values.  It still lacks aliasing rules for faster updates, automatic release summaries for recursive-inductive call results, and a public ABI for nested heap graphs. | The compiler has explicit layouts for supported arrays and slices, a documented ownership model for returned buffers, and correctness tests for reads, writes, copies, aliases, and returned memory. |
