...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix and composition prove the aligned stride-five capacity, empty free-list scan, bump allocation, semantic allocation store, and exact result frame.  The two copy loops store the smaller length, prove the unchanged prefix and shifted suffix, reconstruct `os.eraseIdx i`, and return the replacement pointer; the next proof boundary composes allocation and copying with source ownership and allocator state.
