...
| 6 | Internal recursive inductives | Add self-recursive data values and monomorphic recursive instances with arena pointers at strict boundaries | Constructors, matches, branch values, helper values, a fuel-recursive list traversal, a direct structural traversal, generated `Array`-child traversal, ordinary `List UInt64` construction and traversal, and limited direct-lambda `List` library calls pass WASM tests; public recursive ABI, mutual recursion, arbitrary well-founded recursion, hidden carried arguments, broader `List` APIs, and GC remain planned |
...
| ---- | ----------- | ------------------ |
| Control flow | The extractor accepts a restricted tail-recursion shape over `Nat` fuel, including local step lets before the tail call or tail-call `if`; list-shaped structural recursion over internal recursive inductives; generated well-founded recursion for `Array` child descent; scalar-accumulator `ByteArray.foldl`; scalar-accumulator `Array.foldl`; pure `Id.run` `for` loops over `ByteArray`, fixed-width arrays, and ranges with supported accumulators that contain no `ByteArray` field; `break` and `continue` in those accepted pure loops; `ByteArray.findIdx?`; and array scans through `Array.find?`, `Array.findIdx?`, `Array.any`, `Array.all`, and `Array.filter` over fixed-width arrays.  Early returns, general `break` or `continue` outside the accepted pure-loop shape, structural recursion over branching direct fields, and arbitrary well-founded recursion are absent. | Lean programs can express common loops over counters, byte buffers, arrays, and simple inductives in accepted source forms, with exact rejection for unsupported recursion. |
...
| Data types | Structures, user-defined inductives, `Option`, and `Except` now work as local values, parameters, and returned values when their runtime fields fit the current ABI.  Monomorphic self-recursive inductives and monomorphic recursive instances such as `List UInt64` work internally but do not cross the public ABI.  Arrays of structured values now cover the fixed-width operations listed above, while unspecialized polymorphic data, mutual recursion, arbitrary well-founded recursion, and recursive public values remain missing. | Supported structures, small inductives, recursive internal values, `Option`, and `Except` can appear according to one documented representation, with public ABI limits stated explicitly. |
...
| Functions | Higher-order arguments and closures are rejected.  Direct lambdas are accepted only when the extractor can specialize them into first-order code, which now covers selected array and byte-array operations and monomorphic `List UInt64` `map`, `filter`, `find?`, `foldl`, and `any` helper calls.  Escaping functions and hidden carried runtime arguments remain unsupported. | The first Go-level milestone can remain first-order, but the plan should keep a later escape-restricted function-value design open. |
