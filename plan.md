...
| ---- | ----------- | ------------------ |
| Control flow | The extractor accepts a restricted tail-recursion shape over `Nat` fuel and scalar-accumulator `ByteArray.foldl`, but ordinary Go-style loops, early returns, breaks, and structural recursion over data are absent. | Lean programs can express common loops over counters, byte buffers, arrays, and simple inductives in accepted source forms, with exact rejection for unsupported recursion. |
