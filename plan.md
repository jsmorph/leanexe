...
The `clob_depth` artifact is registered outside the aggregate gate while its proof is in progress.  `ClobDepth.Representation` states the stride-two level layout and owned fixed-array predicate, and `ClobDepth.Properties` proves side filtering, first-price append order, unique prices, exact per-price modular aggregation, and its bounded natural-number interpretation.  `ClobDepth.Entry` divides level update into bounded regions, `ClobDepth.Scan` proves its first-price search, and the missing branch has verified preparation, empty free-list search, bump allocation, finalization, old-word copying, appended-field stores, exact result locals, and reconstruction of the owned extended array; the remaining work must compose that branch, prove the found replacement path, prove the per-side fold, and compose both owned export results with page and input-memory frames.
