...
| Data types | Structures and user-defined inductives are useful for local values and returned values, but structure and inductive parameters are missing.  `Option` and `Except` now use the internal tagged-value path, but they cannot cross the public ABI. | Supported structures, small inductives, `Option`, and `Except` can appear in local values, parameters, and results according to one documented representation. |
