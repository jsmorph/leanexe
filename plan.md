...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix computes the aligned stride-five capacity and initializes the exact full-book search frame.  The next proof boundary composes that prefix with the empty free-list scan and bump allocation.
