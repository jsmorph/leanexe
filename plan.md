...
| 5 | Arrays with richer elements | Extend `Array` beyond `Array UInt64` to supported scalar, structure, and small-inductive element types | One-cell scalar arrays are specified and tested; structure and small-inductive arrays have fixed-width literal, read, safe-read, default-read, write, modification, push, pop, append, extract, insertion, erasure, swapping, reversal, and returned-pointer support; the remaining mapping and replication operations still need multi-slot lowering |
...
| Memory model | The current memory model has arena allocation and copy-on-write arrays for one-cell scalar element types, plus fixed-width structure and tagged arrays for literals, reads, default reads, writes, modifications, pushes, pops, appends, extracts, insertions, erasures, swaps, reversals, and returned pointers.  It lacks multi-slot replication, multi-slot mapping, owned byte outputs, and aliasing rules for faster updates. | The compiler has explicit layouts for supported arrays and slices, a documented ownership model for returned buffers, and correctness tests for reads, writes, copies, aliases, and returned memory. |
