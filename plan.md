...
The `findBest` step completed on 2026-07-14.  Its artifact theorem returns the exact source `Option Nat` for every represented order array, every taker, and every input length below `2^32`, while preserving the store.  Its source theorem proves that a returned maker is eligible, no eligible maker has a better price, and the first array index breaks equal-price ties.  The focused branch gate covers empty input, rejected makers, an eligible maker after a rejected prefix, both taker sides, better candidates, worse candidates, and equal-price FIFO ties.  The remaining functions need the same branch inventory before proof work begins, using the established order and result layouts without source wrappers.
