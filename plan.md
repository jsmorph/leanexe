...
| Functions | Higher-order arguments and closures are rejected.  Direct lambdas are accepted only when the extractor can specialize them into first-order code, which now covers selected array and byte-array operations and monomorphic `List UInt64` `map`, `filter`, and `find?` helper calls.  Function-valued structural-recursion motives still block current `List.foldl` and `List.any`, and escaping functions remain unsupported. | The first Go-level milestone can remain first-order, but the plan should keep a later escape-restricted function-value design open. |
