...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  The theorem uses the shared nested-branch continuation instead of expanding the allocation body.  The next proof boundary computes the aligned stride-five capacity and initializes the empty free-list search.
