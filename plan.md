...
The `clob_depth` artifact is registered outside the aggregate gate while its proof is in progress.  `ClobDepth.Representation` states the stride-two level layout and owned fixed-array predicate, and `ClobDepth.Properties` proves side filtering, first-price append order, unique prices, exact per-price modular aggregation, and its bounded natural-number interpretation.  `ClobDepth.Entry` divides level update into bounded regions, and `ClobDepth.Scan` proves its first-price search with exact found and missing frames; the remaining artifact proof must verify both allocation and copy paths, the per-side order fold, both owned output arrays, allocator counters, page preservation, and the input memory frame.
