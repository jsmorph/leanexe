...
The full-fill book preparation proves both selected-index bounds checks and computes the erased length and exact prefix and suffix word counts before entering an opaque allocation-and-copy continuation.  Its allocator prefix and composition prove the aligned stride-five capacity, empty free-list scan, bump allocation, semantic allocation store, and exact result frame.  The complete book update stores the smaller length, copies the unchanged prefix and shifted suffix, reconstructs `os.eraseIdx i`, retains both owned books and exact allocator state, and returns the replacement pointer; the following trade prefix prepares the shared stride-four update, and its suffix records the new trade array and subtracts the maker quantity from the remainder.
