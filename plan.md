...
| Artifact proofs | Eighteen completed byte-pinned proof cases exist, including the self-compiled LEB128 encoder and exact CLOB quote, cancel, `findBest`, `postOnly`, `matchFuel`, and `limit` behavior.  The source matcher has exact step and natural-number quantity-conservation theorems. | `market` and `depth` remain unproved. |
| Documentation and tools | The current documents identify all eighteen checked proof cases as complete.  The compiler and proof workspaces pin Lean 4.31.0, Node pins 24.13.0, `wasm-tools` pins 1.251.0, Wasmtime 44.0.0 archives have checked hashes, repository instructions constrain every Lean process, CLI failures use tested statuses and contextual stderr records, and Talos artifact comparisons run before a separate proof-freshness check. | Older handwritten proof files retain linter warnings.  Warning-only builds of `SharedPair` and `LebU32.Iter` exceeded 30-minute and 15-minute limits under memory pressure, so those warnings remain until substantive work requires the same elaboration. |
...
The `clob_market` case is registered outside the aggregate artifact gate while its proof is in progress.  Its byte-pinned input exports function 21, retains the shared runtime at functions 22 through 25, and has function definitions 0 through 20 identical to `clob_limit`.  Region certificates transport the complete matcher, validity, and status-helper specifications, while the valid exported branch proves the unlimited-price transformation and exact matcher result.  The invalid path now has separately compiled validity, preparation, empty-search, and bump-header theorems, leaving its final length and counter stores before branch and primary composition.
