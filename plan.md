...
| `IO` | No Lean `IO` compiles.  WASI command adapters now wrap pure zero-argument `ByteArray` results, bounded `ByteArray -> ByteArray` stdin transforms, and bounded `ByteArray -> Except ByteArray ByteArray` error-aware transforms with stderr and failure exit status.  Streaming input, argv, files, and environment variables are still absent. | Restricted `IO` is represented in the IR through declared host imports, after pure byte output and tagged error results are stable. |
