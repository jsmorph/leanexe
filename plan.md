...
| 5 | Arrays with richer elements | Extend `Array` beyond `Array UInt64` to supported scalar, structure, and small-inductive element types | One-cell scalar arrays are specified and tested; structure and small-inductive arrays have fixed-width literal, read, safe-read, write, push, pop, append, extract, and returned-pointer support; the remaining insertion, erasure, swapping, reversal, modification, mapping, replication, and default-read operations still need multi-slot lowering |
...
| Memory model | The current memory model has arena allocation and copy-on-write arrays for one-cell scalar element types, plus fixed-width structure and tagged arrays for literals, reads, writes, pushes, pops, appends, extracts, and returned pointers.  It lacks slices, the remaining multi-slot copy operations, owned byte outputs, and aliasing rules for faster updates. | The compiler has explicit layouts for supported arrays and slices, a documented ownership model for returned buffers, and correctness tests for reads, writes, copies, aliases, and returned memory. |
