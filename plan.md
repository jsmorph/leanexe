...
Current program-output progress: WASI command mode accepts a zero-argument `ByteArray` entry, wraps it in `_start`, and writes the returned bytes to stdout through `wasi_snapshot_preview1.fd_write`.  This gives pure byte-producing programs an observable command-line result under Wasmtime without compiling Lean `IO`.  Input through stdin, argv, files, and environment variables remains planned.
...
| `IO` | No Lean `IO` compiles.  A first WASI command mode wraps pure zero-argument `ByteArray` results and writes them to stdout, but stdin, argv, files, environment variables, and exit status are still absent. | Restricted `IO` is represented in the IR through declared host imports, after pure byte output and tagged error results are stable. |
