...
The proving task now permits and imports `FixedArraySearchTree` when checked less-than recipes identify a tree candidate, and its proof-library text explains the descriptor and theorem.  The next implementation step must derive the descriptor and a checked region equality from the annotation paths so the proving agent receives the parent-child graph instead of reconstructing it.  A fresh Demo 3 reproof will then measure whether the 266-line reduction and eliminated graph search reduce Stage 5 time.
