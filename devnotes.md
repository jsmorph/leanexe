...
`PairFree.Spec` is intrinsically heavy: it ran 85 minutes solo without a diagnostic under the serialized policy, and a controlled rebuild with the sweep edits stashed also exceeded a twenty-minute cap, which exonerates the five bound-proof conversions in that file.  The module joins the division list beside its sibling `SharedPair.Spec` and the other recorded no-diagnostic modules.  Serialization itself is confirmed effective: `ClobCancel.Spec` built in 35 seconds where the collided run had held it for 53 minutes.  The heavy modules that took forty minutes or more under collision build at full speed serially: `ClobPostOnly.FindBest` in 207 seconds and `ClobMatchFuel.FindBest` in about six minutes.
