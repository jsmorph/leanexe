...
The first fresh composition run preserved Demo 3's 7,186-byte artifact and SHA-256 digest `1a93ec55974666ad54fad73d321b8b9e1f7d67970b272c13bcc77055c8e7631d`.  The accepted 144-line proof took 405.385 seconds, including 361.651 seconds in Codex and 30.551 seconds in outer acceptance.  A separate `tools/leanexegen verify -s` run accepted the published package.

The preceding journal-guided node proof took 770.948 seconds and 421 lines, so the complete composition reduced that run by 365.563 seconds, or 47.4 percent, and 277 lines, or 65.8 percent.  The 278.656-second pair-result reference remains 126.729 seconds faster than the new screen despite containing 672 lines.  The 392.544-second optional-search screen also remains 12.841 seconds faster, and the large spread among prior Demo 3 runs prevents a causal timing conclusion from one new result.

The journal records the intended proof process.  The agent ran the starter check before editing, selected the complete composition before node tactics, and wrote one proof using the normalized length dispatch, checked invalid result, saved-key loader, and `Tree.program_spec`.  Its first build after that edit succeeded without node-level expansion or allocator arithmetic, providing direct evidence that the generated descriptor removed the graph-reconstruction failures.
