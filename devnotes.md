...
The descriptor trial increased total proof time by 1,674.982 seconds, or 83.0 percent.  Its 704-line accepted proof uses `AnnotationMatches.function_0_while_loop_0_eq` and `ScalarTransition.whileProgram_spec`, compared with 734 lines in the baseline, but the descriptor file is 735 bytes larger.  The journal records 79 entries rather than 65 and identifies repeated work on descriptor-evaluator equalities, scratch-local updates, and conversions between `ScalarTransition.State` and the exact WASM local frame.

The next compiler-theorem increment will add generic effect computations and preservation lemmas, but Demo 1 cannot obtain much benefit from an outside-write theorem because its body touches almost every combined local.  The timing-critical addition is a neutral, artifact-checked transition certificate that states branch updates over application locals and existentially hides or normalizes compiler scratch state.  The retained baseline and descriptor packages now reside under `benchmarks/leanexegen/demo1-array` so another machine failure cannot erase this evidence.
