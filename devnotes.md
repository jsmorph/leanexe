...
## 2026-08-09: Checked fold setup and result placement

The accepted Demo 9 journal spent several iterations reducing the compiler-generated fold setup through two length-load bounds, effective-stop selection, and a nested local-update frame.  Its loop-exit helper then repeated the same problem for the final accumulator-to-result transfer.  Both boundaries depend on emitted structure rather than the wrapping-sum application.

`Project.ProofKit.FixedArrayFold` now defines continuation-generic semantics for the exact common shapes.  `forwardSetupProgram_spec` covers a forward, one-word, one-accumulator fold over the complete array input and returns a named `forwardSetupFrame`, while `resultProgram_spec` covers one selected accumulator transfer and returns `resultFrame`.  The artifact matcher advertises these declarations only after the decoded subregions equal their generic programs byte for byte.

`tools/leanexegen annotate` rebuilt Demo 9 without changing its 1,979-byte WASM digest and compiled the generated setup equality over instructions 39 through 62 and result equality over instructions 63 through 65.  Independent package verification accepted both equalities with the retained behavior and artifact theorems, while structured LTG exposes the support through `fixed-array-fold-structure`.  The next fixed-artifact reproof will determine whether a fresh agent retrieves and composes these boundaries and whether the remaining seven-update successor frame still dominates the proof.

## 2026-08-10: Demo 9 fold-structure and capacity screens

The first fixed-artifact reproof with the checked fold setup, traversal, and result-placement boundaries completed Stage 5 in 2,082.889 seconds.  This is 34.7 percent below the preceding annotation run and 39.3 percent below the baseline.  Its independently verified proof contains 508 lines and applies all three fold-region theorems, though two local capacity-prefix theorems increased the proof by 92 lines relative to the preceding run.

`FixedArrayCapacity.constantProgram_spec` now executes the complete eighteen-instruction constant-capacity prefix for arbitrary result length, one-word stride, destination local, following program, and postcondition.  The annotation consumer checks the exact prefix in each length branch and emits a Lean equality plus recipe declaration only for a complete match.  `Frame.internal_getElem?_of_get` and `internal_getElem_of_get` also convert loop-invariant `Locals.get` facts to internal-local getters without repeating the parameter-offset proof.

The next fixed-artifact reproof used both checked capacity equalities and the expanded twenty-entry LTG.  Independent verification accepted its 642-line proof in 3,070.994 monotonic seconds, which is 47.4 percent slower than the retained fold-structure proof but 3.7 percent faster than the earlier annotation proof.  Its UTC timestamps span 12,764.900 seconds and differ from the monotonic measurement by 9,693.906 seconds; accessible system logs did not identify that interval, so timing comparisons use the preserved monotonic total.

The journal confirms that the capacity theorem removed both local capacity derivations and that structured retrieval selected dispatch, allocation, traversal, result, and memory support while rejecting unrelated worked examples.  Two broad reductions still exhausted the heartbeat budget at the traversal exit and accumulator-update edges, and the agent recreated combined-to-internal getter conversions because the traversal entry did not direct it to the frame entry.  The generation package remains under `demos/demo-9/experiments/capacity-frame.proof`, and `capacity-frame-current.proof` passed independent verification after re-freezing the same proof under the next proof-kit identity; the faster fold-structure package remains primary.

`FixedArrayTraversalInput.continuingProgram_exit_spec` now proves the equal-index guard edge through `Break 1` without reducing the dynamic loader or result suffix.  `FixedArrayResult.finishProgram_spec` proves the final root-to-destination-to-return transfer with a named frame, and traversal guidance now indexes the frame-projection declarations directly.  The wrapping-add accumulator and index update remains local comparison evidence because the current corpus has only one annotated fold consumer for that exact emitted step.
...
The investigation record separates the source technique, fixed-artifact evidence, open hypotheses, and planned cross-artifact tests.  A small compiled ProofKit module and a different fold operation will now test reuse, while the accepted local theorem remains evidence rather than promoted automatic support.  Local irreducibility of the generated function remains a separate controlled experiment because entry and frame proofs still require selected explicit reductions.

## 2026-08-10: Compiler-described guarded back edge

The compiler now attaches a versioned `ScalarTransition.PostTest` descriptor to a fixed-array fold annotation when the emitted body fits the existing typed scalar language.  The annotation consumer checks the standard forward continuing edge, names the exact instruction interval, and generates body and condition evaluator theorems from the descriptor.  Older fold annotations remain valid without these optional fields and do not receive the new recipe declarations.

`Project.ProofKit.ScalarTransition.guardedBackEdgeProgram_spec` executes an arbitrary scalar body and condition through either `Break 1` or an arbitrary scalar continuation followed by `Break 0`.  Its theorem type contains compact states and an arbitrary postcondition rather than the enclosing loop or decoded artifact function.  The structured LTG records the theorem as provisional compiler-motif support because one wrapping-sum artifact does not establish cross-operation use.

Demo 9's unchanged artifact generated an exact equality between nested loop instructions 16 through 37 and the generic guarded-back-edge program.  The generated body descriptor performs the wrapping-add accumulator transition, and the continuing descriptor increments the loop index.  Both annotation-package construction and a separate silent verification accepted the equality, transition equations, ProofKit source inventory, and existing artifact theorem.

The first package attempt failed before Lean acceptance because the new module was absent from the canonical proof-kit import allowlist.  Adding the module to the source inventory and generated declaration check resolved that authority boundary without changing the theorem.  The fixed-artifact reproof below measures agent retrieval, theorem use, proof structure, and generation time before a different fold operation tests reuse.

The fixed-artifact reproof selected `guarded-back-edge` on its first structured LTG query and reached the exact generated guarded program after applying the checked traversal prefix.  `guardedBackEdgeProgram_spec` accepted the compiler-derived body and condition equations, the fold-prefix successor invariant, and the strict remaining-input measure.  The complete artifact theorem and a separate package-verification run accepted the unchanged 1,979-byte WASM artifact and SHA-256 digest.

Stage 5 took 1,972.223 seconds, including 1,872.127 seconds in Codex and 79.123 seconds in outer acceptance.  This is 110.665 seconds, or 5.3 percent, below the retained fold-structure proof and 325.653 seconds, or 14.2 percent, below the artifact-local compact-boundary experiment.  The 589-line proof is 81 lines longer than the retained primary but 66 lines shorter than the local compact-boundary proof.

Generic evaluation of the continuing index assignment exceeded the simplifier step limit after the shared theorem had discharged every other loop-step premise.  Rewriting with `ScalarTransition.Stmt.eval_toState` and reducing the finite `U64State` assignment closed the goal without increasing any limit.  The annotation generator should emit a continuing-transition theorem beside the current body and condition transitions before the out-of-sample fold test.

The journal exposed two additional general support gaps.  Structured retrieval omitted the existing `FixedArrayPairResult.input_preserved_by_alloc` theorem, and dependent premise inference for `FixedArrayTraversalInput.continuingProgram_spec` caused several shifted applications before an all-named invocation succeeded.  The same journal restored the exact diagnostic and intended residual after agent-context compaction, supplying direct evidence that frequent prose entries preserve proof progress.

The annotation generator now emits a compact `step_continuing_eval` theorem for the checked index-increment descriptor.  Demo 9's exact annotation package compiled this theorem and added it to the fold recipe, while a separate package-verification run accepted the unchanged artifact and existing behavioral proof.  The fixed-array allocation LTG entry now advertises `FixedArrayPairResult.input_preserved_by_alloc`, and traversal guidance records the all-named application form for dependent premises.
...
## 2026-08-10: Generated traversal-adapter experiment

The annotation generator now emits a continuing frame, its item-local validity theorem, and a complete continuation-generic traversal theorem.  A manual substitution in Demo 10 replaced thirteen lines with six and passed the complete `ArtifactResult` build.  The checked proof decreased from 572 to 565 lines, from 1,931 to 1,896 words, and from 30,577 to 30,173 bytes.
...
The length-dispatch consumer now emits concrete named valid and invalid branch programs, a generic dispatch program, a named suffix, an exact dispatch-region equality, and a complete function equality when the dispatch starts at top-level instruction zero.  JavaScript tests cover the unsigned, normalized-equality, and normalized-inequality constructors.  Complete annotation packages compiled and passed separate verification for Demo 10's unsigned bound and Demo 7's normalized equality with a nonempty suffix.

The manual Demo 10 proof marks `func0` locally irreducible, changes the post-entry goal to the named program, and rewrites through the generated function equality.  Its established capacity, allocation, fold, traversal, guarded-step, and result proof then checks unchanged apart from explicit reductions of the named empty suffix at both terminal continuations.  The complete `ArtifactResult` target passed under the standard runner profile.

The modified proof contains 578 lines, 1,951 words, and 31,087 bytes, increases of six lines, twenty words, and 510 bytes.  One warm acceptance took 8.2 seconds for `Behavior` and 2.1 seconds for `ArtifactResult`, while a reducible build against the same annotation package took 9.0 and 2.3 seconds.  The generated annotation module grew by 13,333 bytes to 24,930 bytes, so fresh proof-generation behavior and the concrete-branch duplication cost remain part of the next screen.

## 2026-08-10: Compact Demo 10 suffix boundary

The fresh fixed-artifact reproof received the generated entry-dispatch package but used the ordinary length-dispatch tactic, providing no evidence that a proving agent will select local irreducibility without stronger direction.  Four related loop-rule builds ended without diagnostics while carrying the complete result suffix, with three explicit six-gibibyte child-memory peaks; an inline compact consequence retained the failure.  A separate `productSuffix_spec` declaration restored diagnostics, and independent verification accepted the completed 687-line proof in 2,331.276 seconds over the unchanged artifact.

This result promotes no timing claim because the run was 46.0 percent slower and 115 lines longer than the retained proof.  It adds a provisional structured-LTG method for a compact loop-suffix declaration and links that method from the fold and result entries.  The artifact-proof prompt now prohibits unchanged retries after timeout, memory-limit, or silent target failure and requires a smaller theorem, module, or checked lemma before another Lean check.

## 2026-08-10: Generic singleton fold suffix

`FixedArrayFold.singletonResultProgram_spec` composes accumulator placement, a zero-index singleton payload store, and the final root transfer for arbitrary local roles, frame, root, value, module, store, and host environment.  Its `singletonResultPost` endpoint records only the returned root and represented singleton array.  The annotation matcher extracts the local roles and emits the theorem recipe only when the complete decoded suffix matches that program through the end of its enclosing instruction list.

The production Demo 10 annotation package compiled the new exact-region equality without changing the 1,979-byte artifact or its digest.  Separate `leanexegen verify -s` accepted the package with the retained behavior and artifact theorems.  The package establishes capability and exact linkage, while a fresh reproof remains necessary to establish agent selection or an effect on proof construction.

## 2026-08-10: Fresh singleton-suffix proof screen

The fixed Demo 10 reproof selected `compact-loop-suffix-boundary` through structured LTG and used the generated suffix equality with `FixedArrayFold.singletonResultProgram_spec`.  The local `productSuffix_spec` retained only the concrete frame premises and consequence to the public artifact postcondition, while the shared theorem executed accumulator placement, singleton payload storage, and final root transfer.  The complete `ArtifactResult` build passed all 3,081 jobs, and a separate `tools/leanexegen verify -s` run accepted the preserved package over digest `a981c7882a51a0660e6dd1e17956b958f7b2e25cbc30618d12458601ef2d4baa`.

Stage 5 took 2,998.613 seconds, with 2,875.208 seconds in Codex and 110.879 seconds in outer acceptance.  The proof contains 707 lines, 3,072 whitespace-delimited words, and 34,081 bytes.  This is 28.6 percent slower and twenty lines longer than the preceding compact-suffix screen, so the result establishes retrieval and composition rather than a time or line-count improvement.

The journal records that the one-million-heartbeat limit exposed a second boundary at `wp_loop_cons`.  An exact `productFoldFrame_eq` removed both invariant-initialization and loop-step failures, and a continuation-generic local `productLoopBody_spec` then composed the generated traversal, compiler-derived multiplication, condition, and index transitions, guarded back edge, completion callback, repeat invariant, and decreasing measure.  The next increment will extract the operation-independent composition into shared ProofKit, generate the exact adapter evidence, screen it on fixed Demo 10, and use Demo 9 as the out-of-sample sum-fold test.

## 2026-08-10: Shared continuing fold body

`FixedArrayFoldBody.continuingGuardedProgram_spec` now composes the active fixed-array traversal guard and indexed load with arbitrary compiler-described body, condition, continuing statement, and guarded back edge.  Its callbacks leave the fold operation, invariant, successor equation, measure, and public postcondition outside the shared theorem.  A focused module build and the generated `LTGCheck` target accepted the theorem under the standard runner limits.

The annotation generator emits an exact loaded-frame equality for each matched array-fold region and advertises the equality with the shared theorem in the fold recipe.  Structured LTG now contains a provisional `fixed-array-fold-body` entry linked to traversal, guarded control, fold structure, prefix mathematics, and frame projection.  JavaScript tests, catalog checks, and the production Demo 10 annotation package accepted the new declaration and recipe.

The first manual Demo 10 application exposed one positional-premise problem, so the checked adapter supplies every dependent premise by name.  Lean then accepted both the 704-line `Behavior` module and the complete `ArtifactResult` target; the source removes the artifact-local loaded-frame theorem and three lines of net scaffolding from the 707-line fresh singleton-suffix proof.  A regenerated capability package passed separate `leanexegen verify -s`, while the retained package proof itself predates the theorem and therefore records availability rather than use.

## 2026-08-10: Fresh fold-body proof screen

The fixed Demo 10 reproof retrieved `fixed-array-fold-body` on its second structured query and used `continuingGuardedProgram_spec` at the strict-index edge.  The shared theorem consumed the generated loaded-frame, multiplication, condition, and increment equations, while the artifact proof retained the product-prefix successor, invariant reconstruction, and measure decrease.  The complete artifact theorem and separate package verifier accepted the unchanged 1,979-byte WASM digest.

Stage 5 took 1,913.092 seconds, including 1,778.739 seconds in Codex and 97.743 seconds in outer acceptance.  The run was 36.2 percent faster than the preceding 2,998.613-second singleton-suffix reproof, and its source fell from 707 lines and 3,072 words to 650 lines and 2,102 words.  It remained 19.8 percent slower and 78 lines longer than the retained primary proof, so the primary package remains the timing and size reference.

The journal records twelve accepted import-check edits and three avoidable iterations at the new theorem application.  Lean postponed dependent `hItem` and `hIndex` premises and shifted positional bullets, while broad simplification of the generated increment evaluator exceeded its step limit.  The LTG entry now requires named premises, direct evaluator rewriting, and local continuing-frame unfolding before the Demo 9 transfer screen.

## 2026-08-10: Demo 9 fold-body transfer

The current annotation pass recognized the fold-body composition on Demo 9's unchanged 1,979-byte WASM artifact and digest `aa263bbfa89c333f9fab497f1a2c370f476afc3419015d17b368cb7c8a6086d5`.  A capability package passed independent verification before proof generation, and the fresh agent selected `fixed-array-fold-body` in its first annotation-driven catalog query.  Its accepted proof applied `continuingGuardedProgram_spec` to the generated wrapping-add body, false condition, index increment, loaded-frame equality, and exact traversal and guarded-step programs.

Stage 5 took 2,074.169 seconds, including 1,984.791 seconds in Codex and 69.416 seconds in outer acceptance.  The run was 0.4 percent faster than the 2,082.889-second fold-structure primary, 5.2 percent slower than the 1,972.223-second guarded-back-edge screen, and 9.7 percent faster than the 2,297.877-second local compact-boundary screen.  Its 588-line, 2,484-word, 29,973-byte proof has one fewer line, 276 more words, and 1,175 fewer bytes than the guarded-back-edge proof, so neither timing nor source size improved uniformly.

The named-premise instructions prevented the dependent-premise bullet shift seen in Demo 10.  The journal instead exposed two reusable proof-construction rules: fix the module, environment, store, and frame when callback inference generalizes the weakest-precondition context, and name all projections of a proof-local invariant frame before composing body or suffix theorems.  The LTG entry now records those rules and advances to promoted evidence because independently verified proofs used the same compiler-motif theorem for wrapping multiplication and wrapping addition.

## 2026-08-10: Generated fold-frame accessors

Both fresh fold-body journals reported the same residual work: define a semantic continuing frame, then prove its parameters, internal-local length, empty operand stack, and required getters before applying traversal or suffix theorems.  The annotation generator now emits those exact facts for every compiler-described continuing fold frame, including one `Locals.get` theorem for every combined parameter-and-local index.  The proof recipe publishes all generated names, and JavaScript tests check the first and final getter names together with the three frame-shape declarations.

`FixedArrayFold.resultFrame_get_result` reads the accumulator value written to a valid nonparameter result local.  `FixedArrayFold.resultFrame_get_of_ne` preserves a distinct valid nonparameter getter across the same list update.  A focused `FixedArrayFold.lean` check and the generated `Project.ProofKit.LTGCheck` target accepted both theorems under the standard local runner limits.

Current annotation packages compiled and passed independent verification for Demo 9 digest `aa263bbfa89c333f9fab497f1a2c370f476afc3419015d17b368cb7c8a6086d5` and Demo 10 digest `a981c7882a51a0660e6dd1e17956b958f7b2e25cbc30618d12458601ef2d4baa`.  A Demo 9 manual substitution removed ten local projection theorems and used restricted simplification with the generated accessors plus the two shared result-frame theorems.  Independent package verification accepted its complete artifact theorem, and the proof decreased from 588 to 555 lines, from 2,484 to 2,084 whitespace-delimited words, and from 29,973 to 29,291 bytes.

Unrestricted `simp [foldFrame]` failed because it reduced `Wasm.Locals.get` into parameter and internal-list branches before the generated accessor could match.  `simp only [foldFrame, <generated-accessor>]` preserved the intended boundary and removed that representation detour.  The structured `annotated-fold-frame-accessors` entry records this use rule, the result-frame declarations, both artifact consumers, the proof-size result, and the absence of a fresh proof-generation timing.

## 2026-08-10: Fresh frame-accessor proof screen

The fixed Demo 9 reproof found `annotated-fold-frame-accessors` through the structured LTG indexes and used the generated parameter, local-length, operand-stack, and combined-local getter declarations.  It also used `FixedArrayFold.resultFrame_get_result` and `resultFrame_get_of_ne` at the singleton suffix.  The accepted source declares no proof-local frame-projection theorem, and a separate `leanexegen verify -s` run accepted the exact 1,979-byte artifact and digest `aa263bbfa89c333f9fab497f1a2c370f476afc3419015d17b368cb7c8a6086d5`.

Stage 5 took 2,230.869 seconds, including 2,173.264 seconds in Codex and 39.702 seconds in outer acceptance.  The UTC timestamps span 4,843.670 seconds, leaving a 2,612.801-second difference from the monotonic metric used for comparisons.  The run was 7.6 percent slower than the preceding fold-body reproof and 13.1 percent slower than the guarded-back-edge screen.  Its source contains 616 lines, 2,541 whitespace-delimited words, and 30,901 bytes, so neither proof time nor source size improved.

The journal records seventeen edited import-check candidates in one accepted Codex task.  The generated accessors eliminated local projection declarations, while the remaining revisions concerned dispatch, capacity, allocation, loop initialization, dependent fold-body premises, and result construction.  This evidence retains the accessor interface as provisional compiler-motif support and directs the next screen to another fold artifact.

## 2026-08-10: Demo 10 frame-accessor transfer

The fixed Demo 10 reproof used the generated continuing-frame value-stack and index getters and both shared result-frame getter theorems.  It did not retrieve `annotated-fold-frame-accessors` by name, although the task included the entry, selected strategy guidance, and every exact accessor in the recipe.  A separate `leanexegen verify -s` run accepted the 1,979-byte artifact with digest `a981c7882a51a0660e6dd1e17956b958f7b2e25cbc30618d12458601ef2d4baa`.
...
## 2026-08-11: Structured tactic retrieval

Schema 2 adds checked tactic records to structured LTG.  Each record names one command, its defining ProofKit module, the residual goal shape, required premises, applicable annotation kinds, and a fallback declaration.  The first inventory contains five commands across four entries, while catalog validation, generated JSONL indexes, metrics, and `Project.ProofKit.LTGCheck` verify the inventory against the same source and declaration boundaries as ordinary LTG support.

The first fixed Demo 9 screen selected `wp_fixed_array_length_le_dispatch_from` for the bounded input branch and `wp_block_loop` for the annotated fold.  The journal compared each selected record with the current goal and recorded why the result-shape, singleton-wrapper, and nested-read tactics did not apply.  It used neither indexed fallback declaration, supplying direct evidence that the structured fields governed tactic choice rather than naming the tactics without affecting the proof.

The independent outer Lean check rejected the candidate after the in-session agent reported success.  Its diagnostics identify a broad simplification that exceeded the step limit, a missing fixed-width successor bound, and a measure rewrite blocked by a callback frame whose parameter and local fields had advanced while its operand-stack projection still came from the predecessor.  `benchmarks/leanexegen/demo9-fold-sum/structured-tactic-retrieval-rejected-1` preserves the digest-bound candidate, journal, task inputs, full outer diagnostic, and a separate repaired source.

The repaired source constructs the semantic successor as `UInt64.ofNat (index + 1)`, uses `UInt64.ofNat_add` only for the compiler-generated evaluator equation, and changes the callback frame to `afterContinue.toState.toLocals []` before rewriting the invariant and measure.  A complete `ArtifactResult` build accepted the repair under the standard runner limits.  The fold-body guidance now records both rules, and another fixed-artifact task may run because the journal exposed a general interface-use defect rather than an unexplained or unchanged failure.

## 2026-08-11: Constant capacity and allocator composition

`FixedArrayAllocatorWindow.constantCapacityRegion_spec_withTail` composes the compiler's constant-capacity prefix with its standard shifted allocator window.  The theorem derives capacity-local validity, capacity-frame projections, the internal capacity getter, and the normalized eight-byte minimum before applying the existing allocator theorem.  Its statement retains the source frame dimensions, memory fit, memory mode, allocator globals, exact following program, and post-allocation continuation.

A manual substitution applied the theorem to both branches of Demo 9's unchanged wrapping-sum artifact.  The complete `ArtifactResult` target accepted the binary identity, decoding, validation, behavioral theorem, and artifact theorem, while the proof source decreased from 526 to 498 lines and from 28,132 to 26,131 bytes.  The annotation recipe and structured LTG now expose the composition theorem and its exact prefix and allocator conditions for a fresh proof-generation screen.

Repository tools now create generated workspaces below the repository's ignored `tmp/` directory.  The shared helper validates each directory prefix and covers LeanExeGen generation, annotation, reproof, and verification together with the artifact, Talos, conformance, release, and related test workspaces.  The machine-wide Lean lock remains in the operating-system temporary directory because it is synchronization state rather than a work product.

## 2026-08-11: Accepted capacity-to-allocation screen

A fresh fixed-artifact Demo 9 task selected `FixedArrayAllocatorWindow.constantCapacityRegion_spec_withTail` from structured LTG and applied it to both branches.  This crossed the capacity elaboration boundary that had censored the preceding task before allocation.  The agent then selected `wp_fixed_array_length_le_dispatch_from` and `wp_block_loop` from their structured tactic records and reached the corrected fold-continuation guidance.

The proof reconstructs the callback from `afterContinue.toState.toLocals []`, applies the generated scalar evaluators and frame accessors, and closes the fold-prefix successor and strict measure.  It uses the generated singleton-result adapter at the completed edge and the shared result representation theorems at the public return.  Outer acceptance and a separate `tools/leanexegen verify -s` run accepted the complete theorem over artifact digest `aa263bbfa89c333f9fab497f1a2c370f476afc3419015d17b368cb7c8a6086d5`.
...
## 2026-08-12: Divided behavioral-specification builds

The cold release check at revision `7147d6dfd5b86c6a8eb57fb3839677b361560937` rebuilt the 3,001-target shared Talos library, the artifact translator, all twenty embedded byte modules, all twenty exact-artifact theorem chains, and seventeen behavioral specifications.  The run then reached the sixty-minute limit assigned to `Project.ClobLimit.Spec` without a Lean diagnostic.  The last successful output came from `Project.ClobLimit.InternalPartialBookFinish`, after many earlier ClobLimit modules had each completed within their own elaboration times.

`Project.ClobLimit.Spec` contains eighty-three direct imports and no theorem body, so the failure records accumulated dependency-build time rather than a failed specification theorem.  The artifact driver now parses the leading Lean import block, resolves repository-local modules, rejects cyclic local imports, and orders the transitive closure before its root.  It builds each distinct dependency once per aggregate run under a separate thirty-minute target limit, while the requested specification retains its sixty-minute limit.

The release-input test now covers nested block comments, trailing line comments, multiple imports on one command, root exclusion, and dependency order against the ClobLimit graph.  The new closure contains 124 repository-local inputs for `Project.ClobLimit.Spec`, including shared proof modules used by its direct imports.  The warm artifact gate passed all twenty byte identities, exact-artifact theorem chains, behavioral specifications, and declaration checks for release-input digest `842605514a0f78034795b1c0416483412d910029f1d6ab2fbdd51e20f4c88a3c`.

The matching conformance gate reported 3,853 Talos passes, six configured imported-memory failures, 627 skips, twenty-five Wasmtime file passes, and fifteen expected invalid-module classifications.  Refresh consumed both receipts and left the immutable source revision and cold-checkout result as the two release blockers.  A second cold checkout remains required before the divided build constitutes release evidence.

## 2026-08-13: Divided cold conformance build

The independent cold conformance run compiled 2,407 of the testsuite target's 2,413 jobs before the unchanged aggregate target reached its 90-minute limit without a Lean diagnostic.  That target had combined the complete cold `Mathlib.Tactic` dependency graph, the Talos testsuite library, and the executable link under one process limit.  Repository policy therefore barred another unchanged attempt.

The conformance driver now reads the direct public imports from the pinned Mathlib `Mathlib/Tactic.lean`, builds them in groups of sixteen with a separate 30-minute limit for each group, then builds `Interpreter.Testsuite.Exec` and the `testsuite` executable as separate 30-minute targets.  The checked pinned source supplies the dependency frontier, so the repository does not maintain a second list that could drift from Mathlib.  Parser tests cover multiple public imports, line comments, nested block comments, and exclusion of private imports.

The divided warm run passed the complete conformance gate on 2026-08-13.  It classified all fifteen configured invalid or malformed artifact modules, recorded 3,853 Talos passes and the six exact imported-memory warnings, and passed all twenty-five selected files with Wasmtime.  The next release evidence must use the new release-input digest and a source revision containing this driver change.

The next cold run passed all twenty exact-artifact theorem chains and behavioral specifications, then stopped before the conformance build while reading `CodeLib/.lake/packages/mathlib/Mathlib/Tactic.lean`.  The warm workspace happened to contain that private CodeLib dependency cache, while a fresh proof-workspace update placed the pinned Mathlib checkout at `proofs/talos/lean/.lake/packages/mathlib`.  The conformance driver now reads the import frontier from the proof workspace that owns the dependency, and its unit test rejects a return to the optional CodeLib cache path.
