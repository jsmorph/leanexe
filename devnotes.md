...
The checked theorem does not yet connect a Demo 1 annotation to its decoded loop body.  The next increment will reify the supported compiler IR into the neutral descriptor and prove exact agreement with the compiler emitter, while the generated artifact package independently checks descriptor-program equality against the decoded WASM.  A loop theorem will then compose the checked condition and body transitions with the application invariant and decreasing measure.
